    ...
    pluginConfiguration {
        ...
        description = """
            ...
            <ul>
                ...
                <li><b>Jolli API Key</b> &mdash; for Push to Jolli Space (contact <code>support@jolli.ai</code>)</li>
            </ul>
...
tasks.register<com.github.jengelman.gradle.plugins.shadow.tasks.ShadowJar>("hookJar") {
    ...
    configurations = listOf(project.configurations.getByName("runtimeClasspath"), hooksRuntime)
...
// After prepareSandbox completes, copy hooks JAR into the sandbox lib dir
tasks.register("copyHookJarToSandbox") {
    ...
    doLast {
        ...
        val pluginLib = layout.buildDirectory.dir("idea-sandbox/plugins/jollimemory-intellij/lib").get().asFile
        pluginLib.mkdirs()
        hookJar.copyTo(File(pluginLib, "jollimemory-hooks.jar"), overwrite = true)
        logger.lifecycle("Copied hooks JAR to: ${pluginLib}/jollimemory-hooks.jar")
    }
}
...
// After buildPlugin creates the zip, inject the hooks JAR into it
tasks.named("buildPlugin") {
    dependsOn("copyHookJarToSandbox")
    doLast {
        val hookJar = layout.buildDirectory.file("idea-sandbox/plugins/jollimemory-intellij/lib/jollimemory-hooks.jar").get().asFile
        val zipFile = layout.buildDirectory.dir("distributions").get().asFile
            .listFiles()?.firstOrNull { it.name.endsWith(".zip") } ?: return@doLast

        // Add hooks JAR to the existing zip
        // Use ant zip task to add file to existing zip
        ant.withGroovyBuilder {
            "zip"("destfile" to zipFile.absolutePath, "update" to true) {
                "zipfileset"("dir" to hookJar.parentFile.absolutePath, "prefix" to "jollimemory-intellij/lib") {
