...
class JcefBrowserPool(private val project: Project) : Disposable {
    ...
    private fun buildEntry(reason: String): Entry {
        ...
        // Deliberately no prime loadHTML. Earlier we loaded a themed blank page here
        // to "warm" the native NSView, but that broke real-content loadHTML on macOS
        // JBR: after `loadHTML(themedBlank, PRIME_URL)` set the browser's URL to a
        // fake sentinel, a subsequent `loadHTML(realHtml)` (no URL) never actually
        // replaced the document — onLoadEnd fired against the still-prime page. The
        // browser is now returned pristine; the caller does exactly one loadHTML.
        log.info("buildEntry [%s]: browser #%d build took %dms (pristine, no prime)", reason, id, buildMs)
    ...
    class Entry internal constructor(
        ...
        // Idempotent guard: pool.dispose() and a straggler lease.release() during
        // project close can both target the same entry (pool.dispose walks leased
        // eagerly; the SummaryPanel.dispose invokeLater arrives later). Second
        // dispose is a JCEF-internal warning trap — the try/catch below already
        // swallows the throw, but the CAS avoids paying the cost.
        private val disposedOnce = java.util.concurrent.atomic.AtomicBoolean(false)
...
class PooledBrowserLease internal constructor(
    ...
    // Guards release() idempotency. `release` is contractually EDT-only, but
    // SummaryPanel.dispose can invoke it from a pooled Disposer thread via
    // invokeLater on shutdown — @Volatile makes the flag's happens-before edge
    // explicit if we ever loosen that EDT contract.
    @Volatile
