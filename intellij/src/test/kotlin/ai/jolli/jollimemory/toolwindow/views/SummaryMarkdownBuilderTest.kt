...
class SummaryMarkdownBuilderTest {
    ...
    private fun makeSummary(
        ...
        excludedContext: List<ExcludedContext>? = null,
    ) = CommitSummary(
    ...
    inner class BuildMarkdown {
        ...
        @Test
        fun `renders a collapsed AI-excluded context section when present`() {
            val excluded = listOf(
            ...
            val md = SummaryMarkdownBuilder.buildMarkdown(makeSummary(excludedContext = excluded))
            md shouldContain "<details>"
            md shouldContain "AI judged 2 context item(s) unrelated (not included)"
            md shouldContain "- Unrelated Plan"
            md shouldContain "— different feature area"
            md shouldContain "- JOLLI-9 — Old ticket"
            md shouldContain "</details>"
        }

        @Test
        fun `omits the AI-excluded section when there is nothing excluded`() {
            val md = SummaryMarkdownBuilder.buildMarkdown(makeSummary())
            md shouldNotContain "unrelated (not included)"
        }
