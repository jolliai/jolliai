...
- **The `local-agent` provider drives one of four CLIs**, selected by `localAgentTool` (`claude-code` | `codex` | `cursor-agent` | `opencode`, defaulting to the first). Two asymmetries bite: only `claude-code` is capability-probed with the real run flags and only it receives a resolved model id (the other three run whatever model they are configured with, yet the stored metadata still records the Claude alias); and `opencode` deliberately keeps provider credentials in the child environment, so it spends the user's own provider credit and has no auth-failure classification at all.
...
`npm run all` is the **pre-commit gate, not the inner loop** — the CLI unit suite alone is 336 files / ~8.9k tests / 3.5-9 min with coverage depending on machine load (plus 5 acceptance files and the 107-file vscode suite). The cost is concentrated: **12 real-`git`/real-fs files carry 96% of the runtime** and the median file takes 14 ms, so the tiers split along that line. Inner loop: `npm run test:fast` (the 324 light CLI files **with coverage**, ~25-45 s), then `npm run test:slow` when you touch `sync/`, `install/`, or git plumbing. Both exist at the repo root and delegate to the `cli` workspace; `test:fast` covers **CLI only** — the vscode suite has no tier worth carving (107 files, ~54 s with coverage), so run `npm run test:vscode` whole when you touch `vscode/`. `test:fast` runs as `vitest --mode fast`, which drops the 12 slow test files *and* the 16 source files they are responsible for (`SLOW_TEST_FILES` / `SLOW_ONLY_SOURCES` in `cli/vite.config.ts`) so the 97/96/97/97 thresholds stay enforced instead of failing on an absent denominator. That coverage exclusion must never leak out of the `fast` branch — in the gate it would silently stop the floor from protecting `sync/` and `install/`. `test:slow` is `vitest --mode slow`, which **inverts the same list** into `include`; `SLOW_TEST_FILES` is therefore the single source of truth for both tiers (it used to be duplicated into the npm script, where a one-sided edit silently left a file in both tiers or neither). Also available: `npm run test:changed`, `npx vitest related --run <src file>`, or a single file + `-t "<case>"`. The tiers partition the suite exactly (324 + 12 = 336) — see [`cli/DEVELOPMENT.md`](cli/DEVELOPMENT.md) → "Two tiers: inner loop vs gate" for the profile and how to re-derive it. Caveat: `--changed` implies `passWithNoTests`, so it exits **0** when nothing matched — check the file count before reading that as a pass.
