...
| **Cursor CLI** (`cursor-agent`) | _(no hook)_ | Sessions discovered from Cursor's plaintext `~/.cursor` session store (`meta.json` + JSONL). Shares the `cursorEnabled` flag with the Composer IDE. |
...
**Local dashboard launcher** — the branch footer's **Dashboard** button (`jollimemory.openDashboard`) sends `jolli dashboard` to an **integrated terminal**; it does not run in the extension host. It used to, with three editor-shaped plugs — an output channel for the command's lines, an Electron-as-node spawner for the server, and `vscode.env.openExternal` for the URL — and every one of them made the command quieter than it is on a command line, while its longest phase (the history import) ran for minutes after the browser opened with the progress notification already gone. A terminal returns all three plugs to their command-line defaults and gives the user the one thing the in-process version could not: the command's own output, as it happens. It is also why `jollimemory.stopDashboard`, its `deactivate` call and its `package.json` declaration are **gone** — `jolli dashboard` binds the port in its own process and serves until Ctrl+C, and in a terminal the shell owns that process, so Ctrl+C *is* the stop.

What is left in [`DashboardLauncher.ts`](src/services/DashboardLauncher.ts) is one decision — which command line to send — in three tiers:

0. bare `jolli`, the readable spelling, used **only** when a globally-installed CLI both wins the `dist-paths` version competition and is at least as new as the core this bundle carries. Presence is not the gate: an ungated bare-`jolli` tier bypasses that competition, so a stale global CLI would shadow a newer bundled one and answer `unknown command 'dashboard'`. The competition is not reimplemented — `readWinningGlobalCliVersion` calls the same two functions `resolve-dist-path` mirrors, so tier 0 fires exactly when tier 1 would have chosen the global CLI anyway.
1. `~/.jolli/jollimemory/run-cli`, the machine-global dispatcher, which resolves the highest-versioned registered dist. This one tier covers both "the user has a global CLI" and "only the VS Code bundle exists" — which is why it, not tier 0, is the fallback that must always work.
2. the extension's own `dist/Cli.js`, run by the host's Electron under `ELECTRON_RUN_AS_NODE` — reached when `run-cli` is absent or the terminal's shell cannot execute a bash script.

Two costs are product decisions rather than oversights, and both are recorded in the source. Every click opens a **new** terminal and the command carries **no `--cwd`**, so a `cd` in the user's shell rc overrides `createTerminal`'s spawn directory and points `registerRepo` / `maybeAutoCutover` at whatever repo the shell landed in. And the line is sent into a shell that has not started yet, so it appears **twice** — the bytes reach a pty still in canonical mode, the line discipline echoes them above the first prompt, and the shell's line editor redraws them after it. One execution, printed twice. Both fixes were implemented, measured and reverted: waiting for `onDidChangeTerminalShellIntegration` does not help at all (it fires at zsh's `precmd`, still inside the echoing window), and waiting plus `shellIntegration.executeCommand` does fix the echo but took click-to-CLI-start from 1.0–3.9 s to 5.8–8.4 s, because that call additionally waits for VS Code's own command detection. Do not re-attempt either without new numbers.

**Wiki freshness banner** — the "N updates behind · rebuilt X ago" banner above the Memory Bank tree, and its **Rebuild** action, render a CLI-owned answer: `WikiFreshness` lives in `cli/src/core` so this panel and the web dashboard print the same numbers and the same warn threshold. Since 0.99.13 a commit no longer triggers an ingest (`wikiRebuild` defaults to `manual`), so this banner is the *only* thing telling the user the wiki has fallen behind — it is not decoration. It sits in a persistent `wikiBannerEl` slot, shows only on the `kb` tab, and a rebuild in progress takes precedence over the freshness snapshot ("Updating the wiki/graph…", keeping the warn tint if it had one). The dismissal is deliberately **not** a persisted boolean: closing it must not silence it while work keeps piling up. The webview snapshots the `{total, summary, severity, repos}` it was showing and sends that with `wiki:dismiss`; the host keeps it in **process memory only** and re-shows the banner once the backlog grows, gets more urgent, or reaches a repo that was not behind at dismiss time — and forgets the dismissal entirely once the wiki is fresh, so the next behind episode starts clean. A Reload Window also clears it, which is why no persistence is needed here: the host process's lifetime *is* the reset boundary. (The web dashboard has no such boundary, so it does the same job with a `wikiBannerNonce` in `localStorage`.)
...
**Token-usage bars** — all three token bars (sidebar Committed Memories, the NextMemory working panel, and the shared memory-detail palette) set exact percentages via an `el.style.width` property write rather than bucketed CSS width classes; a JS property write is allowed under the webview CSP even though an inline `style="width"` attribute is not (the same approach `SummaryScriptBuilder` uses). The denominator is the breakdown sum (input + output + cached), not the scalar total, so segments fill the bar exactly, with the cached segment absorbing the rounding remainder. `branch:tokenStats` is posted unconditionally — including `total === 0`, which the webview treats as the self-healing reset that hides a stale bar when you switch to an empty branch. The palette (input = green, output = blue, cached = gray) is unified across all three surfaces.
...
**Issue & page references in the Summary Webview** — When the AI conversation calls one of the fifteen recognised MCP servers (Linear / Jira / GitHub / Notion / Slack / Zoom meetings / Zoom docs / Confluence / Asana / monday.com / context7 / Jolli's own lookups / Vercel / Figma / Sentry), the referenced issues, pages, threads, meetings, deployments, design files, and error reports are extracted at commit time on the CLI side (`references/ReferenceExtractor` → `references/ReferenceStore`, an opaque `ReferenceField` bag so adding a source is a binding entry rather than a schema change). On the extension side `ReferenceService` surfaces them; they render grouped by source inside the **Plans & Notes** card (`summary.references`), and are embedded into the Markdown export (`SummaryMarkdownBuilder`) and the PR description (`SummaryPrMarkdownBuilder` / `SummaryPrAggregateMarkdownBuilder`). References are hoisted onto the consolidated root on squash / rebase by `QueueWorker.runSquashPipeline` exactly the same way Plans and Notes are, so the link follows the commit through history rewrites. For Claude Code these are extracted at commit time; for Codex (no commit-time hook) they are extracted on the sidebar's 60s polling tick — `vercel`, `figma`, `sentry` and `zoom-doc` carry no `match.codex`, so those four never resolve from Codex transcripts. Kimi resolves through `match.claude`, so the three of them carrying a generic `mcp__<server>__` prefix — `vercel`, `figma`, `sentry` — do reach it; `zoom-doc` (`mcp__claude_ai_Zoom_for_Claude__` only) is the one that is genuinely Claude-only. `vercel` / `figma` / `sentry` are additionally **track-only**: the extension renders, exports and shares them like any other reference, but the CLI never passes them to the summarize prompt (a build log or stacktrace is the *input* to the work and would read as a reason for the change), so do not add them to any host-side "what the model saw" surface.

**Skill usage in the sidebar and the Summary Webview** — Captured skill usage is surfaced by `SkillService` and rendered as **one aggregate row** wherever Context items are listed (the sidebar's live Context list via `SidebarWebviewProvider`, and `summary.skills` in `SummaryHtmlBuilder`). One row for however many skills, deliberately: the per-skill figures live in the archived `skills--<hash8>.md` table and no individual skill is addressable in the UI, so the Context chip counts skills as a single row. The relevance ranker is never fed skills, so a per-skill keep/drop verdict cannot exist and none is rendered. OpenCode has no hook, so its skills are read on the sidebar's polling tick rather than at commit time — skills describe how the work is happening *right now*, and surfacing them only at commit time would leave the live Context list empty for a whole session. Label formatting is shared with the CLI through `buildSkillsSummaryLabel` (`cli/src/core/SkillsAggregateMarkdown.ts`) so the two surfaces cannot drift.

**Per-repo outbound push control** — **Settings → Sync to Jolli** renders `pushControlList`, one immediate-apply toggle per repo Jolli tracks on this machine (no **Apply Changes** step, unlike every other settings row). State lives in the CLI's machine-global `push-control.json` via `PushControl` / `PushControlStore`; the extension never enforces the flag itself — enforcement sits at the CLI's push boundary, so a blocked repo is blocked for the pre-push hook and the manual **Share in Jolli** button alike. An unreadable store fails closed to OFF for every repo, and the UI must show the reason rather than a bare OFF.
