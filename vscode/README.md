...
* **Issue, page & conversation references** (Linear / Jira / GitHub / Notion / Slack / Zoom): any issues, tickets, pages, Slack threads, or Zoom meetings referenced in the AI conversation (via the corresponding MCP server) are extracted and rendered as first-class items — title, status / identifier where available, and a deep link back to the source. They follow the commit through squash / rebase the same way Plans and Notes do. For **Claude Code** these are extracted at commit time; for **Codex** (which has no commit-time hook) they are extracted on the sidebar's 60s polling tick.
