...
export function buildTranscriptEntriesScript(): string {
	...
	return [
		...
		"      // modal-tab is pure navigation (switches the active session panel) and is",
		"      // safe in every readonly mode — including foreign-readonly. The catch-all",
		"      // .page.foreign-readonly button:not([data-foreign-safe]) rule would hide",
		"      // these tabs and silently strip the session-switcher in cross-repo views",
		"      // (.session-delete-btn is a span, opted out separately via CSS).",
		"      tabsHtml += '<button class=\"modal-tab' + activeClass + '\" data-tab-key=\"' + groupOrder[t] + '\" data-foreign-safe>';",
