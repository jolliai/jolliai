...
function buildFullStatusItems(
	...
	// Anthropic API Key warning — shown ONLY when the user has explicitly
	// chosen `aiProvider: "anthropic"` but no key is resolvable (neither
	// `config.apiKey` nor `ANTHROPIC_API_KEY` in the env; the env case is
	// already reported green as "Anthropic (env) ✓" by the provider row above).
	// For any other choice the row is pointless: Jolli / Local Agent don't use
	// an Anthropic key, and when no provider is set the "AI Summary Provider"
	// row above already surfaces the "not configured" gap — a second
	// Anthropic-specific nag would wrongly imply Anthropic is the only option.
	if (
	...
	pushIntegrationItem(
		...
		undefined,
		"Codex Integration",
		"Codex sessions directory found — session discovery is enabled",
		"Codex detected but session discovery is disabled in config",
		undefined,
		counts.codex,
	);
	pushIntegrationItem(
		...
		"Gemini Integration",
		"Gemini AfterAgent hook installed — session tracking is enabled",
		"Gemini detected but session tracking is disabled in config",
		"Gemini detected but AfterAgent hook is not installed",
		counts.gemini,
	);
...
function pushProviderItem(
	...
	switch (source) {
		...
		case "jolli-proxy":
			...
			break;
		case "local-agent":
			description = `Local agent - ${localAgentToolLabel(config?.localAgentTool ?? "claude-code")}`;
			icon = ICON_OK;
			tooltip =
