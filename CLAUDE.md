...
2. **Git hooks** drive a unified queue under `.jolli/jollimemory/git-op-queue/`. `post-commit`'s enqueue + detached-`QueueWorker` spawn is still synchronous and <5 ms. What changed (0.99.0+, the desktop-cockpit work): after that fast path the hook **entry** may block to display live capture feedback — in an interactive TTY or AI-agent session (`commitFeedback: "auto"`, the default; GUI git clients set none of the gating TTY/agent-env signals and keep the original fast, silent return) it foreground-tails the detached worker's `capture-progress` file via `runCommitFeedback`/`watchCaptureProgress`, printing each milestone inline and holding `git commit` until the worker emits a terminal event, is detected dead (orphaned lock), or `DEFAULT_FEEDBACK_TIMEOUT_MS` (90 s) elapses. The worker runs detached regardless of whether the entry watches. (Edge case: if the worker never starts — e.g. an incomplete dist so `launchWorker` no-ops — no lock is ever written, so the dead-worker probe can't fire and the watch runs the full 90 s before printing "continues in the background".) The worker holds a 5-min file lock, drains entries in timestamp order, runs the LLM where needed, and chain-spawns a successor if new entries appear after it finishes. Squash entries (and rebase-squash) now go through the LLM-driven `generateSquashConsolidation` pipeline (`cli/src/hooks/QueueWorker.ts runSquashPipeline`) — the old "skip LLM, mechanical merge only" behavior is now the **fallback** when the consolidation call fails. Rebase-pick entries skip the LLM and just migrate hashes 1:1. `prepare-commit-msg` writes `squash-pending.json` so the worker recognizes squash before picking a consolidation strategy. See [`cli/DEVELOPMENT.md`](cli/DEVELOPMENT.md) for the queue rationale (each op gets its own file precisely because the previous single-slot pending files lost summaries during rapid amend/rebase sequences).
