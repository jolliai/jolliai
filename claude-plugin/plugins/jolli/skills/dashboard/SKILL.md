...
simply absent from the page). There is no MCP tool for this; the bundled CLI is the
only route.
...
dies with it. So start it detached, and use the Bash tool's background mode as
well when it is available.
...
another's URL) and something another user can pre-create as a symlink for this shell
to write through. Keep the printed `LOG <path>` — later steps need it, and it is not
derivable.
...
- **an empty log** — it may simply still be starting (a first run migrates the
  ...
  `LOG` first to the path printed as `LOG <path>` above, since every command runs in
  a fresh shell and the name is unique per launch — before concluding anything.

If `$HOME/.jolli/jollimemory/run-cli` does not exist at all, the bundled dispatcher
has not been written yet — the plugin's SessionStart hook is what writes it. Tell
the user to start a fresh Claude Code session (or run `/jolli:init`) and retry, then
stop; do not guess at other paths, and do not run `node` or `npx` against a
