...
| -- | -- | -- |
| **CLI** — `@jolli.ai/cli` | [`cli/`](cli/) | Standalone command-line tool. Installs git hooks, generates summaries on commit, and offers `view` / `export` / `recall` / `search` / `configure` / `doctor` / `mcp` commands. Works independent of any IDE. **Jolli Site** (`new` / `dev` / `build` / `start` / `convert`) for generating documentation sites from Markdown + OpenAPI is available as a separate `@jolli.ai/site-cli` plugin. |
...
- **Ask your AI agent about past work** — `jolli mcp` exposes your history to AI agents over the Model Context Protocol, so Claude Code can search memories, recall a branch, and trace how a decision evolved without leaving the conversation. Registered automatically when Jolli Memory is enabled.
