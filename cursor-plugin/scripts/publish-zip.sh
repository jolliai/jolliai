...
#
# Usage:
#   bash cursor-plugin/scripts/publish-zip.sh [output.zip]
#
set -euo pipefail
. "$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/_publish-lib.sh"

OUT="${1:-$HOME/Desktop/jolli-cursor-plugin-marketplace.zip}"
case "$OUT" in
	/*) ;;
	*) OUT="$PWD/$OUT" ;;
esac
...
# exist on their machine). Name the shape of the path instead.
