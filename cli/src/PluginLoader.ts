...
import { isPluginBundleBuild } from "./core/ClientHeader.js";
...
async function discoverKnownPlugins(opts?: LoadPluginsOptions): Promise<DiscoveryResult> {
	...
	// An AI-host plugin bundles this CLI purely to run `mcp` and
	...
	// surface never uses. Kill discovery for those bundles (same effect as
	// JOLLI_NO_PLUGINS=1). Gated on ANY plugin bundle, not just Claude's: a
	// second host embedding this CLI has the identical fixed surface, and keying
	// this off one host's kind would leak standalone-CLI behavior into the other.
	// `isPluginBundleBuild()` reads the build-define
