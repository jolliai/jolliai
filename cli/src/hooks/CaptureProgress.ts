...
import { isPidAlive, readLockOwnerPid } from "../core/LockPrimitives.js";
...
/**
 * Absolute path to a commit's capture lock (see {@link file://./CommitCaptureLock.ts}).
 * Lives here alongside the progress dir so a watcher can probe the worker's
 * liveness without importing CommitCaptureLock (which would be circular).
 */
 ...
 *   - `<sha256>.lock`  — {@link CommitCaptureLock}'s per-hash lock file. A
