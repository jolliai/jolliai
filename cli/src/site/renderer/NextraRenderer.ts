...
/**
 * Resolves the accent hue passed to `generateApiCss`. Precedence:
 * `colors.primary` > `primaryColor` > `primaryHue` > internal default (220).
 * External theme packs handle their own accent colours via their CSS builder.
 */
function resolveApiAccentHue(config: SiteJson): number | undefined {
	if (config.theme?.colors?.primary) {
		const hsl = hexToHsl(config.theme.colors.primary);
		if (hsl) return hsl.h;
	}
	...
	return config.theme?.primaryHue ?? undefined;
}
