...
import { SCOPE_PAGE_MAP_RUNTIME_SOURCE, SOCIAL_PLATFORMS, sanitizeUrl } from "@jolli.ai/site-core";
...
export async function initNextraProject(
	...
	await mkdir(join(buildDir, "plugins"), { recursive: true });
	await writeFile(
		join(buildDir, "plugins", "remark-admonitions-to-callout.mjs"),
		generateRemarkAdmonitionsPlugin(),
		"utf-8",
	);
...
const withNextra = nextra({
  contentDirBasePath: '/',
  mdxOptions: {
    remarkPlugins: [remarkDirective, remarkAdmonitionsToCallout],
  },
})
...
/**
 * Returns the contents of `plugins/remark-admonitions-to-callout.mjs` — a
 * remark plugin registered with Nextra's MDX pipeline that turns Docusaurus
 * `:::info` / `:::tip` / `:::warning` / `:::danger` / `:::note` / `:::caution`
 * / `:::important` container directives into Nextra `<Callout type="...">`
 * JSX elements.
 *
 * `remark-directive` (registered before this plugin in `next.config.mjs`)
 * parses `:::xxx` syntax into `containerDirective` AST nodes. This plugin
 * replaces those nodes with `mdxJsxFlowElement` `<Callout>` nodes. Unknown
 * directive names are left untouched so later remark plugins can still handle
 * their original `containerDirective` nodes.
 *
 * Standard label syntax `:::info[Title]` is preserved: the label child is
 * promoted to a bold paragraph at the top of the Callout. Legacy Docusaurus v2
 * title syntax `:::info Title` is normalized to `:::info[Title]` in
 * `ContentMirror.normalizeLegacyAdmonitionTitles` before the file reaches
 * Nextra.
 */
export function generateRemarkAdmonitionsPlugin(): string {
	return `// AUTO-GENERATED by jolli. Do not edit by hand; regenerated on every build.

import { visit } from 'unist-util-visit'

// Docusaurus admonition type -> Nextra Callout type. Nextra's Callout only
// supports the styles default / error / info / warning, so note (grey in
// Docusaurus) and tip (green) both fall back to the neutral default style,
// and important — which has no dedicated colour — falls back to info. caution
// is an alias of warning in Docusaurus (orange), not a red error.
const TYPE_MAP = {
  info: 'info',
  note: 'default',
  tip: 'default',
...
function isDirectiveLabel(node) {
  return node && typeof node === 'object' && node.data?.directiveLabel === true
}

function renderTitleAsStrongParagraph(children) {
  if (!Array.isArray(children) || children.length === 0 || !isDirectiveLabel(children[0])) {
    return children ?? []
  }

  const [label, ...rest] = children
  // Promote the directive label to a bold paragraph, dropping the leftover
  // directiveLabel marker — it only means something to remark-directive and is
  // inert once the node is a plain paragraph inside the Callout.
  const promoted = {
    ...label,
    children: [
      {
...
function directiveHtmlAttributes(attributes) {
  const result = []
  if (!attributes || typeof attributes !== 'object') return result

  const id = stringAttribute('id', attributes.id)
  if (id) result.push(id)

  const className = stringAttribute('className', attributes.className ?? attributes.class)
  if (className) result.push(className)

  return result
}

function makeCalloutNode(node, calloutType) {
  return {
    ...
    children: renderTitleAsStrongParagraph(node.children),
    position: node.position,
  }
}

function wrapWithDirectiveAttributes(node, child) {
  const attributes = directiveHtmlAttributes(node.attributes)
...
export default function remarkAdmonitionsToCallout() {
  return (tree) => {
    visit(tree, 'containerDirective', (node, index, parent) => {
      if (!parent || typeof index !== 'number') return

      const directiveName = typeof node.name === 'string' ? node.name.toLowerCase() : ''
      const calloutType = TYPE_MAP[directiveName]
      if (!calloutType) return

      parent.children[index] = wrapWithDirectiveAttributes(node, makeCalloutNode(node, calloutType))
    })
  }
}
`;
