...
describe("SiteJsonReader.readSiteJson", () => {
	...
	// ── $schema field is silently preserved ────────────────────────────────

	it("preserves $schema field without error", async () => {
		const { readSiteJson } = await import("./SiteJsonReader.js");
		const siteJson = {
		...
		expect(result.config.title).toBe("T");
		expect((result.config as Record<string, unknown>).$schema).toBe("https://jolli.ai/schemas/site-config.json");
	});
