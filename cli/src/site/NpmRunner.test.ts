...
describe("NpmRunner.runNpmDev", () => {
	...
	// 触发 stdout/stderr 的 data 事件，覆盖 child.stdout?.on / child.stderr?.on 的回调主体
...
// ─── Windows 平台分支 ─────────────────────────────────────────────────────────
//
// 模块顶层的 `IS_WIN = process.platform === "win32"` 与 `shellCmd` 中的三元判断
// 是常量，模块加载时一次性求值。要覆盖 IS_WIN === true 的分支，需要在 stub
// `process.platform` 之后通过 `vi.resetModules()` 强制重新加载模块。
describe("NpmRunner on Windows", () => {
	...
	it("joins cmd and args into a single string with shell: true for runNpmBuild", async () => {
		...
		const call = mockSpawnSync.mock.calls[0];
		// Windows 下应该是单字符串 "npm run build"，args 为空数组
