...
import { removeGitExcludePaths } from "./GitExclude.js";
...
export async function updateSkillsIfNeeded(
	...
	// Retire the Cursor per-repo mirror from here too, not only from the Cursor
	// bootstrap.
	//
	// The mirror is gone — the bundle ships all four host-neutral skills directly (see
	// {@link CURSOR_RETIRED_MIRROR_SKILLS}) — so what is left is a one-way sweep of what
	// earlier versions planted. It belongs on THIS path, host-neutral, for the reason the
	// reconcile did: a user who removes the Cursor plugin through Cursor's own UI runs no
	// Jolli code at that moment, and the Cursor bootstrap's own opt-in gate means an
	// un-opted-in repo never reaches it either. Every other host's session start does.
	//
	// Ownership-guarded per entry, so a repo that never had the mirror — or that has a
	// `.cursor/skills/jolli-recall` the user wrote — is untouched.
	//
	// The exclude lines come out with it, and BOTH callers need that. `Installer` runs
	// the same pair inside its `repoHooksOnly` branch, which never reaches this function;
	// this call site covers the full `jolli enable`, which never reaches that branch.
	// Leaving it to one of them would keep hiding `.cursor/skills/` from `git status` on
	// whichever path did not sweep — and that directory now belongs to the user.
	await removeCursorRepoSkills(projectDir);
	await removeGitExcludePaths(projectDir, CURSOR_RETIRED_SKILL_GIT_EXCLUDE_PATHS);
}
 ...
 */
export async function removeJolliOwnedSkillDir(skillDir: string, reason: string): Promise<void> {
...
/**
 * The four host-neutral skills a PREVIOUS version mirrored per-repo into
 * `.cursor/skills/`. Retained for exactly one purpose: knowing what to sweep away.
 *
 * The Cursor bundle now ships all four directly (`CURSOR_PLUGIN_SKILLS`), so nothing
 * writes this set any more — {@link removeCursorRepoSkills} is its only consumer.
 *
 * **Why the mirror was retired.** It existed to give Cursor's flat, un-namespaced menu
 * exactly one entry per skill name: `.agents/skills/` and a plugin bundle's `skills/`
 * land in the same pool and neither collapses the other, so bundling these four
 * doubled each entry in a repo that had run a full `jolli enable`. The cost of avoiding
 * that duplicate turned out to be higher than the duplicate: a Cursor-only user — the
 * one this bundle exists for — got NO recall or search at all, because the mirror was
 * planted by a bootstrap whose opt-in gate (`isGitHookInstalled`) is false in a fresh
 * repo. So the plugin's own core capability was absent from its store page and absent
 * from the menu until the user found `/jolli-init`. Four more failure modes came with
 * it, each silent: symlinks resolved through a `cursor-plugin-root` record that a
 * marketplace upgrade invalidates, a host-neutral reconcile that opened Cursor's
 * private `state.vscdb` from every Claude and Codex session, `.git/info/exclude`
 * upkeep for the planted paths, and a machine-global `/jolli` that any OTHER runtime
 * would delete during that same upgrade window. Duplication is now ACCEPTED: two
 * identically-named entries differing only by a brand icon is a cosmetic cost, and it
 * is paid only by users who also run another host.
 *
 * The `jolli` umbrella is deliberately not in this list — it was never mirrored
 * per-repo. It is written MACHINE-GLOBAL by `ensureCursorGlobalMenu`, which lives in
 * `CursorPluginSkills.ts` with the builder it writes.
 */
export const CURSOR_RETIRED_MIRROR_SKILLS: ReadonlyArray<SkillRegistration> = SKILLS.filter(
	(skill) => skill.name !== "jolli",
);

/**
 * Git-exclude paths the retired mirror used to need, kept so they can be REMOVED.
 *
 * Nothing writes `.cursor/skills/` any more, so an exclude line for it is no longer
 * inert bookkeeping — it is a leftover that hides a directory the user may now be
 * using for their own skills. {@link Installer} passes this set to
 * `removeGitExcludePaths` on every install, beside the mirror sweep itself.
 *
 * The containing directory keeps its own entry, and that is not redundant with the
 * per-skill ones: git reports an untracked DIRECTORY as a single `?? .cursor/` line
 * rather than walking into it, so the per-skill lines alone never suppressed the
 * parent (measured on a fresh repo after `/jolli-init`). An install that planted the
 * mirror therefore wrote both shapes, and both have to come back out.
 */
export const CURSOR_RETIRED_SKILL_GIT_EXCLUDE_PATHS: ReadonlyArray<string> = [
	`/${CURSOR_SKILLS_DIR.join("/")}/`,
	...CURSOR_RETIRED_MIRROR_SKILLS.map((skill) => `/${CURSOR_SKILLS_DIR.join("/")}/${skill.name}/`),
];

/**
 * Sweep away the retired Cursor per-repo mirror, whatever this repo received of it.
 *
 * Two callers, one behaviour. {@link updateSkillsIfNeeded} runs it as a MIGRATION on
 * every install path — the bundle ships these four now, so a leftover symlink is a
 * second entry for a name the menu already has, and after a marketplace upgrade it is
 * a *dangling* second entry. `uninstall` runs it as teardown, for the reason
 * `removePluginJolliMenu` exists: these files live in the REPOSITORY, so removing the
 * plugin through Cursor's own plugin manager cannot reach them.
 *
 * Both placements the retired mechanism produced are handled: the symlink it normally
 * planted, and the real directory it fell back to on Windows (where creating a symlink
 * needs Developer Mode). Ownership-guarded per entry, so a `.cursor/skills/jolli-recall`
 * the user wrote themselves is left alone.
 *
 * What this deliberately does not try to solve: a user who removes the plugin through
 * Cursor's UI runs no Jolli code at all, so nothing here executes for them. Harmless
 * now — a leftover symlink into a deleted bundle is dropped by Cursor's own scanner,
 * and the next session of ANY other Jolli host sweeps the entry itself.
 */
export async function removeCursorRepoSkills(projectDir: string): Promise<void> {
	...
	for (const skill of CURSOR_RETIRED_MIRROR_SKILLS) {
 ...
 */
export function isJolliOwnedSkill(content: string): boolean {
 ...
 * therefore revision **9** (above the standalone's current revision 8, and above
 * anything a legacy `.claude/` copy carries) so {@link installPluginJolliMenu}
