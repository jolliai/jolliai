	...
	function runCommandInPanel(argv: string[]): void {
		...
		void deps
			...
			.then(({ output, exitCode }) => {
				...
				// A successful run needs no `exit 0` footer — it's noise. Only surface a
				// non-zero exit (red), where the code is a useful failure signal.
				appendLines(exitCode === 0 ? outLines : [...outLines, { text: `exit ${exitCode}`, tone: "red" }]);
				setRunning(null);
				onComplete?.(argv);
			})
			.catch((e) => {
