/**
 * CommitMessageMerge — string-only merging of several commit messages into one.
 *
 * Used by the squash flows on every surface: the VS Code extension (Amend =
 * merge HEAD message + new AI message; Squash = merge N commit messages) and
 * the `jolli generate squash-message` bridge command, where it is both the
 * no-LLM-provider path and the fallback when the LLM call fails.
 *
 * Moved verbatim from `vscode/src/util/CommitMessageUtils.ts` so the logic has
 * a single home in `cli/src` (the VS Code module re-exports from here).
 */
...
export function mergeCommitMessages(messages: ReadonlyArray<string>): string {
	...
	if (commonPrefix) {
		const descriptions = messages.map((m) => m.substring(commonPrefix.length).trim());
	...
	if (ticketPrefixes.every(Boolean)) {
		...
		const tickets = ticketPrefixes.map((p) => TICKET_PATTERN.exec(p as string)?.[0]?.toUpperCase());
		...
		if (allSameTicket) {
			...
			const descriptions = messages.map((m, i) => m.substring((ticketPrefixes[i] as string).length).trim());
