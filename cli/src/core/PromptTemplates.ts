...
/* `decision-gist` lived here: it compressed a mined `decisions` field into one
   sentence for the dashboard's Decisions card, on demand at display time. The
   card now shows the decision's own topic title (JOLLI-2192), so the action had
   no consumer left and is retired rather than left as a template nothing sends
   (JOLLI-2209 — where it was also found to have no row in the backend's
   `llm_prompt_templates`, so every proxy-authenticated call 404'd). */
...
export const TEMPLATES: ReadonlyMap<string, PromptTemplate> = new Map<string, PromptTemplate>([
	["summarize", { action: "summarize", version: 3, template: SUMMARIZE }],
	["summarize-strict", { action: "summarize-strict", version: 3, template: SUMMARIZE_STRICT }],
