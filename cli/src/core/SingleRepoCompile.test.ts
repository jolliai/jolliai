...
vi.mock("./SummaryStore.js", () => ({
	...
	// Lock free: the write guard's inner orphan-write lock just runs its body.
...
describe("compileSingleRepo", () => {
	...
	it("rebuild resets the processed-set + topic index and purges orphan pages", async () => {
		...
		// Twice: the reset discards every page up front (an empty keep-list — writing
		// an empty index is not a reset on a store that synthesizes the index from the
		// pages), then the post-drain purge keeps the slugs still in the index.
		expect(purgeTopicPagesExcept).toHaveBeenCalledTimes(2);
		expect(vi.mocked(purgeTopicPagesExcept).mock.calls[0][0]).toEqual([]);
		expect(vi.mocked(purgeTopicPagesExcept).mock.calls[1][0]).toEqual(["keep"]);
	});
