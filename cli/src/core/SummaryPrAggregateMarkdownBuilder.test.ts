...
const mocks = vi.hoisted(() => ({
	...
	formatDate: vi.fn(),
	// pushFooter (real) now receives a synthetic summary and calls this to derive
	// the "· via <provider>" footer segment.
	formatProviderLabel: vi.fn(() => "Anthropic"),
...
describe("buildAggregatedPrMarkdown", () => {
	...
	it("appends the AI-provider identifier to the footer (via <provider>)", () => {
		const summaries = [makeSummary({ hash: "AAAA1234" }), makeSummary({ hash: "BBBB5678" })];
		const md = buildAggregatedPrMarkdown(summaries, 0);
		// The aggregate path previously called pushFooter() with no summary,
		// dropping the provider segment the single-summary path already had.
		expect(md).toContain("Generated by Jolli Memory");
		expect(md).toContain("· via Anthropic");
		// Provider is derived across ALL commits: pushFooter receives a synthetic
		// root whose children are the aggregated summaries.
		const calls = mocks.formatProviderLabel.mock.calls as unknown as Array<[{ children?: unknown }]>;
		expect(calls.at(-1)?.[0]?.children).toEqual(summaries);
	});
