...
vi.mock("./MemoryBankScanner.js", () => ({
	listUserKnowledge: vi.fn().mockResolvedValue([]),
}));
...
const mockListUserKnowledge = vi.mocked(listUserKnowledge);
...
describe("listBranchCatalog", () => {
	...
	it("tolerates catalog entries with no topics field (?? [] fallback)", async () => {
		mockGetIndex.mockResolvedValue(
			makeIndex([
				{
					commitHash: "h1",
		...
		// `topics` is absent on the catalog entry → the `cat.topics ?? []` guard
		// must kick in instead of throwing on `.map`.
		mockGetCatalog.mockResolvedValue({
			version: 1,
			entries: [{ commitHash: "h1" } as unknown as { commitHash: string; topics: { title: string }[] }],
		});
		const catalog = await listBranchCatalog("/test");
...
describe("compileTaskContext", () => {
	...
	it("period reflects the loaded summaries, not the full index, when summaries are missing", async () => {
		// Orphan-branch hole: the index spans Jan–Dec but only the middle summary
		// loads. period must match the loaded content (commitCount), otherwise the
		// header reads "1 commit, Jan 1 – Dec 31" while the single loaded summary
		// only covers June.
		mockGetIndex.mockResolvedValue(
			makeIndex([
				{
				...
				},
				{
					...
					parentCommitHash: null,
					commitMessage: "June",
					commitDate: "2026-06-15T00:00:00.000Z",
					branch: "feature/test",
				...
				},
				{
					commitHash: "dec",
					parentCommitHash: null,
					commitMessage: "December",
					commitDate: "2026-12-31T00:00:00.000Z",
					branch: "feature/test",
					generatedAt: "2026-12-31T00:00:00.000Z",
				},
			]),
		);
		// Only the June summary is loadable; the Jan/Dec edges return null.
		...
		const ctx = await compileTaskContext({ branch: "feature/test" }, "/test");
		expect(ctx.commitCount).toBe(1);
		expect(ctx.period.start).toBe("2026-06-15T00:00:00.000Z");
		expect(ctx.period.end).toBe("2026-06-15T00:00:00.000Z");
	});
...
// ─── raw-summary path — no compiled cache ────────────────────────────────────

describe("compileTaskContext — raw-summary path (no compiled cache)", () => {
	beforeEach(() => {
		vi.clearAllMocks();
	});
	...
	it("returns raw-summary context with no compiledTopics field", async () => {
		...
		const ctx = await compileTaskContext({ branch: "main" }, "/test-cwd");
...
describe("renderContextMarkdown", () => {
	...
	it("renders the Memory Bank Knowledge section when userKnowledgeTopics is non-empty (spec 108 Correction 1)", () => {
		const summary = makeSummary();
		const ctx = {
			branch: "feature/test",
			period: { start: "2026-03-28T10:00:00.000Z", end: "2026-03-28T10:00:00.000Z" },
			...
			userKnowledgeTopics: [
				{
					...
					content: "Use PKCE",
					source: "jolliai/feature-test/auth.md",
					scope: "branch" as const,
				},
				{ title: "Inbox", content: "global thought", source: "Inbox.md", scope: "global" as const },
			],
			stats: {
				topicCount: 1,
				planCount: 0,
		...
		const md = renderContextMarkdown(ctx);
		expect(md).toContain("## Memory Bank Knowledge");
		expect(md).toContain("### Auth decisions [branch: jolliai/feature-test/auth.md]");
...
describe("buildRecallPayload", () => {
	...
	it("pre-pass 2 leaves a topic untouched when it already lacks response (>8 commits)", () => {
		// 9 commits → tier fires. One topic genuinely lacks `response`, so the
		// pre-pass 2 map must early-return it (the `t.response === undefined`
		// branch) without flipping `truncated` for it; the other commits' topics
		// do have response and get stripped.
		const summaries = Array.from({ length: 9 }, (_, i) =>
			makeSummary({
				...
				topics: [
					i === 0
						? {
								...
								trigger: "t",
								// biome-ignore lint/suspicious/noExplicitAny: deliberately omitting response for tier early-return coverage
								response: undefined as any,
								decisions: `dec-${i}`,
								importance: "major",
							}
		...
		const payload = buildRecallPayload(makeCtx({ summaries, commitCount: 9 }), 100_000);
		expect(payload.commits).toHaveLength(9);
		// Every topic ends up without response (the present ones stripped, the
		// absent one left as-is).
		expect(payload.commits.every((c) => c.topics[0].response === undefined)).toBe(true);
		// At least one commit DID carry response → truncated flips true.
		expect(payload.truncated).toBe(true);
	});

	it("pre-pass 2 does not flip truncated when no commit had a response to drop (>8 commits)", () => {
		// 9 commits → tier fires, but EVERY topic already lacks response, so the
		// map's `anyDropped` stays false and `truncated` is never set by the tier.
		const summaries = Array.from({ length: 9 }, (_, i) =>
			makeSummary({
				commitHash: `aaaa1111000000000000000000000000000${String(i).padStart(4, "0")}`.slice(0, 40),
				topics: [
					{
						title: `T${i}`,
						trigger: "t",
						// biome-ignore lint/suspicious/noExplicitAny: deliberately omitting response so the tier finds nothing to drop
						response: undefined as any,
						decisions: `dec-${i}`,
						importance: "major",
					},
		...
		const payload = buildRecallPayload(makeCtx({ summaries, commitCount: 9 }), 100_000);
		expect(payload.commits).toHaveLength(9);
		// Nothing was dropped by the tier and budget is wide → not truncated.
		expect(payload.truncated).toBeUndefined();
	});
...
describe("compileTaskContext — recursive plan/note collection", () => {
	...
	it("surfaces user knowledge topics, derives H1 titles, and falls back to basename", async () => {
		mockGetIndex.mockResolvedValue(
			makeIndex([
				{
					commitHash: "abc12345def67890",
		...
		mockGetSummary.mockResolvedValue(makeSummary());
		// Three files exercise deriveUserKnowledgeTitle's three branches:
		//  - leading "# H1" → title from heading
		//  - no heading, path with "/" → basename (strip .md)
		//  - no heading, path without "/" → whole name (strip .md)
		mockListUserKnowledge.mockResolvedValueOnce([
			{
				path: "jolliai/feature-test/auth.md",
				absolutePath: "/mb/jolliai/feature-test/auth.md",
				scope: "branch",
				branch: "feature/test",
				fingerprint: "f1",
				content: "#  Auth Decisions \n\nUse PKCE",
				mtime: "2026-03-28T10:00:00.000Z",
			},
			{
				path: "jolliai/notes/scratch.md",
				absolutePath: "/mb/jolliai/notes/scratch.md",
				scope: "repo",
				fingerprint: "f2",
			...
			{
				path: "Inbox.md",
				absolutePath: "/mb/Inbox.md",
				scope: "global",
				fingerprint: "f3",
				content: "global thought",
				mtime: "2026-03-28T10:00:00.000Z",
			},
		]);

		const ctx = await compileTaskContext({ branch: "feature/test" }, "/test");
		expect(ctx.userKnowledgeTopics).toBeDefined();
		const topics = ctx.userKnowledgeTopics ?? [];
		expect(topics).toHaveLength(3);
		// H1 heading wins, trimmed.
		expect(topics[0].title).toBe("Auth Decisions");
		expect(topics[0].source).toBe("jolliai/feature-test/auth.md");
		expect(topics[0].scope).toBe("branch");
		// No heading, nested path → basename without .md.
		expect(topics[1].title).toBe("scratch");
		// No heading, flat path → name without .md.
		expect(topics[2].title).toBe("Inbox");
	});

	it("skips user knowledge entirely when includeUserKnowledge is false", async () => {
		mockGetIndex.mockResolvedValue(
			makeIndex([
				{
					commitHash: "abc12345def67890",
		...
		mockGetSummary.mockResolvedValue(makeSummary());
		mockListUserKnowledge.mockClear();

		const ctx = await compileTaskContext({ branch: "feature/test", includeUserKnowledge: false }, "/test");
		expect(mockListUserKnowledge).not.toHaveBeenCalled();
		expect(ctx.userKnowledgeTopics ?? []).toHaveLength(0);
	});

	it("defaults cwd to process.cwd() when omitted (still scans user knowledge)", async () => {
		mockGetIndex.mockResolvedValue(
			makeIndex([
				{
					commitHash: "abc12345def67890",
		...
		// No cwd argument → loadUserKnowledgeTopics falls back to process.cwd().
		const ctx = await compileTaskContext({ branch: "feature/test" });
		expect(ctx.commitCount).toBe(1);
		expect(mockListUserKnowledge).toHaveBeenCalledWith(process.cwd(), "feature/test");
	});
