 ...
 *
 * SHARED BY BOTH IDE HOSTS — see the PlanService header for the contract. In
 * particular `removeNote`'s "delete the backing file only when it lives inside
 * `.jolli/jollimemory/`" rule is load-bearing: an IntelliJ re-implementation
 * used a different predicate (uncommitted + snippet) and therefore deleted a
 * different set of files than VS Code did for the same user action.
 */
...
import type { NoteEntry, NoteFormat, NoteInfo, NoteReference } from "../Types.js";
...
import { storeNotes } from "./SummaryStore.js";
...
export async function detectNotes(cwd: string): Promise<Array<NoteInfo>> {
	...
	log.info("detectNotes found %d notes (%d in registry)", result.length, Object.keys(notes).length);
	return result;
}
...
export async function saveNote(
	...
	log.info("saveNote: %s %s (%s)", id ? "updated" : "created", noteId, format);
...
export async function removeNote(id: string, cwd: string, expectedCommitHash?: string): Promise<void> {
	...
	if (entry.sourcePath && isPathInside(entry.sourcePath, getJolliMemoryDir(cwd)) && existsSync(entry.sourcePath)) {
		try {
			unlinkSync(entry.sourcePath);
			log.info("Deleted note file: %s", entry.sourcePath);
		} catch {
			/* ignore — file deletion is best-effort */
		}
	}

	log.info("Removed note %s from registry", key);
}
...
export async function archiveNoteForCommit(
	...
	const contentHashAtCommit = createHash("sha256").update(noteContent).digest("hex");
	...
	if (entry.format === "snippet" && entry.sourcePath && existsSync(entry.sourcePath)) {
		try {
			unlinkSync(entry.sourcePath);
			log.info("Cleaned up snippet file: %s", entry.sourcePath);
		} catch {
			/* ignore — cleanup is best-effort */
		}
	}

	log.info("Archived note %s → %s for commit %s", id, newId, shortHash);
...
function toNoteInfo(entry: NoteEntry): NoteInfo | null {
	...
	if (entry.contentHashAtCommit) {
		...
		if (
			currentContent === null ||
			createHash("sha256").update(currentContent).digest("hex") === entry.contentHashAtCommit
		) {
			return null;
		}
	}
	...
	if (entry.commitHash === null && entry.sourcePath && !existsSync(entry.sourcePath)) {
		return null;
	}
	...
	if (entry.format === "markdown" && entry.commitHash === null && entry.sourcePath && existsSync(entry.sourcePath)) {
