...
/**
 * Entry 0 — the schema as it first landed. The intermediate shapes it went through
 * were this branch's own development history, not a compatibility contract with
 * anyone's installed database, and were collapsed into it rather than carried as
 * dead upgrade steps. See `migrations/index.ts`'s docblock for the rules the whole
 * list — this entry included — must follow.
 *
 * Its bytes are frozen and SHA-pinned in `MigrationFingerprints.test.ts`: every
 * database on earth has already applied this one. See the `commit_branches` note
 * below, and the rest of this file, before touching a character in here.
 *
 * This entry is split in two SQL sections because the halves have different
 * natures, and confusing them is how data gets lost: the ACTIVITY section is a
 * projection of git and each agent's storage and can be rescanned, while the
 * MEMORY section is the only copy there is.
 *
 * Three properties of the memory section are load-bearing and easy to undo by
 * accident:
 *
 ...
 *    `SOT_INSPECTION_QUERIES` (`SotSchema.ts`). The previous version carried nine
 *    triggers (revision monotonicity, pointer validation, two FTS mirrors, cascade
 ...
 * built for, and that comment is FROZEN: this constant is `MIGRATIONS[0]`
 * (`BASELINE_DDL`), whose bytes are SHA-pinned in `MigrationFingerprints.test.ts`.
 * Editing a character in there fails that check on the author's own machine before
 * it can ship — there is no runtime byte-compare any more (see the note at the end
 * of `verifyMigrationLog` in `DashboardDb.ts`), so an edit that slipped past CI
 * would not be caught on a user's machine either; it would just silently leave
 * every existing database's logged `schema_migrations.ddl` holding the older text
 * forever. So the current behaviour is documented here, outside the frozen text.
 *
...
export const BASELINE_DDL: DbMigration = sqlMigration(
	"BASELINE_DDL",
	ACTIVITY_SQL +
		`
