...
function render(
	...
	const model = {
			...
			selected:
				opts.selected === false
					...
					: {
							...
							topics,
							files: [],
							// When Jolli wrote THIS memory. Deliberately a different
							// instant from the model's stamp below, so the footer case
							// proves which of the two it reads.
		...
		scope: { kind: "all" },
		// When the PAGE's payload was built — "now" in production, and the wrong
		// answer for the footer. Fixed here so every case renders a stable footer
		// rather than one that changes between runs.
		generatedAtMs: Date.parse("2026-08-19T10:49:00Z"),
		timeZone: "UTC",
	};
...
/**
 * The reading pane's closing footer — the privacy note plus the product
 * signature the editor panel has always printed and this page did not.
 *
 * The count is the thing worth pinning, and it is the SAME number the pane shows
 * twice already — the Conversations section header and the counts line. It
 * shipped once as a count of transcript FILES, on the argument that the editor
 * panel counts that population: wrong, because a file is not a unit this page
 * ever displays, so a real memory holding six sessions in two files printed
 * "Conversations · 6" directly above "(2)".
 *
 * The NOUN is pinned with it. The editor's wording states the figure as a number
 * of transcripts (`transcripts (N)`), which under this page's population would
 * claim six transcripts for a memory that has two files — so the sentence names
 * conversations instead. See JOLLI-2245 for the editor half.
 */
describe("memories.js — detail footer", () => {
	it("counts the conversations the pane itself lists, and names them", () => {
		const rows = [
			{ source: "claude", sessionId: "s1", title: "one", messageCount: 3 },
			{ source: "codex", sessionId: "s2", title: "two", messageCount: 4 },
		];
		const { html } = render([], rows, []);
		expect(html).toContain(
			"Full transcripts for all 2 conversations stay in your repo — never included in shared exports.",
		);
		// The section header is the number a reader can check by counting rows, so
		// the note has to be that number and not one derived some other way.
		expect(html).toContain("Conversations · 2");
		// The editor's noun would make this figure a transcript count, which is the
		// unit this page never shows.
		expect(html).not.toContain("conversation transcripts (2)");
	});

	it("does not claim a count for a memory that lists no conversation", () => {
		// Reachable, not degenerate: `buildConversations` returns nothing when a
		// memory links no readable transcript. "all 0 conversations" is the same
		// empty claim that keeps the footer off the no-selection placeholder, so the
		// figure drops out and the sentence stands on its own.
		const { html } = render([], [], []);
		expect(html).toContain("Conversation transcripts stay in your repo — never included in shared exports.");
		expect(html).not.toContain("all 0");
		// The footer itself is still there — the note is true, and the generation
		// stamp below it is about the memory, which does exist.
		expect(html).toContain("mem-detail-foot");
	});

	it("keeps the sentence plural for a single conversation", () => {
		// One conversation can still be several transcript FILES — an amend chain
		// slices one session across three — so "transcripts" is not a slip.
		const { html } = render([], [{ source: "claude", sessionId: "s1", title: "one", messageCount: 3 }], []);
		expect(html).toContain("Full transcripts for this conversation stay in your repo");
		expect(html).not.toContain("all 1 conversation");
	});

	it("stamps the MEMORY's generation time, not the page's", () => {
		// The page's own stamp is "now", so reading it would date every memory ever
		// recorded to the current minute and change the line on each 30 s refresh
		// with nothing under it changing.
		const { html } = render([], [], []);
		expect(html).toContain("Generated by Jolli Memory · August 14, 2026 at 3:22 AM");
		expect(html).not.toContain("August 19, 2026");
	});

	it("names the provider when the summary recorded one", () => {
		const { html } = render([], [], [], { detail: { provider: "Anthropic" } });
		expect(html).toContain("· via Anthropic");
	});

	it("omits the provider segment rather than printing 'via unknown'", () => {
		// Summaries written before `llm.source` existed carry no provider at all.
		const { html } = render([], [], []);
		expect(html).toContain("Generated by Jolli Memory");
		// Scoped to the segment's own markup — a bare "via" would match prose
		// anywhere else on the pane and pass for the wrong reason.
		expect(html).not.toContain("mem-generated-via");
	});

	it("renders no footer when no memory is picked", () => {
		// The placeholder is not a memory — a privacy note about "(0) transcripts"
		// under it would be a claim about nothing.
		expect(renderNoSelection().html).not.toContain("mem-detail-foot");
	});
});
