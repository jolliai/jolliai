...
((JD) => {
	...
	/* The pane's closing footer: a transcript-privacy note above the product
	   signature, the same pair the editor panel prints (`buildFooter` in
	   vscode/src/views/SummaryHtmlBuilder.ts). It was missing here, so the one
	   surface a user can open in a browser — and therefore the one that looks
	   most like something shareable — was also the one that never said the
	   transcripts stay local.

	   The count is `conversations.length` — the SAME number this pane already
	   prints twice, in the Conversations section header and in the counts line
	   under the title. It shipped once as a count of transcript FILES instead, on
	   the argument that the editor panel counts that population and the two
	   surfaces should agree: wrong, because a file is not a unit this page ever
	   shows. One memory here stores six sessions in two files, so the section
	   read "Conversations · 6" and the sentence directly below it read "(2)" —
	   a number the reader can neither verify nor interpret. A count in a sentence
	   the reader is meant to trust has to be one they can check by counting the
	   rows above it; the editor printing a different figure is a bug there
	   (JOLLI-2245 — it labels the FILE count "conversations" in three places).

	   So the NOUN counts conversations too. The editor's wording — "Full
	   conversation transcripts (N)" — reads as N transcripts, which is the unit
	   this page deliberately does not use: with six sessions in two files it would
	   claim six transcripts where there are two. Naming the conversations instead
	   leaves the sentence true under either population and keeps the figure the
	   one a reader can check.

	   Zero conversations drops the figure rather than printing "all 0": the
	   sentence is still true (the transcripts, of which there are none, are not
	   exported) but a count of nothing is the same empty claim that keeps the
	   footer off the no-selection placeholder entirely.

	   The stamp is THIS MEMORY's own `generatedAtMs` — when Jolli wrote it. Not
	   the clock, and not the model's `generatedAtMs` beside it: both of those are
	   "now", so every memory ever recorded would be stamped with the current time
	   and the line would change on each 30 s refresh with nothing under it
	   changing. (The editor panel builds its footer from `new Date()` because it
	   is assembled per render; this pane has the memory's own stamp on the wire,
	   so it uses it.)

	   `provider` is absent on summaries written before `llm.source` existed; the
	   segment is then omitted rather than printed as "via unknown". */
	function detailFooter(model, detail) {
		var count = detail.conversations.length;
		/* One conversation can still be several transcript FILES (an amend chain
		   slices one session across three), so the plural "transcripts" is right
		   even at a count of one — and the verb stays `stay` in all three cases. */
		var subject = "Conversation transcripts";
		if (count === 1) subject = "Full transcripts for this conversation";
		else if (count > 1) subject = "Full transcripts for all " + count + " conversations";
		return (
			'<div class="mem-detail-foot">' +
			'<p class="mem-privacy"><span aria-hidden="true">🔒</span> ' +
			subject +
			" stay in your repo — never included in shared exports.</p>" +
			'<div class="mem-generated"><span>Generated by Jolli Memory · ' +
			JD.esc(JD.fullDateTime(detail.generatedAtMs, model.timeZone)) +
			"</span>" +
			(detail.provider ? '<span class="mem-generated-via"> · via ' + JD.esc(detail.provider) + "</span>" : "") +
			"</div></div>"
		);
	}
