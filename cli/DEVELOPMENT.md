...
| [dashboard/ServerEntry.ts](src/dashboard/ServerEntry.ts) | `dist/DashboardServerEntry.js` | The detached process `jolli dashboard` spawns. "`dist` contains `DashboardServerEntry.js`" is a build contract asserted in `cli/vite.config.ts`, same as `QueueWorker` |
...
Everything under [`cli/src/dashboard/`](src/dashboard/) serves one machine-global SQLite database, `~/.jolli/jollimemory/jollimemory.db`. `DashboardCommand` runs the import sweep and then spawns the detached server entry (`dist/DashboardServerEntry.js`); [DashboardServer.ts](src/dashboard/DashboardServer.ts) is the loopback-only HTTP service behind it, with a mutation token on every state-changing route. Pages: `/dashboard` (overview + standup), `/memories`, `/decisions`, `/repositories`, `/knowledge` (wiki browsing), `/graph` (the same graph runtime the export uses), plus the Settings modal ([SettingsPageQuery.ts](src/dashboard/SettingsPageQuery.ts) / [SettingsMutations.ts](src/dashboard/SettingsMutations.ts)).

Four rules are easy to break and each has already cost something:

- **GET never writes, and the server never migrates the schema.** Renders open the database read-only; producers write it directly through [StatsWriter.ts](src/dashboard/StatsWriter.ts), so whether the server is running has no effect on capture. This is the one long-lived process whose build can lag (a launcher reuses a recorded server after probing `/health`, which reports no version), so it must never be the process that upgrades the schema. The daily snapshot used to fire from `startDashboardServer` — and `opportunisticSnapshot` opens a *writable* handle, which migrates — which is why both halves of the backup schedule now live in processes that already write.
...
- **Backfill is not part of the request surface.** [DbBackfill.ts](src/dashboard/DbBackfill.ts)'s import sweep is the command process's startup step, and generating memories stays `jolli backfill`. The one browser-reachable exception is Settings → Generate Missing Summaries, added for parity with the VS Code panel and serialised by an in-flight guard — without it, a refresh or a second tab would start a second backfill over the same commits and pay for every summary twice.
...
| [GlobalDaemon.ts](src/daemon/GlobalDaemon.ts) (`global-daemon`, hidden) | machine + user | **no idle timeout** — only a retire, a lost bind race, or shutdown | Runs work that must happen when nobody is working: today the daily `jollimemory.db` snapshot, which every other trigger only reaches opportunistically. |
