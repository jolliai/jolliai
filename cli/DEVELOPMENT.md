...
**The version watershed.** `AGENT_DIMENSION_SINCE_VERSION` ([TelemetryAgent.ts](src/core/TelemetryAgent.ts), currently `"0.99.14"`) is the first client version that emits `agent`, printed into the public [TELEMETRY.md](../TELEMETRY.md) so agent-sliced charts start there. `TelemetryAgent.test.ts` asserts it is never *below* `cli/package.json`'s version — the guard exists because a hand-written prediction of "which release" can go silently stale ([0569fb75b]).
