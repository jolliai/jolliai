...
| [core/localagent/](src/core/localagent/) | The `local-agent` AI-provider backend, selected by `aiProvider: "local-agent"`. `LocalAgentRunner` drives a locally-installed agent CLI to generate a summary instead of calling an API; `BackendRegistry` maps `localAgentTool` (v1: only `claude-code`) to a backend; `ClaudeCodeBackend` builds the invocation and `ClaudeExecutableResolver` locates the binary (honoring `localAgentPath`, else `PATH` discovery). Surfaces as the `local-agent` value in `LlmCredentialSource`. |
...
The server exposes nine built-in tools, all pure handlers in [McpTools.ts](src/mcp/McpTools.ts):
