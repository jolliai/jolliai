...
The write is **gated on an explicit user decision** (0.99.6). A tri-state `globalInstructions` config switch — `undefined` (undecided) / `"enabled"` / `"disabled"` — is resolved by `resolveGlobalInstructionsDecision`: interactive `jolli enable` prompts once and persists the answer, while `-y`, non-interactive, and VS Code auto-enable stay undecided (no write) until the user confirms. Opting out is not just "don't write" — `removeGlobalInstructions` strips the block from every host (ungated), and both the CLI decision path and the VS Code disable transition funnel through the shared `syncGlobalInstructions` helper, which writes or removes with host-gating in one place. The prompt wording is a single `GLOBAL_INSTRUCTIONS_PROMPT` string shared across the CLI prompt, the VS Code notification, and the settings toggle so it can't drift between surfaces.
