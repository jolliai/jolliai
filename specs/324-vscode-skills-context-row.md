...
- `jollimemory.openSkillsAggregate` (live) and `jollimemory.previewCommittedSkills` (committed), the tab title each gives its preview, and why neither opens a file on disk.
...
**Out of scope:**
- The `plans.json.skills` registry, `SkillEntry`, `archivedTotals`, `uncommittedDelta`'s own definition, and the shared projection this panel's read path delegates to — its delta filter, its sort, and its counters-from-the-delta / timestamps-from-the-row rule (all owned by spec 319).
- The virtual-document scheme both open commands render into — its URI shape, its body cache, and how a preview tab survives a window reload (owned by spec 329).
...
### `SkillInfo` — an alias, not a declaration

`SkillInfo` in [`vscode/src/Types.ts`](../vscode/src/Types.ts) is a **type alias of the CLI projection's `ActiveSkill`** (spec 319), not a shape declared here. The same rows are served to the JVM host over a bridge operation, so the shape belongs next to the projection that builds it; re-declaring it in this package would let the two drift silently — a field this panel reads could disappear from the projection and still typecheck.

What it carries is therefore spec 319's contract: a discriminated `kind: "skill"`, `mapKey` (the `plans.json.skills` key, `<source>:<skill>`), `source`, `skill`, optional `plugin`, `entryPaths`, `invocationCount`, `firstUsedAt`, `lastUsedAt`, optional `usage`, `sourcePath`, optional `detection: "heuristic"`, and `lastModified` mirroring `lastUsedAt` so a skill sorts against plans / notes / references in one list, exactly the way `ReferenceInfo` mirrors `updatedAt`. `usage` is **absent, never zero**, when the source could not attribute tokens.

It is structurally assignable to spec 323's `SkillTableRow`, which is what lets the live document and the committed file share one renderer.
...
### Read path — `detectSkills` is a one-line delegate

[`vscode/src/core/SkillService.ts`](../vscode/src/core/SkillService.ts) is a shim: `detectSkills(cwd)` returns `projectActiveSkills(cwd)` and does nothing else. The module survives only so the rest of the extension keeps importing the name it already uses.

Everything that used to be described here — walking `registry.skills`, skipping a row whose `uncommittedDelta` is `undefined`, the newest-first sort, and the rule that the **counters** come from the delta while the **timestamps** come from the row — now lives in the CLI (spec 319) and is shared verbatim with the JVM host, which reaches the same function over a bridge operation (spec 336).

That move is the point rather than a refactor: this panel is where a hand-copied version of the delta rule drifted out of sync and hid every re-used skill, and a second host asking the same question is exactly how a second copy would have appeared. What remains true for this surface is the consequence — the list is what the *next* commit will carry, not the running total, and a skill that was archived and never re-entered is absent from it even though its registry row still exists.
...
The `mouseover` listener reaches this dispatch for **any** Context row carrying an id — it applies no per-kind gate of its own, and only skips a hover that started inside the row's inline-action cluster (those buttons own their own feedback, so the card is dismissed instead of stacking on a button tooltip). `lookupBranchHoverById` returns `{ kind: "skills", hover }` when the serialized row carries `skillsHover`, and the renderer dispatch is **exhaustive** rather than falling through to the reference renderer — a new kind reaching that default would render with the wrong card and read `h.source` / `h.url` off a payload that has neither.
...
| Command | Source of rows | Preview tab title | Empty case |
|---|---|---|---|
| `jollimemory.openSkillsAggregate` | `bridge.listSkills()` — the working registry | `Skills used — uncommitted` | information message: "No skills have been captured for this working session yet." |
| `jollimemory.previewCommittedSkills(commitHash, …)` | `bridge.getSummaryAnyRepoWithSource(commitHash)` → `summary.skills` | `Skills used — <hash8>` | information message: "This memory has no archived skill usage." |

Neither opens a file on disk, and **neither opens an untitled document any more**. Both render their Markdown into the in-memory snapshot preview scheme (spec 329) and show the *rendered* table: the live command under the `skills-live` namespace, the committed one under `skills` keyed by the commit hash. The two distinct titles are what makes the before-commit and after-commit tabs read as a pair rather than as two anonymous buffers, and the namespaces are what let a reopened live table refresh in place while the session keeps adding rows. Everything about how the scheme re-renders a tab after a window reload is spec 329's.
...
- **Skills filter on read; references do not** — and that filter is no longer this package's. The difference is a lifecycle difference (deleted-on-commit versus guarded-on-commit), not an optimisation, and collapsing the two read paths would resurrect every skill ever used onto the working panel. This surface now inherits the rule from the CLI rather than restating it, which is what stops a second host from arriving with a third version of it. (Surprising if the two kinds are assumed symmetric because both are auto-captured.)
- **The rows this panel renders carry the DELTA's counters and the ROW's timestamps.** Mixing them either way is wrong: the row's counters overstate a re-used skill, and the delta has no time fields to read. The split matches exactly what archival stamps, so the preview and the commit agree. Decided in the CLI (spec 319); recorded here because every figure on the row and in its hover card is a consequence of it.
- **`SkillInfo` is an alias, so a projection change is a compile error here rather than a silent one.** The panel deliberately does not own its own row shape — the same rows go to another host, and a locally re-declared copy could quietly diverge from what is actually sent.
- **The hover card reaches the skills row, and it took deleting a second list of kinds to get there.** The Context tab's `mouseover` listener carries **no `data-context` allowlist**. One used to sit at that line naming `plan` / `note` / `reference`, which made it a second enumeration of hoverable kinds beside the renderer dispatch — and it was the one that went stale, so the `skills` row returned before ever reaching its own card. The hover-payload lookup already answers "does this row have hover data", so it is the only list that needs maintaining. The `mouseout` listener is deliberately un-gated for the same reason: an asymmetric pair would leave a kind able to open a card and unable to close it. Leaving a row that never had a card is a harmless no-op.
- **`SkillsGroupItem.tooltip` was never the fallback for that, and still is not.** The extension contributes only a **webview** view (`jollimemory.mainView`), with no native `TreeView`, so a `SerializedTreeItem.tooltip` MarkdownString is inert for every kind — the hover card is the whole hover affordance.
...
- The `plans.json.skills` registry, `SkillEntry`, `archivedTotals`, `uncommittedDelta` and the shared `projectActiveSkills` / `ActiveSkill` projection this panel delegates to are owned by spec 319.
- The rendered-preview scheme both open commands use — URI shape, body cache, post-reload resolver, refresh-in-place — is owned by spec 329.
- The JVM host's adapter over the same projection and renderers is owned by spec 336; its panels by specs 132, 123 and 222.
