...
- **The gate is read once, at entry, from the durable flag only — so a worker already past it keeps draining.** Two consequences follow, and both are intended. First, the durable on-disk flag is the only signal consulted here: the in-memory mirror of that flag which the editor process maintains (spec 304) is irrelevant in this process, because the worker is a separate, freshly-started process where that mirror is never set. Second, there is no re-check anywhere in the drain, so a user who disables the repository *after* this worker passed the gate gets a worker that finishes the entries it already has — which is the point: commits made while the project was still enabled still get their memories instead of being stranded in the queue. Disabling a project uninstalls its git hooks, so no *new* worker is normally spawned while disabled; the entry gate additionally covers the narrow race where the disable lands between a hook spawning the worker and the worker starting. (Notable; a once-only, entry-time check, not a continuously-enforced suppression.)
...
- **The commit pipeline's relevance sub-stage can hold the drain for minutes, and the heartbeat is what makes that safe.** Its per-call wall-clock is provider-dependent and, under a locally-run agent-CLI provider, can approach three minutes (spec 258). That call sits inside the summary hold, so a single entry's processing can exceed the five-minute staleness threshold's order of magnitude — but the sixty-second heartbeat keeps bumping the summary-drain lock throughout, so a long relevance call cannot cause a competing worker to judge the lock stale and reclaim it. The call fails open, so even a timeout only costs the relevance verdicts, never the summary. (Notable.)
...
- **The ingest phase's outcome report enumerates held topics, not just counts.** A held topic is a topic whose page was left untouched and whose sources stayed unprocessed, so those sources resurface and are retried on **every** later drain. Counts alone cannot express that: a drain that folded some sources while permanently stalling one topic reports the same shape as a fully clean drain, and the stall is then invisible for as long as it persists — indefinitely, since nothing else surfaces it. Naming each held topic by slug and structured code is what makes a repeating hold recognizable across successive drains. The list is emitted even when empty, deliberately, so its absence always means "the run did not report" rather than "the run had no holds". (Surprising; the counts-only shape read as clean while work silently stalled.)
