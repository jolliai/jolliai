...
- The one raiser of three of these names that is deliberately **not** a consumer, and the different meaning it gives them.
...
The `401` → not-authenticated / `403` → permission-denied split is also what the session-statistics endpoint raises, unchanged — it is the *meaning* the two names carry there that differs, not the status they come from (below).
...
- **Three further classes are raised by the session-statistics endpoint and are deliberately outside the set as well.** A **cursor-ahead conflict**, which is not a refusal at all — it says the backend holds less than the client believed and is answered by re-sending that range. An **endpoint-missing** signal, which is a property of the deployment rather than of the repository or the credential. And a **precondition-failed** signal, which is the one worth pinning: it arrives on the *same* status as the documented precondition slugs — the binding-required, allowlist and document-type ones — but is classified **by status alone, with no slug read**, because that channel needs no binding and so has no slug it could expect — it is documented in place as expected-unreachable, kept as its own class precisely so that a server which did start demanding a binding is findable rather than retried into the ground.
...
### The endpoint that raises these names without consuming the set

The session-statistics endpoint raises not-authenticated, permission-denied and client-outdated from the same statuses the document push does, and its handler deliberately consumes **neither** the shared membership test nor the per-repo outbound opt-out. There is nothing for it to abort — the channel has no per-document loop — and the three conditions mean something different there: they are facts about the backend **scope** (origin plus tenant slug), not about a repository. So they are answered by recording a **24-hour silence against that one scope** rather than by a repo-wide refusal, and the endpoint-missing and precondition-failed classes above are answered the same way. The one exception is a rejected credential, which is *not* silenced: a key is re-issued by the user, and the attempt after they do has to go through.

Its classifier is therefore a second, hand-written one over the same conditions, matched on the error **class** rather than through the shared name predicate — which works only because that handler and the definition are in the same language and package, and is why nothing about that classifier is protected by the shared set.
...
- **The same names now carry two divergent classifications, and the second one is not a consumer of this set.** The session-statistics endpoint raises three canonical members from the same statuses and reads them as properties of the backend scope, silenced for 24 hours, instead of as repo-wide refusals — with its own hand-written classifier, matching on class rather than through the shared predicate. A shared name set with two divergent classifiers is exactly the drift shape this module exists to prevent elsewhere: the definition can be edited without either the endpoint's classifier or its silences noticing, and nothing type-checks the relationship. (Surprising; the divergence is deliberate — the meanings really are different — but the coupling is unenforced.)
...
- The HTTP status → error mapping for each member (`426` → outdated client, `412` with the allowlist slug and `403` → permission-denied, `401` → not-authenticated) and the deliberately separate document-type mapping are defined by **Summary Push to Jolli Space** (94), with **Plugin Outdated Flow** (96) owning the outdated message. That topic also owns the session-statistics endpoint's own mappings for the same statuses, and the three classes it raises that this set excludes.
