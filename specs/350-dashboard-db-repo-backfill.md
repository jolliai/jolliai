...
- The commit tier: multi-checkout collection, the exclusion of this product's own storage references from every git-side read, the branch-reachability cap, the merge before writing, the projection-skip comparison, the prune, the per-file statistics skip rule, and the bisect that salvages a failed statistics batch.
...
Two properties are deliberate. HEAD alone is not the commit signal: deleting a branch, rebasing it, or committing from another checkout moves no HEAD this pass can see, and a HEAD-only value would call that unchanged and skip both the prune and the reachability refresh indefinitely. And this product's own storage references are excluded — by the same rule the commit collection applies (see *The storage-reference exclusion* below) — because that reference gains a commit on every memory write: including it meant the value could never converge on a repository that is actually in use, so every launch re-swept the whole history the cursor exists to skip.
...
This product keeps its memories on an ordinary local branch under a reserved reference namespace, and that branch gains **one commit per memory**. Every git-side read in this pass therefore drops it, by one shared rule: the commit walk, the per-file statistics pass, the branch enumeration behind reachability attribution, and the change-detection fingerprint the whole tier is gated on. Without it, the memory-storage commits are imported as the user's own work, the storage reference outranks the primary branch in branch attribution, and every commit-derived figure the dashboard shows is dominated by commits about the other commits.
...
#### The branch-reachability cap

Reachability is computed by unioning a per-branch reachability walk — never a per-commit "which branches contain this" query, which would be one subprocess per commit times per branch. The branch list is bounded at **fifty**, most recently committed first, purely to cap that fan-out.

The list is fetched **in full** and capped in the consumer, **after** the storage-reference filter. Asking git for one more than the cap and filtering afterwards spends a slot on a reference that is never walked — and the storage reference sorts first by commit date essentially always, because it is written on every memory. Enumerating every local branch name is one cheap reference walk; the per-branch reachability walk is the expensive half. Truncation is consequently detected by the **listed length exceeding the cap**, not by the older trick of asking for one past it so that overflow is visible.

Under truncation the field is decided per commit rather than suppressed wholesale: a commit the listed branches do reach keeps getting its (capped) attribution refreshed, while a commit **nothing** listed reaches emits *nothing at all*. An empty set is the positive claim "no branch reaches this commit", which the projection honours by deleting every link — and under truncation that claim may only mean the branch that does reach it sits past the cap. A failed branch enumeration, or any single branch's reachability walk failing, marks the whole scan incomplete and suppresses the field for **every** commit; both cases are announced at warning level.
