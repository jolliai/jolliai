...
- The cross-language obligation this table creates against the desktop editor's equivalent, the mechanism that permits this side to lag, and the observable consequence while it does.
...
The rows cover every source this host's own enumeration carries, and **at HEAD that enumeration is the whole shared reference-source catalogue** — nothing is omitted. The enumeration is *permitted* to lag behind the catalogue, and the mechanism that permits it plus the observable consequence when it does are described in the note below; the lag list is currently empty.
...
- **Two letter collisions are deliberate.** `J` is shared by Jira and Jolli Memory, whose colours differ and where the first-party brand takes precedence — the same call the desktop editor made. `Z` is shared by Zoom Doc and Zoom Meeting, which additionally share one hue and are therefore **visually indistinguishable at badge size**. Only the first collision is acknowledged in place. A third collision — the issue-tracking / error-monitoring source reusing Slack's `S` on a different hue — exists in the shared catalogue, in the desktop editor's table, and now **here too**, since that source is mirrored: it could not arise while this host's enumeration omitted it. (Intentional; the second and third are undocumented in place.)
- **This side is permitted to lag the catalogue, and while it does the gap is invisible until a user has such a reference.** Three sources — a deployment-platform one, a design-file one and an issue-tracking / error-monitoring one — were each absent here for a period after shipping on the other two surfaces. **All three are now mirrored and the lag list is empty**, so there is no live drift at HEAD.

  The mechanism matters more than the current state, because the state will move again. Lagging is allowed **only** by naming the id in the catalogue's lockstep test, which is checked in **both** directions: an entry that has since been mirrored also fails, forcing the list to shrink in the very change that closes the gap. A prose note here or in the catalogue is explicitly not a substitute — a comment cannot fail, and each of those three sat as one for weeks and each was re-reported as a bug.

  The observable consequence while a source is unmirrored is exact, and is why the gap is worth permitting rather than ignoring: the reference deserializes to an **absence**, so it renders as the neutral placeholder — placeholder letter, neutral hue, the label "Reference" — in the working-context list, the working-memory review and the pinned list, while its row is **dropped entirely** from the committed-memories context group, and the desktop editor renders its real letter, hue and label from byte-identical stored data. (Previously live drift; now closed, with the mechanism retained.)
