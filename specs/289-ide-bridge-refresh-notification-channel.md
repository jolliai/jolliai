...
**Both memory-writing surfaces are watched because a repository can be on either side of the storage cutover**, and neither watcher can tell which: before it, memories land on the ref and the database is a projection; after it, the ref is frozen and only the database moves. The freeze, and the swap that applies it, are owned by spec 345; nothing in this channel consults it — watching both unconditionally is what removes the need to. Watching the ref alone left a cut-over repository pushing nothing at all — the sidebar silently stopped updating, with no error anywhere.
...
- **The cutover fence and swap (345)** — owns the freeze that stops the ref moving and leaves the database as the only writing surface. This channel watches both surfaces unconditionally rather than consulting it.
