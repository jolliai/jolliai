...
- **The whole write side of a launch is the registration, the history import, the routing attempt and the snapshot pass — and deliberately *not* the session-statistics upload.** That upload is asked for on the resident daemon's own clock (cross-ref 365), and the only two other callers that ask for it are the post-commit worker and the diagnostic command's explicit sync flag. This command's tail brings that daemon *up*, which is a different thing from asking it for an upload — nothing in this command asks. Worth stating plainly because a comment in the daemon's own source names this launcher among that upload's opportunistic triggers, and it is not one. (Notable; the comment is wrong, this list is what the command does.)
