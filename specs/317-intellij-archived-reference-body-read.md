...
`PATH_UNSAFE_SOURCES = setOf(github, context7)` — the code-forge source and the documentation-lookup source. **This is no longer the same set the CLI declares.** The shared source definitions now mark a third source path-unsafe — the issue-tracking / error-monitoring source added to the catalogue since — and this host's set does not carry it. Identity therefore applies to every source this host enumerates other than those two (`linear`, `jira`, `notion`, `slack`, `jollimemory`, `confluence`, `asana`, `monday`, `zoom-doc`, `zoom-meeting`); the catalogued sources this host's enumeration does not list at all — the deployment-platform, design-file and issue-tracking / error-monitoring sources — never reach this function, because parsing answers nothing for their identifiers first (spec 313).
...
- **The two path-unsafe sets have diverged, and the wrong-stem read that follows from it is latent, not live.** The shared catalogue declares the issue-tracking / error-monitoring source path-unsafe; this host's set names only the code-forge and documentation-lookup sources. Nothing can be observed today: this host's source parsing answers nothing for that identifier, so the row is dropped before any path-key mapping happens — the read is never issued, and the committed context group simply omits the reference (spec 313). The defect appears the moment this host's enumeration is extended to include that source **without** its unsafe set being extended in the same change: identity would then apply, and the read would ask for the archived body under a stem the writer never used — a wrong-filename lookup that returns null and silently falls back to opening the whole commit summary, which is byte-for-byte the regression this shape exists to prevent. (Unreachable today; a latent defect, not current behavior.)
