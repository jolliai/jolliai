...
- The race-loser path when the binding-create call returns the "already exists" verdict: the spaces list is hidden, a banner names the winning space, the primary button is relabelled to a single-confirm action that adopts the winning binding on click.
...
| --- | --- | --- |
| Space committed | `{ binding id, space id, space name, repository name }` | The server accepted a binding, either freshly created or adopted via the race-loser banner. |
...
4. Reveal the race-loser banner with the text `Another teammate just bound this repo to <bolded winner space name>. Using that one.` framed in a bordered area.
...
- **Repository URL is HTML-escaped before display in the header.** The dialog uses HTML labels to bold portions of the header and the banner; user-controlled strings (repository URL, error message, winning space name) are passed through an HTML-escape that handles ampersand, less-than, greater-than, and double-quote so that a hostile remote URL or server message cannot inject markup. (Notable; defensive.)
