...
Twenty-six actions. Actions whose work is a family of named operations take the operation name as a request field.
...
| `auth` | `site-url`, `is-signed-in`, `parse-api-key`, `validate-api-key`, `assert-origin`, `should-request-fresh`, `build-login-url`, `exchange-and-save`, `save-legacy-credentials`, `sign-out` |
| `jolli-api` | `serialize-summary` (no key needed); `push`, `delete`, `list-spaces`, `create-binding`, `create-share`, `update-share`, `revoke-share`, `invite-share`, `list-org-members` — each requires `apiKey`, with an optional `baseUrl` |
