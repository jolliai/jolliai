...
Thirty-one actions. Actions whose work is a family of named operations take the operation name as a request field, and the family's operation names are **not** counted here as separate actions — `git-remote`'s four operations, for instance, are one row and one action.
