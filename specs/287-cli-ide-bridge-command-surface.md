...
| `transcript-repair-state` | `commitHash?` → `{ state }`, one of `present` / `repaired` / `repairable` / `unrepairable`. A **missing** hash is deliberately not an error — it resolves to a summary lookup that misses — while a present non-string throws the ordinary string-field message. A summary that is not found answers `unrepairable` **without the predicate being invoked at all**, rather than throwing: that is the mildest verdict, and "repair may still be possible" is the one wrong direction to guess in, because it invites a repair with nothing to work from. The action catches nothing of its own, so a throw from the predicate becomes the ordinary error envelope; the JVM adapter degrades **every** failure — an unreadable body, an absent state, a bridge that is down — to `unrepairable`. It is a read: not in the write-refusal set below, and not gated on a Jolli sign-in. **The predicate is command-line-owned, and this action exists for no other reason than that the JVM host cannot import it** — the desktop-editor host, which bundles that code, calls the same predicate in process. The hosts-are-adapters rule working as intended rather than a duplication |
...
- **Part of the catalogue still has no shipped consumer, but authentication, the platform API and the transcript-repair verdict are no longer in that set.** Three authentication operations (sign-out, login-URL construction, whole-callback handling), nine platform-API operations (push, delete, space listing, binding creation, and all five live-share / org-member operations), and the transcript-repair-state action now have a shipped IDE caller — that last one is the only action in the catalogue that exists *because* of its consumer, since the JVM host has no other way to reach the predicate behind it. What genuinely remains consumer-less: the **pricing**, **plan-grouping**, **reference-store**, **PR-description**, **raw git**, **git-remote** and **main-worktree-root** actions, and all four **telemetry** actions; the platform API's one purely local operation; the seven unused authentication operations (site URL, signed-in check, key parsing, key validation, origin assertion, freshness check, exchange-and-save); and, within the shared-store action, everything except the three pin operations, the skills family, and the three selection operations that read and write the commit-exclusion set. The fourth selection operation is the one in that group with no consumer, and deliberately so: the host derives the storage key itself rather than paying a round trip for it. Those are surface, not observed behavior.
...
- **The domain specs behind each action** — storage, summary store and tree, Memory Bank path/metadata, plan grouping, session state and locks, pins and other shared stores, active-session aggregation, transcript reading, the transcript-repair verdict, conversation overlays, authentication, platform API, pricing, sync rounds, status, compile, PR description, and telemetry each own their own behavior. This spec owns only the names, the required fields, and the envelope.
