...
The local-agent description is the literal `Local agent`, a space-hyphen-space separator, and the display label of the configured agent tool. The four supported tools' labels are `Claude Code`, `Codex`, `Cursor`, and `OpenCode` — so the row reads e.g. `Local agent - Cursor`. When no tool identifier is stored the label defaults to `Claude Code`. A **stored identifier the label registry does not recognize** (a newer build's value read back by an older one, or a hand-edited config) degrades to the generic label rather than throwing, which renders as the doubled `Local agent - Local agent`. The single generic tooltip above is used for every tool — it does not name the tool.
...
- **The provider row is the only place the chosen local-agent tool is named.** The tool label is part of the row's description; the tooltip is a single generic sentence shared by all four tools. An unrecognized stored tool identifier degrades to the generic label instead of throwing, which reads as the doubled `Local agent - Local agent`. (Notable.)
...
- **The local-agent tool-label registry** — the closed set of supported tool identifiers and their display labels — is shared with every surface that names the active tool, including the settings panel's agent-tool dropdown (110) and the pre-regeneration confirmation in the memory panel (109).
