...
- The contract with the queue worker: it reads the user EXCLUDE set read-only, and it WRITES (clears) the AI layer — on more paths than the single one that actually consumes a ranking (see the consumer section).
...
**The squash path reads the user layer exactly once and shares that one read.** Its skill archival step and its working-area Context consumption both take the same read result rather than each reading for itself. An archive write may run between the two steps, so two independent reads could straddle it and disagree — leaving the two halves of one commit's archival honouring different exclusion sets.

The worker's relationship to the **AI layer** is different and includes a write:

- It reads the AI per-item ranking list + fingerprint on the **plain-commit path only**. On a fingerprint match it reuses the persisted list (skipping a recompute); on a mismatch (or no panel run) it recomputes the ranking itself (see [258]). No other op consults the layer: an amend always re-ranks from scratch, and a squash or rebase-pick ranks nothing at all.
- It **clears** the AI layer (empties the list, drops the fingerprint) via the clear operation, under the same cross-process lock, on the plain-commit path, on **every** amend entry regardless of which amend route ends up running, and on the shared squash consolidation that serves both squash routes. This is a real write to this file — the earlier invariant that the worker "only reads, never writes" this file is **false** for the AI layer.
- Only the plain-commit path has actually *consumed* a ranking by the time it clears. The amend and squash paths clear a ranking they never read: the trigger is not consumption but the fact that the op **moves the branch tip**, which makes the stored change fingerprint stale for whatever commit comes next, so a LATER commit must not be able to reuse this ranking's exclude decisions. The 1:1 rebase-pick migration is the one op that neither reads nor clears the layer.
- **The clear is positioned ahead of the path's long model window, not after it.** The plain-commit path clears as soon as it has settled the ranking (reused or recomputed) and before the summary call; the amend and squash paths clear at the very top, before any model call at all. Clearing early is deliberate: a user who re-ranks in the pre-commit panel while a summary or consolidation call is in flight must not have that fresh ranking clobbered by a late clear.
- On every one of those paths the clear is **best-effort**: a failure is logged and swallowed, never breaking summary generation or squash consolidation.
...
The plan / note / reference bullets above each have two arms — a prompt-side one and an archive-side one — because the plain-commit and summary-generating amend paths build context prompt blocks. **A squash has only the archive-side arm.** Consolidation merges the source commits' existing topic structures and never assembles a plan / note / reference prompt block at all, so on a squash the exclusion decides purely what gets associated with the consolidated commit; there is no prompt input for it to filter.
...
- **Cleared by the worker.** The worker empties the list and drops the fingerprint, returning the layer to Absent. This happens on the plain-commit path (once that commit has consumed the ranking), on every amend, and on a squash — the last two clear without ever reading the layer, because the tip has moved and the fingerprint is stale for the next commit. A rebase-pick leaves the layer alone. Unlike a user-exclude key, an AI entry is therefore **consumed or invalidated per commit**, not sticky.
...
- **The worker is read-only on the user layer but WRITES the AI layer.** The queue worker never mutates the four user-exclude arrays. But it **does** write `commit-selection.json` in one specific way: it clears the AI per-item ranking list + fingerprint. So the blanket "the worker only reads this file, never writes it" invariant no longer holds — it is true only of the user layer. (The worker also writes the plans/notes/references registry when it *associates* the checked items — a different store — but it deletes no excluded row.)
- **"Clear after consume" describes only one of the clearing paths.** The clear fires on the plain-commit path, on every amend, and on a squash consolidation, but only the plain-commit path has read a ranking by then. The real trigger is that the op **moved the branch tip**, a staleness the stored fingerprint cannot detect on its own — so an op that rewrites history invalidates the ranking on principle rather than because it used it. Reading the clear as strictly post-consume would predict that a squash or an amend leaves a panel ranking behind for the next commit to reuse; it does not. Only the 1:1 rebase-pick migration, which writes no new memory of its own, leaves the layer untouched. (Surprising; intentional.)
- **The squash path takes one read of the user layer and hands it to both of its consumers.** Its skill archival and its working-area Context consumption share a single read result. Two reads could straddle an archive write that runs between the two steps and disagree, so one commit's archival would honour two different exclusion sets. This is a real obligation on a consumer with more than one exclusion-sensitive step, not an incidental optimization.
