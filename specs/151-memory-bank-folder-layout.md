...
- The internals of the second, direct reader of the hidden layer (covered by spec 314); only the subset of this layout it consumes is defined here. The larger content-reading path that used to read this layout directly is retired (307).
...
- The two operations a user-initiated Refresh of the desktop editor's folder tree performs on the parent folder — archiving every discovered subdirectory that provably holds nothing, and folding the several subdirectories that hold the current repository into one survivor. Both move subdirectories, and both leave the naming, reuse and adoption rules below untouched (covered by the Memory Bank unused-folder archival spec and the Memory Bank duplicate-folder consolidation spec).
- The host-alias resolution step inside the remote-URL normalization used by the same-repository comparison — its cost, its cap, its fail-safe fallback, and its never-invalidated memo (covered by the repo-identity-and-folder-naming spec).
...
- **Same repository** (subdirectory present, holds a configuration document whose recorded remote URL normalizes-equal to the host's remote URL, or where both remote URLs are absent and the recorded name matches or is unset): reuse in place. Normalization folds the non-HTTPS transports to HTTPS — **resolving each folded remote's host token through the local SSH client configuration, so a locally-defined host alias and the real host it names compare equal** — then strips a trailing `.git` and any trailing slashes and lowercases the whole string.
...
- `manifest.json` — registry of every file the system has emitted into the visible or wiki layer, keyed by a stable identifier; carries a content fingerprint per row to support hand-edit detection and a human-readable display title. Its row-type vocabulary is `commit` | `plan` | `note` | `wiki` | `skill`. (Document shape and mutation semantics: see folder-based summary storage spec.)
...
- `shadow-status.json` — **per-device** dirty marker. Present only when the most recent shadow-write into this repository's hidden layer failed and was suppressed; absent otherwise. Carries `dirty: true`, an ISO-8601 timestamp, and the message of the failing batch. This document is **not** synced across devices — it represents recovery state local to one device's mirror operations. **No consumer parses its body**: every reader tests only whether the file exists. It used to double as a per-device *read* switch, because the second reader of this layer refused to serve while it was present; that reader is retired (307) and the surviving one never consults the marker, so the marker is now a recovery record and a "your mirror is behind" signal only. It remains an existence-signalling file of its own rather than a field inside one of the aggregate documents above.
...
- `references/<source>/<pathKey>.md` — one per archived external-entity reference; the stem is a sanitized transcoding of the reference's native identifier.
- `skills/<source>/<stem>-<hash8>.md` — one per archived skill invocation record, where `<stem>` is the sanitized skill identifier and `<hash8>` the archiving commit's short hash. The `<source>` segment is required (two producers can hold the same skill id, and a flat layout would archive one over the other). This path lands here with no folder-specific code: the writer's batch write mirrors every relative path it is handed into the hidden layer unconditionally, before the visible cascade.
...
**Two readers of the hidden layer.** The layout above is written by exactly one writer, but read by two independent implementations in two languages. Alongside the writer's own read path, one host reads the hidden layer directly off disk to avoid a per-repository process hop while building a multi-repository tree. That second reader consumes exactly **two** names: `manifest.json` and `index.json`. It never writes, and it fails soft — a missing file yields an empty payload silently, an unparseable one yields the same payload plus a warning.

**Nothing in the content directories is consumed there, and neither is the dirty marker.** That host used to read the per-commit summary documents, the plan bodies, the note bodies and the archived reference bodies straight off this layout, gated on the marker's presence; that path is retired (307) and those reads now travel back through the writer's own storage over a cross-process bridge. So the two-language obligation **inverted**: the two aggregate documents named above, their locations and their schemas require a coordinated change in both languages, whereas the content directories' names as *this layout's* shape, and the dirty marker's semantics, may now evolve on the writer's side alone for as long as nothing reads them there. The surviving reader's own internals — its two entry points, its error policy, and its deliberate absence of a marker gate — are defined by spec 314.

One obligation survived the move rather than being retired with the path that carried it: that host still re-implements the sanitize rule that turns a reference's native identifier into a filename stem, and still composes `references/<source>/<stem>.md`. It now hands that relative path to the writer's storage over the bridge rather than opening a file in this layout, so the rule binds the storage-provider path namespace — which this layout mirrors verbatim — instead of this layout itself. A drift in it still returns nothing for every archived reference, silently and uniformly, with no parse error to notice (spec 317).
...
- `<branch-folder>/skills--<hash8>.md` — the per-commit skill-usage table, **one file per commit** rather than one per archived skill. Its lifecycle is the summary's: it is emitted, healed, and deleted alongside the summary Markdown that shares its hash, and it is registered in the per-repository manifest under a **namespaced** file identifier (`skill:<commitHash>`) rather than the bare hash, so it cannot evict the summary's own manifest row. The rendering, the delete/heal ordering, and the manifest rules are owned by the folder-based summary storage spec.
...
0. **Precondition (caller-side).** A caller whose repository name is derived from an *ambient* working directory must first clear the Memory Bank write boundary and, on a refusal, choose a different storage backend instead of calling this resolution mode at all. The reason is that this mode is a claiming mode, not a lookup: it creates directories and writes identity (step 4 and the claim step below). Invoked from a working directory that is not a real project, it therefore permanently materializes a subdirectory named after that directory's basename — junk only the user can clean up. The predicate itself, its refusal reasons, and their evaluation order are defined by **Memory Bank Write Boundary and Effective-State Reporting**; this step defines only that the precondition is the caller's obligation and which callers carry it.
   ...
   **Exempt callers** are the flows a user invokes deliberately against a project they already have open, where the resolution is the point of the command rather than an incidental side effect: the sync round's mirror initialization and the one-shot back-fill migration into the parent folder. The exemption is about the *invocation* being deliberate, not about the inputs being different — both still derive the repository name from a working directory exactly as the gated callers do. (The desktop editor's folder re-target flow is exempt for a different reason: it never reaches this mode at all, resolving through the peek mode and then claiming the chosen path explicitly.)
   ...
   The exempt set is wider than that list in practice: the desktop editor's activation-time mirror initialization, and the resolution adapter the JetBrains host reaches, both call the claiming mode with no boundary consultation. On that host the claiming resolve-then-initialize pair runs at activation, from the settings surface's apply, from its rebuild action, and from the Memory Bank tree's own initialization — all of them write-intent flows. (It previously also backed a purely-*read* capability, the direct hidden-layer read path, which resolved the subdirectory in order to read it; that path is retired, so attaching a reader is no longer one of the ways a subdirectory the boundary would have refused comes into existence. See 307.) A refused placement is therefore closed only on the three gated seams, not on every surface. (See the boundary spec for the cross-host asymmetry.)
...
- A presence check `exists?` is the only consumer-visible read. Its body is never parsed by anything. Downstream code uses the presence check to decide whether to surface a "your mirror is behind" UI indicator. It used to serve a second purpose — gating every individual read in the second, direct reader of the hidden layer, so a repository whose mirror was known to be behind was served from the version-controlled ref instead — and that reader is retired (307), so the marker is no longer a read switch anywhere.
...
- **No silent relocation *by the resolver*: any repository that resolved to a given subdirectory before still resolves to the same one.** The write-boundary precondition is a pure precondition — when it allows, resolution proceeds under exactly the naming rule, the reuse / adoption / suffix-ladder policy, the peek mode's mirroring of that selection, the fresh-mode allocator, the archive destination, and the remote-URL normalization documented above, all unchanged; when it refuses, nothing is created. No previously-resolved per-repository subdirectory moves, is renamed, or is re-numbered *as a consequence of resolution*. The only observable difference at this seam is *whether* a subdirectory is claimed, never *which*. (Notable; load-bearing.)

  **The guarantee is scoped to the resolver, and there are now two operations outside it that relocate.** A user-initiated Refresh of the desktop editor's Memory Bank tree archives every discovered subdirectory that provably holds nothing — including a subdirectory belonging to a *different* project the user has open elsewhere — and then folds the several subdirectories that hold the current repository into one survivor, moving that survivor's losers into the archive and their memories into it. Both are recoverable moves rather than deletions, and after either one the resolver still answers the same way it always did: it re-claims a subdirectory at the same path on the next write, or reuses the survivor. So "the resolver never relocates" survives intact while "a repository's memories never move without the user saying so" does not. (Notable; the carve-out matters to anyone reading the guarantee as a property of the layout rather than of the resolver.)
...
- **Generated naming patterns are duplicated in two places.** The visible layer's `-<8-hex>.md` suffix and `plan--`/`note--`/`topic--` prefixes are recognized as system-generated by both the writer (which uses them) and the user-content scanner (which excludes them). The skill aggregate needed no new pattern: `skills--<hash8>.md` already ends in the hex suffix, so the existing rule covers it — which also means renaming it away from that shape would silently reclassify it as user knowledge whenever the manifest is unreadable (see spec 195). Changing the convention in one place without updating the other would either let generated files leak through as "user knowledge" (double-folding their content) or hide newly-renamed visible files from the system. (Notable; intentional duplication is called out at both ends.)
...
- **The dirty marker's contract is its existence.** No consumer parses the document; the presence of the file is the whole signal. It is no longer a cross-language read gate — the reader that gated on it is retired — so folding it into one of the aggregate documents would now break a UI badge rather than a read path. (Notable; the file's existence *is* the API.)
- **The hidden layer has two readers and one writer, and the two-language part of it inverted.** The second host now reads exactly two names — the manifest and the hidden index copy — while the content directories and the dirty marker it used to read directly are reached through the writer's own storage over a bridge. So the names that were deliberately *outside* the coordinated set are now the whole of it, and the ones that were inside it can change unilaterally. The rule did not change: the obligation is scoped to the names actually consumed, not to the whole layer. (Notable; and the reason to state the set explicitly rather than by category — it has already flipped once.)
- **The reference stem binds a naming *rule*, not a name, and it outlived the reader that made it visible here.** The other host still re-implements the writer's sanitize of the native identifier (spec 317) — it just composes the relative path for a bridge read now instead of a filesystem read of this layout. A change to that sanitize, to the `<source>` segment, or to the folder still makes that host return nothing for every archived reference, silently and uniformly, with no parse error to notice. It is the one surviving coupling whose contract is an algorithm rather than a literal, and it is no longer enforced by anything in *this* layout — which is what makes it easy to miss. (Surprising; the sharpest edge in the lockstep set, and it moved rather than disappearing.)
- **The per-commit skill aggregate is registered under a namespaced manifest identifier, not the commit hash.** Manifest rows are keyed on that identifier and a write replaces any row sharing one, so a bare hash here would evict the summary's own row — and the superseded-descendant cleanup, which looks a hash up and skips anything that is not a commit-typed row, would then silently stop cleaning up superseded summaries. Plans already namespace for the same reason. (Surprising; a real cross-feature hazard.)
...
- **Same-repository reuse is normalized, and the normalization reaches past the URL string into the machine's SSH configuration.** Comparison folds the non-HTTPS transports to HTTPS, resolves each folded remote's host token through the local SSH client configuration, then strips trailing slashes and a `.git` suffix and lowercases. So `https://Example.com/Foo.git/`, `https://example.com/Foo`, `https://example.com/foo.git`, `git@example.com:foo`, and an SSH remote spelled with a locally-defined alias for that host all reuse one subdirectory. The alias step is what makes this predicate depend on state outside the repository: the same two remotes compare equal on a machine whose SSH configuration defines the alias and unequal on one that does not, so two developers can legitimately end up with a different number of vault folders for the same pair of clones. (Notable; the rule is defined by the repo-identity-and-folder-naming spec, including the resolution's cost, its cap, and its never-invalidated memo.)
...
- The second, direct reader of the hidden layer — its two read entry points, its fail-soft error policy, and its deliberate absence of a dirty-marker gate — is defined by spec 314. This spec defines only which names in the layout that reader makes into a two-language contract, and states the writer's half of the obligation: the manifest's and the hidden index's locations and schemas may not change without the same change on that host, in the same delivery. The larger content-reading path that used to impose the opposite obligation is retired (307). Its archived-reference arm is owned by 317, and that arm's stem-derivation rule is still re-implemented on that host — it now composes a bridge request path rather than a path into this layout, so this layout no longer enforces it while a drift in it still fails silently there.
- The per-commit skill aggregate in the visible layer, its rendering, and the archived skill bodies in the hidden layer are written by the folder-based summary storage spec (02) and the skill archival spec (322); the table's content is owned by spec 323.
- The orchestration of the system-of-record backend with the per-repository folder mirror (which gets the primary read role, when each is written, and how dirty-flag state is set or cleared) is defined by the dual-write summary storage spec. **This mirror is written in every writable routing state, not under one configured storage mode; the only condition that omits it is a working directory that may not claim a per-repository subdirectory** (that routing is owned by spec 344).
...
- The two operations that relocate a repository's subdirectory from outside the resolver — the archival sweep of provably-empty subdirectories and the consolidation of the several subdirectories that hold one repository — are defined by the Memory Bank unused-folder archival spec and the Memory Bank duplicate-folder consolidation spec. This spec defines only the naming, reuse and adoption rules those operations leave intact, and records the carve-out they open in the no-silent-relocation guarantee.
