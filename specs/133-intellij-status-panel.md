...
- The hooks row's compact summary (`5 Git + 2 Claude + 1 Gemini CLI` style) and the multi-line tooltip detailing each hook family.
...
| - | -------------------- | ------ | -------------------------------------------------------------------- | ------------- |
| 1 | `Hooks`              | OK if git hook installed; ERROR otherwise | `5 Git + 2 Claude + 1 Gemini CLI` (only the parts whose hooks are installed are joined; if none, `none installed`) | Three lines: Git hooks status (post-commit, post-rewrite, prepare-commit-msg, post-merge, pre-push), Claude Code hooks (Stop, SessionStart), Gemini CLI hook (AfterAgent). |
...
| -------------- | ------------------- | ---- | ----------- | ------------- |
| true | true | OK | `active` | MCP tools + the three skills are active. |
...
[refreshUI]
  ...
  listModel.clear()
  add row: Hooks          // "5 Git + 2 Claude + 1 Gemini CLI"
...
- **The hooks row's description is a "+"-joined list of installed hook families.** When all three families are installed it reads `5 Git + 2 Claude + 1 Gemini CLI`; when only Git is installed it reads `5 Git`; when none it reads `none installed`. The numbers are part of the literal string, not a count.
- **The Hooks health icon hinges on a four-of-five check, so a repository missing one hook still renders healthy.** The git-hook flag the snapshot supplies is computed from only four of the five installed git hooks — the push-time hook is deliberately excluded from it. The row's description and tooltip both say "5", but the OK/ERROR decision does not consider the fifth. A repository with all five hooks and a repository missing only the push-time hook therefore render identically: OK, `5 Git`.
