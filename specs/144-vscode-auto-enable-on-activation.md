...
- **Preconditions 2 and 3's reads are now shared with a sibling auto-repair path** (reinstalling hooks in a new worktree of an already-enabled project) — both reads happen once before either branch runs. This does not change what the three preconditions are.
