...
- The single time-zone engine every local-date decision goes through, why the database's own local-time functions are never used, and the class of zone where its day-boundary inversion does not converge.
- The per-window **day plan** — how each activity render splits its window into days already settled in the per-day cache and days that must be computed live — and the membership rule the live pass has to filter on.
...
- The per-day rollup cache itself — how a day is settled, how a settled day expires, the horizon past which caching stops paying, and the write-side builder that fills it — together with the local-calendar arithmetic both sides derive their day boundaries from, now a shared module with a write-side caller. This layer only *reads* settled days and cuts plans over them; the rest is owned by the per-day statistics rollup and local-day spec.
...
| `menus` | Which **optional** sidebar rows this machine shows — `{ knowledge, graph }` — read from machine-global configuration by the transport and passed in, defaulting to **both false** when it is absent, so a caller that does not know about them cannot switch one on by omission. See *The optional menu flags* below. |

A view payload slot exists for every view name above, and exactly one of them is filled per request — the rest are absent, because building the others would be wasted queries and the page only reads its own. Two of those slots are never *built* here: the knowledge and graph payloads read the Memory Bank folder rather than the database (366), so this layer only passes through what it was handed, substituting an empty placeholder when it was handed nothing.
...
Optional markers, each present only when true, so an ordinary active row's shape is unchanged:
...
#### The optional menu flags

Present on **every** view, the settings payload included, because the sidebar is shell furniture rendered on all of them — a flag read only on the settings view would leave the navigation guessing everywhere else. One fact, one place: there is deliberately no copy of it inside the settings payload, which would be a second owner with no tie-breaker, and the settings modal reads it off the envelope like every other view.

`false` hides the navigation **row** only. Both views stay routed and answer normally, so a bookmark and the cross-links between them keep working — switching a row off is a decision about the sidebar, not about reachability.

The flag defaults **off** rather than on, matching the configuration file's own polarity: the transport's configuration read fails open to an empty configuration, and that has to land on hidden rather than on showing two rows nobody asked for.
...
| A bound that is not a real local day (wrong shape, or a date the calendar does not have) | **Rejected** — falls back. A day key is validated by resolving it and checking the resolved instant's own day key matches, which is what catches a normalized non-date. It also silently rejects a day that *does* exist but whose local midnight the inversion cannot resolve — see *The time-zone engine* below. |
...
That fallback is resolved **before the per-day cache is consulted**, and never reaches it. It is a display decision, so a repository that crosses the tier later must not find stored days of one axis holding another axis's data.

The two halves also read different sources, and the first half changed with the per-response rows. `model`, `agent` and `project` read the **per-response usage rows**, each carrying its own instant, and fall back to the session-level tables for the sources that cannot report per-response usage; `branch`, `ticket` and `category` still window on the commit's committer date and the memory's landing date.

### The time-zone engine

Every local-time decision — day boundaries, day keys, hour buckets — is computed in one zone (the machine's, unless overridden), never by the database's own local-time functions, which answer with the process's zone at query time and cannot agree with the boundaries computed here.

**"Filtered by UTC millisecond range in SQL, bucketed afterwards" is now true for one half of the work only.** Each activity render cuts a **day plan** over its window, splitting it into days already settled in the per-day cache and days that must still be computed. A settled day is read back **already bucketed** — the bucketing happened at write time, in one zone, keyed by day string — and only a live day is filtered by millisecond range and bucketed after the fact. Both halves derive their boundaries from the same arithmetic, which is what makes a cached day and a live day the same number rather than two numbers that happen to agree.

The live pass must therefore filter on **day membership, not on its enclosing range**. The plan's live days are not contiguous — a settled day can sit between two live ones — so a row from that settled day falls inside the live range and would be counted twice, once out of the cache and once out of the live query.

Local midnight is derived by inverting a wall-clock read: guess the UTC value, measure the error, correct, repeated up to **three** times.

**On a day whose local midnight does not exist, that inversion does not always land on the earliest existing instant of that day, and for one class of zone it does not converge at all.** Where the spring-forward transition falls exactly at local midnight and the offset is positive, the correction oscillates between two candidates and the three iterations run out, returning an instant that lies **in the previous local day**. Two consequences follow:

- A custom range naming such a day is **silently rejected** by the round-trip validation above — the resolved instant's own day key is the day before — so the request falls back to the default preset.
- The day-walk built on that midnight **does not terminate**: stepping forward from it is a fixed point.

The day-stepping guarantee is bounded by the same case. Stepping by days steps through midday rather than adding 24-hour multiples, so 23- and 25-hour days cannot skip or repeat a day — for every day **whose local midnight exists**. For these zones it fails in both directions: stepping forward is a fixed point, and stepping backward skips the day entirely.
...
Each render also cuts a **day plan** per window — **twice**, once for the selected window and once for the prior comparison window, because a plan is only valid for the range it was cut for. Everything below that says "per day" is drawn over one of those two plans, half out of the per-day cache and half computed live; see *The time-zone engine* above for the split and the membership rule.

- One sweep of sessions and commits over the 84 days feeds the heat map, the hour histogram and the records. When the selected window sits **inside** that sweep, its rows are filtered out of what is already in memory; a custom window reaching outside it pays for its own scan.
- **KPI row** (all over the selected window, each labelled with the window so a figure cannot be misread as today's): session count; tokens (input + output + cached, from the token breakdown below rather than from the window's session rows); estimated cost.
  - Token magnitudes render as one decimal plus `k` at or above a thousand, one decimal plus `M` at or above a million.
  - **A `% cached` share and a streak were described here and are on no payload** at either end of this delta — see *Notable / Surprising Behavior*. Nothing in the layer computes either one.
- **Series**: one point per local day across the whole window (empty days are seeded, so gaps are real points rather than missing ones), each carrying total tokens, estimated cost, and a per-series-key token split. Values are rounded at emission rather than per row.
  - Days come from the **plan**, so a point may be a cached row or a live one — and both halves read through the **same** row reader. That is not tidiness: two of the axes apportion against a window count, so a second, differently-shaped query would make a cached day and a live day disagree by an apportioning fraction, producing plausible-but-wrong numbers with nothing to flag them.
  - The per-series split is emitted **in sorted key order**, and that is load-bearing rather than cosmetic. The cache returns a day ordered by series key while a live day arrives in query order, and object key order survives serialization all the way to the browser — so without the sort a day's legend visibly reorders itself the moment that day settles: same numbers, different chart.
  ...
  - The `branch` axis records **one row per commit — the branch it was committed on** — so the axis answers what the work on this branch cost, and a commit counts once. (Predates this delta: it was described here as a per-branch *union*, in which a trunk commit was also listed under every feature branch based off it. The apportioning division survives, but only as a safeguard for rows an older client wrote in that shape; against anything this build wrote the divisor is one.)
  - The `ticket` axis does **not** apportion; commits with no ticket group as `(no ticket)`.
- **Token breakdown**: input / output / cached over the window, plus a per-day series of the same three. The shape is unchanged but the population is not — it is **no longer the window's session rows summed**. It is built from the day plan: settled days from the cache's token rows, live days from a per-response read (one row per **model response**, each on its own instant), with a per-session fallback to the session-level total for any session whose per-response rows do not account for all of its tokens. A session carries one cumulative total under a single timestamp, so a three-day conversation used to put all of its tokens on the day it ended.
- **Cost trend**: percentage change against the immediately preceding window of equal length. Absent when that prior window has no priced sessions, so no trend is ever claimed against a zero. The prior window cuts its **own** day plan, and its lower bound is a raw millisecond subtraction from the selected window's start — so across a daylight-saving transition it can cover one more or one fewer local day than the window it is compared against.
...
| `provider` | A provider label derived over the whole memory **tree**, which is why it is a separate field rather than a third value on `summarizedBy`: the two read different populations, so a consolidation whose root records no generator but whose folded children do still has a provider and no `summarizedBy`. Absent for memories written before the generator was recorded, and the surface then omits the clause entirely rather than printing a placeholder. |
| `generatedAtMs` | **Required.** When Jolli wrote *this* memory, falling back to the coalesced committer date when the memory's own generation instant is absent or unparseable (it is persisted as an empty string on some paths). Explicitly **not** the payload's own clock beside it — reading that would stamp the current time under every memory ever recorded and change it on every refresh tick with nothing under it changing. The substitute is the same committer date the row's committed instant carries, never the author date, so a rebased memory cannot stamp one instant here and another in the tree row beside it. |
| `transcriptRepairState` | Optional, one of four verdicts, and **the only asynchronous part of this detail** — it costs a filesystem read that only the detail view pays for, so it is computed per *selection* rather than per render and threaded in from outside the same way the reachable-commit set is. Every way it can fail answers **absent**, which the client renders as the plainest wording, never the optimistic one; an unrecognised value reads the same way. |
...
Several fields a reader might expect are **deliberately absent rather than invented**: a conversation kind, an immutability flag (regeneration is an in-place update, with no revision history to point a lock at), a minted sequential id (the short hash is already stable and content-bound), per-file add/modify/delete status (only line counts are stored), tool-call arguments (only counts are stored, so a row reads "×22", never what was read), and **a second conversation count**. A transcript-*file* count briefly rode this type and was removed: the pane already prints the conversation count, and one memory storing six sessions in two files rendered a six directly above a two. A count a client cannot reconcile with what it already prints does not belong here, so there is deliberately none for a client to pick the wrong one from.
...
Built entirely from configuration plus one cheap folder-state peek — no database read, no subprocess on the common path — and passed straight through this layer. Its slices: the agent toggles, the summary settings, `sync`, the memory-bank state, and the remainder.
...
- The **`sync` slice** holds a single field, `syncSessions`, on the same "not explicitly false" rule as the agent toggles — an absent key reads as **on**, because rendering it off would show a switch that disagrees with what is actually happening. It only *seeds* the switch's initial position: that switch writes through its own immediate endpoint on change and is never submitted with the batched save, so every switch on its tab applies immediately. Sign-in state is deliberately not in this slice (another slice needs it too) and neither is the per-repository push list, which loads from its own endpoint.
...
The counter stayed at **4** across the additions above — the envelope's menu flags and the memory detail's three new fields. An additive field an old tab simply ignores cannot break it, so the bump is reserved for the removals and reshapings the table lists.
...
- **There is no `% cached` figure and no streak on this payload at all.** Both were described here — the cache share with its *input + cached* denominator, and the consecutive-active-day count ending today or yesterday — and neither exists in the model at either end of this delta, so both claims predate it. Nothing in this layer computes either one. (Corrected; the stale claim predates this delta.)
- **The local-midnight inversion does not converge for one class of zone, and returns an instant in the WRONG local day.** Where a spring-forward transition falls exactly at local midnight with a positive offset, the three correction iterations oscillate between two candidates and the last one returned lies in the *previous* local day. A custom range naming such a day is therefore silently rejected by the round-trip validation and falls back to the default preset, and the day-walk built on that midnight does not terminate — stepping forward is a fixed point, stepping backward skips the day. (Surprising; the section's own claim that this "lands on the earliest existing instant of that day" holds only for zones whose local midnight exists.)
...
- **Nothing in this layer can reach a write.** It issues no statement that could write one, and every handle it is given is opened read-only — but that read-only-ness is enforced by the **connection**, not by the handle's type, which still exposes a general execute. The guarantee is a runtime one. That is worth stating plainly now that a write-shaped thing sits under the read model: the per-day cache's readers issue only selects, and the cache is written **exclusively** from the write protocol. This layer reads settled days and cuts plans over them; it never settles one. (Notable; load-bearing.)
- **The token figures can sit slightly BELOW the session-level ones, and that is the right side to err on.** A usage line the parser could not date is counted in a session's own totals and dropped from the per-response rows the token breakdown is built from — and an undatable token has no day to be shown on, so a per-day card is better short than misplaced. (Notable.)
- **The transcript repair verdict does not decide whether the empty-conversations block renders.** An empty conversation list still does, on its own; the verdict only chooses the wording. Nor is a "present" verdict proof of renderable conversations — an older summary reads as present unconditionally. (Surprising.)
...
- The write protocol that produces every row read here is owned by spec 354, which is also the **only** writer of the per-day cache this layer reads settled days out of.
- The per-day rollup cache's own rules — how a day is settled, how it expires, its horizon — and the local-calendar arithmetic both this layer and that builder derive their day boundaries from are owned by the per-day statistics rollup and local-day spec. Sharing one implementation of those boundaries is what makes a cached day and a live day the same number rather than two numbers that happen to agree.
