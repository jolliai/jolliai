...
| `jolliUrl`        | string | optional | The Jolli tenant URL the credentials belong to. **Persisted** — with trailing slashes stripped and the value origin-allowlisted before it is written. Two independent classes of writer produce it (below). It exists so downstream consumers can recover the tenant when the API key is missing or stale. |
...
### Two classes of `jolliUrl` writer

`jolliUrl` is no longer written only by sign-in. Every path that persists a **hand-pasted** product API key also derives the tenant from that key and writes it in the same partial:

| Writer class | Where the value comes from | On failure to derive |
| --- | --- | --- |
| **Sign-in** (the combined credential save) | An ordered candidate list — the freshly minted key's embedded tenant, then the already-on-disk key's, then the launch origin — skipping any candidate off the allowlist. Always yields a value, because the launch origin is the terminal fallback. | Not possible; the field is **required** on this path and a missing one refuses the whole write. |
| **Pasted key** (every path that persists a key the user supplied directly — a configuration-set command, a first-run prompt, or a settings surface's key field, on any host) | The key's own embedded tenant claim, trailing slashes stripped, then screened against the origin allowlist with the **non-throwing** predicate form. | Yields *nothing*. There is no origin to fall back to, so the field is simply **omitted from the partial** and the stored value is left exactly as it was. |

The pasted-key derivation answers "nothing" in three cases, all treated identically: no key at all; a legacy or hand-typed key carrying no embedded tenant claim; and a claim whose origin is off the allowlist. Every one means *leave the stored value alone* — **never** *clear it*.

One pasted-key writer carries a fourth condition of its own: the command-line configuration-set path writes the derived site only when the key **survives the same invocation**. Setting a key and removing it in one command leaves the derivation holding the removed key's tenant, and writing it would name a tenant the user never signed into and has just discarded the key for.

**Constraint — the "leave it alone" answer MUST be expressed by the field's absence, never by an explicit undefined.** This is a correctness requirement of this storage layer, not a style preference: by the merge semantics above, a key that is *present in the partial with an undefined value* is a **delete**, because the serializer omits it and the merge has already let it win over the stored value. So a writer that assigns the derived value unconditionally would erase a `jolliUrl` the moment the user pasted a legacy key — a key that by definition cannot supply a replacement. Every pasted-key writer therefore adds the field conditionally, and all of them do; and the one writer whose surface has an editable key field but whose own configuration document deliberately carries **no** `jolliUrl` field relies on exactly that absence to make the merge preserve the stored value (its serializer emits nulls, and a transmitted null is *worse* than an undefined — it survives serialization and persists as a literal null).

**The pairing is for display and outbound targets — it is NOT what routes requests.** The request client derives its target from the key's **own embedded claim** and never consults the stored `jolliUrl`. So a stale `jolliUrl` never misroutes anything; what it does is make the surfaces that report "which site am I on" name the wrong tenant. Those surfaces split in two, and only one half drifts: the command-line status output, the equivalent tool, and the guided front door read `jolliUrl` and deliberately **never** decode the key (a decoded key reaching a console is what the repository's clear-text-logging gate objects to), so they keep naming the old tenant; the editor and IDE site rows read the key first and fall back to `jolliUrl`, so they self-correct. That asymmetry is why the mismatch is silent: the user pastes a key in an IDE, sees the new tenant confirmed right there, and nothing indicates the command-line surfaces still name the old one.
...
- **Because of that, "I have nothing to write" and "delete this" are the same expression, and only the field's absence separates them.** Any writer that computes an optional value must add the field conditionally rather than assign it. The tenant URL derived from a pasted key is the live case: assigning it unconditionally would delete a stored tenant every time the user pasted a key that cannot supply one. (Surprising; the trap follows directly from the removal channel.)
- **The stored tenant URL is a display value, not a routing value.** Requests target the tenant encoded in the API key itself; nothing consults the stored field to build a request. A drifted value therefore mislabels rather than misroutes — and mislabels only on the surfaces that read it without decoding the key, which are exactly the ones a user is least likely to be looking at when they paste a key. (Surprising; intentional.)
