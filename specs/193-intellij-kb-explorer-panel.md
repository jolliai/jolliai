...
- The visible-layer heal pass that regenerates missing rendered Markdown — across the boundary; this panel only reads what is on disk.
...
- **Is-directory**: false.
- **Badge**: a single-letter classification code derived from the manifest entry's type (see "Classification badge" below); the empty string when no manifest entry exists.
...
The badge letter is consumed by the row renderer to render a colored single-letter glyph after the display title. The empty-string case suppresses the glyph entirely. A file with no manifest entry has no badge at all (an absent field, distinct from an empty-string badge).
...
- every per-repository manifest read, every projected-index read, and the migration-state write the reset action performs.

What remains **native in-process**: the on-disk directory enumeration and recursive descent that produce the branch-folder and file rows, the hidden-or-internal filter, the sorts, the tree-model assembly, the selection/expansion bookkeeping, and the default parent-folder path.

The practical shape of one refresh is therefore: a handful of delegated calls to establish the KB root and reconcile, **one delegated discovery call, then two delegated reads per discovered repository** (manifest and index) — i.e. it scales with the number of repositories, not with a fixed constant — followed by a full native directory walk of every repository subtree.
...
4. For each discovered repository, in the order produced by discovery:
   1. Read the per-repository manifest and projected index from the repository's hidden-metadata folder. **Both are delegated calls, so this step costs two cross-process round-trips per discovered repository.**
   2. Compute the hidden-children set (see Data Contracts above).
   3. Compute three lookup maps over the manifest's row list, keyed by repository-relative path: a badge-letter map, a display-title map (only for rows whose recorded title is non-empty), and a source-branch map (only for rows whose source record carries a branch).
   4. Project the repository row.
   5. Enumerate the immediate-child directories of the repository root, apply the hidden-or-internal filter, sort by name. For each surviving directory: project a branch-folder row, recursively descend (see Data Contracts above), attach to the repository row.
   6. Enumerate the immediate-child files of the repository root, drop dot-prefixed names and the literal `index.json`, sort by name. For each surviving file: skip if its repository-relative path is in the hidden-children set; otherwise project a root-level file row populating the badge, display title, source branch, and owning repository root from the three lookup maps. Attach to the repository row.
   7. Attach the repository row to the synthetic root.
...
8. On the UI dispatcher, open the summary in a synthetic editor tab whose underlying virtual file is constructed from the parsed summary and the read-only flag (across the boundary).
...
- **Lazy expansion in this surface is UI-only.** Despite the row-flag contract distinguishing repository rows from branch-folder rows, the tree-model build precomputes every descendant of every repository — files, branch folders, and nested sub-trees — eagerly, before the model swaps onto the UI dispatcher. The IDE's tree component then only lazily renders rows whose parents are expanded. A repository with thousands of memories pays the full enumeration cost on every refresh; the rendering deferral is not a fetch deferral. **That cost is now materially higher**: on top of the native directory walk, each refresh pays two cross-process round-trips per discovered repository for the manifest and index, plus the KB-root, initialize, reconcile and discovery calls. A parent folder hosting many repositories therefore pays a per-refresh cost that grows with the repository count *in round-trips*, not just in filesystem operations. (Notable; contrasts with spec 175's VS Code sibling, which does defer fetching.)
...
- **The classification badge map collapses one source type (`commit`) to a different letter (`C`).** Other types pass through unchanged. A future manifest type that this map does not enumerate yields an empty-string badge and a row that renders with no badge glyph. (Notable; intentional graceful degradation.)
...
- **The memory-viewer that opens when a memory row is double-clicked** (its layout, its read-only mode, its summary-document parsing) is defined by a separate spec.
...
- **The delegated transport itself** (how a request reaches the command-line surface, the daemon-first / one-shot-fallback behavior, and the timeouts) is owned by the IDE-delegation spec. This panel's observable coupling to it is that every metadata read can fail as a transport error and surface as the panel's generic error message.
...
- **The sibling VS Code Memory Bank Folder Browser** (spec 175) covers the same surface for the VS Code extension. The two diverge on lazy expansion (VS Code defers child fetches, this panel precomputes); on classification (VS Code surfaces `wiki` as a separate kind, this panel collapses it into "no badge"); on the "is-diverged" flag (VS Code computes per-file fingerprint divergence, this panel does not surface divergence at all); on the heal pre-pass (VS Code runs heal as well as reconcile, this panel runs reconcile only). Behaviors common to both surfaces — manifest-derived display title, hidden-children-set construction, dotfile filtering, directories-first sort, single-letter classification badge, current-repository auto-expansion — are described in both specs at the level of structural-flag contracts.
