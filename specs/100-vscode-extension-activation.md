...
4. **Read-only document providers.** Virtual document content providers are registered, one per scheme, so the read-only previews launched from Context rows and from memory-detail rows can resolve content on demand. Registering them before any such row can be clicked guarantees the click never races a missing scheme handler. They are registered in a fixed order — plan, then note, then the in-memory snapshot scheme — and that order is deliberate rather than incidental: the activation's own tests reach for the first two by registration index.

    | Provider | Serves | Body source | Cached by | Fallback body |
    | --- | --- | --- | --- | --- |
    | Plan preview | A committed plan | Read from the summary store at open time, optionally through a foreign repo's storage | The plan's slug | "Plan not found" |
    | Note preview | A committed note | Read from the summary store at open time, optionally through a foreign repo's storage | The note's id | "Note not found" |
    | In-memory snapshot preview | The skills aggregate (live and committed) and reference snapshots (archived and active) | Composed in memory by the opener, and **re-composed on demand by a resolver** | The whole encoded URI query, in a bounded LRU | An explanatory "snapshot no longer available" document |

    The third registration is what makes a preview tab survive a window reload: the host restores the tab and re-asks its provider for a body the extension host no longer holds. Activation supplies that provider with a resolver closing over the repository bridge and the workspace root — the two things needed to re-read the working skill registry, a commit's summary from any known repo, the active-reference registry, and an archived snapshot (rebuilding a foreign repo's reader from provenance carried in the URI itself). The first two providers have no resolver and no reload survival; their maps refill only when a row is clicked again.

    All three build their URI the same way — a sanitized title as the path segment, an identifying payload in the query — and all three now encode that payload as a single `ref` parameter holding base64url-encoded JSON, replacing the plain percent-encoded key-value form the plan and note schemes previously used. Cross-reference: spec 329, which owns the encoding's rationale, the snapshot scheme's namespaces, cache, refresh helpers and its one fallback out of the scheme.
...
9. **Initial branch read and status-store subscription.** A single asynchronous probe reads the current branch name (which fires the branch-change emitter when it resolves). The host subscribes to the status store's change event; the subscription mirrors the store's `enabled` and `configured` derived booleans into local state and posts the corresponding `enabled:changed` and `configured:changed` messages to the sidebar webview only on actual change. The `configured` boolean is a **three-armed** derivation — a cloud session, a non-empty vendor API key, or the recorded summarization provider being the local-agent one — the third arm keyed on the recorded *choice* and never on whether that tool is still installed. On a `configured` true→false flip only, the subscription additionally re-runs the local-agent presence sweep and pushes the refreshed list **before** posting `configured:changed`, so the re-raised onboarding panel paints from a current sweep rather than the (deliberately empty) startup snapshot. The previous fire-and-forget standalone status probe is replaced by this subscription path. A deferred `initial-state-readiness` barrier is created at this step; it resolves when the parallel-refresh step (15) completes, and the sidebar webview's `init` handler awaits it before posting `init` so the loading placeholder is visible until real state is available.
...
12. **Watcher subscriptions.** In order: sessions-file watcher, external-markdown-note save subscription, HEAD watcher (for branch switches; uses git's resolved path so it works in worktrees), orphan-summary-ref watcher (which gates Memories refresh on the lazy-load flag), worker-lock watcher (which sets a `workerBusy` context key and falls back to a manual refresh on lock release), references-tree watcher (create and change only — it pushes an out-of-band reference rewrite into that reference's open snapshot preview, and is gated on such a preview being open before it pays for the registry lookup; see spec 329). Watchers are subscribed in this order because each one's callback assumes the stores from step 6 already exist.
13. **Command registrations.** Every command listed in the manifest is registered with its real handler. The full set covers refresh, enable/disable, AI commit, squash, push, file/commit selection toggles, plan and note add/edit/remove/preview, memory search/clear/load-more/view/copy-recall, summary panel openers, settings, sign-in/sign-out, and the two onboarding-panel credential handlers (save an Anthropic API key; adopt a locally-installed agent tool).
...
15. **Initial data load.** A bootstrap routine first loads the exclude filter (so the very first file list is already filtered), then runs the four panel refreshes in parallel, then updates the status bar and the `enabled` context key.
    ...
    Once that settles — on the success *and* the error path — two further resolutions run inside the `initial-state-readiness` barrier before it is released, so the first `init` the sidebar receives already carries them and nothing flashes:

    1. The per-repo cold-start signals for the back-fill card.
    2. **A presence-only sweep for locally-installed agent tools**, whose result rides the initial state and decides whether the onboarding panel offers a local-agent card. The sweep is filesystem-only and launches **no subprocess** — the expensive per-tool capability probe is deliberately not run here, because activation runs on the extension host's single shared thread where a blocked event loop stalls every extension in the window. It is **skipped entirely for a user who is already configured** (they never see the card), so their list stays empty; that is a cost optimization, not a one-shot, and the step-9 subscription re-runs it if `configured` later flips back to false. It is run with **no explicit-executable-path override**, so a tool installed only at a hand-configured path is reported absent in onboarding. Any failure leaves the list empty, which renders the panel exactly as it read before this option existed. Cross-reference: spec 142 (onboarding panel), spec 280 (the detection predicates themselves).
...
- **The virtual document providers are registered per-extension-window, not per-preview.** The schemes are stable for the window lifetime. The plan and note schemes each keep one content map keyed by the previewed artifact's identifier, and an explicit change event invalidates the entry when the user re-previews the same id with new content; neither refills its map after a window reload, so a restored tab of either falls back to its "not found" body. The third scheme is the one that survives a reload, via the resolver activation hands it.
- **The providers are registered in a fixed order, and the order is depended upon.** The in-memory snapshot provider is registered third, after plan and note, because the activation's own tests address the first two by registration index. (Notable: a registration-order dependency that nothing in the product code needs.)
- **The status-store loads the global config unconditionally**, not only when the project is enabled. This keeps the configured-state derivation (a cloud session, a vendor API key, or a recorded local-agent provider) accurate across enable/disable cycles. Without this, disabling would flip configured back to false and replace the disabled panel with the onboarding panel, trapping the user.
- **The activation-time agent-tool sweep answers only "is it on disk", never "does it run".** The two questions differ by roughly three orders of magnitude in cost, and the second one launches a subprocess per tool. Activation pays the cheap one for every tool; the expensive one is paid exactly once, later, for the single tool the user picks in onboarding. (Surprising; deliberate cost split — see spec 142.)
- **An already-configured user's detected-tool list stays empty, and that is correct rather than a missing refresh.** The sweep is skipped for them because they never see the card. The one path where it matters — `configured` returning to false mid-window — re-runs the sweep and pushes the result before raising the panel. (Notable.)
...
- **Onboarding panel** — full-viewport sidebar viewport states (loading / onboarding / disabled / tab UI), the three-armed configured boolean, and everything the detected-tool list does once it reaches the webview are owned by spec 142.
- **Local Agent CLI Provider Backend (spec 280)** — owns the presence and runnability predicates that the step-15 sweep and the onboarding selection consume. Activation owns only when the cheap one runs and that the expensive one does not.
...
- **Context snapshot markdown preview (spec 329)** — owns the third document provider registered in step 4: its URI and query encoding, the four snapshot namespaces, the bounded body cache, the refresh and open-preview helpers, the frontmatter rewrite, and the shared title sanitiser. Activation owns only the registration order and the resolver it supplies.
