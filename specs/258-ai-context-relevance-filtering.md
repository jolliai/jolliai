...
Before a commit summary is generated, a pure-LLM relevance ranker assesses each active CONTEXT item (plan, note, reference) against the commit's change and uses a single batch call to (a) attach a per-item categorical tier (`high` / `mid` / `low`) emitted directly by the model, (b) attach a one-line reason, (c) order the kept items by tier for display, and (d) conservatively soft-exclude every item the model tiered `low`. The layer is always attempted **wherever it is invoked**, is credential-driven, and fails open on every error path: any failure keeps every item and excludes nothing. The call is now issued for **every** credential provider — including a locally-run agent-CLI provider, which previously short-circuited to keep-all without issuing anything; only the per-call wall-clock differs by provider. The provider is no longer a reason to skip; the **commit kind** still is. The layer belongs to the paths that derive a new memory from a change; a path that consolidates or migrates existing memories issues nothing and applies no tiering.
...
- Which commit kinds issue the call at all, which reuse a persisted ranking instead, and which issue nothing and therefore soft-exclude nothing.
...
### Which commit kinds issue the call

The layer has no gate of its own beyond an empty candidate list, but not every commit reaches it. The callers divide as follows:

- **The interactive pre-commit surface** ranks the pending change before any commit exists. Its ranking is persisted for reuse and is non-authoritative (spec 247).
- **The plain-commit path** — the same path also serves cherry-pick and revert — is the authoritative one. It builds a change signal from the commit against its parent, then either **reuses** the pre-commit surface's persisted ranking (when that ranking's change fingerprint matches its own, via orchestrator 2) or **issues** the call fresh (orchestrator 1). A plain commit therefore always ends up with a ranking, but only sometimes pays for one.
- **An amend** always ranks fresh — it never consults a persisted ranking — but only when it gets that far. The trivial-delta route returns before reaching this layer, so it issues nothing at all. The route taken when the delta summary comes back with no topics *does* issue the call (the call happens while the delta prompt is being assembled, ahead of that outcome being known), but then discards the verdicts: that route carries the previous memory's topics forward and associates the user's full selection minus their hard excludes, recording neither a soft-excluded set nor kept-item relevance records. Only the full amend route both issues the call and keeps its verdicts.
- **A squash — either route (an explicit squash commit, or a rebase with squash/fixup, which share one consolidation path) — issues nothing.** Consolidation merges the source commits' existing topic structures rather than re-deriving relevance from a diff and a transcript, so there is no fresh change for candidates to be judged against. The user's whole working-area selection is archived and associated with the consolidated commit, minus only their hard excludes; the consolidated record carries no soft-excluded set and no kept-item relevance records **of its own** (the source commits it absorbs keep whatever their own commits recorded). This is the same choice the amend routes that skip the call make.
- **A rebase-pick** carries an existing memory to a new hash 1:1 and likewise issues nothing.

The consequence worth stating plainly: **"no relevance call" and "no soft-exclude" are the same thing.** On the kinds that issue nothing, the only filter still applied to the user's working-area selection is their own hard-exclude set, there is no tiering, and nothing is set aside for audit — not because the layer failed open, but because it was never asked.
...
- **Exclusion selection store (spec 188)** — the persisted **full per-item ranking** (kind/key/tier/reason + original-exclude + optional dismissed flag) and change fingerprint this layer's output feeds, plus the dismiss/clear operations over them. Note the asymmetry between the two axes: the paths that *clear* the persisted ranking are a **wider** set than the paths that issue this layer's call — a squash and an amend clear a ranking they never read, because their op moves the branch tip and staleness, not consumption, is the trigger.
- **Squash consolidation (spec 13)** — the consolidation path that issues no ranking call at all: it merges the source commits' existing topic structures, so the working-area selection it archives is filtered by the user's hard excludes only.
