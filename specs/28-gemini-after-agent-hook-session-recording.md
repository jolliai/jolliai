...
The directory used as the project root is selected as follows, in order:
...
3. Otherwise, the `cwd` field from the payload.

If none of these is available, the handler falls back to the runtime's current working directory.
...
1. If the Gemini-specific project-directory environment variable is set, configure the diagnostic log directory under that path; otherwise fall back to the cross-agent project-directory environment variable for the same purpose.
