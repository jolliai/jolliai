...
- The normalization rules applied to the remote-origin URL so that trivially different forms of "the same remote" — including SSH transport variants, host aliases defined in the local SSH client configuration, and HTTPS clones of the same repository — collapse to one identity.
- The host-alias resolution step inside that normalization: what it asks, its cost, its cap, its fail-safe fallback, and the fact that its memo is never invalidated.
...
2. **Fold SSH and git transport forms to HTTPS, resolving the host token through the local SSH client configuration.** Any URL whose transport is not already `https://` (or `http://`) is rewritten:
   - `ssh://[user@]host[:port]/path` and `git+ssh://[user@]host[:port]/path` → `https://<resolved-host>[:port]/path`. The SSH default port 22 is dropped; any other port is preserved.
   - `git://host[:port]/path` → `https://<resolved-host>[:port]/path`. The git default port 9418 is dropped; any other port is preserved.
   - SCP form `user@host:path` (requires the `user@` prefix) → `https://<resolved-host>/path`. A bare `host:path` without a `user@` is not folded (to avoid mangling Windows drive paths like `C:/repos/foo` or bare Linux basenames that legally contain `:`), and therefore never reaches host resolution either. The path segment of the SCP form is appended directly after the host; an absolute SCP path (`host:/srv/repo`) becomes `https://<resolved-host>//srv/repo`.
   - `https://`, `http://`, `file://`, and all other scheme-prefixed URLs pass through this step unchanged — including their host token, which is **not** alias-resolved.

   **Host alias resolution.** The host token of every folded form is first passed through the local SSH client's own configuration, so a user-defined alias resolves to the real host it names. Two spellings of one remote — the alias and the real host — therefore produce one identity, and consequently one vault folder. Without this, a repository cloned through an alias and the same repository cloned through its real host folded to different keys and split into two folders on disk.

   The resolution is performed by asking the local SSH client to print its effective configuration for that host token and reading the resolved host name out of the answer. It is a configuration parse, not a network operation. Its properties:
   - **One subprocess per distinct host token per process**, memoised. A host with no matching alias block resolves to itself (the client echoes the token back), so the common case is a no-op that still costs the one subprocess.
   - **Bounded** by a five-second cap on that subprocess.
   - **Fail-safe**: any failure — the client absent from the search path, the cap expiring, unparseable output — yields the literal host token unchanged, and never raises.
   - **Never invalidated.** The memo lives for the process's lifetime.
...
The combined effect is that the same repository reached via SSH (`git@github.com:Foo/Bar`), via SSH through a locally-defined host alias (`git@my-alias:Foo/Bar`), and via HTTPS (`https://github.com/foo/bar`) all produce a single canonical identity string. Two different clients that cloned the same repository with different transports, or through different host spellings, will not create duplicate rows in the registry — and, for the same reason, will not create duplicate vault folders.
...
- **The folded host token is resolved through the local SSH client configuration, so a host alias and the real host collapse to one identity — and one folder.** This is the fix for a repository cloned through a locally-defined alias splitting into two vault folders alongside the same repository cloned through its real host. It applies only to the folded transports; an HTTPS remote's host is never alias-resolved, because an alias is an SSH-client concept and an HTTPS host is already the real one. (Notable.)
- **Alias resolution costs one synchronous subprocess per distinct remote host per process, and it runs inside the editor extension host too.** It is a configuration parse rather than a network call, and the common no-alias case still pays for it because a host with no matching block is resolved by asking and getting the token back. The cap is five seconds, and a failure of any kind falls back to the literal host — but a wedged client (most plausibly one whose configuration routes that host through a proxy command that hangs) blocks the caller for that full budget, synchronously, in a process that is also servicing a user interface. (Surprising; the fail-safe path bounds the damage without removing the stall.)
- **The alias memo is never invalidated, so editing the SSH configuration has no effect until the process restarts.** A user who adds, removes, or re-points an alias while an editor window is open keeps the previously-resolved answer for every remaining resolution in that process. The memo is keyed per host token, so the stale answer is scoped to the tokens already resolved. (Notable.)
...
- **Identity stability across `git remote set-url` is a non-property — except for transport changes.** Re-pointing the remote to a genuinely different host or path (e.g. moving from GitHub to GitLab) yields a different normalized URL and therefore a different `repoIdentity`. The vault layer treats the post-rename remote as a distinct repository: a new identity row will be appended on the next sync round, and the old folder will stay associated with the old identity (now orphaned but not deleted). Users who deliberately change their remote URL need to either accept the new folder or manually edit the registry. **Two exceptions.** First, switching between `git@github.com:owner/repo` (SSH/SCP) and `https://github.com/owner/repo` (HTTPS) for the same remote produces the same canonical identity after normalization — the transport switch is invisible to the registry. Second, switching an SSH remote's host token between a locally-defined alias and the real host it names is likewise invisible, because the token is resolved before comparison. The converse of that second exception is the one case where the identity moves **without the remote changing at all**: editing the SSH client configuration so a host token resolves somewhere else re-points the identity of a remote nobody touched. This is documenting code reality.
...
- The transport-folding step described here, host-alias resolution included, is shared verbatim with the local-disk folder-reuse predicate that decides whether an existing vault subdirectory belongs to this repository, so "is this the same repository?" gets one answer on both sides. The **separate** normalizer that produces the server-side binding key is deliberately alias-blind and is defined by the canonical-repo-URL-and-name-derivation spec, which records the divergence.
- The consolidation of vault folders that an earlier, alias-blind normalization already split on disk — which this fix cannot retract — is defined by the Memory Bank duplicate-folder consolidation spec.
