...
`firstUsedAt` / `lastUsedAt` come from the **row**, not the delta, because `SkillArchivedTotals` carries no time fields. The shared display projection reads them from the row for exactly the same reason, which is what keeps the pre-commit preview and the committed record in parity (spec 319).
...
**Every one of those paths also attaches the refs to the summary it writes**, each guarded on non-emptiness (`...(skillRefs.length > 0 ? { skills: skillRefs } : {})`) so a commit that archived nothing carries no field. The amend fresh-leaf path spells this the same way as its siblings; it previously guarded and archived the rows while building its leaf with `plans` / `notes` / `references` only, so the orphan-branch record existed and that commit's summary reported no skills.
...
- **`commitHash` is deliberately NOT the archival predicate.** It is set on every archive and never cleared, so gating on it froze a re-used skill out of every later commit permanently. The predicate is `uncommittedDelta`, i.e. the counters moving past `archivedTotals`. This is the single most load-bearing difference between skills and the plan/note lifecycle they otherwise follow. **The rule now has exactly one home** ([`cli/src/core/skills/SkillDelta.ts`](../cli/src/core/skills/SkillDelta.ts)) and every consumer imports it — the archival pass here, `detectActiveSkillsForBranch`, and the shared display projection (spec 319) that both IDE hosts read through. It is imported rather than restated because a local re-derivation of the guard rule already drifted out of sync once and hid every re-used skill from a panel.
...
- **`detectActiveSkillsForBranch` has no production caller.** It exists in `SessionTracker.ts` and is exercised only by tests; `SkillArchive` reads the registry itself, and the IDE surfaces read the shared projection (spec 319). All three import the same `uncommittedDelta`, so this one is redundant rather than divergent. Declared-but-unreachable.
