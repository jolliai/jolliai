...
- QueueWorker wiring: where archival runs on the plain-commit, amend and squash paths, and how the exclusion set is assembled.
...
**Rows are guarded BEFORE the orphan write**, which is the opposite of the reference flow's write-ahead ordering. That is safe here precisely because a guarded skill row is never deleted: a failed orphan write leaves the row recoverable rather than lost, and the guard clears no data.
...
Four call sites reach `consumeWorkspaceContext`: the normal commit pipeline (`executePipeline`), the amend short-circuit, the full amend path, and the amend fresh-leaf path. When `skillFiles` is non-empty it calls `storeSkills(files, "Archive N skill(s) for <commit|amend> <shortHash>", cwd, branch)`. Refs are attached to the summary only when non-empty (`...(skillRefs.length > 0 ? { skills: skillRefs } : {})`).

The **squash** pipeline archives separately, before `mergeManyToOne`, so the refs can be hoisted onto the root it writes — there is no second write to patch them in afterwards. A squash lands new work, so uncommitted skill rows belong on the squash commit exactly as they would on a plain commit. Unlike plans and notes there is no separate detect step; `associateSkillsWithCommit` reads the registry itself. It honours the user's exclusions the same way, read fresh via `readExclusions(cwd)`.
...
- **A row is guarded before the orphan write, which inverts the reference flow's ordering.** Safe only because the row is never deleted; do not copy the reference flow's write-ahead reasoning here, and do not copy this ordering back to references.
