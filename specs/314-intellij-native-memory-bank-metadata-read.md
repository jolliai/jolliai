...
- The *other* IntelliJ direct-filesystem reader, `FolderStorageReader`, and its per-read dirty-gate, containment guard, and attach preconditions (owned by spec 307; extended by spec 317). This spec states only how the two differ.
...
`FolderStorageReader` (spec 307) probes `<kbRoot>/.jolli/shadow-status.json` before **every** read and declines when it exists, because the reads it serves participate in a multi-file transaction: an amend or squash whose shadow write failed part-way would otherwise serve a pre-write summary for the rest of the session.

This reader deliberately omits that gate (`KBFolderReader.kt:36-43`) on two grounds:
...
- **Eventual consistency of the consumer.** Both callers feed a tree render. A manifest that is one transaction stale renders a tree that is one transaction stale, and the next refresh corrects it — which is not true of a summary body served to a viewer.
...
- **This is the only IntelliJ direct-filesystem reader with no out-of-sync gate.** That absence is exactly what distinguishes it from the accessor in spec 307, and the justification does not transfer: atomicity protects against a *torn* file, not against a file that is intentionally behind because a shadow write was swallowed. The trade is deliberate — a stale tree row corrects itself on the next refresh; a stale summary body does not. (Central design point.)
...
- **The writer's own lockstep note says this file does not exist.** `cli/src/core/FolderStorage.ts:24-26` still reads "`.jolli/index.json` is CLI-only (no read path in IntelliJ today) so its schema can evolve independently; if a future Kotlin read path starts consuming it, add it to the list here AND to AGENTS.md". That read path now exists, and AGENTS.md's "Critical rules" *does* register `KBFolderReader` as a second Kotlin reader parsing both `manifest.json` and `index.json` — so the repository currently carries a direct contradiction between the writer's header comment and its own contributor rules. The obligation is real regardless of which text a contributor reads first. (Surprising; a documentation hazard rather than a behavior.)
...
- **The reader takes an already-resolved root and resolves nothing itself.** It performs no root discovery, no claiming write, and no bridge call — unlike the *attach* path in spec 307, where locating the root is itself several bridge round-trips and can create a directory. Here the cost of resolving `kbRoot` sits entirely with the caller (`KBRepoDiscoverer.discover`, which is a bridge call).
...
- **IntelliJ Direct Memory-Mirror Read Path (307)** — owns the sibling native reader, its per-read out-of-sync gate, its containment guard, and its attach lifecycle; the contrast with this reader's ungated design is stated above.
