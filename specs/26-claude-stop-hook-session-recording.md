...
Record one entry into the local session registry every time the Claude Code agent completes a response turn, by reading a small JSON payload from the hook's standard input and persisting only metadata, then trigger a single incremental discovery pass that runs the shared plan, reference, skill and owner scanners against the same transcript — the first two off one shared watermark, the other two each off their own — all without any language-model call.
...
- The relationship between this handler and a third, skill-discovery scan that runs after the reference scan against its own independent watermark, and the ways it is deliberately decoupled from the plan-and-reference pair.
- The relationship between this handler and a fourth, owner-discovery scan that runs after the skill scan against its own independent watermark, the durability condition its mark advance is gated on, and the fact that it is the only machine-global write of conversation-ownership state this handler performs.
...
- Deriving which worktree roots a session visited or authored into, and the machine-global ownership ledger those roots are merged into — its shape, its own lock, its session cap, and every consumer that later reads it (covered by a neighbouring ownership-ledger topic). What this handler contributes is the trigger — passing the session identifier, the transcript locator and the resolved project directory — and nothing else: the owner scan owns its own watermark read, its own advance, its own ledger write, and its own error containment.
...
12. Run a single incremental discovery pass against the same transcript path. The pass first migrates any legacy per-scan cursors into one merged discovery cursor, then loads that cursor's line watermark, then runs four scans in fixed order:
    ...
    4. **Owner discovery** — invoke the shared owner-discovery driver with the session identifier and the transcript locator. It is likewise handed **no** starting line: it reads and advances its own `owners` per-extractor mark in the same cursor record (spec 24), and it never throws — it absorbs its own failures internally. The worktree roots it derives from its own window are merged into the machine-global ownership ledger under that ledger's own lock, and the mark advances **only when that merge was durable** (that is, the ledger's lock was actually held for it): a merge that ran unlocked because the lock timed out leaves the mark where it was, so the next invocation re-scans the same window and re-emits the same edges. This call is therefore **not** wrapped in a try/catch here either, and its outcome is not captured.
    
    After the first two scans, **only if the plan scan completed without throwing and the reference scan reported progress beyond the starting line**, persist the merged discovery cursor at the reference scan's reported line. Otherwise leave the cursor at its prior value. Neither the skill scan nor the owner scan takes any part in that condition, in either direction: a scan of either kind that failed cannot hold the shared cursor, and a plan scan that threw cannot hold the skills mark or the owners mark.
...
Session recording (step 10) and the telemetry flush (step 14) are **surface-independent and always run** on a non-disabled repository; only the discovery pass is gated. Everything inside the pass is therefore skipped together: a deferring invocation writes no plans, no references, no skills, and — because the ledger is only ever written from inside the pass — no owner edges either.

The gate is fail-safe in one direction only. Running when it could have deferred is harmless — all four scans are idempotent and dedupe on re-scan, so a redundant pass costs work, not correctness. Deferring when the other runner cannot actually run would be harmful, which is why condition 2 exists. The gate therefore removes wasted work and the cross-version cursor race; it is not the only thing preventing corruption.

### Branches

- **All inputs valid** → record is upserted into the session registry, then the discovery pass runs (plan first, references second, skills third, owners fourth), then the merged cursor is advanced and — independently of it and of each other — the skills mark and the owners mark are advanced, then the telemetry buffer is flushed.
...
- **Single-owner gate defers** (plugin invocation, launcher present on disk, and both canonical hooks registered repo-locally) → registry write happens, the discovery pass is skipped entirely, the merged cursor is not written, no owner edges are written, the telemetry flush still happens; logged informational message.
...
- **Both scans throw** → both failures are logged; the merged cursor is held; the skill scan and the owner scan still run; the handler returns success.
- **Both scans complete, reference scan reports the same line as the start** → no cursor write; nothing to advance. The skill scan and the owner scan still run and each may still advance its own mark.
...
- **Owner scan fails internally** → it logs its own warning, writes no owner edges, advances nothing, and returns normally; the plan and reference outcome, the merged-cursor decision and the skills mark are unaffected.
- **Ownership-ledger lock times out** → the merge still runs, unlocked and best-effort, but the owners mark is **not** advanced; the next stop re-scans the same window and re-emits the same edges idempotently.
...
- Zero or more merged writes of the machine-global ownership ledger, each performed under that ledger's own lock. This is the handler's **only machine-global write of conversation-ownership state**; every discovery, cursor and registry effect listed here is repository-scoped.
- At most one write of the merged discovery cursor at the end of the pass, gated as described in Step 12, and — independently of it — at most one advance of the `skills` per-extractor mark and at most one advance of the `owners` per-extractor mark in the same cursor record, the latter gated on the ledger merge having been made under the lock. The recording step itself does not read the transcript or call any model; transcript reads, if any, happen inside the four scans.
...
| Owner-discovery failure     | Any error inside the post-recording owner scan.    | Absorbed by that scan, which never throws: logged at warning by it, no owner edges are written, its own mark is left where it was, and the merged cursor decision is unaffected. |
| Ownership-ledger lock timeout | The machine-global ledger's lock cannot be acquired within its budget. | The merge runs unlocked and best-effort; the owners mark is **not** advanced, so the next stop re-scans the same window and re-emits the same edges idempotently. Not an error condition for the handler. |
...
- **Plan scan runs first; reference scan runs second; skill scan runs third; owner scan runs fourth.** The order is fixed. All four are independent — each swallows its own errors and does not short-circuit the others — but only the plan and reference scans feed the merged-cursor advance decision, which uses the plan scan's completion as its gate.
- **The third and fourth scans are deliberately outside the conjunctive gate, in both directions.** Skill discovery and owner discovery each read and advance their own per-extractor mark in the same cursor record, so a scan of either kind that throws cannot pin the plan/reference watermark, and a plan scan that throws cannot pin either of their marks. That independence is the entire reason per-extractor marks exist: each backlog is its own window, and on an upgrade it is a window the shared watermark may already have passed. (Surprising; load-bearing — see spec 24.)
- **Two of the four calls in the pass carry no try/catch at this site.** Neither the skill scan nor the owner scan needs one: each absorbs its own failures and returns normally, leaving its own mark unadvanced so the next stop retries the same window. Wrapping either here would be dead code, not defence. (Notable.)
- **The owner scan is the handler's only machine-global write of conversation-ownership state.** The other effects listed above — the session-registry upsert, the merged discovery cursor, the per-extractor marks, plan and reference and skill state — all land under the resolved project root. The ownership ledger is machine-wide by design, because the checkout that later needs a session's owner edges is frequently not the checkout whose stop hook recorded them, and it therefore carries its own lock rather than the worktree-local one the cursor writes share. (Notable.)
- **The owners mark advances only on a durable ledger write.** A ledger merge that had to run unlocked may have been clobbered by a concurrent peer, and advancing past the transcript lines that produced those edges would strand them for good; holding the mark instead makes the next stop re-scan the same window, and re-emitting the same edges is idempotent. This is the one advance in the pass gated on something other than the scan's own completion. (Surprising; intentional.)
...
- The owner-discovery scan that runs fourth — what counts as a worktree root a session visited or authored into, the machine-global ownership ledger it merges those roots into, that ledger's record shape, its lock and its session cap, and the commit-time consumers that read it back — is owned by a neighbouring ownership-ledger topic. The `owners` per-extractor mark it reads and advances, its legacy-seeding rule, and its independence from the shared watermark are owned by spec 24.
