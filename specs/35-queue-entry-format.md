...
- **trace id** (optional, string): a correlation identifier the producer stamps from its own ambient trace scope, which the consumer adopts while draining the entry so the producing hook's log records, the consumer's, and the outbound model and push calls made for this operation all carry one id across the process boundary. Both producers stamp it. Absent on entries written before the field existed, in which case the consumer mints a fresh id rather than going untraced. Serialized after the creation timestamp.
- **executing-session id** (optional, string): the agent session that the **post-commit hook itself** was running inside, read from the environment that hook inherited from the version-control process. It is stamped so the detached consumer can attribute a commit that landed in one checkout to the agent session that authored the change from a different checkout — evidence no field derived from the repository can supply, because a session driving one working tree leaves no trace in the working tree it commits into. Because only the post-commit hook writes it, it can appear on `commit`, `cherry-pick`, `revert` and `squash` entries and **never** on `amend`, `rebase-pick` or `rebase-squash`. It is read only by the kinds whose handler reads transcripts. Absent for a plain-terminal or graphical-client commit, and for every agent host that advertises no session id (exactly one host does today). Serialized after the creation timestamp and after the trace id, and omitted rather than emitted as `null`.
...
The two producers no longer write the same field set. Only the post-commit hook reads the environment for an executing-session id, so no entry written by the post-rewrite hook ever carries one — the field is structurally absent from every `amend`, `rebase-pick` and `rebase-squash` entry rather than merely usually absent. Everything else (the branch capture, the commit-source resolution, the trace id) is written the same way by both.
...
The producer constructs the in-memory record with the canonical field set, in the canonical order, and serializes it with two-space-equivalent tab indentation. Optional fields (`source hashes`, `commit source`, `branch`, `trace id`, `executing-session id`) are present only when applicable — `source hashes` and `commit source` when the kind requires them, `branch` whenever the producer could read the current branch, `trace id` whenever the producer has an ambient trace scope, `executing-session id` only from the post-commit hook and only when the environment it inherited named a session — and are never emitted as `null` placeholders. Both producers capture the branch the same way: they read the working tree's current branch at the instant the entry is built, and drop the field if that read yields nothing.
...
- **The producer partition decides which entries can ever carry an executing-session id, and that makes one consumer path inert.** The field is read from the environment by the post-commit hook alone, and the post-commit hook returns early on a detected amend — so an `amend` entry is written by the producer that does not stamp the field, and the consumer's amend handler, which does accept and forward the field, receives nothing on every real amend. The asymmetry is invisible from either side in isolation: the field is declared optional, both a stamping and a non-stamping producer are correct against the format, and an absent value is indistinguishable from a commit made outside an agent session. (Surprising; a consequence of the partition rather than a decision made about the field. The consumer-side consequence is stated by the **Amend Summary Migration** topic.)
...
- What the consumer does with an entry's executing-session id — the machine-global ownership ledger it backfills before resolving owners, and the owner-derived lower bound that then floors the transcript window — is defined by the **Claude session-ownership** topic; which handlers forward the field and which ignore it is defined by the **Git Operation Queue Worker** topic.
- The consumer's preference for the entry's captured branch over a live read (for summary attribution and stale-child cleanup, with a live-read fallback only for legacy entries) is defined by the **Git Operation Queue Worker** topic; the amend-specific use of the captured branch for the prompt-context gathering and the fresh-leaf branch label is defined by the **Amend Summary Migration** topic; the consolidation paths' use of it when archiving working-area context onto the consolidated commit — including the further fallback they apply when the field is absent *and* a live read fails — is defined by the **Squash Consolidation Summary** topic.
