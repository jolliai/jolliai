...
| `error_occurred` | A structured error code was raised. | `where` (discriminator — stage/subsystem), `code` (discriminator — enumerated code), `source?` (discriminator, optional), `retryable?` (boolean, optional). Content-free — never a message, stack, or path. |
