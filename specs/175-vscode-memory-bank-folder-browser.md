...
- **File kind** (file nodes only): one of `memory`, `plan`, `note`, `wiki`, or `other`, derived from the owning repository's manifest (see classification below). Absent on directory nodes.
...
- **The classification map collapses one source type (`commit`) to a different display kind (`memory`).** Other source types pass through unchanged. The asymmetry is historical and is duplicated nowhere else; renaming on either side would silently drop the labelling. (Notable.)
...
- **The manifest's record schema — its entry types (`commit`, `plan`, `note`, `wiki`), recorded path, file identifier, source-branch field, fingerprint, and recorded title — and the manifest's reconcile-on-rename pipeline** are defined by the folder-based-summary-storage spec. This spec only reads the manifest's `files` list and projects per-file rows.
