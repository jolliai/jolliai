...
**In scope:** the three modes of the entry-point command (the default proxy launch, the one-shot reindex, and the environment-forced in-process server), to the extent of naming them and what each one initializes; the fixed list of advertised tools with each tool's public name, description text, declared argument shape, required-argument flags, and abstract response shape; argument-validation rules done at the tool boundary; how a successful result is wrapped for the client; how a failed call is wrapped; how unknown tool names are handled; how missing-arguments fields are handled; the working-directory binding the server uses to resolve every tool call; the storage-backend initialization done once per server process; the per-process search-index cache that makes repeated searches cheap.
...
- The active-storage slot is process-global; it is written once in the per-process half of startup and only read by handlers thereafter, so every concurrent call — from any client — observes the same storage handle. That is also why it is never re-established per connection.
...
There is no explicit shutdown handler, and no tool-surface state is ever released early: the active storage handle and the search-index cache live for the life of the process.
...
- **A degraded manifest is retried, and only the platform half.** The storage half is a process-global side effect that is already correct and is the expensive one; re-running it per connection would undo the sharing entirely. (Notable.)
