...
A long-lived server speaks the Model Context Protocol, advertising a fixed set of named tools over one bound working directory's stored memory. Most tools are read-only projections (search, recall, topic timeline, branch catalog, PR description, queue status, installation status); the Jolli Space tools reach the Jolli Space backend (list spaces, bind this repo to a space, push this branch's summaries to the bound space). Every tool returns a JSON-text response.

**The process an AI host spawns is not that server.** It is a per-session proxy that speaks no protocol at all and forwards raw bytes; the long-lived server is a shared per-worktree background process reached over a socket or named pipe, and the same server code also runs in-process over standard input/output whenever that shared process cannot be reached. Which of the two is running changes nothing about the surface below — the same tool list, the same envelopes, the same working-directory binding — so this spec describes the surface, and the daemon/proxy topic owns where it runs.

## Scope

**In scope:** the three modes of the entry-point command (the default proxy launch, the one-shot reindex, and the environment-forced in-process server), to the extent of naming them and what each one initializes; the fixed list of advertised tools with each tool's public name, description text, declared argument shape, required-argument flags, and abstract response shape; argument-validation rules done at the tool boundary; how a successful result is wrapped for the client; how a failed call is wrapped; how unknown tool names are handled; how missing-arguments fields are handled; the working-directory binding the server uses to resolve every tool call; the storage-backend initialization done once per server process, and the one thing that can re-run it mid-life (the per-call re-check that rebuilds it when the repository's storage route has moved); the per-process search-index cache that makes repeated searches cheap.

**Boundaries:**

- The shared per-worktree daemon and the per-session byte-forwarding proxy — the pre-parser shortcut that routes the bare invocation, the singleton key and derived address, the handshake and its version comparison, the generation mechanism, every terminal that ends in an in-process server, the reap windows, and the degraded-manifest retry mechanics — are owned by the **MCP per-worktree daemon and proxy** spec (364). This spec owns the surface those processes serve.
...
- The reindex flag exposed by the same entry-point command rebuilds the local search index and exits without ever opening a transport; the index-rebuild data flow itself is owned by the search pipeline spec — this spec only documents that the same command carries this as one of its three mutually exclusive modes and that the reindex mode initializes the storage backend before doing anything else.
...
### Which tools need a repository

Each built-in entry additionally carries a declaration of **whether the tool needs a git repository to answer**. It is internal metadata, never part of the advertised schema, so it never reaches a client. Every built-in declares it, and the tenant-scoped space-listing tool is the one that declares that it does **not** need one — it asks the backend about the tenant, whereas `bind_space` and `push_memory` act on *this* repo and branch. Platform tools carry no such declaration and derive nothing from the bound working directory (their executor relays the call's arguments to a backend endpoint).

The partition governs two things in the dispatch path: a repository-requiring tool called when the bound directory is not inside a repository is refused, and only a repository-requiring built-in gets the storage-route re-check. This declaration, and the refusal it drives, are **pre-existing** — they were simply never recorded in this document.
...
When the gate is open, the server fetches a tool manifest from the backend **once per server process**, during the per-process half of startup and before any client is served, over the same authenticated channel the Jolli Space tools use: the long-lived product key, the tenant origin/base URL derived from that key, and the same bearer + tenant/org request headers. Each manifest entry carries a tool name, a description string, and an object-typed JSON-Schema argument shape — the same three fields a built-in registry entry carries — and each surviving entry is spliced into the advertised tool list alongside the built-ins. An entry may also carry a REST `binding` (an HTTP method plus a path) that the generic executor uses to reach the tool's backend endpoint; the binding is internal routing metadata, not part of the tool schema advertised to the client. An entry may additionally carry an optional `menu` block (`{ label, description?, order? }`) that flags the tool for the curated `/jolli` menu prompt (see below); it too is metadata, never part of the advertised tool schema.

The manifest fetch is **best-effort**: any failure — the endpoint being absent so the request is refused, the key lacking permission to invoke platform tools, no key configured, a network/timeout/abort error, or a non-JSON body — degrades silently to "no platform tools". A disabled or older backend therefore never breaks server startup; it simply yields the built-in-only surface. An individual manifest entry with a malformed **advertised-schema** field (a missing/blank name, a non-string description, or an argument shape that is not an object schema) is dropped rather than failing the whole manifest. A malformed `binding` or `menu`, by contrast, is internal metadata that is never advertised, so it does **not** drop the tool: it degrades at field granularity (a bad `binding` is discarded and the call falls back to the conventional endpoint; a bad `menu` just omits the menu item), so a partially-rolled-out or buggy backend never loses a working tool. The fetch uses a short timeout so a slow backend cannot stall startup.

**A FAILED fetch and a successful EMPTY manifest are distinguished, and they carry different downstream consequences.** Both degrade the advertised surface identically — built-ins only — so a client cannot tell them apart, but the prepared runtime records which one happened, in a **degraded** bit that is set only when the gate was **open** and the fetch **failed**. A closed gate is never degraded (nothing was attempted, and the closure is a configured choice), and an empty manifest from a healthy fetch is never degraded either: a tenant with no platform tools is a normal, permanent state. That bit is the input to the retry a shared long-lived process performs on a later connection, so conflating the two states — which the first version did, having only the list length to go on — turned a bounded retry into a manifest fetch on every single connection, awaited in front of that client's server construction, for the whole life of the process. The retry mechanics, and the fact that an already-connected client never sees recovered tools, are owned by spec 364; what matters here is that "the manifest is fetched once and any failure degrades silently" is now only half true — the *failure* half is retryable, the *empty* half never is.
...
The server captures one absolute working-directory string at startup and uses that same value to satisfy every subsequent tool call. That string is **not** the raw process working directory: the entry-point command resolves the enclosing git repository root of the launch directory and binds *that*, falling back to the launch directory when the resolution fails (not inside a git repository, or git is unavailable). An AI host that spawns the server from a subdirectory of the repo therefore still binds the repo root. There is no mechanism to change the binding without restarting the process — and that process is no longer the session's own: a shared per-worktree server is bound once, for every session on that worktree, and outlives any one of them. A session whose own launch directory resolved to a *different* worktree root never reaches that server at all (its derived address differs), and a session whose launch directory git could not name is deliberately never given one — both decisions belong to spec 364. The binding is also used as the kb-root argument when initializing storage — i.e. the same path is used for both the project root and the kb-root. The resolution rule itself is owned by the project-state-root-resolution spec (311).
...
The entry-point command exposes **three** mutually exclusive modes:

1. **Default (no flag)** — launches the per-session byte-forwarding proxy, which reaches a shared per-worktree server or, on any of its documented terminals, runs the long-lived server in-process over standard input/output. A bare one-word invocation is routed there by a pre-parser shortcut that never enters the general startup path; anything else falls through to normal parsing. (Spec 364.)
2. **Reindex flag** — performs an immediate full rebuild of the local search index and exits without ever opening a transport.
3. **Environment-forced in-process** — one named environment variable, tested for one exact value, forces the long-lived in-process server exactly as it behaved before the shared process existed. It is an environment variable rather than a flag because the host registrations write a fixed argument list, so a flag would have to reach every registrar and every already-installed configuration first.

The working-directory binding is resolved **once** — the enclosing git repository root of the launch directory, falling back to the launch directory — and shared by every mode (see 311). No mode re-reads the process working directory afterwards. Whether git actually named that directory is also observable, and is what decides whether a session may be given a shared server at all (spec 364).

### Startup sequence (long-lived server, per process)

The long-lived server's startup is split by lifetime: a **per-process** half that is identical for every client of one working directory, and a **per-connection** half that each client needs its own copy of. In the in-process case the two run back-to-back for the single client; in the shared-process case the per-process half runs once and the per-connection half runs once per attaching session, so **one process builds many server objects**. The steps below are the per-process half.
...
0. **Local-agent child guard (runs first, before storage init).** If the process
   ...
   any server object is built, before any request handler is registered, and
   before any transport is opened**, so nothing is served — no protocol
   negotiation and no advertised tools at all. There is no degraded or
   empty-tool-list server — there is no server. In the shared-process case the
   refusal is also **before anything is bound to an address**, deliberately: a
   bound-then-refusing process would look reachable to every future session,
   which would attach and get nothing.
...
2. Storage-backend initialization runs next — the first thing after the guard that touches memory state. The configured backend is constructed for the bound working directory (used both as the project root and the kb-root) and is installed as the process-wide active storage. This must happen before any tool handler runs; otherwise reads through the store would silently fall back to the default backend (the orphan-branch fallback), producing wrong results for folder-mode or dual-write configurations and emitting a warning on every read in this otherwise long-lived process. It is **also** the reason the storage half is never re-run per connection: it is a process-global side effect, and re-establishing it for each client would undo the sharing a per-worktree process exists for. That remains exactly true of *connections* — nothing about a client attaching rebuilds storage — but it is no longer true of the process's whole life: the handle is re-**checked** on every repository-requiring tool call and re-run at most once per observed cutover (see "Storage-route re-check before a repo-scoped built-in").
3. The platform-tool gate is read and, when open, the manifest is fetched — once for the process — producing the advertised tool list, the dispatch map, the curated menu, and the degraded bit (see "Backend-defined platform tools").

The result of these steps is one **prepared runtime**, held for the process's lifetime and handed by reference to every client. It carries no per-connection state.

### Startup sequence (long-lived server, per connection)

Repeated for each client the process serves — once in the in-process case, once per attaching session in the shared-process case:

1. A server instance is created with the advertised name "jollimemory" and the host package version. It declares the tools capability always, and the prompts capability only when the curated `/jolli` menu is non-empty; it declares no other capability. Each client needs its own instance because the protocol lifecycle is per-connection: every client sends its own initialization request, and one server instance binds to exactly one transport.
2. A list-tools handler is registered that, on request, returns the prepared runtime's tool list verbatim. **The list is captured by value at this point**, which is why a client that connected while the manifest was degraded keeps the shorter list for its whole life even after a later connection recovers the missing tools (spec 364).
3. A call-tool handler is registered (see below).
4. A transport is constructed and the server is connected to it. In the in-process case that transport is this process's own standard input and output, and the startup function awaits the connect call, resolving when the transport closes. In the shared-process case it is the remainder of one client's connection, and the process goes on serving its other clients.
5. One log line records the attachment and the working-directory binding. Its **level differs between the two shapes**: informational in the in-process case, debug in the shared-process case — so the attachments of a shared process's many clients are not visible at the level its single-client counterpart logs its one connection at.

### Startup sequence (reindex mode)

1. Storage-backend initialization runs first, identical to the long-lived server's, and for the same reason — otherwise the reindex would read from the wrong (possibly empty) store and report a misleadingly small document count.
...
2. **Not-inside-a-repository backstop** (pre-existing, and previously undocumented here). When the bound working directory is not inside a git repository and the named tool declares that it needs one, the handler throws, naming the tool, the bound directory, and the two ways out (start the AI host from the repository to be served, or enable the product there). The name is matched against the full built-in registry, so an unrecognized name still falls through to the dispatch table's own unknown-tool error rather than being reported as a repository problem.
3. **Storage-route re-check.** When the named tool is a repository-requiring *built-in* — never a platform tool — the handler awaits the storage-route re-check before dispatching (see "Storage-route re-check before a repo-scoped built-in").
4. The bound working directory, the tool name, and the arguments are routed through a fixed switch on the tool name, and the selected handler runs (see per-tool behavior below). Routing is by exact string match; a name belonging to a platform tool goes to the generic executor instead.
5. **One retry on the frozen-branch error.** The built-in dispatch of step 4 — and only it, never the platform-tool path — is wrapped so that a single specific failure is retried: a typed error the dispatcher recognizes without matching message text, raised when a write reaches an orphan branch a cutover has frozen. On that error the re-check's back-off is cleared, the re-check is re-run, and the same call is dispatched **once** more. Every other thrown value propagates unchanged.
6. Any value the handler returns is serialized to JSON and wrapped in a single text-typed content block. The error flag is set on this returned response **only** when the result is a non-null object whose `type` is exactly `"error"` (the result-shape path in the tool-result envelope); otherwise the response carries no error flag — including for a `binding_required` result, which is a normal "needs input" outcome.
7. Any thrown value from the handler is caught. The error message is extracted: an Error-shaped throw yields its message string, any other throw is coerced via string-conversion. The wrapper logs a warning naming the failed tool and the message. The response wraps `{ "error": <message> }` as JSON in a single text-typed content block, with the error flag set true.
8. An unknown tool name throws an error of the form "Unknown tool: <name>", which then flows through the same error-wrapping path above (resulting in a client-visible error response, not a transport-level failure).

### Storage-route re-check before a repo-scoped built-in

The storage backend this process installed process-globally at startup was resolved for the repository's state *at that moment*. A cutover committed later by another surface freezes the orphan branch, and a process that outlived that moment holds a pre-cutover handle: reads keep coming off the frozen ref — successful, and silently missing everything written to the database since — while a write on the same path (the lazy catalog rebuild behind `search` / `recall` is one) raises the frozen-branch error. Before every repository-requiring built-in dispatches, the handler re-checks the repository's current storage route and, when that route has moved the system of record off the orphan branch, rebuilds the process-global handle in place — no restart and no reconnect.

- **Per call, not per connection.** The failure being repaired is one connection that spans the cutover, so a check made when a client attaches would never fire for it.
- **Throttled to five seconds, and one-way.** A repository that probes as still orphan-backed is trusted for five seconds before the route is probed again, so the ordinary pre-cutover path pays at most one cheap probe per window; once the handle reads the database the check latches off, because a cutover never reverts. Concurrent calls coalesce onto one in-flight probe rather than each firing their own.
- **Anything inconclusive leaves the working handle alone.** A route probe that fails, and a route reporting that the database is unreachable, both back off and change nothing — a readable-but-stale handle beats turning a working read into a hard throw. A rebuild that itself fails also leaves the previous handle installed and backs off, so a later call retries.
- **The five-second window is exactly what the single retry covers.** A cutover landing inside the window is invisible to the check, so the dispatch throws the frozen-branch error; that throw is the proof the window is stale, so the back-off is cleared, the re-check re-runs unthrottled, and the call is dispatched once more.
- **Platform tools are excluded.** They take nothing from the bound working directory, so they can neither go stale nor raise that error.
...
1. After storage initialization and before any client is served, the server reads the activation gate. If it is closed (an explicit opt-out — the config flag set to `false` with the env override unset), the server skips everything below — no client is constructed and no manifest is fetched.
2. If open, the server fetches the manifest once for the process, drops entries whose name collides with a built-in (built-in wins, with a warning) and drops malformed entries, and builds the advertised tool list as the built-in registry followed by the surviving platform tools. The built-in registry array is never mutated; when no platform tools survive, the built-in array is advertised as-is.
...
4. When the fetch **failed** (the gate was open and no manifest came back — as distinct from a manifest that legitimately carried nothing), the runtime is marked degraded, and a shared per-worktree process re-fetches the **platform half only** on the next attaching client, never the storage half. That retry is bounded to one in flight and its clients-already-attached blind spot are owned by spec 364; the surface consequence here is that the advertised list can grow between two sessions of one process without any client being told.

### Concurrency

The server does not serialize requests itself; a transport may deliver call-tool requests concurrently and each handler runs as its own asynchronous task. **The real concurrency is now across independent clients, not within one client's request stream:** a shared per-worktree process holds many server instances at once, each bound to a different session's transport, and every one of them reads through the same process-global resources. Two of those guard themselves:

- The active-storage slot is process-global, and a call handler **can write it**. It is written once in the per-process half of startup and at most once more for the worktree this process serves, when the re-check above first observes a cutover. Every concurrent call — from any client — observes whichever handle is installed at the moment it reads the slot, so concurrent calls, **including calls belonging to other sessions on the same process**, may straddle that swap. Concurrent probes coalesce onto one, and the slot is still never re-established per connection.
- The search-index cache is a process-wide map keyed by the resolved index directory; concurrent `search` calls on the same directory may race to populate the same entry, but the recompute is idempotent and the source-signature check detects staleness on every call, so a concurrent loser's work is harmless overwrite. With many sessions on one process the cache is shared across all of them, which is the intended win, not a hazard.

One client's failure never reaches another: a client whose setup fails is dropped on its own, and a call-tool handler crash produces a client-visible error response rather than tearing anything down.

### Authentication and authorization

The server performs no authentication on its caller, and what that means depends on which of the two shapes is running:

- **In-process** — the transport is this process's own standard input and output, so the client is whoever launched it and there is nothing to authenticate.
- **Shared per-worktree** — the process listens at an address and accepts a connection from **any process that can reach it**. The only check it applies is a handshake in which it states its own protocol version, build version, process id and bound working directory, and the peer states its intent; the fields the *peer* writes are the only thing that could identify it, so a peer able to reach the address is a peer that is served. The protections that actually bound who can reach the address (an owner-only per-user directory on one platform, and its total absence on the other) belong to spec 364, including the known limitation that one platform has none.

In both shapes, tool responses include whatever data the bound working directory's memory contains, and there is no per-tool capability gating.

### Shutdown

There is no explicit shutdown handler, and no tool-surface state is ever released early. The search-index cache lives for the life of the process. The active storage handle lives that long too, but it is no longer the *same* handle for that whole life: the storage-route re-check replaces it in place when it observes a cutover.

- **In-process** — the startup function's awaited connect-promise resolves when the transport closes (the client closes its side, or the controlling process exits), and the process ends.
- **Shared per-worktree** — one client closing ends only that client's server instance. The process itself reaps **itself**, on whichever of three conditions arrives first: an idle window with no clients, a first-client window after binding with no client ever arriving, or a retirement request from a strictly newer build (which it honours only after its remaining clients drain, so an in-flight tool call always completes). The windows, the retirement negotiation and the stop reasons are owned by spec 364.

## State Transitions

The server has three observable lifecycle states, the last of which is now per-client rather than per-process:

1. **Pre-init** — process started, action entered. Every mode shares this state.
2. **Runtime-prepared** — the configured storage backend has been constructed for the working-directory binding and installed as the process-wide active storage, and the platform-tool half has been resolved into the prepared runtime. Reindex-mode stops one step short of this (it establishes storage, then performs its rebuild and confirmation print and exits). **This transition is skipped entirely** when either working-directory guard fires: nothing is bound, no runtime is prepared, and — in the shared-process case — no address is bound either.
3. **Connected (per client)** — a server instance has been built over the prepared runtime, its list-tools and call-tool handlers are registered, and one transport is open. In the in-process case there is exactly one such state and the process ends when it closes. In the shared-process case there is one per attached session, each entered and left independently, and the process outlives all of them until it reaps itself. There is no intermediate degraded state — a call-tool handler crash never tears down anything; it only produces a client-visible error response.

A fourth, process-level state exists only for the shared shape and belongs to spec 364: **retiring**, entered when a strictly newer build asks it to stand down, in which it stops accepting new clients (or, on the platform that cannot release its address, keeps listening while serving only its existing ones) and exits once they drain.

## Notable Behavior

- **The process an AI host spawns speaks no protocol.** It forwards raw bytes to a shared per-worktree server, or — on any of a documented set of terminals — runs the server described here in-process instead. Nothing on the surface distinguishes the two: same advertised list, same envelopes, same binding. (Mechanics: spec 364.)
- **Storage-backend setup is the first memory-touching thing every mode does, deliberately.** (In the long-lived server exactly one step precedes it — anchoring the diagnostic-log directory, which touches no memory state.) Without it, store reads inside the tool handlers would silently fall through to the orphan-branch fallback. For users running in folder-mode or dual-write mode this would return wrong data and would emit a warning on every store read — once per call in an otherwise long-lived process. The same trap motivates threading the storage handle through the search-index open and rebuild calls (so the index directory resolves under the memory-bank kb-root, not the checkout).
...
- **The bound working directory is fixed at startup, and it is a repository root, not the launch directory.** The one resolution happens in the command action, before the mode branch, and is reused for storage, for the kb-root, and for the diagnostic-log directory. A host that spawns from a subdirectory therefore gets the repo's own state rather than a second one rooted at the subdirectory — which for a long-lived, per-tool-call-logging process is the difference between one state directory and one per launch directory. The server has no mechanism to switch to a different repository while running; a client that wants a different repo's memory must reach a server bound to that repo. **What has changed is who "the process" is:** a shared per-worktree server is bound once and answers every session on that worktree, so the binding now outlives any single session, and sibling worktrees of one repository deliberately get separate servers rather than sharing one — several tools are branch- or worktree-scoped, so a repository-wide binding would answer for the wrong branch, silently. (Resolution rule: spec 311; keying and lifetime: spec 364.)
...
- **One process now builds many server instances, and each captures its tool list by value.** The expensive half of startup — the guards, the log anchor, storage, the manifest — runs once; the cheap half runs per client, because the protocol lifecycle is per-connection (each client sends its own initialization request and one instance binds one transport). The consequence a caller can observe: the advertised list a client got is frozen at the moment it connected, so a later client of the same process may see *more* tools than an earlier one, with nothing in either client's list saying so. (Surprising; intentional.)
- **A shared server accepts a connection from any process that can reach its address, and the only check is on fields the peer writes.** In the in-process shape there is nothing to authenticate; in the shared shape the handshake states what *this* server is and asks what the peer wants, so reachability is effectively the authorization. What bounds reachability — and the platform where nothing bounds it at all — is spec 364. (Notable.)
- **A degraded manifest is retried, and only the platform half.** The storage half is a process-global side effect and is the expensive one; re-running it per connection would undo the sharing entirely. What has changed is the other half of that reasoning — "already correct" no longer holds unconditionally, because a process that outlived a cutover holds an incorrect storage half. Its repair is per repo-scoped **call**, not per connection, so the per-connection retry described here still never touches storage. (Notable.)
- **A repo-scoped call can rebuild the storage handle for the whole process.** The re-check before every repository-requiring built-in exists because a long-lived process that outlived a cutover otherwise reads a frozen ref successfully and forever, missing everything written to the database since. It is per call (the reproduction is one connection that spans the cutover, so an attach-time check would never fire), throttled to five seconds, one-way, and inconclusive answers leave the working handle in place. Because the slot is process-global, the swap lands for every session attached to this worktree, mid-call ones included. (Notable.)
- **The single retry does not guarantee the caller never sees the frozen-branch error.** It re-dispatches once; if that re-dispatch throws the same typed error, it is surfaced like any other thrown failure — a `{ "error": <message> }` result with the error flag set. (Notable.)
- **One tool can never take that retry at all.** The memory-push tool converts every throw into a structured error *value* rather than throwing, so the type check the retry keys on never runs for it. A cutover landing inside the probe window therefore reaches the caller as that tool's ordinary error result, carrying the frozen-branch message — whose text tells the reader to restart this process, advice that is wrong for this host, since the very next call would have healed it. (Surprising.)
...
- The **storage-route re-check** the dispatch path awaits — the five-second throttle window, the coalescing of concurrent probes, the one-way latch, and the rule deciding which routes have moved the system of record off the orphan branch — is shared with the editor-extension host and owned by spec 370. This spec records only what this surface does with it: which tools it runs ahead of, that it rebuilds the process-global handle, and the one retry the dispatch path wraps around the resulting error.
- The per-client configuration files that point a client at this server (so the client knows to spawn the entry-point command) are owned by the MCP-client-registration spec. Those files were unaffected by the move to a proxy — the descriptor they write is still a plain standard-input/standard-output spawn of the same command.
- **Where this surface runs** — the per-session byte-forwarding proxy, the shared per-worktree background server, the pre-parser shortcut that routes a bare invocation into the proxy, the singleton key and derived address, the handshake and its version comparison, the generation mechanism on the one platform that needs it, the enumerated set of terminals that end in an in-process server, the reap windows and stop reasons, and the mechanics (and blind spot) of the degraded-manifest retry — is owned by the **MCP per-worktree daemon and proxy** spec (364). It also records the per-tool telemetry a session loses when the shortcut bypasses the general startup path, and the known limitation that one platform has no address-ownership protection at all.
