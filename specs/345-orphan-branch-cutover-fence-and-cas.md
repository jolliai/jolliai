...
- The consumer-side repair a long-lived process performs when it catches one of the ref-backed write batch's two refusals — the lazy route re-probe, its throttle and latch, and the in-place storage rebuild — covered by **Stale Storage Heal After an Unwitnessed Cutover**. This spec states only that the refusals are typed and what a catching host does instead of restarting.
...
1. **The ref-backed write batch refuses outright.** Immediately before the plumbing write — after the manual-disable gate — the batch re-reads the marker from disk and throws if it is present, with a message telling the caller its process holds a pre-freeze storage object and must be restarted. The throw is **typed**: the message text is unchanged, but a dispatcher can recognise the condition without matching on it. Reading from disk at write time is what closes the window for a long-lived process (an editor host, a server, a worker started before the freeze) that is holding a storage object built when the repository was still unfrozen. The hosts that carry the consumer-side repair now catch that typed throw, and for them the message's restart is no longer the remedy: such a host treats the throw as proof the route moved, clears its route-probe back-off, rebuilds its storage against the current system of record, and retries the operation **once** — so the refusal reaches the caller only if the retry fails too. Those same hosts also re-probe the route lazily on their own call paths, which *replaces* the held object with no external signal at all, so the claim that no cache invalidation can reach one no longer holds either. For every other surface the write-time re-read remains the last line, and the drift probe below remains what catches a writer that bypassed the freeze outright.
2. **The same batch then asks the second witness.** It probes whether a swap has been recorded for this working directory's identity and throws if so, because the marker is per-clone and a clone the swap never enumerated would otherwise write onto a ref nothing will read again. It raises the **same typed throw** as refusal 1 — both say one thing to a caller: rebuild against the database. This probe **fails open**: an unreadable or absent database answers "no", so an unfrozen repository is never blocked by a broken database — and therefore a clone with no marker, of a repository that *has* been cut over, can still write the retired ref whenever its database cannot answer. The drift probe below is what catches that.
...
10. **Old runtimes are NOT stopped, and that reverses the original design.** The marker used to be one of two inputs to the derived composite that already-shipped runtimes read as "this repository is disabled", so that such a runtime would stop writing the frozen ref. It no longer contributes: the composite mirrors the user's own opt-out alone. The reason is that the composite is the only field those runtimes read, and at least one shipped plugin build treats it as a user decision and *uninstalls the repository* on session start — deleting the shared version-control hooks and every worktree's tool registrations, which a current surface then reinstalls, in a loop the user sees as the integration turning itself off after every commit. Since those builds cannot be changed from here, the brake was removed rather than the wrecking ball. What replaces it is the drift probe: an old runtime's writes move the ref, the probe reports it and catch-up imports them, so they are stranded rather than lost. The accepted residual is that such a runtime still READS the ref and shows a view missing everything written to the database after the swap. A profile still carrying the old fence-derived composite is repaired on the next read, because a derived field is otherwise only recomputed by a write. (Spec 145 owns this field and its precedence.)
...
**Already recorded.** Open the database and check for the state row; if present, the outcome is `already-cutover`. This open is deliberately unguarded. It used to be wrapped, because a database whose schema was *ahead* of this build could not be opened writably at all and turned the command into a stack trace that changed nothing; the database no longer refuses a build over its version, so a newer format opens and writes normally with one warning, and the only errors left here are real faults that should propagate.
...
1. **Pin every clone's tip.** A clone with **no parallel ref at all** pins a sentinel empty tip rather than refusing: there is nothing to migrate, so nothing could ever make a refusal turn into a success, and the marker is what stops a later runtime from creating the ref and writing memories nothing reads. Such a clone is imported from an empty listing — not skipped, because the import is also what registers the repository's database row, which the recording transaction looks up. All subsequent reads of a clone that does have a ref go through the pinned commit, never through the ref name. Every later comparison against a live tip folds "unresolvable" onto the sentinel, in both the re-verification and the drift probe; a ref that exists at either of those points but did not at pinning time is therefore a moved tip / real drift.
...
4. **Compare every clone at its pinned tip** by containment, and treat the result as a **report rather than a veto**. Extra paths in the database are never a reason to refuse; neither, any longer, are paths the database fails to reproduce. Every finding across every clone is collected, logged in one line, and recorded on the state row, and the run proceeds. Two refusals survive, and both are asked of the **import** rather than of the compare: a clone whose tip lists memories the import stored none of, and a clone the import stored no row of *any* kind from while its tip listed artifacts in any family at all. Either means the import did not land, and either is `not-ready` with nothing frozen.
...
**Failure policy after the marker is up.** Every step that can fail past the freeze returns an outcome rather than throwing: the import and compare on a retry (they run with the marker up and can fail for reasons unrelated to readiness — a concurrent writer holding the database's writer lock past its busy timeout, a version-control read failure, a migration entry hitting a damaged file), the recording transaction itself, and a repository row that has vanished between the import and the transaction. All of them carry the "you are frozen, writes go to the database, re-run to finish" guidance, because the alternative is a stack trace out of an uncaught command action that never tells the user what state the repository is in.

### The containment compare

**What it decides.** Nothing, in the ordinary case — it is the switch's *receipt*, not its gate. The gate is the import: a fault there throws and nothing downstream runs. This is deliberate and replaces the opposite rule, under which a single path that would not read back kept a repository unfrozen forever. That was not a safety property in practice. A real parallel ref accumulates a small number of paths the import can **never** store — a memory whose embedded child list names a commit that has no memory file of its own, a body rewritten in place after its row was written — and those are stable and self-sustaining, so every attempt failed on the same path and the repository could never leave the legacy ref. What the freeze actually costs is knowable and small: the marker **freezes** the ref rather than deleting it, so every unreconciled path is still readable at the pinned tip the record itself names.

So the compare collects **every** finding rather than returning the first, logs one bounded line naming them, and records an exact count with a capped sample on the state row. The command prints that note both when it commits and on a later status query.

**Two shapes still refuse, and both are measured on the import rather than on the comparison.** They are one rule seen from two sides, not a rule plus an escalation, which is why neither may be merged into the other:

- A clone whose tip lists memories the import stored **none** of. This may not be weakened to "did anything land at all": the memories *are* the repository's content, so a repository whose memories all failed while one plan imported would freeze on the strength of that plan and then report every memory it has as unreconciled. Nor may it be expressed as a comparison ratio — "every visited path mismatched" reads as equivalent and is not: a repository holding a single dirty memory mismatches one of one, and was refused by the very rule this change exists to remove.
- A clone the import stored **no row of any kind** from, while its tip listed an artifact the import would take in *any* family. This exists solely for the tip the first refusal cannot see: one carrying no memory at all, which the first test answers "lists nothing" to however little the import took — a reachable shape, since a plan can be written to the ref with no commit and no memory behind it. Its stored-side condition is deliberately the whole-import question rather than a per-family one, because "the tip lists notes and no note was stored" would strand a repository forever on one un-importable note.
...
The compare reports the number of paths it visited, the first finding as a one-line detail, and the full list of paths that did not read back.
...
It reads the recorded state row through a **read-only** database handle and tolerates a database it cannot open: a writable open would migrate the schema as a side effect of a diagnostic. A schema *ahead* of this build is not one of the cases it is tolerating — there is no version gate, so such a file opens and writes normally. An absent or unreadable database, or an identity with no recorded row, means "no drift to report".
...
**The probe also runs without being asked**, and that is what makes it a safety net rather than a diagnostic. The same automatic trigger that presses the swap runs the probe instead whenever it finds a repository already recorded as cut over. Its properties:
...
- **prepared → prepared** — a `not-ready` before any marker was written (unregistered repository, or either of the import's two refusals). Nothing was frozen and nothing changed.
...
- **A repository with no parallel ref cuts over immediately.** It used to be refused for having nothing to migrate, which is the one refusal that can never be retried into a success. Freezing an absent ref is not a formality: the marker is what keeps a runtime that never saw it from creating the ref later. (Surprising; the empty case is the easiest cutover, not an error.)
- **The compare cannot refuse the switch, and that inverts its original design.** It was the gate; a single path that would not read back kept the repository unfrozen permanently, and on a real ref a handful of such paths are stable and self-sustaining, so the repository could never leave the legacy ref at all. It is now a receipt: findings are collected, logged, recorded on the state row, and printed — the gate is the import, which throws. **Two import refusals survive** — no memories stored from a tip that lists some, and no row of any kind stored from a tip that lists an artifact in any family — because each is an import that did not land rather than dirty data. Neither may be merged into the other, softened into a per-family veto, or spelled as a comparison ratio, which a one-memory repository trips. (Surprising; the safety argument reversed once the failure was measured on real data.)
...
- **There is no admission check on installed surfaces, and its removal is the decision.** Refusing the swap whenever any registered surface sat below a version floor meant one un-upgraded editor extension pinned every repository on the machine at `uncutover` forever, with the reason reaching only the debug log. The risk it guarded — an old surface still writing the frozen ref — moves to the drift probe, which both reports such a write and imports it. (Surprising; a guard deliberately deleted, not forgotten.)
...
- **The attempt is stamped before it runs.** A crashing attempt still burns its throttle slot. (Notable; intentional.)
- **Two throttles live next to each other here, at different scales and over different questions.** The attempt throttle above suppresses a repeat *swap* for two hours. The consumer-side repair described at refusal 1 carries its own throttle of **five seconds** over the route probe that drives it — a bound on how often a long-lived process re-asks whether the route moved, not on how often the switch may be pressed. Neither bounds the other. (Notable.)
...
- The consumer-side repair that catches those typed refusals — the lazy route re-probe on a long-lived process's own call paths, its five-second probe throttle, its one-way latch, the in-place storage rebuild, and the single retry — is defined by **Stale Storage Heal After an Unwitnessed Cutover**.
