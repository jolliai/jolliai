...
4. **Compare every clone at its pinned tip** by containment, and treat the result as a **report rather than a veto**. Extra paths in the database are never a reason to refuse; neither, any longer, are paths the database fails to reproduce. Every finding across every clone is collected, logged in one line, and recorded on the state row, and the run proceeds. The single exception is asked of the **import**, not of the compare: a clone whose tip lists memories the import stored none of did not land, which is `not-ready` with nothing frozen.
...
So the compare collects **every** finding rather than returning the first, logs one bounded line naming them, and records an exact count with a capped sample on the state row. The command prints that note both when it commits and on a later status query. One shape still refuses, and it is measured on the import rather than on the comparison: a clone whose tip lists memories the import stored **none** of. Expressing it as a comparison ratio — "every visited path mismatched" — reads as equivalent and is not: a repository holding a single dirty memory mismatches one of one, and was refused by the very rule this change exists to remove.
...
- **prepared → prepared** — a `not-ready` before any marker was written (unregistered repository, or an import that stored nothing from a non-empty tip). Nothing was frozen and nothing changed.
...
- **The compare cannot refuse the switch, and that inverts its original design.** It was the gate; a single path that would not read back kept the repository unfrozen permanently, and on a real ref a handful of such paths are stable and self-sustaining, so the repository could never leave the legacy ref at all. It is now a receipt: findings are collected, logged, recorded on the state row, and printed — the gate is the import, which throws. Only an import that stored *nothing* from a non-empty tip still refuses, because that is an import that did not land rather than dirty data — and it is deliberately not spelled as a comparison ratio, which a one-memory repository trips. (Surprising; the safety argument reversed once the failure was measured on real data.)
