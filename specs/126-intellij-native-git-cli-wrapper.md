...
A separate worktree-resolution helper returns the project directory unchanged. It takes no arguments, reads nothing from disk, and cannot fail; its nullable return type is vestigial (see "Worktree resolution" below).
...
The wrapper exposes a method that returns the **current** worktree root — which is simply the project directory it was constructed with, returned unchanged. It parses nothing, inspects no `.git` entry, walks no parents, and has no failure path; the declared nullable return type is never null in practice, and callers' `?: basePath` fallbacks are dead.
...
- Worktree resolution touches no filesystem at all — it is a field read. The method name still says "worktree", and the field that stores its value on the project service is still named for the *main* repo root (spec 124), but both now hold the current worktree root; renaming the field is deferred as a large mechanical cleanup.
...
- **The wrapper's own self-description no longer matches its call sites.** It documents itself as an IDE-only surface scoped to display-time reads, on the grounds that domain-level orphan-branch input/output has moved to the command-line surface. Its remaining callers contradict that: the summary reader still performs every orphan-branch **content** read through this wrapper as direct tree-listing and blob-read calls — unconditionally for the summary list and for committed-conversation rendering, and as the fallback leg for the three single-item content reads (an individual summary, a plan body, a note body) — and the plugin's commit, amend, squash, and push mutations still drive git through it directly. The actual split (below) is the behavior; the self-description is aspirational.
...
- **Orphan-branch reads and writes are split.** Orphan-branch **display reads** still call this wrapper for every plumbing operation, but two of them do so unconditionally and three do so only as a fallback. The summary reader lists tree entries and reads blobs from the orphan branch directly through it: **always** for the summary list and for committed-conversation rendering (including the stored-transcript reads those rendering paths perform), and **only when the direct memory-mirror read source is absent, declines, or holds no such item** for the three single-item content reads — an individual summary, a plan body, and a note body. Those three consult the mirror first when one is attached; that read source, its eligibility rules, and its decline conditions are owned by spec 307. This wrapper remains the only path to the orphan branch, so every mirror decline lands back here. The **storage-provider surface does not**: the IDE's storage handle is a command-line-backed provider that routes read / list / exists / ensure / write-files through the bridge's `storage` action, and it **never dereferences the git-wrapper argument it still accepts** — that parameter survives only for source compatibility with the previous constructor shape. So an orphan-branch write on this surface never touches this wrapper, while an orphan-branch read for display always does. This spec defines only the subprocess plumbing, not the orphan-branch semantics.
