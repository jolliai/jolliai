...
A repository whose user recorded the durable, repo-wide "memory capture is off here" opt-out is supposed to receive no writes at all. The mechanism that delivers most of that promise is a **process-wide in-memory suppression flag** that mirrors the durable opt-out, consulted by twenty-odd write sites that sit on synchronous or latency-sensitive paths where the durable reader (async, one source-control subprocess plus a file read, and itself capable of persisting a migration decision) is unaffordable. Only the **editor host process** ever sets that flag. Every gate built on it is therefore inert in command-line invocations, source-control hooks, agent hooks, the background queue worker, the compensation worker, the host bridge, and the notification daemon. Those surfaces are covered instead by **durable** reads: each hook and worker at its own entry point, plus — no longer only there — a set of write sites that read the durable opt-out directly because the mirror could never reach them. The zero-write contract is consequently still a property of one process for its mirror-based half, but no longer only that: the long-lived host bridge now refuses an enumerated set of data-write actions on its own durable read, and mid-session plan registration reads both. On top of that sit two deliberate carve-outs — the telemetry recording primitive is not gated at all, and machine-global writes are outside the contract entirely — plus one **undeliberate** carve-out, a pair of Memory Bank mutations on the editor's folder tree that read neither signal and one of which can empty a disabled repository's Memory Bank into the archive; and one refusal that is reported as a **success** carrying a manually-disabled discriminator, which is why no caller may read success alone as "the write happened".
...
- The complete inventory of writes suppressed while the flag is set, each one's trigger, what happens in its place, and whether reads keep working — and which rows are gated on the **durable** opt-out instead, because the mirror could never reach the host that also triggers them.
- The **carve-out rows**: writes into a disabled repository's Memory Bank that no gate covers at all.
...
- The **bridge write refusals**: which of the long-lived server's actions refuse on a durable read, which operations stay readable, and why the refusal is explicit rather than a silent success.
- The **zero-write success discriminator**: the one refusal that is reported as a success, why, and the obligation it places on every caller.
...
Most rows below are gated on the **in-memory flag**; every row that is not says so explicitly, and there are now four such rows (the startup skill reconciliation, the mid-session plan registration, the rebuild action, and the settings-driven hook sweep). "Reads" records whether the corresponding read path keeps working while the write is suppressed.

The inventory is **not** the complete set of writes a disabled repository can receive. Beyond the two carve-outs at the end of this spec (telemetry recording and machine-global writes), a separate table of **ungated Memory Bank writes** follows the scope limitation below — writes into the disabled repository's own Memory Bank folder that no gate in this contract covers.
...
| 20 | The plan-directory watcher's registration | An agent session saving a plan file while the repository is disabled (the watcher keeps firing) | The plan is not registered. It is not lost either: saving it again after re-enable, or the agent-hook path once hooks are reinstalled, registers it. **Gated on the mirror OR the durable opt-out, read through the synchronous read-only reader** — the mirror alone left the whole gate inert inside the long-lived bridge server, which is where mid-session plan registration actually runs on the JVM host. The registration entry point re-applies the same two-part gate | Unaffected |
...
| 22 | The "Migrate to Memory Bank" rebuild | User click, from **either** the editor's settings panel or the local dashboard's settings page | **Refused outright with a message.** A partial run would be worse than useless: the identity write is suppressed, but the folder creation and the re-point configuration write are not — one click would de-identify the previous folder while migrating nothing. The sequence itself is shared core driven by both hosts, and its refusal is therefore gated on the **durable** opt-out read from disk, ahead of the entire sequence — the mirror is inert in the dashboard's process, so a mirror-only gate would leave that host's button unprotected. The editor additionally checks the mirror before dispatching, so on that host the refusal fires twice for the same reason | n/a |
| 23 | The settings-save integration hook sync | Saving settings, from **either** host | Skipped in **both directions** — neither installs nor removals run. Removals are unnecessary (disable already uninstalled), and installs would silently override the opt-out. The settings write itself still happens (see carve-outs). The dashboard's sweep is the wider one — it walks every registered repository and every worktree of each — and gates **per clone** on the **durable** opt-out read from disk, skipping that clone's worktrees entirely; the editor's own sweep gates on the mirror for its single workspace | n/a |
...
| IDE bridge command | **Partially — no longer wholesale** | Its data-write actions are now **refused at the action layer** on a durable read (see *Bridge write refusals* below), so the memory / transcript / plan / reference / container writes a JVM host requests do not land. What is still unprotected there is everything outside that enumerated set: most visibly its commits listing, which runs the cross-branch alias persist (row 10) with the mirror `false`. |
...
### Ungated Memory Bank writes — the carve-out rows

The rows above are the writes this contract *covers*. Two Memory Bank writes are covered by nothing: they run inside the editor host, where the mirror is set and correct, and they simply do not read it — nor the durable opt-out. Both are triggered by a user-initiated Refresh of the editor's Memory Bank folder tree, which for as long as this contract has existed was a read-only surface.

| Ungated write | Trigger | What it does to a disabled repository |
|---|---|---|
| The archival sweep of Memory Bank folders that provably hold nothing | The folder tree's Refresh, at the Memory Bank scope or the everything scope | Moves every discovered folder that holds nothing — the current repository's own excepted — into the vault's hidden archive directory, creating that directory if needed, under the per-vault write lock. A disabled repository's *own* folder is protected only incidentally, by the current-repository guard that exists for a different reason; every other project's empty folder in that vault is archived regardless of whether **it** is disabled |
| The duplicate-folder consolidation | The same Refresh, immediately after the sweep, on modal confirmation | Copies files between folders, rewrites the survivor's metadata registries, and archives the drained folders — or, on the rebuild classification, archives **every** folder for the repository and then attempts to re-populate a recreated one. On a disabled repository that rebuild path is the worst case in this spec: the archival is not gated, so it happens; the identity write into the recreated folder *is* gated, so it is suppressed; and the copy engine's own entry gate returns its transient skipped outcome, so nothing is copied back. Every memory ends in the archive, nothing is rebuilt, and the completion notification reports the survivor with a memory count of zero |

Both are recoverable — archiving is a move, never a deletion, and the system of record is untouched — but both are writes into a disabled repository's Memory Bank, and the second one empties it. Neither the mirror nor the durable opt-out is consulted anywhere on either path. Contrast the sibling operation on the very same surface: the reconcile-and-heal pre-pass on a listing (row 21) *is* gated, and the rebuild action (row 22) is refused outright for precisely the reason the consolidation's rebuild demonstrates.

### Bridge write refusals — the second gate that escapes the mirror

The long-lived host bridge is a command-line process, so every mirror-based row above is inert inside it. That is not a theoretical exposure: the argument "a disable uninstalls the hooks, so no new writer starts" does not apply to a server the JVM host **already had running** when the disable landed, and every storage backend behind it carries only the same bare mirror gate — so the exposure is not specific to one route.

Two of the bridge's actions therefore carry their own gate, applied **at the action layer** on a durable read, before any storage object is constructed:

| Action | Operations refused | Operations left readable |
| --- | --- | --- |
| Raw container access | The single raw **write** | Read, list, batch-read, and the container-creation step |
| Memory store | Storing a memory, storing its files, writing a plan, writing a reference, writing a transcript batch | Every read operation |

Four properties are load-bearing:

- **The read is the durable one, and it answers the AUTHORED opt-out, not the derived composite.** Reading the composite would stop the current runtime on exactly the repositories whose system-of-record branch is frozen — the ones where the database is the only place a memory can go (spec 145).
- **The gate runs before the write path takes its cross-worktree lock**, matching the hook-reconciliation action's own gate: a disabled repository should not contend for a lock, and the flag is stable across it.
- **The refusal is explicit, not a silent success.** It answers *not ok* plus a manually-disabled marker. A host told a write succeeded when nothing was written has no way to notice; an older host that drops the unknown marker field still sees a failed write, which is the safer of the two things to be wrong about. **This is the opposite polarity from the install refusal below, deliberately** — see the discriminator section.
- **The container-creation step is deliberately excluded.** It creates an empty container, and the install flow that calls it is already gated at its own action.

A third action carries the same durable gate for the same reason: the per-worktree assistant-hook reconciliation a settings save triggers. The settings surface stays reachable while disabled (it is the re-enable entry point), so that action must not silently reinstate hooks; it answers a manually-disabled marker with empty worktree and failure lists, and — like the two above — checks **before** taking the repository hook lock.

### The zero-write SUCCESS discriminator

There is one place in the product where a refusal is reported as a **success**, and it is the highest-consequence one: an install asked to respect the durable opt-out returns *success*, an explanatory message, no written paths, and a **manually-disabled discriminator**.

**The success is deliberate.** Refusing to touch a repository the user turned off *is* the correct outcome, not an error, and reporting failure would make every automatic repair path treat a healthy opt-out as a fault.

**But it means success alone cannot be read as "the install happened".** Any caller whose success path has a side effect — stamping "enabled for this version", clearing a cached opt-out, holding an interface state — must check the discriminator first, or it records an install that never occurred. That is not hypothetical: the JVM host stamped its version marker on the strength of success alone, which made its version gate permanently satisfied for that plugin version so every later integrations catch-up was suppressed, and left an optimistically-cleared cached verdict un-rolled-back.

The discriminator is **absent** on every other path, including a real install of a repository that simply has no opt-out set. Its consumers are therefore obliged to test for it explicitly rather than infer it.

Note the polarity flip across the boundary: the runtime reports this as *success plus a marker*, while the JVM host re-reports it to its own callers as *not success plus a marker* — precisely so no success side effect can fire there — while separately requiring that it never be recorded as an error, since the recorded error is what that surface paints in red.

Note also what "zero-write" does **not** cover on this path: the shared-runtime reconciliation (dispatch scripts, this surface's registry entry, the stale-entry sweep) runs **before** the gate and therefore still writes. The guarantee is zero *repository* writes — no hooks, no configuration, no opt-out clear — not zero machine-global ones, which is consistent with the machine-global carve-out below.
...
| true | Any gated write site fires | **true** | Every gate is read-only. A disabled repository's timers, watchers, and sidebar ticks keep firing and keep returning early forever; nothing self-heals. What still accumulates is what no gate covers: the telemetry buffer and onboarding ledger (carve-outs), and — on a user's Refresh of the folder tree — archived folders in the vault. |

## Notable Behavior

- **The zero-write contract is mostly a property of the editor host process, not of the product.** All but a handful of the inventory rows exist only inside that one long-lived process. Invoke the standalone sync command or the Memory Bank migrate command directly against a disabled repository and they write normally, because nothing on their path consults either flag. (Surprising; the single most important qualification in this spec.)
- **Several kinds of gate escape that limitation**, and each does so by reading the *durable* opt-out rather than the mirror: the startup skill reconciliation on every command-line invocation; the long-lived bridge server's enumerated data-write actions; the mid-session plan registration, whose gate is the mirror **or** the durable flag precisely because the mirror is inert in the process that actually runs it; and the two operations that gained a second triggering host once their sequences moved into shared core — the Memory Bank rebuild, refused on a durable read ahead of its whole sequence, and the settings-driven agent-hook sweep, which the local dashboard runs across every registered repository and gates **per clone** on a durable read. The skill reconciliation uses the synchronous read-only reader specifically because the ordinary durable reader would migrate a legacy marker — a write — on the very path that must not write.
- **"Every row is gated on the in-memory mirror" is no longer even approximately true, and the exceptions are now the load-bearing ones.** Four rows read the durable opt-out instead, and two of those four do so *because* the sequence they gate is shared core with a second host whose process has no mirror to consult. Anyone adding a row should assume the sequence may acquire a second host later, and that a mirror-only gate silently stops protecting it when it does. (Notable; the pattern is now established rather than exceptional.)
- **Two Memory Bank writes are gated by nothing, and they live on the surface this spec previously recorded as the safe one.** A user-initiated Refresh of the editor's folder tree archives every provably-empty folder in the vault and folds duplicate folders together, and neither operation reads the mirror or the durable opt-out. The consolidation's rebuild classification is the sharpest case: the archival half is ungated so it runs, the identity write is gated so it is suppressed, and the copy engine's own entry gate returns skipped — so a disabled repository's entire Memory Bank ends up in the archive with nothing rebuilt, and the notification reports success with a memory count of zero. It is recoverable (a move, not a deletion, and the system of record is untouched) and it is still the largest breach of the contract's own promise. The neighbouring operations on that same surface are gated: the reconcile-and-heal pre-pass on a listing is suppressed, and the rebuild action is refused outright for exactly the reason the consolidation demonstrates. (Surprising; the inconsistency is within one surface, not across layers.)
- **The bridge refuses loudly rather than succeeding quietly, and that is the opposite of the install refusal.** A refused bridge write answers *not ok* with a marker, because a host told a write succeeded when nothing was written has no way to notice, and an older host that drops the unknown marker still sees a failure. A refused install answers *success* with a marker, because refusing to touch a repository the user turned off is the designed outcome and reporting failure would make every automatic repair treat a healthy opt-out as a fault. The two are correct for opposite reasons, and the marker is what makes either readable. (Surprising; the same situation is spelled with opposite polarity in two adjacent layers on purpose.)
- **"Success" from an install can mean nothing happened.** Any caller whose success path carries a side effect must test the discriminator before acting on it. Skipping that test is not a theoretical hazard: it stamped a version marker that permanently suppressed the JVM host's later catch-ups for that plugin version, and left an optimistically-cleared cached verdict un-rolled-back.
- **The install refusal is zero *repository* writes, not zero writes.** The shared-runtime reconciliation runs ahead of the gate and still writes machine-global state — consistent with the machine-global carve-out, and worth knowing before treating the refusal as a total no-op.
...
- **A refusal can be safer than a suppression, and the case that proves it is now demonstrated live by the operation that omits the refusal.** The "Migrate to Memory Bank" rebuild is refused with a message rather than partially suppressed, because its identity write is gated while its folder creation and re-point configuration write are not — a suppressed-but-not-refused click would de-identify the previous folder while migrating nothing. The duplicate-folder consolidation re-implements that same archive-then-rebuild sequence with **no** refusal, and produces exactly that outcome plus the archival of every folder. The reasoning was already written down; the second implementation did not inherit it.
...
- **Reads are never suppressed — but "this surface only reads" is now a claim about the listing paths, not about the surface.** Every row that touches storage leaves its read path fully working, which is what lets the disabled sidebar keep listing memories, folders, plans, and references. The folder tree's *listing* is still purely a read while disabled; its *Refresh* is not, and the two ungated writes above are what changed that.
...
- **The migration engine** — the migration's own passes, idempotence, stale-child reconcile lifecycle, and the shared rebuild sequence's durable-read refusal are owned by the migration topic. That topic also owns the second implementation of the archive-then-rebuild sequence that carries no refusal.
- **The two ungated Memory Bank writes** — the archival sweep of provably-empty folders and the duplicate-folder consolidation, including the per-vault lock both hold and the archive directory both move into — are owned by the Memory Bank unused-folder archival spec and the Memory Bank duplicate-folder consolidation spec. This spec owns only the fact that neither consults either signal and what that produces for a disabled repository.
- **The settings-driven agent-hook sweep's wider form** — the walk over every registered repository and every worktree, and its per-clone durable read — is owned by the local dashboard's settings-mutation topic; the editor's single-workspace form is owned by its settings surface.
...
- **The settings surfaces (both hosts), the folder browser, the sidebar, and the command palette** each own their remaining behavior; this spec owns only what each one refuses, skips, or performs unguarded while the flag is set.
- **The install refusal's consumers** — spec 249 owns the JVM host's outcome set, which re-reports the success-with-discriminator as an explicit non-success so no side effect fires; spec 128 owns the version stamp that must not be written on it; spec 332 owns the cached verdict and roll-backs the same discriminator protects. This spec owns only the discriminator's meaning and the obligation it places on callers.
- **The working-area context service** — spec 337 owns the mid-session plan registration path whose two-part gate is recorded here, including why the durable half must be the read-only reader.
