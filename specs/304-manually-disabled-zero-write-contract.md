...
Repo-local telemetry buffer appends are therefore the one write class a disabled repository genuinely still receives, and because flushes are suppressed they accumulate undelivered until re-enable.
...
- **Telemetry** — the recording choke-point, consent resolution, scrubbing, buffer format, and flush protocol are owned by the telemetry topics; this spec owns only which telemetry writes are and are not gated.
