...
Removal goes through the **same ownership-guarded removal primitive** every other deletion in the installer uses — it is not an unconditional recursive delete. The legacy directory's `SKILL.md` is read first; a document carrying no ownership marker is a user's own hand-authored skill that merely shares the name and is **kept**, with the decision recorded at info level. Only a marked document's directory is recursively removed. An absent directory or document is a silent no-op, and a removal failure is logged at warning level and swallowed.

The guard accepts the **legacy** ownership marker as well as the modern one, which is load-bearing here: these directories were written by pre-revision builds that carried only the legacy marker, so a modern-marker-only guard would leave exactly the installs this pass exists to clean. The names themselves are ordinary enough that a user could plausibly own a hand-authored skill under one, which is why the guard was added at all.

This pass is **not** redundant with the plugin bootstrap's Claude-slot legacy cleanup, whose own legacy-name list spans these same names: the two are reached from different entry points — this one from a full reconciliation, that one from the reduced repo-hooks-only bootstrap — so a user who never installs a plugin passes only through here.
...
1. **Legacy cleanup.** For each name in the legacy-directory list, run the ownership-guarded removal against the corresponding directory under `.claude/skills/<legacy-name>/` — keeping any document that carries no ownership marker. Run this pass once for the whole installer, before processing any registered skill. Failures are logged and swallowed.
...
### Codex-plugin skill duplication is accepted, not cleaned up

The Codex plugin ships its own bundled skills under bare directory names — `jolli`, `init`, `login`, `logout`, `status`, `timeline`, `push`, `recall`, `search`, `local-run`, `remote-run` (that inventory is owned by spec 328). Codex prefixes a plugin's skills with the plugin's own name, so the model sees them as `jolli:recall` and so on — the same namespacing Claude Code applies to `/jolli:*`. Codex **also** scans the repository's `.agents/skills/` directory and presents those entries under their bare names, so a repository that additionally ran a full `jolli enable` shows both sets.

The Codex bootstrap **removes nothing** from that directory. It owns no skill assets at all: unlike the Claude bootstrap (which writes and cleans `.claude/skills/`, a slot with exactly one consumer), the Codex bootstrap has no per-host slot to own, because Codex reads the shared cross-platform directory.

- **The two sets do not collide.** A namespaced plugin entry and a repo-local entry of the same underlying skill carry different names, and neither shadows the other — a repository-local skill and a plugin skill of the identical name appear as two separate entries.
- **What duplicates is function, not name.** `recall`, `search`, `local-run` and `remote-run` are host-neutral, so a repo that ran a full `jolli enable` offers each of them twice in Codex: once from the bundle, once from `.agents/skills/`. The bare `jolli` umbrella duplicates the same way. This is a known cosmetic cost.
- **Why not delete the repo copies.** `.agents/skills/` is the cross-platform directory Cursor, Gemini, OpenCode, Windsurf and Copilot read as well. An earlier revision deleted the Jolli-owned repo copies of the plugin's inventory during the Codex bootstrap; because the directory is shared, that removed the **only** copy those other hosts had, silently, and flapped against every later `jolli enable` or startup auto-refresh — a functional loss on several hosts traded for a cosmetic gain on one. Ownership-marker checks did not help: the files being deleted were genuinely Jolli's. Ownership and consumer-set are orthogonal.
- **The intended fix is on the shipping side.** Stop bundling the four host-neutral skills in the plugin (letting `.agents/skills/` be their single home, with the bootstrap ensuring they exist there) rather than deleting a shared resource on one host's behalf. Deferred pending a decision on whether Jolli should instead write per-host copies into each agent's private skills directory — several hosts do have one (`.opencode/skills/`, `.codex/skills/`, `.claude/skills/`).
- **Retired names remain the one exception.** The retired-skill sweep still runs for the Codex bootstrap and still removes retired names from `.agents/skills/`, because a name the product no longer ships is dead for every host that reads it.
