...
The Codex plugin ships eleven skills in its own bundle under bare directory names: `jolli`, `init`, `login`, `logout`, `status`, `timeline`, `push`, `recall`, `search`, `local-run`, and `remote-run`. Codex prefixes a plugin's skills with the plugin's own name, so the model sees them as `jolli:recall` and so on — the same namespacing Claude Code applies to `/jolli:*`. Codex **also** scans the repository's `.agents/skills/` directory and presents those entries under their bare names, so a repository that additionally ran a full `jolli enable` shows both sets.
