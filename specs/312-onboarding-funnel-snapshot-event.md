...
Six keys, snake_case, emitted at [`OnboardingFunnel.ts:303-310`](../cli/src/core/OnboardingFunnel.ts):
...
- **Path**: `join(getJolliMemoryDir(cwd), "onboarding-progress.json")` — `LEDGER_FILE` at `:170`, joined at `:274`. `getJolliMemoryDir` ([`Logger.ts:201-204`](../cli/src/Logger.ts)) is a plain `join(base, ".jolli", "jollimemory")` against the **literal** cwd passed in, with no git-root walk of its own.
- **Read tolerance** (`readLedger`, `:194-202`): unreadable file, unparseable JSON, or a record missing either a string `sig` or a string `tsIso` all read as **absent** — which is treated as "first emit".
- **Write**: `mkdir(dir, { recursive: true })` then `atomicWriteFile` (`:311-318`). Atomic because a torn ledger reads back as absent, i.e. as "first emit" — so a crash mid-write must produce a duplicate send at worst, never a permanently-stale skip.
- **Heartbeat**: `HEARTBEAT_MS = 24 * 60 * 60 * 1000` (`:172`).
...
Five fields joined by `|` ([`OnboardingFunnel.ts:184-192`](../cli/src/core/OnboardingFunnel.ts)):
...
| # | Step | Site | Behavior |
|---|---|---|---|
| 1 | Consent short-circuit | `OnboardingFunnel.ts:233` | `if (!getTelemetryContext()?.enabled) return;` — **before** git, the status probe, the lock map, and the ledger. An uninitialized or opted-out install pays nothing and writes no ledger. Kotlin: `Telemetry.isEnabled()` at `OnboardingFunnel.kt:74`, before even the config round-trip. |
| 1b | **Manual-disable gate** | `OnboardingFunnel.ts:273` | Returns when the repo carries the durable "leave this project alone" opt-out (spec 145) — after the consent short-circuit, **before** the ledger path is even computed, so a disabled repo emits nothing and creates no ledger file. **Both readers are consulted** (`isManuallyDisabled() \|\| readManualDisableFlagSync(cwd)`) because neither alone covers every trigger: the in-memory mirror is free but process-local and set only by the editor host, so on its own it left every CLI trigger recreating the ledger in a disabled repo; the synchronous disk reader is the durable truth those CLI processes need. It must be the **sync** reader — the async one persists a legacy-marker migration decision, i.e. a write, which is exactly what this gate exists to prevent. Its main-worktree resolution is memoized per cwd, so steady-state cost is one file read. **No Kotlin equivalent** — see Cross-surface divergence. |
| 2 | Per-ledger serialization | `:213`, `:274-278`, `:281-284` | A module-level `Map<ledgerPath, Promise>` chains each call onto the previous for the **same** ledger path; distinct repos run concurrently. The entry is deleted in a `finally` only when this run is still the tail, so the map cannot grow unbounded. |
| 3 | Resolve the capture route | `:84-96`, `:144-145` | `captureMethodOf` collapses `resolveLlmCredentialSource` ([`LlmClient.ts:222-242`](../cli/src/core/LlmClient.ts)): `local-agent`→`local-agent`, `jolli-proxy`→`jolli`, `anthropic-config`/`anthropic-env`→`anthropic`, `null`→`none`. `captureConfigured = captureMethod !== "none"`. Using the *same* function that drives generation is what keeps the funnel from drifting from reality. |
| 4 | Non-git short-circuit | `:146-156` | `isInsideGitRepo(cwd)` ([`GitOps.ts:980-983`](../cli/src/core/GitOps.ts)) false → return with `inGitRepo`, `repoEnabled`, `memoriesGenerated` all false and `memoriesBucket: "0"`, while **preserving** the already-resolved `captureConfigured` / `captureMethod`. No status probe is ever run. |
| 5 | Status | `:115-127`, `:157` | Uses `opts.status` when the caller precomputed one; otherwise `resolveFunnelStatusLight(cwd)` — a lightweight probe reading ONLY the two fields the snapshot uses, in parallel: `enabled` via the SHARED [`isGitPipelineFullyInstalled`](../cli/src/install/GitHookInstaller.ts) predicate (the same function `getStatus()` and the guided front door consume, so the three trigger classes cannot drift apart on `repo_enabled`), and `getSummaryCount`. It lazy-imports `GitHookInstaller` / `SummaryStore`, preserving the import-cycle break the old lazy `getStatus()` import provided. The fallback used to be a full `getStatus()`, which probes every AI host and scans session stores — several times the cost — and now sits on blocking per-session paths (the plugin SessionStart bootstraps). A missing `summaryCount` reads as `0`. |
| 6 | Derive the last two fields | `:164-165` | `memoriesGenerated = summaryCount > 0`; `memoriesBucket = bucket(summaryCount)`. |
| 7 | Dedup decision | `:299-302` | `changed = !prev || prev.sig !== sig`. `elapsed = prev ? now - Date.parse(prev.tsIso) : +Infinity`; `stale = !Number.isFinite(elapsed) || elapsed >= HEARTBEAT_MS`. Return early iff `!changed && !stale`. An absent ledger yields `+Infinity`, and an unparseable `tsIso` yields `NaN` — the `!Number.isFinite` arm treats **both** as stale, so a garbled timestamp re-emits rather than pinning the ledger forever. |
| 8 | Emit, then persist | `:303-318` | `track("onboarding_progressed", …)` first, `mkdir` + `atomicWriteFile` second. |
| 9 | Never throws | `:319-321`, `:230`, `:279-280` | `emitOnce` swallows everything. `maybeEmitOnboardingProgress` wraps its **whole** body — including the `getTelemetryContext()` call — in its own `try`, so even a Telemetry module mocked down to just `track` cannot throw into the caller. `resolveOnboardingFunnel` itself **rejects** on a git or status failure (`:135-141`); the swallow lives only in the caller. |

### Trigger sites

Ten call sites across five surfaces:

| Surface | Site | What it passes | Notes |
|---|---|---|---|
| CLI — `jolli enable` | [`EnableCommand.ts:635`](../cli/src/commands/EnableCommand.ts) | `{ cwd: options.cwd, config: await loadConfig() }` | Tail of `reportEnableResult`, reached on **both** the success and the failure branch, so the non-git failure path reports `in_git_repo: false`. Config is re-loaded because the interactive provider flow may have just added a capture route. Not reached under `--repo-hooks-only`, which returns earlier — that branch carries its own emit (next row). No precomputed status. |
| CLI — `jolli enable --repo-hooks-only` | [`EnableCommand.ts:457`](../cli/src/commands/EnableCommand.ts) via [`capturePluginOnboardingSnapshot`](../cli/src/hooks/PluginBootstrapTelemetry.ts) | `{ cwd: options.cwd, config }` (config read once, shared) | Tail of the early-returning repo-hooks-only branch, on both the success and failure branch, awaited inline. This branch never reaches `reportEnableResult`, and both plugins' `/jolli:init` run exactly this mode — before this emit existed, the plugins' primary install gesture was a funnel blind spot. The shared helper's own `bootstrapTelemetry` re-points the telemetry context at `options.cwd`, keeping the buffer, the bounded flush and the dedup ledger on the SAME directory even under an explicit `--cwd`, and its explicit `BOUNDED_FLUSH_BUDGET_MS`-capped flush replaces the exit flush that `markSkipExitFlush()` disarmed for this mode. No precomputed status. |
| Claude plugin — SessionStart bootstrap | [`PluginBootstrapHook.ts:148`](../cli/src/hooks/PluginBootstrapHook.ts) via [`capturePluginOnboardingSnapshot`](../cli/src/hooks/PluginBootstrapTelemetry.ts) | `{ cwd: worktreeRoot, config }` (config read once, shared) | On **both** the success and failure branch of repo-hook reconciliation. On success the snapshot chain is STARTED inside the context phase, right after `ensurePluginDefaultProvider` seeds the capture route — any earlier and a fresh install's first snapshot misreports `capture_method: "none"` for a state the same run repairs — and runs fully deferred (config read → `bootstrapTelemetry({ cwd, sessionId, deps })` → emit → flush capped at `BOUNDED_FLUSH_BUDGET_MS` for both the per-POST timeout and the whole-flush deadline), overlapping the briefing build; the hook awaits `done` in a `finally`, so no throw or future early return can orphan the in-flight chain. Paths that skip the seeding (manual disable re-check, `claudeEnabled` off, context-lock contention) still snapshot via the finally-fallback with the unseeded config. The failure branch (`:123`) awaits the chain inline. Events carry `surface: "claude-plugin"`. |
| Codex plugin — SessionStart bootstrap | [`CodexPluginBootstrapHook.ts:178`](../cli/src/hooks/CodexPluginBootstrapHook.ts) via the same helper | `{ cwd: worktreeRoot, config }` (config read once, shared) | Mirrors the Claude row exactly — same helper, same seeded-then-snapshot ordering, same deferred chain awaited in a `finally` (failure branch at `:147`) — except the Codex hook input carries no session id, so no `sessionId`. Codex executes plugin command hooks only after the user reviews and trusts them in `/hooks`, so on an untrusted install the `--repo-hooks-only` trigger above can be this surface's only emitter. Events carry `surface: "codex-plugin"`. |
...
|---|---|---|---|
| Trigger sites | 6 (enable, enable `--repo-hooks-only`, front door ×2, status, ingest) — plus the two plugin-surface SessionStart bootstraps that ride the same TS emitter | 1 (`StatusStore.refresh()`), fired from ≥5 uncoordinated callers — e.g. `Extension.ts:1888, 1951, 2335, 2444, 2499, 4324` | 1 (`JolliMemoryService.refreshStatus()`) |
...
| From | Event | To | Notes |
|---|---|---|---|
| Telemetry uninitialized / opted out | Any trigger | Unchanged | Returns at `:233` before git, status, the lock map and the ledger. No ledger file is created. |
| Repo carries the durable manual-disable opt-out | Any TS trigger | Unchanged | Returns at `:273`, before the ledger path is computed. No event, no ledger file, no ledger update. The IntelliJ mirror has no such gate and proceeds normally. |
| No ledger for this path | Any trigger, consent on | Ledger written with the current signature | `prev` absent ⇒ `changed` true ⇒ emit. |
| Ledger present, same signature, `<24 h` old | Any trigger | Unchanged | The only path that returns without emitting (`:302`). The state was still resolved first. |
...
- **A deduped trigger still costs a git subprocess.** The state is resolved *before* the ledger is read (`:295-297`), so `isInsideGitRepo` — and, where no status was precomputed, the lightweight `resolveFunnelStatusLight` probe (git-hook section checks plus `getSummaryCount`; a full `getStatus()` until the plugin bootstraps put this on a blocking per-session path) — runs on every trigger past the consent gate, including the overwhelming majority that then return at `:302` having sent nothing. The consent short-circuit at `:233` is the only free path.
...
- **A failed ledger write produces a duplicate, never a loss.** `track()` runs before the persist (`:303` then `:311-318`) and the inner `catch` swallows a write failure (`:319-321`), so the event is already buffered when the ledger fails; the next trigger sees no (or a stale) ledger and emits again. The atomic write exists for the same reason at a finer grain — a torn file reads back as absent (`:194-202`), i.e. as "first emit".
