...
- The ordered emit pipeline: consent short-circuit, per-path serialization, capture-route resolution, the non-git short-circuit, status resolution, the dedup decision, emit-then-persist, and the double error swallow.
...
- **The funnel is not gated on the manual-disable opt-out on any surface.** None of the seven call sites consults `readManualDisableFlag` before emitting, and the in-memory suppression flag that spec 304 describes explicitly carves telemetry recording out of its zero-write contract. A repository the user turned Jolli off in therefore keeps reporting `repo_enabled: false` snapshots — which is arguably the point (that is the drop-off the funnel exists to see) but is worth stating, because the ledger *write* is a per-project `.jolli/` file write into a repo the user asked to be left alone.
...
- The durable `manuallyDisabled` opt-out is owned by spec 145, and its zero-write contract — including the explicit telemetry carve-out — by spec 304.
