...
| 5 | Status | `:114-131`, `:157` | Uses `opts.status` when the caller precomputed one; otherwise `resolveFunnelStatusLight(cwd)` — a lightweight probe reading ONLY the two fields the snapshot uses: `enabled` (replicating `getStatus()`'s own derivation — the post-commit hook plus its three sibling sections) and `getSummaryCount`. It lazy-imports `GitHookInstaller` / `SummaryStore`, preserving the import-cycle break the old lazy `getStatus()` import provided. The fallback used to be a full `getStatus()`, which probes every AI host and scans session stores — several times the cost — and now sits on blocking per-session paths (the plugin SessionStart bootstraps). A missing `summaryCount` reads as `0`. |
...
| CLI — `jolli enable --repo-hooks-only` | [`EnableCommand.ts:454-455`](../cli/src/commands/EnableCommand.ts) | `{ cwd: options.cwd, config: await loadConfig() }` | Tail of the early-returning repo-hooks-only branch, on both the success and failure branch. This branch never reaches `reportEnableResult`, and both plugins' `/jolli:init` run exactly this mode — before this emit existed, the plugins' primary install gesture was a funnel blind spot. Followed by an explicit `flushTelemetryNow(cwd, { timeoutMs: 2000, deadlineMs: 2000 })` because the mode calls `markSkipExitFlush()`, disarming the CLI's exit flush (`deadlineMs` bounds the whole flush, not just each POST). No precomputed status. |
| Claude plugin — SessionStart bootstrap | [`PluginBootstrapHook.ts:123`](../cli/src/hooks/PluginBootstrapHook.ts) via [`capturePluginOnboardingSnapshot`](../cli/src/hooks/PluginBootstrapTelemetry.ts) | `{ cwd: worktreeRoot, config }` (config read once, shared) | After repo-hook reconciliation, on **both** the success and failure branch. The shared helper (`PluginBootstrapTelemetry.ts`, one copy for both plugin hooks) reads the config ONCE and injects it into `bootstrapTelemetry({ cwd, sessionId, deps })` — a one-shot hook process otherwise has no telemetry context, so `track()` would be a no-op — then emits, then STARTS `flushTelemetryNow` with `timeoutMs` = `deadlineMs` = 2 s (`deadlineMs` bounds the WHOLE flush; `timeoutMs` alone is per-POST and a full buffer is several sequential POSTs). The hook awaits the returned `flushed` promise only at its returns, so the network wait overlaps the briefing build. Skipped when the repo is manually disabled or the repo-hooks lock is contended (both early-return before the `install()` call). Events carry `surface: "claude-plugin"`. |
| Codex plugin — SessionStart bootstrap | [`CodexPluginBootstrapHook.ts:149`](../cli/src/hooks/CodexPluginBootstrapHook.ts) via the same helper | `{ cwd: worktreeRoot, config }` (config read once, shared) | Mirrors the Claude row exactly — same helper, same bounded overlapped flush — except the Codex hook input carries no session id, so no `sessionId`. Codex executes plugin command hooks only after the user reviews and trusts them in `/hooks`, so on an untrusted install the `--repo-hooks-only` trigger above can be this surface's only emitter. Events carry `surface: "codex-plugin"`. |
