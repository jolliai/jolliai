...
5. **Mirror the tree** into the destination with a delete-extraneous sync, after refusing any destination that is neither an existing checkout of this same marketplace nor empty. The plugin's own build/generator script directory is excluded from the mirror here (the sibling plugin ships its build script into its marketplace repository). Beyond that explicit exclusion list the mirror honours no exclusion *rules* whatsoever — it is a plain file sync, not a git operation, so no ignore file of any kind can drop a file from it. The global-exclusion neutralisation belongs to the staging step below, not here.
...
7. **Stage everything**, with the operator's machine-global exclusion file neutralized — a developer's global rule matching a skill filename has silently dropped skills from a published plugin before. The destination repository's *own* exclusion rules are still honoured here — see the green-no-op notable below. If nothing changed, report "already up to date" — and then, unlike the sibling plugin, check whether a previous run left a commit unpushed and push it — so a commit-without-push rehearsal is recoverable rather than stranded locally forever.
...
- **The comparison is strictly greater, and whole-match numeric on both operands.** Both the candidate version and the baseline must match a three-numeric-component shape in full; if either does not, the answer is "not greater" — nothing is padded, truncated, or sorted, so a malformed version *blocks* the release rather than being guessed at. Components are compared left to right as integers, base ten, so a leading zero carries no octal trap. **Equality is rejected**, not accepted.
...
- **The inventory gate covers this plugin's non-git targets too, and the sibling plugin's covers neither of its own.** The skill / required-configuration / freshness assertion runs on the archive and local-directory paths here as well as on the two git targets, so a missing licence copy cannot reach an offline archive. The sibling plugin's archive is **ungated on configuration**: it checks the bundle inventory but never reads its required-configuration list at all, and after packing confirms only that one single bundle entry survived into the archive — and its local-directory path asserts nothing about configuration either, so the same omission ships silently there. (Notable; a divergence in this plugin's favour. Spec 282.)
- **An ignore rule in the destination that swallows only the licence produces a green no-op.** The mirror writes the file; the staging step honours the destination repository's own exclusion rules and skips it; the change check then finds nothing staged and the run reports the target already up to date — *before* the index assertion that would have caught it, which sits after that check. Exclusion rules do not affect an already-tracked file, so the case this bites is exactly a destination that has never carried a licence copy, which is the case the duplicated licence pair was added for. Both plugin trees exit this way; here the hole is the ordinary no-op only, since the unpushed-recovery branch above does run the index assertion before pushing. (Surprising; the silent shipping the required list is claimed to prevent is the one shape it cannot see. Spec 282.)
