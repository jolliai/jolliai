...
- The tab wrapper's own lifecycle (what opens a memory as an editor tab, tab reuse, tab titles) — owned by **121. IntelliJ Summary Virtual-File Editor**.
...
- The two other embedded-browser views in the plugin — the branch-level pull-request draft (**251**) and the working-memory review tab (**222**). Both construct their own instance directly, outside the pool, and destroy it themselves. They are deliberately not pooled, are not counted against the pool's ceiling, and are not assigned a sequence number.
...
All three attachment operations refuse (throw) once the handle has been handed back. No live caller can reach that state: the tab performs all three attachments during its construction, while the handle is fresh.
...
The tab's teardown detaches its own UI-component observers first — those sit on the shared instance's UI component and are not the lease's responsibility — and only then hands the lease back, so no next tenant can attach in between. Because the pool's hand-back asserts the UI thread while the tab can be torn down from any thread during project close, the tab hops to the UI thread when it is not already there.
...
Both refusals reach the memory tab through its construction path, which catches them along with any genuine construction failure and substitutes the tab's pre-existing read-only plain-text rendering of the memory (owned by spec 120). No handle is retained in that case, so the tab's later teardown hands nothing back.
...
[tab open]  (UI thread required)
  [pool disposed]           → refuse → tab renders plain-text fallback
  [not the UI thread]       → refuse → tab renders plain-text fallback
...
[tab close]  (hops to the UI thread if teardown was raced elsewhere)
  tab detaches its own UI-component observers
...
- **The ceiling is a hand-back constraint, not admission control.** Checkout never consults it and never refuses for capacity, so N simultaneously open memory tabs hold N instances even when N exceeds 5. The population is trimmed back toward the ceiling only as tabs close, one destroy per over-capacity hand-back.
...
- **A cold checkout does not schedule a replacement.** Top-up is scheduled only when a checkout hit an idle instance (or by prewarm). Open two tabs in a row from an empty pool and both pay synchronous construction on the UI thread; the standby spare is only re-established once a tab closes or a later hit-checkout schedules one.
...
- **Both pool refusals are indistinguishable from a genuinely unavailable embedded browser.** A disposed pool, a wrong-thread call, and an environment without the embedded-browser capability all surface identically: the memory tab shows its read-only plain-text rendering, logged as the browser being unavailable.
...
- **Only pool-constructed instances are numbered.** The pull-request-draft view and the working-memory review tab each construct their own instance directly and are never stamped, so the numbering is a count of *pooled* instances, not of embedded browsers in the session — and the elapsed offset is measured from the counter's first use, so it is an inter-instance spacing signal, not a time-since-startup one.
- **The counter is application-wide while pools are per project.** Numbers from two open projects interleave in the log.
- **The memory tab is the pool's only live consumer.** A legacy standalone-dialog wrapper around the same view still exists and would also check out an instance, but nothing constructs it.
...
- **121. IntelliJ Summary Virtual-File Editor** — the tab wrapper whose construction checks an instance out and whose disposal hands it back.
...
- **222. IntelliJ Working Memory Web View Editor** and **251. IntelliJ Create-PR View** — the two embedded-browser views deliberately outside the pool: each constructs and destroys its own instance and is neither counted against the ceiling nor numbered.
