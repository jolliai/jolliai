...
In that turn, in order: clear the pending flag (so a request arriving later re-arms), re-check pool/project disposal, then decide under the lock whether to build — required only when the idle count is **below the standby target** *and* idle + checked-out is **below the ceiling**. If a build is required it happens in this same turn; a construction failure is logged as non-fatal and abandoned. That catch is deliberately widened to the **whole throwable hierarchy, not just exceptions**: on the IDE build where the embedded-browser toolkit was split into a separate platform module the plugin does not yet declare, merely referencing the browser class raises a class-loading `Error`, which an exception-only catch would let escape and take the plugin's startup with it. The log line falls back to the throwable's class name when it carries no message, precisely because that class-loading failure has none. After a successful build, disposal is re-checked under the lock: if the pool was disposed in the meantime the fresh instance is destroyed instead of filed.
...
**Those consumer catches are exception-only, and so is the containment.** The throwable-wide catch described under "Background top-up" covers the top-up path alone. The **cold-checkout** path — the one a user-initiated tab open takes when no idle instance is available — calls construction uncaught, and each of the three surrounding catches (both consumers' construction paths and the project service's prewarm wrapper) catches exceptions only. So in an environment where the embedded-browser class cannot be loaded at all, the two behaviors diverge: eager warm-up degrades cleanly and silently, while opening a memory tab or a pull-request draft propagates a class-loading `Error` past every catch on that path. Widening the top-up catch stopped the startup crash; it did not make the whole surface tolerant of a missing browser toolkit.
...
- **Both pool refusals are indistinguishable from a genuinely unavailable embedded browser** — but only for failures that arrive as exceptions. A disposed pool, a wrong-thread call, and an environment without the embedded-browser capability all surface identically: the consumer shows its read-only plain-text fallback, logged as the browser being unavailable. A **class-loading `Error`** does not join them: it is contained on the top-up path and uncontained on the cold-checkout path, so the same missing toolkit produces a quiet degraded warm-up and a propagating failure on the user's click.
- **The throwable containment is partial, and asymmetric in a way worth stating plainly.** Exactly one call site — the background top-up — catches the full throwable hierarchy. The cold-checkout construction is uncaught, and all three catches around it (the memory tab's, the pull-request draft's, and the project service's prewarm wrapper) are exception-only. The widening was made to stop a startup crash on an IDE build that moved the browser toolkit into an undeclared module; the user-initiated path was not covered by the same change. (Notable; the containment reads as complete and is not.)
