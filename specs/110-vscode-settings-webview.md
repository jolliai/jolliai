...
**Build and post** (`vscode/src/views/SettingsWebviewPanel.ts:506-542`). The host reads the Memory Bank root from the machine-global config and calls `listPushControlRepos({ localFolder, currentCwd })`, then posts `pushControlLoaded { repos, status? }`. Two failure behaviors are deliberately different:
...
- A failure from `listPushControlRepos` itself re-posts the **last-known rows** *plus* an `unreadable: <message>` field. That combination is the point: the list is built from the machine-global store and the outbound gate fails **CLOSED** on that same store, so silently rendering stale rows would show every repo as "Push ✓" while every push is in fact blocked — the exact opposite of the truth. The catch is kept deliberately narrow because `listPushControlRepos` enumerates defensively, so an unreadable store is the only thing that can reach it (spec 310).

**Render** (`vscode/src/views/SettingsScriptBuilder.ts:200-265`). When `unreadable` is set, an error banner is prepended **before any row**: *"Can't read the outbound-push setting, so pushing is blocked for every repository until it's fixed. The states below are the last known ones and may be wrong. Detail: `<message>`"*. With no rows and no `unreadable`, an empty-state hint is shown instead; with no rows *and* `unreadable`, only the banner renders. Each row's checkbox is `checked = !repo.pushDisabled` and carries a per-repo `aria-label` (the visible label is just "Push", identical on every row).

**Toggle** (`SettingsWebviewPanel.ts:559-592`). Changing a checkbox posts `setPushDisabled { repoIdentity, disabled, isCurrent }` and immediately shows a **pending** status only ("Disabling/Enabling outbound push for `<repo>`…") — never a success line, because the host confirms by re-posting. The host then:

1. Writes **by identity** via `setRepoPushDisabledByIdentity` — the key of the row the user actually clicked. `isCurrent` decides only whether the re-enable drain can run (that needs a working tree, so it goes through `triggerReenableDrain(workspaceRoot)`); it deliberately does **not** also re-derive the target, which would give "which repo did they mean" two sources of truth that disagree once the workspace's remote changes after the list was rendered.
...
3. On a `recoveredFromCorrupt` write, **replaces** that status with the machine-wide-reset warning naming the preserved `.corrupt-<epoch>` file — a bare "Enabled ✓" over a settings reset is exactly what spec 310 forbids.
