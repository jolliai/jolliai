...
| AI Summary | Model | Dropdown, one option per model the selected tool pins. | Local-agent card only, and **hidden entirely for a tool that pins no model** — the four tools that defer to their own configuration have nothing to offer. The document carries every pinned tool's options at once, each tagged with its tool, because the document is built once while the tool dropdown changes in the page; switching tools hides the other tool's options and, when the selected model belongs to none of the visible ones, falls back to the first. Default is the mid-tier option, which saves as unset (default elision), and one option means "send no model flag", i.e. run whatever the tool is configured with. |
