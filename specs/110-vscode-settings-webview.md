...
The settings webview that edits the per-user configuration — Anthropic API key, model alias, max-output tokens, Jolli API key, six independent agent-integration toggles, AI-summary provider selection, Memory Bank local folder, Memory Bank cloud-sync controls, DCO sign-off, and exclude-pattern globs — and persists them to a single per-user config file with no per-workspace overrides, validating the Jolli API key against an HTTPS-only host allowlist before any disk write.
...
- The integration-toggle side effect: enabling or disabling the Claude or Gemini toggle installs or removes the corresponding hook across every worktree of the repository. The other agent toggles (Codex, OpenCode, Cursor, Copilot) gate discovery-only integrations and have no per-worktree hook to install.
- The AI Summary provider selection (Anthropic direct vs. Jolli proxy) and the conditional cards rendered for each: an Anthropic card with key / model / max-tokens; a Jolli card with three sub-states (signed-in-with-key, signed-in-but-key-missing, signed-out) and a sign-in button that hands off to the shared auth service.
...
| AI Agents | Codex CLI | Slider toggle. | Default on. Discovery-only — no hook is installed; the flag gates whether Codex sessions are scanned and surfaced. |
| AI Agents | Gemini CLI | Slider toggle. | Default on. Toggling installs / removes the Gemini AfterAgent hook across every worktree. |
| AI Agents | OpenCode | Slider toggle. | Default on. Discovery-only. |
| AI Agents | Cursor | Slider toggle. | Default on. Discovery-only. |
...
| AI Agents | "At least one must be on" | Form-level rule. | At least one of the six toggles above must be checked, otherwise Apply Changes shows a red feedback string and refuses to send. |
| AI Summary | Provider | Dropdown ("Anthropic", "Jolli"). | Selects which of the two cards renders below. |
...
| Integrations (six toggles, collectively) | At least one must be checked. |
...
| `settingsLoaded` | Host → webview | Initial population. |
| `setLocalFolder` | Host → webview | Picker result. |
| `rebuildKnowledgeBaseDone` | Host → webview | Carries `success` and `message`. |
| `settingsSaved` | Host → webview | Save succeeded. Webview shows a 2-second green feedback. |
...
4. Send `settingsLoaded` with the masked keys, the model alias (defaulting to "sonnet"), the max-tokens value (or null), the six integration toggle states (defaults true), the local-folder path (or empty), and the comma-joined exclude patterns.
...
5. Resolve the repository root and enumerate worktrees. For each worktree, install / remove Claude and Gemini hooks per the toggles. Collect failures; throw a single error if any occurred.
6. Save the partial config to the per-user file.
7. Update the cached full keys to the resolved values.
8. Send `settingsSaved`. Invoke the post-save callback if one is registered.
...
- **Hook syncing runs across every worktree of the repository.** Toggling Claude on while three worktrees exist installs the hook in all three. A failure on any worktree throws a concatenated message and aborts the save before the config is written, so the on-disk state cannot disagree with the toggle state. (Surprising; intentional.)
...
- **The "at least one integration enabled" check happens client-side.** The host does not re-run this rule — the webview is the only enforcer, because all six toggles being off is not a "destructive" save, just a "the product won't do anything useful for you" save. The check is to prevent accidental footgun, not enforce an invariant. (Notable.)
