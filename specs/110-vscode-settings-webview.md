...
- The local-folder browse button, the "Migrate to Memory Bank" button, the "Generate Missing Summaries" button and its count-gated enablement, the compile-exclude-folders input, and their per-action feedback strings.
...
| AI Summary | Model | Dropdown, one option per model the selected tool pins. | Local-agent card only, and **hidden entirely for a tool that pins no model** — the three tools that defer to their own configuration have nothing to offer. The document carries every pinned tool's options at once, each tagged with its tool, because the document is built once while the tool dropdown changes in the page; switching tools hides AND disables the other tool's options and, when the selected model belongs to none of the visible ones, falls back to the option **marked** as that tool's default — never to the first visible one, since each list is ordered by capability with its default in the middle. Every pinned tool offers the "send no model flag" option under one shared identifier, so the document genuinely contains several options with that value differing only by tool tag; the page selects among the current tool's options by position rather than by assigning the value, because assigning it would select the first match — another tool's, hidden, and labelled with that tool's name. Default is the mid-tier option **of the selected tool**, which saves as unset (default elision). |
...
| Memory Bank | Generate Missing Summaries | Button + a count line + a status line. | Back-fills this workspace's own un-summarized commits — one AI call per commit. **Ships disabled** and stays disabled until the count message arrives; from then on its enabled state is decided by **testing the count against zero**, so **only a count of zero disables it**. The count arrives on its own message, off the settings-load critical path, and names the repository. A count that could not be computed is sent with **no number at all**, which fails that comparison — so the line goes blank and the button becomes **enabled**. |
...
Rendered instead of the Anthropic / Jolli cards when the provider dropdown is on the local-agent option. It holds two controls, one read-only status line, and one hint, and nothing else: a label reading `Agent tool` with its dropdown, a label reading `Model` with its dropdown (present in the markup but hidden unless the selected tool pins a model), the availability line described below, and the hint line
...
| `generateMissingSummaries` | Webview → host | Sent on the Generate Missing Summaries click. Forwarded to the standalone back-fill command; the host responds with `generateMissingSummariesDone` and then re-requests the count. |
...
| `generateMissingSummariesDone` | Host → webview | Back-fill finished (or threw); the webview reports it on the button's own status line. |
| `missingSummaryCountLoaded` | Host → webview | Carries the count and the repository name, sent **separately** from `settingsLoaded` so computing it never blocks the panel's population. **A failed count is sent as this same message with no count on it**, deliberately, so the webview stops waiting — which blanks the line and leaves the button **enabled**. |
...
**The folder is picked, never typed, and that is a real divergence from the other surface that writes the same field.** Here the field is read-only and a native picker supplies its value, so the path is an existing directory by construction and this form performs no path validation of its own. The local dashboard's equivalent control is deliberately a **free-text absolute path** — it has no picker to offer from a browser — so it carries a validation layer this panel does not: an advisory existence/writability verdict on blur, and a save-time gate that rejects a relative, missing, non-directory or non-writable path with a client error **before** anything is persisted, and that never creates the folder. Neither surface's behaviour is a subset of the other's: this one cannot produce a bad path, and that one cannot pre-fill a good one.

### Generate missing summaries

1. The button ships disabled; on every fresh load the count line is reset to its pending text and the button is re-disabled, so it is not clickable before the count message arrives.
2. The count arrives on its own message and is rendered as either "all your commits … already have summaries" or "N of your commits … still need summaries", naming the repository. A count of **zero** re-disables the button.
3. **A count the host could not compute arrives on the same message with no count on it**, which renders no line at all and, because the enabled state is a comparison against zero, **releases the button from its initial disabled state**. That is the host's own stated intent — the user should not be stuck behind a number the host cannot produce, and the back-fill is idempotent for a commit that already has a summary.
4. A click forwards to the standalone back-fill command, whose outcome is reported on the button's own status line, after which the count is re-requested.

The count is deliberately computed **off** the settings-load critical path, because it walks the repository's whole history.
...
- **Two of the values this panel writes are rejected outright by the command-line configuration surface**, and the reverse holds as well — that surface accepts values this form has no field for. The two are not subsets of one another in either direction. (Notable.)
- **The machine-wide session-statistics switch has no field on this panel, and none on the sibling editor host's settings dialog either.** It governs a channel that uploads session statistics — session titles and the names of the AI tools that ran among them — for **every** repository Jolli is enabled in on this machine, bound to a Space or not, which is exactly what neither the include-transcripts toggle nor the per-repo outbound-push list means, so none of the controls on this panel stands in for it. Its only two surfaces are the local dashboard's sync section, where it applies immediately like the push rows beneath it, and the command-line configuration setter. It is a further member of the reverse asymmetry above: a value that surface accepts and this form cannot express — this time for the one channel that uploads from repositories the user never connected to anything. (Notable.)
...
- **The Generate Missing Summaries button is gated on a count it does not have at load.** It ships disabled and is re-disabled on every fresh load, so it is not clickable before the count message arrives. From then on the gate is a comparison against zero, which means **an empty candidate set is the only thing that disables it** — an unknown count does not. (Notable; the gate is what stops an AI-call-per-commit action firing against a set that is already complete.)
- **A count that FAILED to compute is the one state in which this budget-spending button becomes clickable, beside a blank line.** The host answers a failed count with the same message minus the number, and the webview's disabled state is set by testing that number against zero — a comparison a missing number loses, so the button ends up **enabled** with no count text next to it. It is deliberate on the host's side (the user should not be stuck behind a number the host cannot produce, and the back-fill is idempotent for commits that already have a summary), but the result inverts the reading the surrounding wording invites: the least-informed state is the permissive one, and the only explanation offered to the user is the absence of a line. (Surprising; intentional.)
- **The folder field here is picked, not typed, so this panel validates no path at all.** The read-only field plus a native picker makes a bad path unrepresentable. The dashboard's counterpart is free text and therefore carries the validation this panel has no need for — an advisory blur verdict and a save-time rejection of relative / missing / non-directory / non-writable paths, neither of which ever creates the folder. Two surfaces write one configuration field under opposite guarantees. (Surprising; neither behaviour is a subset of the other.)
...
- **Dashboard Settings Apply (spec 363)** — the other surface that edits this same per-user configuration file, and the one this panel's five tabs are partly mirrored into. It owns its own validation order, its all-repositories hook sweep, its free-text folder field's verdicts, and the two Memory Bank controls it drops while still round-tripping their values.
