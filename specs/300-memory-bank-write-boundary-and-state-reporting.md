...
### An ungated read consumer
A further consumer resolves the per-repository subdirectory in order to **read** the folder mirror, and does not consult the boundary at all. It exists on the host that has neither the boundary nor the report, and it resolves through the **claiming** resolution path — the same asymmetry the read-side storage resolution was gated to close, re-opened at a different seam. Two consequences:
...
- **A read path now claims through the same ungated adapter.** That host reads the folder mirror directly for latency, and the resolution that tells it *where* the mirror is goes through the claiming resolver with no boundary consultation — so attaching a purely-read capability creates the subdirectory the boundary would have refused, both at activation and again on every settings save. This is the same defect the canonical read-side resolution was gated to close (**Consumer 2**), re-appearing at a different seam, and it is the sharpest illustration of why the precondition being enforced socially at each caller is a real cost rather than a theoretical one: a new caller derived its root the ordinary way and inherited the ordinary claiming behavior, with no automated check to notice. (Surprising; the gap the "gated from outside, not defensive from within" choice predicted.)
- **No state text reports a read path at all.** The wording table answers "will folder-layer *writes* land, and where". A host serving reads from the mirror has no line, no severity, and no field in either report — and on the host where that read path exists, neither the boundary nor the report exists either, so the attach outcome is recorded only in an internal activation log that no surface displays. (Notable; observable gap.)
