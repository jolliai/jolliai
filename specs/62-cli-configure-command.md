...
| `localAgentModel` | enum | Which model the local agent is told to run, for the tools the product pins one for. The accepted set is the union across those tools, and the failure message groups the identifiers BY tool — with two pinned tools whose namespaces are disjoint, a flat list reads as one menu and tells nobody that half of it does nothing under the tool they have selected. Distinct from `model`, which names an API model for the direct / proxy providers and does not reach this one. |
| `wikiRebuild` | enum | When the topic knowledge base and its graph are rebuilt from newly-summarized commits. One of `manual`, `auto`. **An absent key means `manual`**, and the polarity is deliberate — the predicate tests for `auto`, so every existing install reads as manual without a migration. Under `manual` no git operation auto-triggers a rebuild and the user rebuilds on demand from the dashboard or the editor sidebar; `auto` is the legacy behavior in which every commit, merge and back-fill enqueues an ingest pass. Recall and commit search stay fresh either way, because they read the per-commit memories rather than the topic pages. |
| `syncTranscripts` | boolean | Include raw AI conversation transcripts in cloud sync. |
| `syncSessions` | boolean | Sync session statistics — token counts, cost, tool names, session titles — to the user's Jolli organization, for **every repository on this machine**, bound to a shared Space or not. **An absent key means on**, because the reader tests only for an explicit `false`. It is deliberately its own switch rather than riding an existing one: every other sync and push switch means "this repository's memories", and this one does not — reusing one would make that setting describe something it does not control. |
...
**`--list-keys` documents a strict subset of what `--set` accepts** — 30 of the 32 keys in the table above. The two backup keys above are accepted, coerced and validated exactly like any other key, and appear in neither the listing nor its usage lines — so the only way to discover them is to already know their names. The listing also orders its entries differently from the accept-list, so the two are not a reordering of each other. (The listing is a static help table; the accept-list is what actually gates a write.)

Several fields the configuration carries are deliberately **not** in the accept-list and are rejected as unknown keys, including the tenant site (written only as a side effect of setting a Jolli API key — see below), the compile-exclusion list, the storage mode, the commit sign-off flag, the auto-sync flag, the install identifier, every telemetry field, and the two dashboard sidebar-menu switches — one for the Knowledge menu row, one for the Graph menu row. Three of those — the compile-exclusion list, the sign-off flag and the auto-sync flag — are editable from the desktop-editor settings surface, so the two surfaces are not subsets of one another in either direction. The two sidebar-menu switches are editable from **neither** this command nor either editor panel: they are the first configuration fields whose only writer is the dashboard.
...
- **`localAgentModel`** — stored **verbatim**, including an explicit default, unlike the two settings panels, which store the default as an absent field. The difference is deliberate and only becomes visible on an upgrade that changes the default: an absent value follows the new default, a literal one stays pinned. Selecting the default option in a picker means "I expressed no preference"; typing the model's name here means "I chose this model". Returning to "follow the default" from this command is `--remove localAgentModel` — an empty value is rejected, as it is for every enum key. Must be exactly one of the identifiers any pinned tool offers. The failure message does NOT follow `localAgentTool`'s flat form: it groups the identifiers under the tool that offers each, because the namespaces are disjoint and a flat union reads as one menu that would tell nobody half of it does nothing under the tool they have selected. The accepted set is the **union across every pinned tool**, derived from the same registry, deliberately rather than being scoped to the tool currently stored: this key and `localAgentTool` are set by two independent invocations in either order, so a per-tool check would reject a legal value merely because the tool had not been switched yet. Applying the value to the tool actually in force happens at read time, where an identifier that tool does not offer falls back to **that tool's own** default rather than reaching the agent CLI. Whose default matters: the identifiers are each CLI's own namespace, so falling back to another tool's would send an identifier the agent answers with a client error rather than a milder model.
...
- **The two dashboard sidebar-menu switches are the first configuration fields whose only writer is the dashboard.** They are rejected here as unknown keys and carry no field in either editor panel, so the dashboard's own settings surface is the only place a user can change them. No configuration field before these two had the dashboard as its sole write path. (Notable.)
...
- The `syncSessions` switch set here is the gate the machine-wide session-statistics push channel reads before it uploads anything; the diagnostic command's `--sync-sessions` form forces a run past that channel's throttle and any backend silence, but never past this switch (spec 59). The dashboard's own Sync tab writes the same field.
