...
| `localAgentModel` | enum | Which model the local agent is told to run, for the tools the product pins one for. One of `sonnet`, `haiku`, `opus`, `inherit`. Distinct from `model`, which names an API model for the direct / proxy providers and does not reach this one. |
