...
- The per-source filename sanitization rules: identity for sources whose native id is already filesystem-safe and globally unique within the source; a replace-unsafe-then-append-content-hash transformation for every source whose native id carries a directory separator or is otherwise collision-prone.
...
- The second, independently-declared merge rule a source may opt into — restoring the superseded file's harvested title, url, display fields and body when the incoming observation recovered less — the load-bearing order between it and the body merge, and the fact that the write reports the *effective* title and url it actually persisted rather than echoing its input.
...
- A per-source subdirectory under the root, named exactly by the source id. A source id is a plain string; the id space is **open** (spec 255) — the built-in definitions supply the ids seen in practice, but the layout does not assume a fixed set and never enumerates them.
...
- **Non-path-safe sources** (flag false) — the sources declaring this are the code-host source, whose native id is `<owner>/<repo>#<number>` (both the slash and the hash mark are unsafe filename bytes, and two different repositories can share the same issue number); the library-documentation source, whose native id is a slash-prefixed multi-segment library identifier; and the error-tracking source, whose native id is a hostname followed by a slash and an issue identifier (which is also what keeps two tenants' same-numbered issues apart). The sanitized key is computed in two steps:
...
A source definition may declare an **accumulate-body** flag (spec 255 owns the flag's contract; spec 154 records which built-ins declare it — the self-referential memory lookup and the design-file lookup do). Its identity is an act rather than an entity, so successive writes of the same key must *collect* rather than overwrite.
...
4. Determine the **effective reference** to render, by folding the existing file into the incoming reference. This is a general merge that runs for **every registered source**, gated per rule on what that source's definition declares. When an existing file was read and parses:
   - **Body merge** — for a source declaring the accumulate-body flag, the incoming body is replaced by the merge of the existing body with the incoming one (see "Accumulating bodies").
   - **Harvest restore** — for a source declaring a title-fallback pattern (spec 255), the parsed file's **title, url, display fields and body** are folded back over the incoming reference when the incoming title matches that pattern and the parsed one does not — i.e. when this observation recovered *less* than the one it supersedes. Each value is restored only when the parsed reference actually carries one, so an absent value there never erases one the incoming record did resolve. The body is exempt for a source that also accumulates.

   **The order is load-bearing: the body merge runs first, the harvest restore second.** The restore deliberately leaves an accumulating source's body alone, so an already-merged body survives it; running the restore first would make that exemption depend on the merge not having happened yet.

   For a source declaring neither, and for a first write (no existing file, or one that does not parse), the effective reference is the incoming one unchanged.
...
6. Capture the **effective title, and the effective url when present**, for the return value as well. These are built **once**, before the unchanged-bytes check below, so the skip path and the writing path cannot report different values for the same call — a caller must not get a different answer depending on whether the bytes happened to change.
7. If the existing bytes match the freshly rendered bytes exactly, log a "skipped" debug event and return without writing.
8. Create the parent directory recursively if it does not exist. A pre-existing directory is not an error.
9. Write the rendered bytes to the target path. Encoding is UTF-8. The write replaces the file in place (no temp-file rename dance; not atomic against external readers).
10. Log a "wrote" debug event with the path and rendered byte count.
11. Return the target path, the content hash, the effective title, and the effective url when one is present.
...
**The returned title and url are the point of the return value, not decoration.** Because the harvest restore can keep values the incoming reference did not carry, the caller that maintains the working-context registry row stores **what was written**, not what it handed in — so the row can never disagree with the document it points at, which is the document a click opens and the label the sidebar renders. That caller is out of scope here; what this layer guarantees is that the two values it reports are the ones in the bytes on disk.
...
- **The hash-suffix on each non-path-safe source — the code-host source, the library-documentation source and the error-tracking source — is computed over the original native id, not the post-replace form.** A pathological input where two different originals would replace to the same form is still resolved by the hash; the hash never sees the lossy replacement.
...
- **Atomic-write is not provided, and for an accumulating source the caller MUST serialize.** A reader racing the writer can see a partially written file. There is no temp-file rename, no fsync, and no advisory lock inside this layer; serialization is the caller's responsibility. For a source declaring neither merge rule that responsibility is soft — the rendered bytes are a pure function of the incoming reference, so two interleaved writers of one key produce identical bytes and the race is harmless. Either merge rule removes that property, and the accumulate-body flag removes it destructively: the write becomes a read-modify-write, so two writers that both read before either writes each merge into the same pre-merge body and the later write silently drops the other's entries. This is reachable in practice, because an accumulating source's key is the *act* (the tool), not the agent — so independently-scheduled writers (an agent-stop hook and a polling discovery tick) contend for one file. The single caller therefore holds the project's plans lock across the whole write. Note what that lock does *not* cover: the plans-registry writer's per-key merge is a residual mitigation for a failed lock acquisition, and it works only because a registry row is overwritten wholesale; nothing mitigates a lost update on an accumulated body.
- **The write is a read-modify-write for any source declaring either merge rule, and the two rules are ordered rather than independent.** The body merge must run before the harvest restore, because the restore's "leave an accumulating body alone" exemption exists precisely to protect an already-merged body; reversing them would silently make that exemption a function of evaluation order. The two rules are otherwise deliberately not nested — a source can declare either alone — and a source declaring neither still gets the incoming reference back untouched, so the write remains a pure function of its input there.
- **What the write returns is what it wrote, not what it was handed.** Alongside the path and the content hash, the call reports the effective title and (when present) the effective url — necessarily, since the harvest restore can substitute values the caller never supplied. Both are computed before the byte-equality short-circuit, so a skipped write and a performed write answer identically; a caller storing the incoming values instead would leave its own row disagreeing with this file.
...
- **The note's exact sentinel string is duplicated in a downstream reader and must stay in lockstep.** The editor-extension reader that builds a short description preview for its reference rows performs the same cut before truncating, using its own hard-coded copy of the sentinel rather than importing this layer's. A change to the sentinel that misses that copy would leak the note text into every preview. The JVM port's parser implements **no** equivalent cut at all, so it is not in lockstep — and that is no longer moot. That port's closed source enumeration was widened to cover the product's own memory-lookup source and the documentation-lookup source, and **both** of those declare the two flags that make the writer append an auto-note. The port can therefore now represent a flag-declaring source, and when it parses one of those files it folds the note — the horizontal rule, the explanatory blockquote, and the accumulated per-invocation block — into the parsed description wholesale. That parser is production-live on the JVM surface — it is the one function in that file with a caller (the reference hover popup, spec 132), the rest of the module being test-only — so the divergence is reachable rather than hypothetical. What limits the blast radius today is the caller, not the parser: the popup consumes only the parsed field list and never renders the description, so the folded-in note is currently produced and discarded. Any future JVM consumer of the parsed description inherits the leak.
