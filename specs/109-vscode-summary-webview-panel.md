...
- The meter's refresh control: which panels emit it, why it is present in *every* meter state including the one that reports no usage, the command it posts, and the fact that it is re-bound on each swap rather than once at load.
...
- The one shared in-place meter swap both correcting paths use, and the markup restriction that makes replacing an element's whole outer markup acceptable there.
- Which stored conversation records the conversations list and the per-source count chips deliberately hide — a rule this panel now *consumes* rather than owns.
- The empty-conversations line: the three sentences it can be, the per-memory repair verdict that picks one, the reply it rides on, and the fact that the client latches that verdict rather than re-asking for it.
- The footer's two blocks: a transcript-privacy note carrying a count, and the render-time attribution line with its optional provider clause.
- The collapsible behavior on topic cards and the "expand / collapse all" toggle.
- The context-relevance display layer inside the Context panel: a tier chip (High / Med / Low) + one-line reason under each *kept* plan / note / reference row (sourced from the summary's `contextRelevance` record), and each *excluded* item rendered as an inline dimmed, struck-through row with an "Excluded" chip, an optional reason, and a single delete affordance (sourced from the summary's `excludedContext` record). Fail-open (empty-reason) verdicts render no chip/reason. The excluded rows are rendered uniformly whether the exclusion was a model verdict or a deterministic foreign-repository classification — the panel does not read the reason text to tell them apart.
- The single aggregate "Skills used" row inside the Context panel: how it counts toward the header chip, that it carries no per-row affordances, and the message its click posts.
- The Push button's handling of the per-repo outbound-push opt-out: an information message, not an error.
...
- What a detach does to the persisted figures — ownership resolution, subtraction, cost re-derivation, write ordering, and the failure path's re-derivation and notification gate (spec 306). This spec owns only the row's UI contract.
- What the meter's refresh control actually does once its command reaches the host — the derivation from the archived conversations, its evidence contract, its per-node gate, the stale-identifier cleanup that rides along, and the four outcome messages it reports (spec 333). This spec owns only the control's presence, its command, and the reply that swaps the meter.
- The repair engine behind the conversations verdict — what a real repair reads, the two bounds it insists on, every reason it refuses, and the write it performs — owned by the transcript-repair topic. This spec owns only the sentence the panel prints and how the verdict reaches the page.
- The archived-conversation grouping rule itself — what counts as one conversation, what order its turns end up in, and which stored sessions are not conversations at all — owned by the archived-conversation grouping topic and shared with the dashboard's memory detail. This spec owns only what this panel's rows, chips and detach do with the result.
...
4. **Task-usage meter** — sits between the header's title line and its property rows. Three states, and no other:
   ...
   The head line also carries a **refresh control** that asks the host to re-derive this memory's token and cost figures from its archived conversations (spec 333 owns what that does). Three things about it:
   - **It is emitted whenever the panel is not read-only**, and omitted otherwise — the handler behind it writes to this workspace's storage, so a read-only panel ships no affordance for it at all rather than hiding one with styling.
   - **It is present in every meter state, including the "usage not reported" one.** That is deliberate rather than an oversight: a memory whose figures were stripped, or never corrected after a lost write, renders precisely as "not reported", so a control attached only to the populated state would be missing exactly when it is wanted. Unlike the help affordance beside it, it is also **not** tagged as exempt from the read-only affordance-hiding rule — a second layer that is moot only because a read-only panel never emits it in the first place.
   - **It is bound to its click on every meter render, not once at page load.** The meter is replaced by swapping the element's whole outer markup, which discards any listener held by the old node — so re-binding is what keeps a second press working after the first one has already swapped the meter.
...
8. **Context** — plans, notes, and references, in one panel with a header count chip and an "add" affordance. Rendered unconditionally: the panel and its header always appear, even when the summary has zero plans, notes, and references, in which case the body shows an empty state instead of the section being omitted. Kept rows are ordered plans → notes → references → the single aggregate skills row (see "Skills row"); reference rows within that block are ordered by the built-in source registry (see "Reference ordering"). Each kept row carries a second meta line — a relevance tier chip + one-line reason — when the summary records a verdict for it. Excluded items render *after* the kept rows as inline dimmed, struck-through "Excluded" rows (see "AI context-relevance display"), replacing the earlier collapsed "AI excluded N" disclosure.
...
12. **Footer** — two blocks, in this order:
    - A **transcript-privacy note**: a lock glyph, then a sentence stating that the full conversation transcripts stay in the user's repository and are never included in shared exports, carrying a **count** in parentheses.
    - An **attribution line**: "Generated by Jolli Memory" plus a timestamp read from the clock **at render time** — not the summary's own generation stamp — followed by an optional `· via <provider>` clause in its own span, **omitted entirely** when no provider resolves for the summary rather than printed as an unknown.

    The count is the number of **transcript files** linked to the tree, not the number of conversations, and it is baked into the markup when the page is built. Neither block is rebuilt by anything short of a full re-render. Both facts have consequences that are recorded under Notable Behavior.
...
The Context panel's header carries a count chip (plans + notes + references + skills) that sits outside the body it sits above. **Skills contribute exactly one, however many were captured** — the panel renders them as a single aggregate row, so the chip counts rows on screen; counting them individually would show `CONTEXT 14` above four visible rows. The same one-or-zero term appears in the body's own total, and the two must stay in step. One shared formula computes this count from the summary; both the initial full render and the partial in-place refresh described below call that same formula, so the two can never disagree about the number for a given summary. Because the chip lives outside the body that the partial refresh replaces, the refresh message that rebuilds the body also carries a `count` field holding the freshly computed number; the webview reads `count` and updates the chip's displayed text directly, independent of (and without recomputing from) the replaced body markup.
...
- **Excluded items** — one entry per item kept out of the summary's prompt, each carrying `kind`, `key`, a `title`, an optional `reason`, and an always-`low` `tier`. Each renders as an inline row *after* the kept rows: a source badge, the title struck through and dimmed (full opacity on hover), an **"Excluded"** chip (always shown, even with no reason), the ✨ reason when present, and a single trash button. The trash button is the row's only action — an excluded item was never archived into this commit, so there is no snapshot to preview or edit.

  **Not every row here is an AI verdict, and the panel cannot tell.** Most entries are the relevance ranker's soft-excludes, whose `reason` is the model's own one-line explanation. But a plan whose source file belongs to a **different repository** is excluded deterministically, upstream of any model, and its entry arrives on the same field with a fixed reason phrase and the same `low` tier. The panel neither reads nor matches that phrase, so such a row renders exactly like a model verdict — the ✨ glyph included. Deleting it does not change the deterministic decision either: it edits this commit's audit trail only, and a future commit re-derives the same exclusion from scratch.

### Skills row

The Context section builder takes the summary's skill records as **one object pairing the record list with the commit hash** — not two trailing parameters. The pairing is enforced by the type on purpose: the row's only action is "open this commit's aggregate table", so records handed over without a hash would render a row that cannot be opened.

However many skills the memory captured, they render as **one** row: an `S` kind tag, the title **"Skills used"**, and a meta line carrying the shared pre-formatted aggregate summary label (the same label the aggregate document uses, so the row and the document it previews cannot disagree), suffixed with ` · some inferred` when at least one member was detected heuristically rather than observed. "Inferred" is spelled out rather than daggered here because this row has no footnote in reach.

The row is **deliberately affordance-free** — no edit, no remove, no pin, no translate button, and no relevance meta line. There is nothing to edit (the record is a measurement, not a document), nothing to dissociate (the row stands for a set, not an artifact), and the relevance ranker is never fed skills, so a verdict for one cannot exist. Its title is the row's single interactive element: clicking it posts `previewCommittedSkills` carrying the commit hash.
...
merged, and the row keeps the first-seen owning commit).

**These rules are no longer webview-local.** They are a CLI-owned product rule
with two consumers — this panel and the dashboard's memory detail — so "how many
conversations does this memory have" has one answer on both surfaces instead of
one answer per surface. The full rule is owned by the archived-conversation
grouping topic; what follows is only what this panel's own contract rests on.

Two membership rules matter:
...
Two more facts about the merge itself, both of which this panel's rows inherit
rather than decide:

- **The carrier predicate reads the first-seen slice's usage share, not any
  slice's.** A conversation is represented by the record the walk met first, so a
  share recorded only on a later slice does not enter the test at all.
- **The slices are merged chronologically, not in encounter order.** A
  consolidated memory's transcript set is not in time order, so each of a
  conversation's slices is placed by its own first parseable turn timestamp, and
  slices with no parseable timestamp keep their first-seen position rather than
  being sorted to the front. That ordering is what the row's turn list and the
  row-click transcript both show.

### Conversations empty state

When the conversations list has no rows, the body carries one line, and which
line it is comes from a per-memory **repair verdict** — a CLI-owned answer to
"why does this memory have no captured conversations", one of `present`,
`repaired`, `repairable`, `unrepairable`:
...
| `present`, `unrepairable`, an unrecognised value, or none received yet | *No conversations were captured for this memory* |

`repairable` means precisely "a real repair run would succeed" — the verdict is a
dry run of the repair engine itself, not a looser "some owned transcript exists"
guess. The engine, its bounds and its refusals are owned by the transcript-repair
topic.

Three properties of how the verdict reaches the page:

- **The host derives it in process**, because this host bundles the CLI. The same
  predicate is what the IntelliJ viewer asks for over its bridge, so the two
  editor surfaces cannot word the same memory differently.
- **It rides the conversations reply**, not the build-time HTML, because both
  places the page prints this line are client-side: the initial render of the
  rows, and the moment a detach empties the list.
- **Every failure path answers the mildest verdict and none of them propagates.**
  No current summary, or any throw while deriving it, yields `unrepairable`; the
  rows are what the user opened the card for, so an unreadable answer must not
  cost them the list. `repairable` is the one direction it would be wrong to guess
  in, because it invites a repair with nothing to work from.

**The verdict is latched from the last conversations reply, not recomputed.** The
host sends no second message carrying it, and the last detach empties the panel
long after that reply arrived, so the page keeps the value it was given. Anything
it does not recognise — including nothing at all — leaves the plainest line.
...
| `previewCommittedSkills` (`{ commitHash }`) | **None** — a read. Dispatches the committed-skills preview command with the hash alone; **no foreign-provenance arguments are forwarded**, unlike the plan and note previews, because the command resolves which repo owns the hash itself rather than having its storage selected up front. | The commit's skills-aggregate document opens. |
...
| `loadConversations`, `openConversation` | **None** — both are reads. The first fills the conversations rows (titles resolved through the same helper the sidebar uses, so the two surfaces show identical labels); the second opens one row's archived turns in a separate read-only panel. | Conversations data for the rows — three fields, the rows plus **this memory's repair verdict**, which picks the empty-state line (see "Conversations empty state"); and, for the second, a new read-only panel. The verdict is derived in process, and every failure answers the mildest one rather than propagating, so an unavailable verdict never costs the reply its rows. Both messages are permitted in foreign-readonly mode. |
| per-conversation detach request (identifies the memory plus the conversation's **producer-and-id pair**) | Removes that one conversation (matched on the pair, not the bare id) from every stored conversation record of this memory's tree — rewriting records that still hold conversations, deleting those left empty — then performs **one** summary write carrying both the transcript-id removal and the corrected token/cost figures (spec 306 owns that correction). Behind the stale-commit guard, and excluded from both the foreign-readonly and the regenerate-in-flight allow-lists, since it writes storage. | A detach acknowledgement naming the row, carrying a rebuilt token-meter fragment **only when the figures actually changed** — by the correction *or* by the failure path's re-derivation. A no-match (already detached / stale row) still acknowledges so the row is cleared rather than left stuck. A failed record write surfaces an error toast and no acknowledgement. A failed summary write always acknowledges the row and first attempts a re-derivation from the surviving conversations; only when *that* derives nothing **and** the correction had attributed something does the user get a warning. Both other combinations are silent (spec 306). |
| meter refresh request (no payload) | Re-derives this memory's token and cost figures from its archived conversations and, in the same write, drops conversation references with no surviving record. Behind the stale-commit guard; excluded from the foreign-readonly allow-list (so a foreign panel never reaches the handler) and from the regenerate-in-flight allow-list (so it is dropped silently while a regeneration is awaiting, whose own final write would otherwise clobber it). | **An outcome is always reported by notification** — a silent no-op would read as a broken control (spec 333 owns the four wordings). A meter-replacement message is posted **only when something actually changed**; when nothing changed there is no reply at all and the notification is the whole answer. |
...
When the push is refused because the user has turned **outbound push off for this repository** (spec 310), the panel shows an **information** message carrying the refusal's own sentence — not the red "Push failed" toast every other refusal gets. This is a deliberate opt-out and memory is still being recorded locally, so presenting the user's own choice as an error would misrepresent it. The error is then rethrown anyway so the push button resets out of its loading state.
...
| Conversation detach | **Two surgical in-place edits, no full re-render.** The acknowledged row is removed by matching the producer-and-id pair (a bare-id match could remove the wrong row, since two producers can mint the same raw id), the conversations count chip is decremented to the surviving row count, and an empty list is replaced by the empty-state line — one of the three sentences under "Conversations empty state", chosen from the verdict **latched from the last conversations reply** rather than recomputed, since no second message carries one. When the acknowledgement carries a rebuilt meter fragment it goes through the shared meter swap below. When no fragment is carried — nothing could be attributed *and* nothing could be re-derived — the meter is left showing its current value. A full re-render is deliberately avoided so a single-row change does not collapse scroll position and expanded sections. |
| Meter refresh | **One in-place edit.** The reply carries only the rebuilt meter and goes through the same shared swap; nothing else on the page is touched, because nothing else changed. |
| Push success | Full re-render so PR section picks up the published Jolli URL and plan/note URLs. |

### The shared meter swap

Both paths that correct the figures — the detach acknowledgement and the refresh reply — hand their fragment to **one** swap routine rather than each performing its own replacement, so the two cannot drift on the re-initialisation step. A swapped meter that skipped that step would render as an empty bar with dead buttons.

The routine ignores a fragment that is not a non-empty string, and does nothing when there is no meter on the page. Otherwise it replaces the meter element's **whole outer markup** and re-initialises the new node.

Two constraints ride on that:

- **Re-initialisation is scoped to the replaced element, never to the page.** The help affordance's listener is bound per element, so re-scanning document-wide would hand a second listener to every other pinnable popover on the page and each click would toggle it twice — a net no-op that reads as a broken tooltip.
- **Replacing the outer markup is acceptable here only because of what the fragment contains.** It is composed host-side from numeric aggregates and literal strings; no commit message, path, or other repository content reaches it. This is explicitly not a pattern to extend to markup carrying such strings.
...
- **A per-repo push opt-out is reported as information, not as a failure.** The opt-out arm sits ahead of the generic error handling and calls the *information* notification with the refusal's own message; every other refusal (plugin-outdated, permission-denied, anything unclassified) gets an error toast. The distinction is that the opt-out is the user's own setting and capture is unaffected — only outbound sync is off. The error is still rethrown so the button leaves its loading state. The **IntelliJ mirror of this panel goes one step further**: it carries an extra arm for "the gate could not be *evaluated*" (nothing was sent — fail-closed), shown as a warning dialog and deliberately kept **out of** the generic arm's `trackError("push", "push_failed")` path, because a setting the plugin couldn't read is not a push failure and counting it would pollute the error metric with a retryable condition. The VS Code side has no such distinct error type today. (Surprising; intentional. See spec 310 for the opt-out itself and spec 327 for which refusals are repo-wide.)
- **A source comment on both push sites still cites the wrong spec number for the opt-out.** The VS Code arm and the IntelliJ arm each say "spec 306" where the per-repo outbound-push control is **spec 310** (306 is the conversation-detach usage correction). Known inaccuracy in the comments only; the behaviour is as described here. (Notable.)
...
- **A detach is a surgical DOM edit, not a re-render — and the meter is only swapped when the figures really changed.** Omitting the swap is the honest rendering of "the removed conversation's share could not be attributed and could not be re-derived either", so the meter keeps its previous value rather than being replaced by a guess. (Notable; the correction itself is spec 306.)
- **The meter's refresh control is rendered in the state that reports no usage, and that is the state it matters most in.** A memory whose figures were stripped or never corrected renders as "not reported", so attaching the control only to the populated meter would omit it from exactly the memories a user wants to re-derive. (Surprising; intentional.)
- **The control's presence is decided by the read-only flag on the first render only.** Every later replacement fragment is built with it hard-coded present, so a swap can never drop a control the first render emitted — but nothing in the fragment re-checks the panel's mode either. What keeps that consistent is that neither writing path is reachable on a read-only panel: a foreign-repository panel is refused by the message dispatcher, and a superseded-commit panel by the stale-commit guard. (Surprising; correct by the gates rather than by the markup.)
- **The refresh control is not on the foreign-safe list, and the handler's own foreign check is therefore dead.** The dispatcher default-denies anything absent from that list, so a foreign panel is refused before the handler runs; the check inside it can never execute. The same dead check pre-exists on the detach handler. (Unreachable path; the dispatcher is the live protection.)
- **A refresh that changes nothing sends no reply at all.** The host reports the outcome by notification instead, so the webview's swap runs only on a real change — which is why the absence of a reply is not a hang. (Notable; the outcome wordings are spec 333.)
- **The meter swap replaces an element's entire outer markup, which is safe only because of what the fragment holds.** It is built host-side from numeric aggregates and literal strings, with no commit, path, or other repository text in it. Extending the same swap to markup carrying such strings would not be. (Notable.)
- **The refresh control is re-bound on every meter render rather than once at load.** The swap discards the old node, so a listener bound at load would leave the second press doing nothing. (Notable.)
...
- **A detach whose summary write fails now attempts a repair before it reports anything, and a successful repair is completely silent.** The handler re-derives the figures from the conversations that survive and drops the now-dangling references in one write; when that works the user sees only the meter change under the removed row, with no notification. Only when the repair derives nothing *and* the correction had attributed something does a warning appear. When neither held, the user is still shown nothing at all — a log line is the whole report. (Surprising; the silent-with-nothing-attributable branch is a reporting gap, not a designed exemption. Spec 306 owns the repair; spec 333 owns the derivation.)
...
- **The empty-conversations line says *why* there are none, and the page latches the answer rather than re-asking.** The verdict rides the conversations reply; the last detach empties the panel long after that reply landed and no second message carries a fresh one, so the line a detach paints is chosen from the latched value. An unrecognised or absent value keeps the plainest sentence, which is also what every failure to derive one resolves to — the rows are what the user came for, and "repair may still be possible" is the one direction it would be wrong to guess in. (Notable; the engine behind `repairable` is the transcript-repair topic's.)
- **For a memory read out of another repository's shared folder, the verdict is answered against *this* repository's evidence.** The panel asks the predicate with the current workspace root, as every other read in it does, so a foreign summary is judged by the owned-session ledger of the repo the user is standing in — and can therefore display *repair may still be possible* for a commit whose repair could never run from here, since a real run resolves the summary in the current repository and would find nothing. The IntelliJ viewer does not have this: its lookup misses and it answers the mildest verdict instead. So the two surfaces can word the same memory differently, which is the divergence the shared predicate was introduced to prevent. (Defect.)
...
- **The skills row is one row and one chip-count, however many skills were captured.** Both the header chip and the body's own total add skills as a single term (`skills.length ? 1 : 0`), because the panel renders them as one aggregate row; the two must move together or the chip disagrees with the rows below it. (Notable.)
- **The skills row is the only Context row with no affordances at all.** No edit, remove, pin, translate button, and no relevance meta line — a measurement is not a document to edit, an aggregate is not an artifact to dissociate, and the relevance ranker is never fed skills so no verdict can exist for one. Its title link is its whole interaction surface. (Surprising; intentional.)
- **The skills preview forwards no foreign-repo provenance, and that is the correct shape.** The plan and note previews must be told which repo's storage to read; the committed-skills command resolves the owning repo from the commit hash itself, so passing provenance would be redundant rather than protective. (Notable.)
- **The relevance layer is two records, two treatments.** Kept-item verdicts (`contextRelevance`) add a tier chip + ✨ reason under a row's title; excluded items (`excludedContext`) render as inline struck-through "Excluded" rows after the kept rows. A fail-open ranking leaves an empty `reason`, so the chip is suppressed rather than fabricating a tier — every surface (this panel, the sidebar hover card, the review panel) gates its chip on a non-empty reason so a ranking failure decorates nothing. (Surprising; intentional.)
- **An "Excluded" row is not proof that a model excluded it.** The `excludedContext` field also carries the plans the pipeline classified as belonging to a different repository, which are excluded deterministically with a fixed reason phrase and the same `low` tier. This panel renders both kinds identically, ✨ glyph included, and never inspects the reason string — so the display attributes a deterministic decision to the AI. (Surprising.)
...
- **`removeExcludedContext` edits the commit's audit trail, not the working set.** Deleting an excluded row drops that one entry from *this commit's* `excludedContext` and re-persists — it does **not** touch the working plan/note/reference registry, because an excluded item was never archived into the commit (there is no snapshot to delete) and the underlying item should stay available for future commits. For a deterministically-excluded foreign plan this is the whole of the effect: the classification is re-derived per commit, so removing the row edits the record and changes nothing about what a future commit does. It trips the same stale-commit guard + confirm modal as `removePlan` / `removeNote`, and re-checks the guard after the modal in case an amend landed while it was open. (Notable.)
...
- **The footer's privacy count counts transcript files while calling them conversations, and it disagrees with the chip above it.** Both figures start from the same number — the tree's linked transcript files — but only the conversations chip is corrected client-side, to the real conversation count once the rows arrive (one per producer-and-id pair, carriers filtered). So a memory whose one session was sliced across three commits, or whose several sessions share one file, shows two different numbers for the same population on the same page. A detach widens the gap: it decrements the chip in place and never touches the footer, which is only rebuilt by a full re-render. (Defect.)
- **The two sibling memory surfaces word this footer differently, in opposite directions.** The IntelliJ viewer's footer carries **no** transcript-privacy note and **no** count at all — only the attribution line, with its provider clause inside the same span. The dashboard's memory detail carries the note, but counts **conversations** rather than files and stamps **the memory's own generation time** instead of the render clock, so its line does not change under a page refresh the way this one does. Nothing reads any other surface's choice. (Notable.)
...
- **Archived-conversation grouping** — the CLI-owned rule that decides what counts as one conversation, how a conversation's slices are merged and ordered, and which stored sessions are usage-only carriers rather than conversations. This panel and the dashboard's memory detail are its two consumers, which is what keeps one memory's conversation count from differing between them. Owned by the archived-conversation grouping topic.
- **Transcript repair** — the engine that rebuilds a memory's missing conversation slice from local history, and the dry run of it that answers this panel's repair verdict. It owns the four verdicts, the bounds a repair insists on, and every reason it refuses; this spec owns only the sentence the empty conversations body prints. Owned by the transcript-repair topic, and asked by the IntelliJ viewer over its own bridge so the wording cannot diverge between the two editors.
...
- **Task-usage meter arithmetic** — the token segments and their formatters come from spec 243, the per-model price table and the per-node / per-bucket cost preference (and the three tooltip wordings) from spec 257, and the tree-aggregation helpers the meter walks from spec 04. This spec owns only the meter's placement, its three render states, its refresh control's presence and binding, and the shared in-place replacement both correcting paths use.
- **Conversation detach correction** — subtracting a detached conversation's persisted share from the owning node, re-deriving the cost, the records-before-summary write ordering, and the failure path's re-derivation and notification gate are owned by spec 306; the share it consumes is written by spec 245. This spec owns the row's action, its acknowledgement, and the DOM edit.
- **Conversation usage recomputation (spec 333)** — what the meter's refresh control invokes: the derivation from the archived conversations, the per-node ownership resolution and forward-only gate behind it, the stale-reference cleanup that rides in the same write, and every wording the outcome notification can take. That spec also owns the third caller of the same derivation, which is the regeneration path. This spec owns only the control itself, its command, and the reply that swaps the meter.
- **Context snapshot markdown preview (spec 329)** — the read-only virtual-document scheme behind this panel's archived-reference and skills-aggregate previews: its URI shape, its cache and post-reload resolver, and the frontmatter rewrite it applies. This spec owns only the rows that post the preview commands.
- **Per-repo outbound-push control** — the identity-keyed opt-out store, the single `isOutboundPushAllowed` predicate every push site consults, and the surfaces that toggle it are owned by spec 310. This spec owns only how the Push button reports the refusal. Which refusals are repo-wide (and therefore stop an attachment loop rather than being collected per item) is owned by spec 327.
- **Skill-usage aggregate rendering (spec 323)** — the per-commit skills-aggregate document the Context row previews, and the shared summary-label formatter whose output the row's meta line renders verbatim. The sidebar's own skills row is spec 324.
