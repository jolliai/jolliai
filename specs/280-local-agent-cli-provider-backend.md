...
- Building the child-process invocation: the shared parts (headless single-shot operation, tool denial where expressible, the isolated temporary working directory, the re-entrancy markers) and the per-tool differences (prompt delivery channel — including the one tool whose channel is chosen by prompt size — system-prompt handling, tool-specific flags, environment posture).
...
- **The degrade-and-retry loop around a run** — how an unrecognized isolation flag is detected after a failure, attributed, dropped, retried, and remembered per tool-and-version in a machine-global store — is owned by spec 331. This spec declares the flags and the request field that carries the omissions; it does not own the loop, the attribution rules, or the store.
...
There is a per-tool model setting, and it is separate from the single global model setting, which still does not reach this provider at all: a tool that declares a model list is handed one of its own identifiers, and every other tool is handed an empty model and omits its model flag (see *Model attribution*).
...
A single request carries the template-filled prompt text, a model identifier (possibly empty), a fixed system prompt, and an optional **set of droppable-flag identifiers this invocation must omit**. There is deliberately **no** output-token cap: no driven CLI exposes a per-call max-output-tokens control, so the API path's max-tokens budget (and its truncation stop-reason) do not apply here.

The omission set is passed *in* rather than read from disk by the backend, which keeps invocation construction a synchronous pure function; the dispatch layer loads the store once per call and owns the degrade-and-retry loop (spec 331). Absent — the common path — means "pass every optional flag".

### Droppable isolation flags

A backend may declare an ordered list of **optional flags**: pure optimizations that strip the user's own MCP servers, plugins, skills and settings out of a prompt that is forbidden from calling any tool. Each declaration carries a stable identifier (the key persisted and matched against a failure message), the argument items it expands to, and optionally extra phrases that also indict it. A backend may additionally declare that its CLI can never name an offending flag, which governs what the degradation layer persists.

| Tool | Droppable flags (identifier → expansion) | Extra match phrases | Can never name a flag |
|---|---|---|---|
| `claude-code` | `--strict-mcp-config`; `--disable-slash-commands`; `--setting-sources` → `--setting-sources` `""` | none | no |
| `codex` | `--disable` → `--disable` `plugins` | `--disable`, `Unknown feature flag: plugins` | no |
| `opencode` | `--pure` | none | **yes** |
| `cursor-agent` | none | — | no |
| `kimi` | none | — | no |

Two identifiers are the **flag alone** while their expansion is a pair, because an argument parser names the unrecognized *token*, not the pair — and dropping the flag must take its value with it, or a tool that receives its prompt positionally would swallow the orphaned value as the prompt. The order above is the order the argument vector emits them, so a CLI that supports all of them builds a byte-identical command line to one built with no degradation machinery at all. The list is expanded through a filter that skips any identifier the request marked omitted.

One environment variable serving the same isolation purpose is deliberately **not** declared droppable: an unrecognized environment variable is ignored by every version, so it cannot fail a run.

### Normalized outcome

Parsing a tool's result yields: the completion text, input-token count, output-token count, a combined cached-token count, a cost figure in the local currency unit, a stop reason (or none), and optionally **the model the tool reports having actually run**. Only Claude Code reports a cost at all; for every other tool the figure is always zero, and every tool except Claude Code and Cursor reports no stop reason. Claude Code is likewise the only tool that names its model — from a per-model usage map, keyed by the model identifier the CLI ran. That map is **not** single-entry: the tool runs a small helper turn of its own that cannot be switched off, and on a short answer that helper produces MORE output than the answer, so the entry naming the **requested** identifier wins, falling back to the entry with the highest total input (the answering turn is the one carrying the conversation cache) when nothing was requested. Its map key retains the context-window variant, which is a distinct product at a distinct price. Every other tool leaves the field unset. This is mapped into the shared LLM-call result, which additionally records the provider source label and the tool identifier; the cost figure has no field in that shared result and is surfaced only in a diagnostic log line, since local-agent spend bills the tool's own account rather than a metered key.
...
4. **Capability probe — deliberately asymmetric.** Each candidate is invoked directly (never through a shell), with its launcher arguments followed by the tool's probe arguments, under a bounded per-candidate timeout. **Only Claude Code is probed with a real run flag (the non-interactive permission mode) alongside the version query**, so a CLI too old to accept that setting is correctly classified incapable. Every other tool is probed with a **bare version query only**. The consequence is real and unmitigated: for those tools, an installed-but-too-old CLI passes discovery and fails at the first actual generation instead, surfacing as a run-time setup failure rather than a discovery failure.

   **No probe argument list carries an isolation flag, and adding one back is a regression** — see *Isolation flags are optional, and unverifiable in advance* below.

   The reported version is **the first whitespace-delimited token of the probe output that looks like a version** (optionally a leading `v`, then digits, then a dot), falling back to the first token when none does. Taking the first token unconditionally is wrong for any CLI that prints its own product name first, and one does: its output begins with a product name, so every build of it collapsed to that one literal string. Two things broke silently as a result — version comparison treated every candidate of that tool as equal, so discovery order picked the winner instead of the newest; and the version-keyed capability store (spec 331) could never expire an entry on upgrade, permanently stranding the machine on a degraded invocation. Only that one tool is affected; the rest already print the number first. Empty output fails the probe, and a token that is not dotted-numeric ranks as zero, so among such candidates discovery order alone decides.
...
- **Model**: the tool's model flag is emitted only when the model identifier is non-empty (see *Model attribution*). In practice Claude Code and Codex receive one.
- **Optional flags**: the backend's declared droppable flags are emitted through a filter that skips every identifier the request marked omitted.
- **No output-token cap** is expressible on any of the tools.

### Isolation flags are optional, and unverifiable in advance

The flags that strip the user's own MCP servers, plugins, skills and settings out of a run are pure optimizations — dropping one costs prompt tokens and nothing else. But an agent CLI that does not *recognize* a flag does not ignore it: it exits non-zero **before running anything**, so one unrecognized optimization flag turns "your CLI is a few versions old" into "every summary on this machine fails", non-retryably.

Nothing can pre-detect this. **One CLI pre-scans its arguments for the version query before validating options, so an unrecognized flag alongside a version query exits zero** — measured. So the probe cannot confirm a flag is accepted, and putting one in the probe arguments would be strictly worse than useless: an *older* CLI that validates options the ordinary way would fail the probe outright, and a failed probe has no recovery path at all (the candidate is discarded, discovery reports no compatible CLI, and the run-time degradation never gets to run). That is why every probe argument list holds only flags that are load-bearing for a run, and why each optimization flag is instead declared droppable and handled after a real failure (spec 331).
...
- Headless single-shot operation with machine-readable output; the model flag only when a model identifier is non-empty; an **empty tool allow-list** (which matches no real tool, denying all tool use); a non-interactive permission mode, so even an attempted tool call never prompts; and session persistence disabled. This is a pure text completion; the agent must not touch the filesystem or shell.
- **Isolation block, all three droppable**: `--strict-mcp-config`, `--disable-slash-commands`, and `--setting-sources` with an empty value. The tool-denial flag above only denies *built-in* tools; without these the CLI still boots the user's own MCP servers and injects their tool schemas, plus skills and settings-sourced content, into a prompt that is forbidden from calling any tool. Measured back-to-back on the same prompt, the isolated invocation used roughly forty-eight times fewer prompt tokens and was roughly thirty-two times cheaper — with most of the baseline weight sitting in the cache-creation and cache-read columns, so reading the plain input count alone makes the bloat look negligible and the win look like a regression. The empty setting-sources value also stops the user's own hooks firing in the child, which reinforces the re-entrancy defenses rather than duplicating them. These flags do not themselves change latency.
- **Deliberately NOT the "bare" flag**, which reads like the perfect fit for this block (it skips hooks, plugin sync, auto-memory and instruction-file discovery in one flag) but *also* stops the CLI reading its keychain OAuth. Since the environment scrub below removes every API-key path on purpose, keychain subscription auth is the only credential left — so that flag fails every call outright with a not-logged-in error envelope and zero tokens. Verified; do not "simplify" to it.
- Environment scrub: `ANTHROPIC_API_KEY`, `ANTHROPIC_AUTH_TOKEN`, `ANTHROPIC_BASE_URL`, `CLAUDE_CODE_OAUTH_TOKEN` and `CLAUDECODE` are all removed — the key or token would bill the API instead of the subscription, the base-URL override would route to a third-party gateway with no credentials, and either of the last two breaks the spawn outright.
...
- **Plugin isolation, droppable**: `--disable plugins` — the user's plugins (and the skills they carry) have nothing to contribute to a pure text completion and are pure prompt weight. Verified by diffing the tool's own authoritative session-state output on standard error: the baseline reports plugins enabled with several loader warnings, the isolated run reports neither, and the resolved model is unchanged.
- **Two forms that look right and are total no-ops**: setting an empty inline table for the plugin or MCP-server keys through the generic config-override flag is *accepted* and changes nothing, because that override is a merging dotted-path set and merging an empty table merges nothing — measured, with byte-identical session state either way. (A scalar dotted path does work; that is the difference.) Judging either by whether an incidental transport error disappeared is what previously made them look verified.
- **And one that works but changes the model**: the ignore-user-config flag genuinely drops the user's MCP servers, but takes everything else in the user's configuration file with it, including the model — measured as a different model on the same machine. That file is where this tool's model comes from whenever no model is pinned — the pre-pinning default, and still the behaviour under the “send no model flag” option — so ignoring it silently changes which model writes those summaries. So the user's MCP servers stay booted here: this tool exposes no lever that drops them without also dropping the model choice.
- Environment scrub: `OPENAI_API_KEY` and `OPENAI_BASE_URL`.

### Invocation construction — Cursor

- Prompt delivered as a **trailing positional argument**.
- No system-prompt flag; the fixed system prompt is **prepended to the prompt text**.
- Flags: print (headless) mode, machine-readable output, and **workspace trust pre-granted**. The trust flag is the one that suppresses the trust gate over an unfamiliar directory — deliberately not the command-approval/auto-run flag, which governs something else entirely; the freshly-created temporary working directory trips the trust gate without it. No droppable flags are declared for this tool.
- Environment scrub: `CURSOR_API_KEY`, so a leaked key can never proxy the run away from the subscription login.

### Invocation construction — OpenCode

- Prompt delivered as a **trailing positional argument**.
- No system-prompt flag; the fixed system prompt is **prepended to the prompt text**.
- Flags: the run subcommand, plus the droppable `--pure` ("run without external plugins"). There is no structured-output option to request.
- **`--pure`'s effect was never observed on the verification machine** — only the plugin SDK was installed there, so there was nothing for it to disable and no log delta to see. It is the tool's documented switch for exactly this intent and costs nothing when no plugins are present, but it is intent rather than measurement.
- Environment: **no credential is scrubbed.** This tool authenticates against whichever LLM provider the user configured inside it, either from its own stored credentials or from provider keys supplied through the environment; scrubbing would break the environment-key logins. The consequence is that a run under this tool can spend the **user's own provider credit** rather than a flat subscription.
- Environment **set**: `OPENCODE_DISABLE_CLAUDE_CODE=1`. This tool implements a compatibility mode that reads another vendor's machine-global instruction file and skills directory on top of its own configuration; for a pure text completion that is dead prompt weight, and the empty temporary working directory does not dodge it because those paths live in the home directory rather than the working directory. Verified against a real run: the baseline logs a duplicate-skill warning naming a file under the user's home directory, and with this set the line is gone and the run still succeeds. This is deliberately an environment variable rather than a flag — an unrecognized variable is ignored by every version, so it cannot fail a run and needs no droppable declaration.
- **MCP servers are deliberately not disabled.** This tool's only lever replaces the user's whole configuration file, and the model can live in that same file while this provider sends an empty model — so redirecting it would silently change which model runs. Same trade-off, and same conclusion, as Codex.

### Invocation construction — Kimi Code

- **Prompt delivery is size-adaptive — this is the one tool not bound by the argv limit the other argv-delivering tools carry.** The tool exposes no standard-input channel (its prompt flag treats a lone dash as a literal prompt) and no prompt-file flag, so a small prompt rides the command line as the value of a trailing prompt flag. Above a **24,000-character budget** on the assembled body, the body is instead written into the run's throwaway working directory and passed by reference, with only a short fixed directive left on the command line.
- **The file-mode contract**, verbatim, because a reimplementer must match it:
  - The file is named `jolli-context.md`, deliberately **not** one of the instruction filenames this tool auto-discovers from its working directory (which is the whole reason the run uses a fresh empty directory).
  - It is passed with `--agent-file`, which the tool treats as a Markdown **agent definition** and therefore **rejects outright without a YAML frontmatter block**. A minimal block (a name and a description) is prefixed to the body; both are inert, since the file is loaded explicitly by path and never auto-discovered.
  - The body is truncated to a **1,000,000-character** backstop, with a marker naming how many characters were dropped. This is a guard against a pathological input overrunning the model's context, not a working limit: the reference channel was verified to carry well over half a megabyte, against a worst-case prompt around 400 KB.
  - The `--prompt` value in file mode is a short fixed directive telling the model to follow the instructions in its agent definition and output only what they ask for.
- The budget exists because the largest prompt would blow the command-line limit on the platform with a hard one, surfacing as a spawn failure — which silently failed the large prompt while the short one succeeded. Small calls keep the proven command-line path unchanged, so only the large-prompt branch is new.
- No system-prompt flag; the fixed system prompt is **prepended to the prompt text** — the same shape Codex, Cursor and OpenCode use — and the prepended whole is what the budget is measured against and what goes into the file.
- Flags, in order: the model flag only when the model identifier is non-empty (see *Model attribution* — this tool declares no model list, so it never receives one), then the output-format flag set to the line-delimited-JSON stream, then either the prompt flag carrying the whole prompt, or the agent-file flag followed by the prompt flag carrying the directive.
...
- The reference channel changes nothing about parsing: it produces no tool calls, just the same assistant line.
- Standard input is **empty**; the prompt does not travel there.
- No droppable flags are declared for this tool.
...
- Environment scrub: `MOONSHOT_API_KEY` **and** `MOONSHOT_BASE_URL` are both removed, which forces the run onto the tool's own subscription login rather than a leaked key or a redirected gateway. The re-entrancy environment marker is set as usual.
...
- **Failure is classified on the event's own stated reason, never on the assistant text** (which is free-form and routinely contains the word "error"), and the two failure event shapes carry that reason in different places. They are split by how *definitive* they are:
  - A **turn-failed** event is the tool declaring the turn over, so it throws **on sight**.
  - An **error-typed** event is only a *candidate* reason — this tool emits them for conditions it can recover from — so the first one seen is held and thrown only at end of stream, and only when no assistant text was ever captured. If text arrived, the tool recovered and the held reason is dropped. Collapsing the two into one throw-on-sight branch would turn a recovered run into a hard failure while protecting nothing extra: a genuinely failing stream carries a turn-failed event anyway.
- Either reason is clipped to **300 characters** and classified: sign-in vocabulary → **auth** error; anything else → **transient** error. It is deliberately **never** a setup error, because that class alone drives optional-flag degradation (spec 331) and no run-time failure reported this way is about the command line — degrading would strip an isolation flag and hit the same wall.
- **No parseable event at all** → **setup** error (with a truncated preview).
- Cost is always zero and no stop reason is reported.

**Some failure had to throw here, and the shape that got past everything is why.** A workspace that had exhausted its credit produced a well-formed event stream (so the no-parseable-event guard never fired) whose reason named no sign-in word (so the only failure check of the day, an auth-vocabulary match, did not either). The parser returned empty text, the summarizer read no topics from it, and a regenerate overwrote a good stored summary with an empty one while the commit printed the plain success line.
...
Once selected, the local-agent path mirrors the direct path's preamble: it resolves the template for the requested action (raising on an unknown action), warns on any unfilled placeholder, fills the prompt, and resolves the model id. It does **not** thread an output-token cap (unsupported by every tool). It then reads the configured tool (defaulting to Claude Code), looks up its backend, discovers the executable (honoring the optional configured path), and enters the **degrade-and-retry loop** (spec 331), which builds the invocation and runs it — possibly more than once, dropping unrecognized optional flags between attempts. It then parses the outcome, rejects an empty completion, logs a completion line including the parsed cost, the tool, the token counts and any dropped flag identifiers, and returns the shared result with the provider source label and the tool identifier attached.

**An empty completion is a failed run, not an answer, and is rejected here rather than in any backend.** Every action's template asks for structured output, so no caller has a use for empty text — and the callers that parse it produce a convincing empty artifact instead of an error. The check is a **transient**-class error naming the tool and the action, and it sits at this shared seam so every tool is covered at once and the next silent-failure envelope a CLI invents cannot slip through a backend that has not learned to recognize it. The backends stay responsible for naming a failure they *can* see, since only they hold the reason text; this only catches what got past them.

**Model attribution.** **The model is pinned per tool, and only for a tool that declares a model list** — Claude Code and Codex today. Such a tool is handed one of its own identifiers (a mid-tier default **belonging to that tool**, overridable per machine, with one option meaning "send no model flag"); every other tool receives an **empty** model, omits its model flag entirely, and runs whatever the user configured *inside* it. The stored preference is a single field shared by every tool, so it is resolved against the list of the tool in force and falls back to **that tool's** default — never to another tool's, which would be an identifier the agent rejects outright rather than a milder model. A tool that pins nothing has no picker, so the stored value passes through it untouched; resolving it there would let merely opening a settings surface under such a tool erase a preference no visible control was showing.

Leaving the model unpinned was the earlier behaviour and was reverted: it did not mean "the user chose", it meant a background, mechanical workload silently rode whatever model the developer had picked for interactive work — measured on one machine as a top-tier long-context model, so a few-hundred-token routing decision cost as much as a page regeneration and the same work cost different amounts on different machines. The identifiers sent are the tool's own rather than resolved API model identifiers, and the two pinned tools differ here in a way that must not be averaged over. One offers **aliases**: an alias tracks the latest of its family, so it does not break when a dated model is retired, and it cannot select a long-context variant. The other offers **dated identifiers** with no alias equivalent, so its list is maintained by release and will eventually name a retired model — which is why a refused model has to degrade rather than fail the machine.

**A model the tool refuses gets ONE un-pinned retry.** A refused model exits nonzero having written its failure envelope to standard output, which the runner deliberately resolves so the backend can classify it — so the failure surfaces during result parsing, downstream of the optional-flag degradation loop, which never sees it. The retry is not recorded (an entitlement can be granted later) and applies only to the **refused-model** failure class with a model actually pinned; a backend that reports a refusal as any other class silently forfeits the retry and fails every generation on the machine while re-running the same rejected identifier. Recognising one is structural where the tool allows it: a tool that reports its own conditions as prose and the API's refusals as a serialised error envelope is classified by parsing that envelope and requiring it to quote the identifier that was pinned, rather than by matching failure wording, which is not a contract. Authentication and server-side statuses are read off the same envelope and take precedence, so a server fault that happens to name a model cannot be mistaken for a refusal and hidden behind a retry. Without any of this, a subscription lacking the top tier would fail every generation on the machine.

Three consequences are given up knowingly: the product's own model setting no longer reaches this provider; a caller that deliberately asks for a cheap model for a high-frequency evaluation has that intent discarded; and the replacement can be a much larger model than the product's own default — measured on one machine as a top-tier long-context model where the default alias would have picked a mid-tier one. Cost and latency both move, and the spend lands on the tool's subscription, which is what makes the trade acceptable: under a subscription the user, not the product, is paying, so their configured default is the right authority — and it remains the only authority for the tools that pin nothing, and for a pinned tool whenever the “send no model flag” option is chosen. No tool but Claude Code reports back which model it ran, so a pin on any other tool is recorded as requested rather than observed.

Stored metadata is nevertheless not allowed to guess: the completion result reports **the model the tool itself said it ran** when the outcome carries one, and falls back to the resolved alias only when it does not. Claude Code is the only tool that reports one today, so for every other tool the persisted metadata still records an alias that did not produce the text — the same guess every other provider records, and the only value available.
...
**Every** temporary working directory the call created is removed in a cleanup step that runs on success, failure and timeout alike — not just the last one, because a degrading retry builds a fresh invocation per attempt and each mints its own directory. A directory is removed **only** if it is under the system temp directory and its name carries the backend's reserved prefix (guarding against deleting an unrelated directory when a test injects an arbitrary working directory).
...
If planting the marker itself fails, the half-built directory is deleted on the spot and the error is rethrown unchanged — that is the one path the call's own cleanup cannot cover, because it learns of a directory only from a *successful* invocation build, and a failure to write into a just-created temporary directory is a broken temp location rather than something a retry without the marker could survive.
...
The executable resolution is additionally exposed as a **non-throwing, per-tool boolean**: "does *this named tool* resolve to a binary that survives its capability probe, right now?" — which for every tool but one means only that it answers a version query, and for none of them means that its isolation flags will be accepted. It takes the tool identifier to test plus the tool-scoped explicit path, and is a plain success/failure wrapper — it performs the identical candidate enumeration and capability probe and simply reports `false` instead of raising the setup error. **Every** failure is absorbed, including a tool identifier this build does not recognize (which resolves to no backend and would otherwise raise a setup error).
...
The backend carries two pieces of state across calls, and neither is a provider preference: the module-level executable-resolution cache (in-process, TTL-bounded, single most-recent slot keyed by tool plus override path, success-only), and the machine-global **on-disk store of flag identifiers already proven unsupported for a tool at a version**, which spec 331 owns and which seeds the first attempt's omission set. Everything else is per call.

A call proceeds: select → look up the tool's backend → resolve executable (cache hit or probe) → load the omission set for this tool and version → build invocation (create temp working directory with its marker, scrub env per tool, set the env marker, emit the surviving optional flags) → run child (sanitize arguments, pin the working-directory variable) → on a setup-class failure with optional flags left, drop and repeat from *build* → parse → reject an empty completion → map result → remove every temporary working directory the call created. A failure at any stage throws the appropriate taxonomy error and still runs cleanup. No provider preference is learned or persisted.
...
- **The capability probe is asymmetric by tool.** Only Claude Code is probed with a real run flag alongside the version query; every other tool gets a bare version query, so for them an installed-but-too-old CLI passes discovery and fails at the first real generation instead.
- **Isolation is unverifiable ahead of a run, by construction.** One CLI pre-scans its arguments for the version query *before* validating options, so an unrecognized flag placed alongside a version query exits **zero** — measured. No probe can distinguish a CLI that accepts an isolation flag from one that does not, which is why every one of them is instead declared droppable and handled after a real failure (spec 331).
- **The flag that reads like the perfect isolation switch for one tool also disables the login the backend depends on.** It skips hooks, plugin sync, auto-memory and instruction-file discovery in one flag — and also stops the CLI reading its keychain credentials. Since that backend deliberately scrubs every environment credential path, the keychain login is the only credential left, so the flag fails every call outright with a not-logged-in envelope and zero tokens. Verified.
- **For one tool, two config-override forms are accepted and are total no-ops.** Handing the generic config-override flag an empty inline table for the plugin or the MCP-server key changes nothing, because that override is a *merging* dotted-path set and merging an empty table merges nothing — measured, with byte-identical session state either way. A scalar dotted path through the same flag does work; that is the entire difference. A third form does drop the user's MCP servers but takes the rest of the user's configuration file with it, **including the model** — and since this provider sends no model, that file is where the model comes from, so it silently changes which model writes every summary.
- **Two tools therefore keep the user's MCP servers booted on purpose.** Neither exposes a lever that drops them without also dropping the user's model choice, and losing the model choice is the worse outcome under a provider that sends no model of its own.
- **One tool's isolation flag has never had its effect observed.** The verification machine had nothing for it to disable, so there was no log delta to see. It is the tool's documented switch for exactly this intent and costs nothing when there is nothing to disable, but it is intent rather than measurement — and it is still declared droppable, because an older build that does not recognize it would fail every call.
- **One tool prints its entire help on an unrecognized flag and names nothing**, and that help is longer than the retained standard-error tail, so even its most distinctive section header is truncated away before anything can match on it. This is why the degradation layer can never depend on attribution.
...
- **Three of the five tools are still told nothing about the model.** Only a tool that declares a model list is pinned; the rest are handed an empty model and run whatever the user configured inside them, because their alias namespaces are not ones the product knows and none of them reports the model it ran, so a pin there would be an unverifiable claim. The product's own global model setting still does not apply to this provider at all — it names API model identifiers, a different namespace — and neither does a caller's deliberate request for a cheap model on a high-frequency evaluation.
- **Stored model metadata is a guess for every tool but one.** The result reports the model the tool itself said it ran when the outcome carries one — only Claude Code does, read out of a per-model usage map, preferring the entry that names the requested identifier — and otherwise falls back to the pinned value, or to the resolved alias when nothing was pinned. So for every other tool, persisted metadata names a model that was never sent and may not have produced the text.
- **OpenCode deliberately keeps provider credentials in the child's environment** — the one backend that scrubs nothing — because it authenticates against whichever provider the user configured inside it and scrubbing would break environment-key logins. A run under it can therefore spend the user's own provider credit rather than a flat subscription. It is nevertheless the one backend that *adds* an isolation variable, switching off a compatibility mode that would otherwise read another vendor's machine-global instruction file and skills directory into the prompt — reached deliberately as an environment variable rather than a flag, because an unrecognized variable is ignored by every version and so cannot fail a run.
...
- **Prompt delivery is argv for every backend except Claude Code, which alone uses standard input.** Codex, Cursor and OpenCode pass the prompt as a trailing positional argument, and Kimi Code as the value of a trailing flag *when it fits*. That split is the whole reason the null-byte strip lives at the shared spawn boundary rather than inside any one backend: it is the single layer that covers every argv-delivering tool at once, and the one place that can leave the standard-input stream deliberately untouched. The split also means an argv-delivering tool inherits whatever limits the platform places on a command line, where the standard-input tool inherits none — the prompt here is a whole diff or transcript, not a short string, so this is a real difference in kind between the two channels and not a formality.
- **Only one argv-delivering tool has been taken out of that trap, and only that one.** Kimi Code measures its assembled prompt and switches to a file-reference channel above a character budget. Codex, Cursor and OpenCode still carry the latent limit: no code in the product measures, checks, or reacts to an argument-length limit for them, so the failure, if it occurs, is the platform's own spawn rejection surfacing as a setup error. (Surprising: the same worst-case prompt is safe on one argv tool and unbounded on three.)
- **An empty completion is treated as a failed run and throws**, at the shared seam after parsing rather than inside any backend, as a transient-class error naming the tool and the action. This exists because a silent empty answer is a data-loss path, not a cosmetic one: a workspace that had exhausted its credit produced a well-formed stream that no guard rejected, the parser returned empty text, and a regenerate persisted an empty summary over a good stored one while the commit line reported success.
- **Two independent defences cover that path, and the second one is the general case.** A backend that can see its CLI's own failure envelope throws on it with the tool's own words (the second-source detail the user would otherwise never get); the shared empty-completion rejection then catches whatever got past every backend, so the next silent-failure envelope some CLI invents cannot reach storage without a backend change.
- **The probe version is not simply the first token of the version output.** It is the first token that looks version-shaped, with the first token as a fallback — because one CLI prints its own product name first, which collapsed every build of it to one literal string. That broke two things silently and in different places: version comparison saw every candidate of that tool as equal, so discovery order picked the winner instead of the newest; and the version-keyed capability store (spec 331) could never expire an entry on upgrade, stranding the machine on a degraded invocation permanently.
- **The file channel that rescues the large prompt is an agent-definition channel, not a prompt-file channel**, and two of its constraints are easy to violate without noticing. The file is **rejected outright without a YAML frontmatter block**, so an inert one is prefixed to the body; and its name is deliberately not one of the instruction filenames the tool auto-discovers from its working directory — which is the very thing the fresh empty working directory exists to prevent. The command line keeps only a short fixed directive pointing at that definition.
