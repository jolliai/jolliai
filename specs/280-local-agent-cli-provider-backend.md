...
There is no per-tool model setting, and the single global model setting no longer reaches this provider at all: every backend is handed an empty model and omits its model flag (see *Model attribution*).
...
- **Model**: the tool's model flag is emitted only when the model identifier is non-empty (see *Model attribution*). In practice no tool ever receives one.
...
- Flags, in order: the model flag only when the model identifier is non-empty (see *Model attribution* — this tool never receives one), then the output-format flag set to the line-delimited-JSON stream, then either the prompt flag carrying the whole prompt, or the agent-file flag followed by the prompt flag carrying the directive.
...
**Model attribution.** **No tool is handed the resolved model identifier.** Every backend receives an **empty** model and therefore omits its model flag entirely, so each runs whatever model the user configured *inside* that tool. The provider defers to the tool uniformly, matching the settings surface, which offers an agent-tool picker and no model picker.
...
- **No tool is told which model to use.** Every backend is handed an empty model and omits its model flag, so each runs whatever the user configured inside the tool, and there is no per-tool model setting to change that. The model-flag branch every backend carries is therefore **never taken in production** — it survives only as the shape a future per-tool model setting would use. The product's own model setting silently does not apply here, and neither does a caller's deliberate request for a cheap model on a high-frequency evaluation; the model that actually runs was measured on one machine as a top-tier long-context model where the product's default alias would have chosen a mid-tier one.
- **Stored model metadata is a guess for every tool but one.** The result reports the model the tool itself said it ran when the outcome carries one — only Claude Code does, read out of a per-model usage map with the highest-output entry winning and the context-window variant retained in the key — and otherwise falls back to the alias that was resolved and never sent. So for every other tool, persisted metadata names a model that did not produce the text.
