...
- **Model**: the tool's model flag is emitted only when the model identifier is non-empty (see *Model attribution*). In practice only Claude Code receives one.
...
- **And one that works but changes the model**: the ignore-user-config flag genuinely drops the user's MCP servers, but takes everything else in the user's configuration file with it, including the model — measured as a different model on the same machine. Since this provider sends an empty model on purpose, that file *is* where the model comes from, so ignoring it silently changes which model writes every summary. So the user's MCP servers stay booted here: this tool exposes no lever that drops them without also dropping the model choice.
