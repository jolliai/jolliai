...
The directory used as the project root is `cwd` from the payload when present, otherwise the runtime's current working directory.
...
1. **Local-agent-child guard.** If the handler detects it is running inside a session Jolli's own local-agent backend spawned, it returns immediately and emits nothing — composing a briefing there would re-enter Jolli against a throwaway temporary working directory (and, under the local-agent backend, recurse into another spawn).
2. Read all bytes from standard input as text and parse them as JSON. Extract `cwd` if present.
3. Resolve the project directory, configure the diagnostic log directory under it, and log that the handler was invoked.
...
- What counts as an LLM credential (including why the `local-agent` provider counts) and the `aiProvider` selection model are owned by spec 10; the local-agent backend that provider drives is owned by spec 280.
