...
- The source-definition model (which payload shapes count as a reference, URL validation, archive-form round-trip) and its evaluation engine. The DSL and engine are **spec 255**; the built-in catalog — Linear, Confluence, Jira, GitHub, Notion, Slack, Zoom Meeting, Zoom Doc, Asana, monday.com, Context7, Jolli Memory (self-referential rather than external), Vercel, Figma and Sentry — is **spec 154**, which owns that inventory and is the place to re-derive it from. Identity resolution — which source a Codex tool call belongs to — is a registry `match` over each definition's Codex match rule; per-source payload reshaping / malformed-output recovery / synthetic tool name remain in per-source Codex bindings looked up by the resolved definition's id. Those mechanics are **spec 153**.
...
- **The source-definition DSL and evaluation engine** the pipeline evaluates are owned by **spec 255**; the built-in catalog is **spec 154**. On the Codex path the reachable sources are Linear, Jira, GitHub, Notion, Zoom Meeting, Confluence, Asana, monday.com, Slack, Context7 and Jolli Memory — each declares a Codex match rule AND has a registered Codex normaliser. The rest of the catalog is **not** reachable here: the hosted-document, deployment, design-file and error-tracking sources (Zoom Doc, Vercel, Figma, Sentry) declare no Codex match rule at all, so such a tool call is never even matched on this path. (Slack was Codex-unreachable in a prior revision, for want of a registered Codex normaliser; that gap is now closed — spec 256.) References from those four are produced only on the Claude path (specs 153, 154), so this pass never yields one regardless of what a Codex transcript contains. Re-derive the split from the definitions rather than trusting this enumeration as a tally.
- **The track-only carve-out downstream of this pass** — a track-only reference (Context7 and Jolli Memory are the two reachable on this path) is persisted, archived, and displayed exactly like any other reference this pass produces, but never reaches the summarization prompt block and never enters the relevance ranker's input. That is owned by **specs 12 and 258**; this pass draws no distinction and applies no filter of its own.
