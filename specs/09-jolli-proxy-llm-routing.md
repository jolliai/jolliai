...
- **Same call interface**: the caller invokes the same dispatch entry as the direct path (`action`, `params`, optional `version`); the dispatcher chooses the path based on credentials. There is exactly one dispatcher, shared by every surface — no surface carries an in-process dispatcher of its own. A caller cannot force the proxy path at the call site; the only ways to reach it are withholding direct credentials, or setting the explicit provider override in shared configuration, which the one dispatcher honors identically wherever it runs (see spec 10).
