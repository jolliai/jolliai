...
- The contents and runtime behavior of the MCP server itself once it has been spawned (its tool surface, lifecycle) — see spec 148 (MCP server tool surface). The process the descriptor actually spawns is a per-session proxy onto a shared per-worktree server, with an in-process fallback; that shape, and both of the spawn-time working-directory refusals noted below, are owned by spec 364 (MCP per-worktree daemon and proxy). The registered descriptor is unchanged by it — still a plain standard-input/standard-output spawn of the same command with the same argument.
...
| Kimi Code CLI | global | `$KIMI_CODE_HOME/mcp.json` when that environment variable is set, else `~/.kimi-code/mcp.json` | key `mcpServers`; entry `{ command, args }` | that same home directory (`$KIMI_CODE_HOME`, else `~/.kimi-code`) exists and is a directory |
...
Kimi Code CLI keeps its MCP servers in `mcp.json`, **not** in the `config.toml` that sits beside it (that file carries only MCP timeout settings), so this host is written by the ordinary JSON writer under the default server-map key and never by the TOML writer the Codex row uses.
...
#### One host overrides the Windows descriptor with a dedicated launcher

Codex — and only Codex — takes an optional upgrade over the Windows branch above. When the platform is Windows, the winning distribution is looked up again and, **if that distribution ships a dedicated MCP-launcher script**, the descriptor becomes `node` with a single argument: the absolute path of that launcher. When the winning distribution ships no launcher (or none can be resolved at all), the ordinary Windows descriptor above is used unchanged.

Two properties of this override matter:

- **The launcher descriptor carries no MCP-server subcommand token.** It is the only registered descriptor in the whole set without one. The launcher appends the subcommand itself when it spawns the resolved main entry script, so supplying one here would pass it twice. Every other entry — including this host's own fallback — carries it.
- **The whole branch is unreachable on any non-Windows platform.** The override is taken only when the platform is Windows; on macOS and Linux the descriptor builder returns before ever looking for a launcher, and no shipped surface on those platforms spawns it. Its purpose is to move the *version* choice from registration time back to launch time: the ordinary Windows descriptor bakes in an absolute path to one distribution's main entry script and freezes that runtime version until the next registration, whereas the launcher re-runs the winning-distribution selection on every server start. POSIX gets that property for free from the dispatch script and needs no launcher.

The launcher file is deliberately **not** part of the runtime-completeness file set a distribution must satisfy to be registered at all, so a winning distribution without one is the normal case rather than an error — which is why the fallback exists. Only one shipped bundle ships a launcher today.
...
Kimi Code CLI is not one of the embedded-database hosts: its conversations are plain files, so its MCP predicate — the home directory exists — is the *same* predicate its discovery path uses, and the two cannot diverge for it.
...
### Registration into the TOML registry (the one non-JSON host)

The Codex host follows the same read-preserve discipline against a hand-written TOML document instead of JSON, but its write path differs from every JSON host in three ways:

1. **Block-level strip-then-append, not a document rewrite.** The product's table is treated as a contiguous run of text from its header line to the next table header (or end of file). The header is matched only at the start of a line, so a header string appearing inside a comment or a value is never mistaken for the table. Any existing block is removed and the freshly rendered one is appended after the remaining content. Everything else is byte-stable — only the seam left by removing the block is normalized to a single blank line; blank-line runs elsewhere are preserved exactly.
2. **No write at all when the rendered document is identical to what is on disk.** This is a correctness guard, not an optimization: one AI-host plugin bootstrap reaches this registrar on **every** session start, and the writer rewrites the whole file — the user's model settings, sandbox policy and other servers' environment blocks included. Re-serializing all of that to produce identical bytes makes two sessions starting together last-writer-wins over the entire file rather than over the product's own table. In the steady state the entry already matches, so the writer returns before touching anything and logs that no write was needed.
3. **Atomic write that preserves the file's existing mode.** Both the upsert and the removal write through a temp-file-then-rename, because most of this file is other tools' configuration and a torn write would truncate far more than the product's own table. A file this code *creates* gets owner-only permissions (it lists commands the machine will spawn and other tools' tokens). A file that already exists keeps its own mode: the current mode is read first and passed back through the rename, because the rename carries the temp file's mode rather than preserving the destination's. Silently re-permissioning another tool's config is refused in either direction.

Removal from this host short-circuits before writing when the table header is not present at line start, so a file with nothing to remove is never rewritten.
...
Gemini, Codex, OpenCode, GitHub Copilot CLI, VS Code Copilot Chat, Cline (every flavor's settings file), Devin CLI, Antigravity, and Kimi Code CLI.
...
Because the descriptor carries no directory, **the spawn-time working directory a host happens to choose is the whole of what identifies the repository** — and a host is free to choose one that is not a repository at all. Two spawn-time working directories are therefore refused a shared, cross-session server; both refusals happen at runtime, and neither is anything a registrar can predict or prevent:

- **Inside a host's plugin-bundle cache** — the case the sections above already discuss, and the reason the Codex plugin ships no MCP manifest. Refused outright: nothing is served.
- **A directory git cannot name as a worktree root** — the direct sequel. Such a session gets **no shared server**: it is served by a private server of its own, and one explanatory line is written to **standard error** naming the directory, saying there is no repository to answer for here, and suggesting either pointing the entry's working directory at a workspace or launching from inside a repository. Standard error, never standard output, because standard output is that session's own protocol stream and a stray byte on it desynchronises the host's framing for the whole session. This is a real registration shape rather than a hypothetical: an editor's *user-profile* (rather than workspace) MCP entry can be spawned at the filesystem root, in which case every such session on the machine would otherwise have collapsed onto one shared server rooted there, each answering its memory tools for nothing.

Both refusals leave the registry entry exactly as written — registration has no way to know, and no reason to care, which directory a host will later pick. What the two look like to a user differs, though: the plugin-bundle case is a server the host reports as failing to start, while the non-repository case is a server that starts, reports healthy, and answers every memory tool with nothing except for that one stderr line. (Both are owned by the MCP daemon-and-proxy spec.)
...
- **The reduced repo-hooks-only install skips registration for every host except Codex, and only when the acting host is Codex.** That mode exists for the AI-host plugin bootstraps and otherwise writes no MCP config at all; this one entry is exempt because it is the *only* path by which MCP reaches a Codex-plugin user. A plugin-shipped MCP manifest cannot serve them: Codex does not expand its plugin-root variable inside an MCP entry, so the entry must give a relative command with a plugin-relative working directory, and every memory tool derives the repository it serves from its working directory — the resulting server answers for the plugin's own cache directory (successful but empty results, plus a placeholder Memory Bank repo named after the bundle's version directory). Nothing recovers the real workspace from inside such a launch: the host declares no roots capability, and a server-initiated request for roots returns empty; the environment the host passes MCP servers is a short allowlist with nothing session-scoped in it. A `config.toml` entry carries no working directory and is launched with the session's, which is the only correct one. Hence three linked facts: the Codex plugin ships no MCP manifest, the server carries a refusal that stops it starting under a working directory inside either host's plugin cache rather than serve the wrong repository, and this exemption is Codex-only — no other global host is registered by a Codex install, which has no business configuring them. **That refusal is a tripwire, not an observed failure mode**: no shipped plugin carries an MCP manifest, so nothing takes the path it guards; it exists to make reintroducing one fail loudly. Nothing enforces the manifest's absence either — the publish inventory simply does not list one, so adding a manifest file would fail no check. The user-visible cost is that the first session after a plugin install has skills but no MCP tools, since registrations are read at session start; the skills' CLI fallback covers it. (Surprising; intentional.)
- **A registered descriptor can be spawned in a directory that is not a repository, and nothing here can stop that.** The descriptor encodes no directory on purpose, so the host's spawn-time working directory is the only thing identifying the repository; an editor's user-profile (rather than workspace) MCP entry can be spawned at the filesystem root. Such a session is deliberately denied a shared cross-session server and served privately instead, with one line on **standard error** naming the directory and saying there is no repository to answer for — never on standard output, which is that session's protocol stream. Unlike the plugin-bundle refusal, this one still *serves*: the host reports a healthy server that answers every memory tool with nothing. Registration is unaffected either way, and no registrar can predict which directory a host will pick. (Notable; the direct sequel to the plugin-bundle refusal above.)
...
- **One host's Windows descriptor can drop the subcommand entirely, and that whole branch is dead on every other platform.** When the acting platform is Windows *and* the winning distribution ships a dedicated MCP launcher, the Codex descriptor becomes the launcher alone, with **no** subcommand token — the launcher supplies it when it re-resolves and spawns the winning main entry. It is the only registered descriptor without that token, and the only one that re-picks the runtime version at every server start instead of freezing it at registration. On macOS and Linux the builder returns the ordinary descriptor before the launcher is ever looked for, so nothing there spawns it. Only one shipped bundle ships a launcher at all, and it is deliberately excluded from the runtime-completeness file set — promoting it would make every already-registered distribution fail that check and de-register itself — so a winning distribution without one is normal and the fallback is the common case. (Notable; unreachable off Windows.)
...
- **JSON-registry writes are full-file overwrites, not appends or patches.** No backup file and no temp-file-then-rename atomicity is used for any JSON host, including the repo-scoped ones. A crash mid-write can leave a truncated registry file, which the next install treats as malformed and leaves alone — the user must repair it by hand. **The TOML host is the exception in both directions**: it returns without writing when the rendered document is byte-identical to what is on disk, and both its upsert and its removal go through an atomic write that carries the file's existing permission bits across the rename (owner-only for a file this code creates). The asymmetry is not stylistic — that one file is reached by an AI-host plugin bootstrap on every session start and holds mostly other tools' configuration. (Notable.)
...
- **The product's MCP-server runtime** that the registered descriptor invokes is documented separately — see spec 148 (MCP server tool surface) for what it advertises, and spec 364 (MCP per-worktree daemon and proxy) for the per-session proxy, the shared per-worktree server it reaches, and the two spawn-time working directories that are refused a shared server.
