...
| `configured`     | Optional boolean. Whether the user has a usable summarization credential — a **three-armed** derivation: `signedIn || hasApiKey || usesLocalAgent`, where the third arm is true when the recorded summarization provider is the local-agent one (which holds no jollimemory credential at all; the agent tool's own login is the credential). The third arm keys on the recorded *choice*, never on live detection of the tool. Undefined is treated by the webview as "not yet known" and defaults to true — the onboarding panel does not show until the host explicitly sends `false`. The first `init` message after the initial-state-readiness barrier resolves is the canonical source of the initial value. |
| `localAgents`    | Optional, ordered list of locally-installed agent tools, each entry carrying the identifier that would be persisted and the display label shown to the user. Drives the onboarding panel's local-agent card: **absent or empty means no card** and a panel identical to the pre-feature one. **Deliberately empty for a user who is already configured** — the host skips the presence sweep for them, since they never see the card. Refreshed mid-window by `localAgents:changed`. |
...
| `indexStatus`      | Changes-tab file rows only: the raw porcelain v1 index column. **Dead payload.** It was carried because the discard handler used to pick a git command from the two raw columns; the discard now takes the path alone and nothing reads this field. Kept for row-shape stability. |
| `worktreeStatus`   | Changes-tab file rows only: the raw porcelain v1 worktree column. **Dead payload**, same reason as `indexStatus`.                          |
| `originalPath`     | Changes-tab file rows only: source path for rename / copy rows (porcelain `R `/`C `). **Dead payload.** The discard rule set recovers a rename's original path from its own status read and reports it back in the outcome — the row never has to carry it. |
...
| `skillsHover`      | Context list only, on the **single aggregate skills row**: a group-shaped hover payload (member `count`, pre-formatted `totalTokensLabel` and its three-way `totalBreakdownLabel`, `anyInferred`, `relativeDate`, a heaviest-first capped `rows` array, and an `overflow` count naming how many members `rows` left out). Group-shaped rather than skill-shaped: the row stands for every skill captured this session, so there is no per-skill hover. The token strings arrive pre-joined because the host owns the compact formatting and the `~`-estimate marker; splitting that across the boundary is how the card and the document it previews would drift. Cross-ref **324**. |
...
One row `id` on the Context list is a **sentinel rather than an artifact key**: the aggregate skills row carries `SKILLS_GROUP_ID` (`"__skills__"`). It addresses no one artifact — the row stands for every skill captured this session — so any consumer that treats a Context row's `id` as an artifact key must special-case it and go back to the store for the real keys; writing the sentinel into an exclusion set matches no skill and silently excludes nothing. The constant is declared beside the wire types (not on the tree provider) precisely so consumers that must not import the IDE API at runtime can still reach it. Cross-ref **324**.
...
| `branch:openSkillsAggregate`        | **none**                                                                                                               | Row-click on the aggregate skills row: opens the view of every skill captured this session. Carries **no id** — the Context list holds one row for all of them, so there is nothing to address; the host reads the current set from the store. Routed through the dedicated open-skills-aggregate command. |
...
| `branch:discardFile`                | `{ filePath, relativePath, statusCode?, indexStatus?, worktreeStatus?, originalPath? }`                                | Inline discard button on a Changes row (also its context-menu item, and the review panel's file row). **`relativePath` is the only load-bearing field**: the host forwards it to the shared discard rule set, which resolves the path against its own status read. `statusCode` / `indexStatus` / `worktreeStatus` / `originalPath` are all **optional and read by nothing** — the discard no longer routes on any status, so nothing downstream may read them back into a decision. The host's handler applies **no** validation to this message at all: `relativePath` must be non-empty, and when a producer's fallback sends the empty string the rule set answers `invalid-path` as a **failure** (never "already clean"), which surfaces as an error notification. |
...
| `branch:toggleSkillSelection`       | `{ selected }` — **no id**                                                                                             | Checkbox toggle on the aggregate skills row: **all skills at once**. Unlike the other three toggles it carries no id, because the Context list holds one aggregate row rather than one row per skill — the only selection this surface can express is "keep them all" or "drop them all". The host reads the current skill keys from the store and writes the whole set. Per-skill exclusion is still representable in the commit-exclusion store (spec 188) and stays reachable from the Commit Memory panel. |
...
| `refresh`                           | `{ scope }` where `scope` ∈ `kb` / `branch` / `branch-current` / `branch-commits` / `status` / `all`                    | Toolbar refresh button; expands per-scope into the corresponding store-level refresh commands. The `kb` scope is **not** a pure reload — it additionally mutates the Memory Bank on disk, can raise a modal prompt, and re-pushes the breadcrumb pickers (see "Refresh scopes"). Two Branch-tab sub-scopes exist: `branch-current` reloads only the Current Memory block (conversations + context + files + pins); `branch-commits` reloads only the committed-history section (git history + the foreign-readonly memory-cache invalidation). `branch` remains the whole-tab refresh and is what `all` expands into for the Branch tab. |
...
| `command` (adopt-local-agent-tool)  | `{ command: <adopt-tool-command-name>, args: [toolIdentifier] }`                                                          | Webview-initiated adoption of a locally-installed agent tool picked in the onboarding card's dropdown. The identifier is **re-validated host-side against the supported-tool registry** before use — this is untrusted webview input. Answered **only on failure**, via `localAgent:selectError`; success writes the provider fields and rides the ordinary `configured:changed` flip instead of acknowledging. Sent via the standard `command` outbound envelope. |
...
| `kb:openEvidenceReference`          | `{ archivedKey, source, title, sourceRepoName, sourceRemoteUrl }`                                                       | Open a committed memory's REFERENCE evidence row. Routes to the archived-snapshot reference preview off the orphan branch (by `archivedKey` + `source`); provenance routes foreign reads. `title` is **required**: the preview names its tab after it, and without it the tab would be named after the storage key while the memory-detail panel's route to the *same* snapshot names it after the human title — one snapshot appearing as two differently-named tabs. The webview falls back through the row's title, then the row's id, then the empty string, so the field is always present on the wire. `source` is re-validated host-side against the closed source-id set before the open is dispatched, and an unrecognized value is logged and dropped. |
| `kb:openEvidenceSkills`             | `{ commitHash, sourceRepoName, sourceRemoteUrl }`                                                                        | Open a committed memory's SKILLS evidence row — the whole per-commit skills table for that commit. Keyed by **commit hash** rather than an artifact id, because the row is an aggregate with no per-skill target. Distinct from the live `branch:openSkillsAggregate`, which renders the working registry: once these skills are archived onto the commit the registry no longer holds them, so the live message would open an empty (or unrelated) table. Provenance travels with it for shape symmetry with the other evidence-open messages, though the committed-skills preview command resolves the owning repo from the hash itself. |
...
| `configured:changed`          | `{ configured }`                                                                                 | Whenever the configured-state derivation (`signedIn || hasApiKey || usesLocalAgent`) flips. Drives the visibility transition between the onboarding panel and the tab UI / disabled panel. |
| `localAgents:changed`         | `{ localAgents }` — the same ordered identifier+label list as the initial-state field            | On any `configured` true→false flip, which is when the onboarding panel is about to reappear. Pushed **before** the `configured:changed` that re-raises the panel, so the local-agent card is correct on its first paint rather than one message late — the host skips detection while the user is configured, so the list on the original `init` is stale (empty) for anyone who signs out mid-window. |
| `localAgent:selectError`      | `{ message }`                                                                                    | When a webview-initiated local-agent tool adoption fails (unrecognized identifier, the tool did not respond as expected, or any other underlying reason). The webview renders the message inline on the local-agent card under an alert role and restores the card's controls. **Dropped when the onboarding cards are not on screen.** |
...
A `FolderNode` carries `name`, `relPath` (relative to the KB root, "/"-joined; empty string for root), `isDirectory`, optional `children` (undefined = lazy/not loaded, empty array = loaded and empty), per-file metadata (`fileKind` ∈ `memory`/`plan`/`note`/`wiki`/`skill`/`other` — `skill` is the per-commit skill-usage aggregate file, manifest-tracked like any other generated file, so a new manifest type must appear in this union or the folders tree stops type-checking, `fileKey` for the manifest's stable identifier, `fileTitle` for the human-readable display, `fileBranch` for the source branch on memory rows, `isDiverged` when the on-disk `.md` sha256 differs from the manifest fingerprint), and directory-only metadata (`isRepoRoot` when the node is a top-level repo folder under the Memory Bank parent, `isCurrentRepo` when that repo matches the open workspace).
...
A `MemoryEvidence` (`kb:memoryEvidence` payload) container carries three arrays of evidence items — `conversations`, `context`, `files` — plus optional provenance `sourceRepoName` / `sourceRemoteUrl` (null = the current workspace; non-null routes previews to the owning repo's storage and gates file-diff opening, since a foreign commit can't be diffed against the workspace git). Each `MemoryEvidenceItem` carries `kind` (`conversation` / `plan` / `note` / `reference` / `file` / `skill`), `id`, `title`, and kind-specific optionals: `source` (conversation transcript provider, or reference source id needed to read the archived snapshot), `transcriptPath`, `relativePath`, `statusCode`, `oldPath` (for a renamed file), and `messageCount` (archived turn count on a conversation item, shown as the trailing "N msgs"). The **files** group prefers git truth for local memories — the host reads the commit's real per-file status (add/modify/delete/rename + rename source path) so the diff opens correctly — and falls back to the summary's path-only topic file list (status defaulting to "modified") for foreign memories or when the git read fails.

The `skill` evidence kind breaks the container's one-row-per-artifact rule twice over. It is an **aggregate** — one row standing for every skill in the memory, the same collapse the live Context list makes and for the same reason (a session routinely enters a dozen skills and the panel is fixed-height) — and consequently its **`id` is the memory's commit hash, not an artifact key**, because what the row opens is the commit's skills-aggregate file. Every other evidence kind keys on its own artifact (session id, plan id, note id, archived reference key, relative path), so code that treats `MemoryEvidenceItem.id` as an artifact identifier is wrong for exactly this one kind.
...
**The `kb` scope is the one refresh that mutates disk and the one that can interrupt the user with a prompt.** It is not a reload; it is a tidy-then-relist, and it runs for `all` too. It holds exactly one store-level command. What the dispatcher does, in order:

1. **Starts the Memory Bank tidy-up chain** — first, and **not awaited**.
2. **Dispatches the memories-store refresh command** — also not awaited.

Nothing in this scope is awaited, deliberately: awaiting the chain would push every other scope of an `all` refresh past a microtask, and the chain contains a modal, so the rest of the refresh would sit behind a dialog. Inside the chain the steps *are* sequential — the folder tidy-up, then the folder-tree root re-listing (which is what makes the tidied folders disappear on the same click), then a re-push of both breadcrumb pickers, `selection:repos` followed by `selection:branches` for the current repository.

Three properties of that chain belong to this protocol:

- **It writes to disk.** No other refresh scope does. A caller cannot treat a Refresh as a read.
- **It can raise a modal confirmation** — blocking until answered — before it merges anything. This is the only gesture in this protocol that can block on a dialog.
- **It ends by re-pushing the two pickers**, which is not a display nicety: without it the dropdowns keep offering a repository the same click archived, and picking one silently does nothing until a window reload.

Everything else about the chain is owned elsewhere and deliberately not restated here: which folders the archival sweep is allowed to move and the two-part guard that spares the current repository, its silent deferral when the vault is busy, the fact that the move is recoverable and propagates to synced peers, and the duplicate-folder consolidation's classification, its case-specific wording, and its opposite busy behaviour. Those are the Memory Bank unused-folder archival and Memory Bank duplicate-folder consolidation topics.
...
- **The `command` outbound is the escape hatch.** Inline buttons and right-click menu items dispatch through it so the protocol does not need a typed message per command. Typed action messages are reserved for cases that need pre-shaped payloads the host has to reconstitute (the discard payload's path, the file-open structural shape, the typed checkbox toggles, the conversation-open payload).
- **The discard payload's status fields are dead weight the protocol still carries.** `statusCode`, both raw porcelain columns and a rename's original path all still travel on the row and on the message, and the host's handler still forwards them — but the discard is decided entirely from `relativePath` against a status read the rule set takes itself, so nothing reads them. Reading any of them back into a decision would re-open the class of bug that removed them: a host-side dispatch on a collapsed or stale status cannot be checked against the shared one, because nothing on the wire fails when they disagree. (Notable.)
- **`relativePath` on the discard message is the one field with no host-side guard, and an empty one is a *failure*.** Every producer falls back to the empty string when it cannot resolve its row's path, and the host applies no validation to this message — so the only thing standing between a malformed message and a confirmed, irreversible click is the rule set's own rejection of a blank path, which reports failure rather than "the state you asked for already holds". The host-side shape guard that used to sit here validated the porcelain columns, which are no longer an input. (Notable.)
...
- **Skills are the one Context kind addressed by a sentinel and an aggregate, not by an id.** Three of the protocol's skill-shaped messages carry no artifact identifier at all: `branch:openSkillsAggregate` (nothing), `branch:toggleSkillSelection` (`selected` only), and the row itself (`SKILLS_GROUP_ID`). The host resolves the real keys from the store on receipt. A consumer that generalises "every Context row has an artifact id" over this row writes the sentinel into an exclusion set and silently excludes nothing. (Surprising; intentional. See 324.)
- **`MemoryEvidenceItem.id` means something different for `skill` than for every other kind.** For a `skill` evidence row it is the memory's **commit hash**; for conversation / plan / note / reference / file rows it is that artifact's own identity. The asymmetry follows from the row being an aggregate with no per-skill target — what it opens is the commit's skills-aggregate file — but it is a real trap for code that keys generically off `id`. (Surprising; intentional.)
- **The live and committed skill-open messages are deliberately different messages.** `branch:openSkillsAggregate` renders the *working* registry; `kb:openEvidenceSkills` renders the commit's archived aggregate. Once skills are archived onto a commit the working registry no longer holds them, so reusing the live message on a committed row would open an empty (or unrelated) table. Same split as the live-vs-archived conversation and reference opens. (Notable.)
...
- **One refresh scope mutates disk, and nothing in its name says so.** `kb` moves Memory Bank folders and can raise a **modal** prompt before merging others; every other scope is a pure reload. A user pressing a Refresh button is not expecting either. Which folders move, and under what guard, is owned by the two Memory Bank tidy-up topics. (Notable.)
- **The `kb` scope also re-pushes the breadcrumb pickers**, which is not a display nicety: without it the repo dropdown keeps offering a folder the same click archived, and selecting it silently no-ops until a window reload.
...
- **Memory Bank folder tidy-up on Refresh** — the archival sweep of folders that hold nothing, and the duplicate-folder consolidation that follows it, both started by the `kb` scope. Their candidate sets, their emptiness and identity predicates, the write lock they share and the two opposite ways they answer a busy vault, the archive destination, and their notifications are owned by the Memory Bank unused-folder archival and Memory Bank duplicate-folder consolidation topics. This protocol owns only that the scope starts the chain first and awaits nothing, that the chain can raise a modal, and the picker re-push that closes it.
- **Sidebar HTML/CSS/JS bundle** — the renderer this protocol talks to. Defined under its own surface; out of scope here.
- **Onboarding panel and viewport states** — when `configured:changed`, `enabled:changed`, `localAgents:changed` and `localAgent:selectError` are acted upon by the webview, and the panel shapes the `localAgents` field selects between, are owned by spec 142.
- **Local Agent CLI Provider Backend (spec 280)** — owns the presence and runnability predicates behind the `localAgents` list and behind the host-side answer to the adopt-tool outbound. This protocol carries only the resulting identifier+label list and the failure message.
...
- **Skills context row (spec 324)** — what the `SKILLS_GROUP_ID` sentinel, the `skillsHover` card and the three id-less skill messages mean behaviourally on the sidebar: when the single aggregate row appears, what its checkbox writes, and what its context menu offers. This spec owns only the wire shapes.
- **Context snapshot markdown preview (spec 329)** — owns what happens after `kb:openEvidenceReference` / `kb:openEvidenceSkills` / `branch:openSkillsAggregate` / `branch:openReferencePreview` reach the host: the virtual scheme those previews render through, and the fact that the `title` carried here becomes the preview tab's name (sanitised and length-capped there).
- **Skill-usage aggregate rendering (spec 323)** — the per-commit skills-aggregate document that `branch:openSkillsAggregate` and `kb:openEvidenceSkills` open, and the pre-formatted token/breakdown strings the `skillsHover` card carries verbatim.
