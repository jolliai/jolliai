...
**Out of scope:**
- **What the structural marker MEANS** — why a repository acquires one, what it freezes, what routes differently once it is present, and who is allowed to remove it. This spec records only how its presence participates in the composite and therefore in the disable decision, and stops there.
...
- **The structural marker's own meaning** — why a repository acquires one, what it changes about where memories go, and who may remove it — is owned by its own topic. This spec owns only its participation in the composite, the enable path's refusal to clear it, and the ordering rule that materializes the authored field alongside it.
