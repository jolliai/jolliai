...
- The two per-request reads that sit outside the model queries: the machine-global configuration read behind the two optional sidebar rows, which is not view-gated, and the one asynchronous read that only a selected memory adds.
...
| `/api/settings/set-sync-sessions` | `enabled` (boolean) | **400** `{"error":"enabled (boolean) is required"}`; **500** `{"error":"could not change the session-statistics setting"}`; **200** `{"ok":true,"syncSessions":<boolean>}` — the machine-wide session-statistics switch, applied immediately like `set-push` above rather than through `apply` |
...
3. Also before the database is opened, read machine-global configuration for the **two optional-sidebar booleans** — the knowledge row and the graph row. This is **not** view-gated: it runs on every model-building route and for every view, because the sidebar is shell furniture rendered on all of them, and a flag read only on the settings view would leave the nav guessing everywhere else. With no configuration directory supplied it falls back to the default one. The read is **fail-open** — unreadable configuration reads as an empty configuration — so the failure mode is both rows hidden, never a thrown request.
4. Open the database **read-only**, then:
   ...
   - For the `memories` view **with a memory actually selected**, read that memory's transcript-repair verdict — the one *asynchronous* read inside the handle, which is why it is computed here and threaded into the model rather than living in the query layer: it consults a machine-global ownership ledger and stats each transcript that ledger names. A tree render with no selection has no detail pane to word and skips it entirely.
5. Build the view's model, then optionally attach the compressed decision gist (below).
...
- **The two optional-sidebar flags are read for every view, not only the settings one**, so every model-building request pays one small configuration read even though only the sidebar consumes it — the sidebar is rendered on every page, and a flag read only on the settings view would leave the nav guessing everywhere else. That read is fail-open, so unreadable configuration hides both rows rather than failing the request. (Notable; the failure mode looks like a setting rather than an error.)
- **The memory detail adds the only asynchronous read inside the database handle**, and only when a memory is selected: a transcript-repair verdict that reaches the filesystem — a machine-global ownership ledger, plus a stat per transcript it names. A tree render with no selection pays none of it. (Notable.)
