...
A second, independent read source for this IDE's memory data: a per-project accessor bound once to one repository's on-disk memory mirror that answers exactly four read shapes straight off the local filesystem — a per-commit memory document as a parsed record, the same document as raw bytes, an archived plan body, and an archived note body — and declines on any doubt so the caller falls back to the canonical version-controlled ref. It never writes and never enumerates, and it re-reads the mirror's per-device out-of-sync marker before every single read; the host that owns it, however, memoizes single-memory reads in a bounded map that is consulted ahead of the accessor and can therefore outlive that check.

## Scope

**In scope:**
- The four read shapes this accessor serves, and the read families that have no shape here and therefore always go to the ref.
...
Relative to that root, the accessor derives and holds four locations: the directory of per-commit memory documents, the directory of archived plan bodies, the directory of archived note bodies, and the per-device out-of-sync marker file. Nothing else under the root is referenced.
...
The project service memoizes the parsed-record shape only (never the raw-bytes, plan, or note shapes):
...
All four shapes follow the same sequence:
...
- **Raw bytes, plan body, note body.** Return the file's exact text with **no schema check of any kind**. Any thrown failure declines with **no log line at all**.
...
The guard exists because every one of the four shapes builds its path from a caller-supplied name, and a name carrying parent-directory traversal or an absolute-path form would otherwise compose a path outside the intended directory. Because canonicalization resolves symbolic links, a link *inside* the directory that points outside it is refused as well. Refusing costs nothing, because the ref fallback can serve the same item and is not reachable by path composition.
...
The writer side's own lockstep note names exactly the four consumed paths plus the marker as the coupled set, matching what this accessor implements.
...
- The four read locations (their folder names and file-naming rules).
- The out-of-sync marker's location and its presence-means-degraded semantics.
- The schema of the one document shape that is parsed (the per-commit memory document). The three byte-returning shapes are opaque to this accessor and carry no schema coupling.
...
A sixth difference sits one level up, in *which configuration value* names the mirror's parent folder: this IDE resolves the root from its own Memory Bank folder setting, which is not the value the canonical write path resolves the same folder from — and no surface on this IDE writes the latter. Re-targeting the Memory Bank folder from this IDE's settings therefore moves where the mirror is **read** from without moving where it is **written**; the newly pointed-at folder is normally unpopulated (so the attach declines and reads fall back), but a folder left populated by an earlier session would be served while fresh writes land elsewhere.
...
- **Three of the four shapes fail completely silently.** Only the parsed-record shape logs (at debug) when a read throws. An unreadable plan body, note body, or raw document leaves no trace anywhere.
...
- **The readiness and out-of-sync predicates are publicly exposed but have no production caller outside the accessor.** Readiness is consulted only by the accessor's own construction path, and the marker test only by its own four reads. Both are reachable from tests, and neither is reachable from any surface that could report degradation to the user — which is the mechanism by which the degradation stays invisible.
