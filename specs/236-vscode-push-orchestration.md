...
Before any attachment is read or pushed, the orchestrator fails fast on the per-repo outbound-push opt-out (spec 310): `if (!(await isOutboundPushAllowed(ctx.workspaceRoot))) throw new PushDisabledError()` (`vscode/src/services/JolliPushOrchestrator.ts:203-204`). Checking here — and not only inside each HTTP call — is what stops a doomed per-attachment push from being issued and then mislabelled as an attachment failure. The HTTP client still re-checks per call, both as defense-in-depth for non-orchestrator callers and because spec 310 requires the flag be read **live**, so a push of N attachments performs 1 + N reads and a mid-push opt-out takes effect immediately.

### The fatal set (abort the loop, don't collect)

`isFatalPushError` (`vscode/src/services/JolliPushOrchestrator.ts:42-53`) is `isRepoWideRefusal(err) || err.name === "BindingRequiredError"`. It is **not** the old "binding-required / plugin-outdated" pair: `isRepoWideRefusal` is imported from `cli/src/core/PushRefusal.ts` — the shared three-surface source of truth (spec **327**) — and covers `ClientOutdatedError`, `PluginOutdatedError`, **`PushDisabledError`** and **`PermissionDeniedError`**. `BindingRequiredError` is added on top *here* because the orchestrator cannot run the binding chooser itself and so must propagate to the caller that can.

The one predicate is applied identically at all three attachment loops (plans `:398`, notes `:471`, references `:528`), so the set cannot drift between them. Membership comes from the shared module rather than a local `instanceof` chain precisely so a new repo-wide type is added once and every classifier picks it up — and it is imported from a module no test stubs, so a partial mock cannot turn the predicate into `undefined`.
...
The **whole-branch share loop** (`vscode/src/services/LiveShareController.ts:645-655`) applies its own stop test at the *summary* level: `isRepoWideRefusal(err) || err instanceof ShareBindingError` (`:654`). Every summary in the loop belongs to the same repo, so a repo-wide refusal — outdated plugin, this repo's push opt-out, the server's allowlist/ownership verdict — fails all of them; collecting them would report one condition as "Shared 0 memories, but N failed" *and* keep firing doomed requests for every remaining commit. `ShareBindingError` is added because by then the chooser has already run without producing a binding. Everything else (network / HTTP 5xx) is recorded per-summary so earlier successes are not discarded.
...
The single-summary push is a leaf operation: it either refuses at the entry gate with `PushDisabledError`, returns a result, raises a fatal error from the set above (which the caller routes to the chooser / outdated / opt-out flow), or — for binding-required on the first attempt — resolves a binding and retries once. Multi-commit pushes iterate this leaf oldest→newest, aborting on the first fatal error (and, for the whole-branch path, collecting non-fatal per-summary failures).
...
- **Per-attachment failures are collected, not thrown.** A single bad plan/note doesn't abort the remaining attachments or the summary; only the fatal set aborts. (Notable.)
- **The fatal set is repo-wide, not just "binding + plugin".** It is `isRepoWideRefusal` (shared with the CLI and IntelliJ — spec 327) plus `BindingRequiredError`, so an outdated client, **this repo's outbound opt-out**, and a server permission/allowlist refusal all abort the loop too. Each is a property of the repo + credential: continuing would fire N doomed requests and report one condition as N per-item failures, and would rob the surfaces of the "re-enable to push" / admin-oriented handling they have for exactly these. (Notable; the set is wider than it reads.)
...
- **The repo-wide refusal set** underlying `isFatalPushError` and the whole-branch loop's stop test — **Repo-Wide Push-Refusal Classification** (327).
