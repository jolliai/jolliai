...
5. **Register the MCP server in detected global-scope hosts** (machine-wide config files shared by every repo — Codex, Gemini, OpenCode, Copilot, Copilot Chat). Written once here rather than per-worktree; detection-gated. (Repo-scoped hosts were already handled inside loop A.)
...
8. **Auto-detect AI agents and update integration toggles** for every integration whose toggle is currently "not yet decided":
...
| Step |
|---|
| Every host detector — the mode forces them all to "not detected", so nothing detection-gated runs |
...
- The dist-paths registry entry is **not** unconditionally overwritten. Identical content is not rewritten at all, and non-identical content may be deliberately **kept**: a complete recorded distribution at a version greater than or equal to this surface's, or a candidate distribution that is itself incomplete, leaves the existing entry in place (spec 50). A re-run therefore does not necessarily move this surface's recorded runtime path.
