...
- **Respect-manual-disable** — when set, the run declines to install anything in a project the user has durably marked "leave this project alone", returning a **zero-write success**: the success flag set, a message saying the repository remains manually disabled, no written paths, and a distinct **manually-disabled discriminator** (see Outputs).
- **An explicit distribution directory** — the absolute path to record in the machine-global runtime registry, i.e. the distribution that hooks installed by this run will execute from. Absent, the run records the directory of the bundle executing it, which is correct only when the process performing the install *is* the distribution being registered. A long-lived server installing on another distribution's behalf must supply it. The value is validated at the cross-process boundary — absolute and existing — rather than trusted. See spec 50.
...
- A **manually-disabled discriminator**, present only when the respect-manual-disable option short-circuited the run.

**The discriminator is what makes the success flag readable.** A zero-write refusal reports success on purpose — declining to touch a project the user turned off *is* the correct outcome, not a fault, and reporting failure would make every automatic repair treat a healthy opt-out as a problem. But it means **the success flag alone can never be read as "the install happened"**. Any caller whose success path carries a side effect — stamping "enabled for this version", clearing a cached opt-out, holding an interface state — must test the discriminator first, or it records an install that never occurred. The field is absent on every other path, including a real install of a project that simply carries no opt-out, so consumers must test for it explicitly rather than infer it. See spec 304 for the consequence when a consumer did not.
...
3. The IDE (IntelliJ-family) plugin's enable action, which drives **the bundled runtime's full enable over its long-lived host bridge** — not a fresh child process — with its own source tag, an explicitly named runtime directory, and exactly one of the two opt-out flags. It no longer installs any agent or git hook in JVM code and writes no hook body of its own; its surface-specific extras are a small set of native preparations performed before the call, owned by spec 128.
...
The **durable manual-disable check happens immediately after the repository lock is acquired**, not during the pre-checks. When the caller asked to respect the opt-out and the project carries it, the run stops there and returns the zero-write success described in Outputs. Reading it under the lock is what prevents a concurrent disable from landing between the read and the install and being silently undone.

**"Zero-write" here means zero writes into the repository, not zero writes anywhere.** The whole machine-global runtime phase — dispatch scripts, the legacy migration, this surface's registry entry, the stale-entry sweep — runs *before* the repository lock is taken, and therefore before this check. So a refused run has still refreshed the shared runtime and re-registered this surface; what it has not done is write a hook, a configuration value, or clear the opt-out. The check is also placed ahead of the explicit-plugin-setup step below, deliberately, so a disabled project stays a true zero-write on the repository side.
...
3. **Offer the per-source dist-paths entry, then prune stale entries.** The source tag is "CLI" when invoked from the CLI, an explicit tag when the caller supplied one, and otherwise derived by inspecting the caller's runtime location. The entry records this surface's version and the absolute path of the runtime that should service hooks installed by this source — **the caller's explicitly supplied directory when it gave one, otherwise the executing bundle's own directory**. Whether the recorded entry actually changes is decided by the registry's own keep-existing gate (see spec 50). Failure aborts the operation.
...
- **A refusal reports success, and a caller that does not check the discriminator will believe an install happened.** This is the single most consequential reading error the result type allows, and it has already been made: a consumer stamped "enabled for this version" on the strength of the success flag, permanently suppressing its own later catch-up runs for that version.
- **The recorded runtime directory is no longer necessarily the caller's own location.** It is inferred from the executing bundle only when the caller supplies nothing. A caller that installs on another distribution's behalf must name that distribution, and the failure of not doing so is invisible: the shared hook dispatcher exits silently by design, so a registry entry pointing at a directory the host has since discarded presents as capture quietly stopping.
...
- The IDE plugin's native preparations (the local state directory, its ignore-file entry, and two legacy sweeps) are its only surface-specific work; every artifact it produces comes from this same orchestrator, because that surface delegates the full enable rather than reimplementing any step. See spec 128.
...
- The durable opt-out this orchestrator respects, clears, or records is owned by spec 145; the full inventory of what a project carrying it refuses is spec 304, which also owns the reading of the zero-write success discriminator returned here.
- The JVM IDE surface's native preparations, its bridge-hosted delegation, and the runtime directory it injects are owned by spec 128; the gestures that reach them and the interface state that consumes the discriminator are spec 332.
