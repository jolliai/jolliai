...
- The project-affinity classification that decides whether a plan's source file belongs to this repository, and the commit path's two applications of it (partitioning the ranker's input, and the archive chokepoint). This panel does **not** apply it; that omission is recorded under Notable Behavior, but the classification itself is owned elsewhere.
- The working-tree file discard the file row's destructive control dispatches — its path-only request shape, the single status read every path is resolved against, its per-path operations and its outcome contract. Owned by the working-tree-file-discard topic.
...
- **Destructive remove/discard (context + file rows only).** A separate trash/discard control that is *not* an inclusion toggle:
  ...
  - On a **file** row it dispatches the discard-file message. **Nothing routes on a status any more:** the message's load-bearing field is the row's repository-relative path, which the host forwards to the shared discard rule set; that rule set resolves the path against its own status read and decides the operation. The collapsed status letter and the raw porcelain columns still travel on the message and are read by nothing. This panel's producer falls back to the **empty string** when it cannot resolve the row's path, and the host validates nothing — a blank path is rejected by the rule set as a failure (never as "already clean"), which is what stops a confirmed, irreversible click from silently doing nothing.
...
**Ranking and persistence.** When the selected-file set changes (and on full refresh), the panel builds a change signal from the currently-*included* files (repo-relative paths, no commit message) and asks the relevance ranker to score every user-kept context item (see [258]). This is the authoritative-style path: it reads full registry content (like the post-commit worker) and **persists the FULL per-item verdict list** — every ranked item, kept *and* excluded — plus a file-based change fingerprint to the exclusion store (spec 188). Each persisted entry carries: the item's kind, its key, its relevance tier (`high` / `mid` / `low`), the one-line reason, and the AI's **original exclude decision** (written once by the ranking, never rewritten). The single full list is what lets the post-commit worker's fingerprint-reuse path rebuild **both** the effective exclude set **and** the kept items' tier + reason (recorded on the summary's context-relevance field) without re-running the LLM. Because the fingerprint is keyed on the file set only, the worker reuses this exact ranking verbatim when its fingerprint matches. The **pre-commit rank is non-authoritative** regardless: the post-commit worker always recomputes on a fingerprint miss, and its recompute is the authoritative result; the worker also **clears this AI layer** after consuming it.

**"One ranking, panel result == final" holds only for the items the commit will actually keep, and this panel does not know which those are.** Its candidate set is the branch's active plans, notes and references filtered by **exactly one** thing: the user's own hard-exclude set. It applies **no project-affinity classification** — no check that a plan's source file belongs to *this* repository. The commit path applies that classification in two places the panel never touches: it partitions the foreign plans out of the ranker's input before ranking, and it excludes them again at the single archive chokepoint every path funnels through.

The consequence is mechanical and silent. A plan whose source lives outside this repository is offered on this panel, ranked by the model, given a tier and a reason, rendered as included, and **persisted into the fingerprint-keyed ranking** — and then, on the reuse path, the worker replays that persisted ranking against a candidate set the plan is simply **absent** from. It never reaches the summary, is never archived, and nothing about its disappearance is attributable to the panel's view of it: the only record is the worker's own foreign-exclusion note, produced after the commit. So the user is shown, and can carefully rank, context the commit was always going to drop.

**This is host-specific, not a product-wide gap.** The JVM host's equivalent surface receives the affinity verdict from the command-line surface — foreign plan identifiers arrive folded into the exclusion set it renders, so that panel strikes them through. This panel reads the plan list in-process instead and never asks for that verdict.
...
- **The panel can offer, rank and persist context the commit will silently drop.** Its candidate set is filtered only by the user's hard-exclude set; the project-affinity classification that removes plans sourced outside this repository is applied on the commit path and **not here**. Such a plan renders as included with a tier and a reason, gets persisted into the fingerprint-keyed ranking, and is then absent from the candidate set the worker replays that ranking against — so it never reaches the summary and is never archived, with the only trace being the worker's own post-commit exclusion note. The JVM host's equivalent surface does not have this gap: it is handed the affinity verdict and strikes those rows through. (Notable; the panel's "what the next commit will save" framing is wrong for exactly this case.)
- **"Panel result == final" is therefore scoped, not absolute.** A fingerprint match makes the worker reuse this ranking verbatim *for the items it still has*; it does not make the panel's candidate set the commit's candidate set.
