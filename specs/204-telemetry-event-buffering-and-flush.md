...
**Cwd is buffer identity.** The queue path is resolved from a literal working directory with no git-root normalization: two different cwd strings (a repo root vs. one of its subdirectories, or two surfaces that resolve the root differently) address two separate buffers. Therefore every writer and every flush trigger for one project must pass the same cwd — the project/workspace root — or events written under one cwd are stranded in a buffer no trigger for the other cwd ever drains.

Current surfaces satisfy this by construction, and on the command-line surface the invariant that makes it true is worth naming rather than assuming: the bin entry's telemetry bootstrap and its exit flush both pass the **git-repository-root-resolved** project directory (spec 311), which is the same value every command's `--cwd` option already defaults to. One resolution, three consumers, one buffer path. This was not always so — before the anchoring, the bootstrap and the exit flush passed the raw process working directory while the commands in between passed the resolved root, so a `jolli` run started from a subdirectory buffered its events under `<subdir>` and then drained a *different* buffer under the repo root. The subdirectory buffer was never a flush target for anything, so those events were stranded on disk indefinitely.

**The long-lived local dashboard server satisfies it differently, and the difference is worth knowing.** That process does **no resolution of its own**: it defaults to the directory it was started in and uses that one value for its bootstrap and for every one of its three flush points — which is what makes those three internally consistent, but says nothing about *which* buffer they land on. What keeps it on the repository-root buffer is the launching command, which resolves the root **before** spawning the detached child and starts it there. That resolution is two steps: validate the raw directory (the caller's explicit choice, else the launcher's own working directory) is a real directory, falling back to the launcher's working directory if not; then take the git top level, keeping the validated base whenever the git query fails or the directory is not in a repository. Launched from a subdirectory without it, the server would buffer under `<subdir>` — a buffer no other surface's flush trigger ever addresses. The invariant therefore lives one process away from the code that depends on it.
...
| Long-lived local dashboard server | Three points: a bootstrap at boot; a repeating **60-second** flush whose timer is detached from the event loop so it cannot by itself keep the process alive; and one final best-effort flush on the stop path (interrupt signal, terminate signal, or the idle shutdown), run before the process exits and idempotent. Every one of these flushes overrides the default timeout with a short **~2-second** cap, because the last of them sits on the exit path. An abrupt kill or a crash has no hook and loses the un-flushed tail — bounded, as always, by the two caps. |
