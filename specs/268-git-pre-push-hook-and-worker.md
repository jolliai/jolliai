...
- Otherwise the enumeration is the two-dot range from the remote tip to the local tip, oldest-first, **also narrowed to commits no tracked remote already has**. Both arms end in that exclusion, and on the ranged arm it is load-bearing rather than defensive — see below.
...
- **The ranged arm carries the same exclusion, and it corrects a real over-report rather than trimming an edge case.** Git hands this hook the old tip of **one** ref; it does not hand it the set of objects the push will actually transfer, which git works out afterwards by negotiating against **every** ref the remote advertises. The two answers diverge sharply after a rebase, because a rebased branch absorbs commits its own remote tip predates — measured on a real force-push, the plain range reported 12 commits of which 11 were already on the base branch and authored by other people. Git transferred none of those 11, yet each was enqueued, and since a memory is generated on the machine that made the commit (the memory ref is local-only and never fetched) none of them could ever gain one here. They rendered as "memory still generating — will sync later" on every subsequent push until they aged out.
- **Dropping them cannot lose a memory**: a commit reachable from a remote ref was already carried by whichever push put it there. A stale remote-tracking ref only makes the enumeration over-report, which is the pre-existing behavior and is the safe direction.
