...
- The push-state half of the pairwise skill fold: which published-article identifier survives, which is banked for deletion, and why the survivor is then removed from that bank.
- Which families of published document can appear in a node's orphaned-identifier list.
- The rule by which a consolidated root's artifact fields (plans, notes, external references, skills) are aggregated — the collection over its children plus the refs the consolidating caller associated with the consolidated commit itself — and the two dedupe-key families that rule uses.
...
- `pricesAsOf`: optional date string, the price table's verification-date stamp at the time `estimatedCostUsd` was **last** computed, co-written with `estimatedCostUsd` (both present or both absent). Lets a reader judge how stale a cost figure is. Because a post-write correction re-derives the cost (see the note on in-place correction below), this stamp can legitimately be **later** than the node's own `generatedAt` — it dates the pricing, not the memory. Defined by [257 — Multi-Provider Pricing and Cost Estimation].
...
- `orphanedDocIds`: array of numeric document identifiers superseded during squash/rebase merge, kept so they can be deleted from the external service after a successful push. It is the **only** queue the push-side cleanup reads, so an identifier not placed here is unreachable forever.

  **Two document families reach it, not one.** The claim that only memory-summary documents are orphaned no longer holds: the list carries superseded *memory-summary* identifiers (the losers of the newest-child-wins hoist, plus whatever the children already had queued) **and** superseded *skill-article* identifiers, which arrive by three separate routes — the losers of the commit-level skill article's own newest-child-wins hoist, the legacy per-(skill, commit) identifiers a pairwise skill fold displaced and banked (drained here by the consolidating caller), and the identifiers a legacy-model conversion reclaims at push time (spec 343). Plan, note and external-reference documents are still never orphaned: each of those is one artifact revised in place, so a fold re-points at the same document rather than superseding it.
- `jolliSkillsDocUrl` / `jolliSkillsDocId`: the commit-level skill article's URL and server-side identifier — the sibling of `jolliDocUrl` / `jolliDocId` for the single article covering all of this commit's skill records. Commit-level rather than per-record: a skill record is a *measurement of one commit's work*, so one article per commit is what every surface already shows.
...
### In-place correction of the usage/cost field group

The five usage/cost fields (`conversationTokens`, `conversationTokenBreakdown`,
`conversationModels`, `estimatedCostUsd`, `pricesAsOf`) are the only fields on a
node that a **post-write** operation recomputes from a durable record other than
the transcripts: detaching a conversation from a committed memory subtracts that
conversation's own recorded share from whichever node owns it, per node rather
than per tree (see [306 — Conversation Detach Usage Correction]). Three
consequences bear on this spec's contracts:

- The group is **stripped, not zeroed**, when nothing remains after subtraction —
  preserving the "absent means this memory reports no usage" reading defined above
  rather than writing a zero that would render as a measured nothing.
- Every segment is floored at zero, so a subtrahend larger than the stored value
  (possible when the node was written before per-response de-duplication existed
  and its aggregate is inflated) collapses the group instead of going negative.
- `estimatedCostUsd` is re-derived from the remaining per-model buckets, never
  scaled from its previous value, and `pricesAsOf` is re-stamped at correction
  time.
...
The fallback is **per segment, not per breakdown**: for each node the aggregator reads each of the three segments independently, substituting zero for that one segment when it is missing, rather than substituting a fully-zeroed breakdown only when the whole breakdown is absent. Summaries load through a bare parse with no schema validation, so a present-but-partial breakdown (hand-edited, older schema, or a future producer that writes only some segments) must not leak a missing segment into the sums: adding a missing segment to a number yields the not-a-number value, which poisons the tree total — and because every display gate asks whether the total is greater than zero, and that comparison is false for a not-a-number total, the aggregate figure would silently vanish from the token bar and the article rather than showing as obviously broken.
...
| legacy per-record article id + URL | **Inherited from the second ref only when the first carries none.** The first ref's own id is never overwritten. The URL travels with the id, always as a pair. |
| banked superseded ids | Recomputed from scratch for every fold — see below. The first ref's existing copy of the field is **explicitly removed before the merge**, rather than left to survive by inheritance. |

The push-state rules deserve spelling out, because three of them are load-bearing and one is a deletion that looks like a no-op:

- **Inherit, never overwrite.** Without the inheritance a fold would simply drop the second ref's published identifier and the next push would create a *duplicate* article instead of updating the existing one. Without the never-overwrite half it would push the merged content at an older article and strand the one the surviving row actually points at. It is the same asymmetry the plan collector's inheritance uses, and the URL rides along because the push-side reuse gate recovers an identifier's owning backend from the URL's origin — an identifier adopted without its URL is worse than untagged, it is unusable.
- **Bank the displaced identifier.** Whichever of the two identifiers did *not* survive now has nothing pointing at it, so the fold records it in the banked set rather than dropping it. It is added only when **both** refs carry an identifier — if the first had none, nothing was displaced. The set also absorbs whatever either side had already banked, so an inner fold's casualties survive an outer one.
- **Then remove the surviving identifier from that set.** This is a correctness guard, not tidiness: deleting the surviving identifier would destroy the very article the merged row still points at. Two paths reach it — the two refs carrying the *same* identifier (one archived record met from both ends of a fold tree, which the archive-key deduplication catches but a caller folding without it does not), and an inner fold having banked an identifier that a later fold ends up inheriting as its survivor. The "survivor" it removes is computed by the same inherit-else-adopt rule as above, not by reading only the first ref, or an adopted identifier would be left marked for deletion.
- **Strip the previous copy first.** The banked set is recomputed whole, so letting the first ref's copy survive by inheritance could resurrect an identifier this very fold just decided *not* to delete. The field is omitted entirely when nothing is pending, so a row that superseded nothing carries no field and the stored JSON sees no churn.

The banked set is a *pending* marker, not a durable field: every caller that persists a folded row owes it a drain into the node's orphaned-identifier list and a removal of the key (removal, not blanking — these rows are stored as JSON, where a null reads back as a field that exists). The drains themselves are owned by the consolidation and normalization specs.

Two of the counting rules also deserve their reason stated:
...
### A consolidated root's artifact fields are the child collection PLUS the consolidating commit's own refs

A root produced by folding many summaries into one commit does not carry merely what its children carried. For `plans`, `notes` and external references the field is the child collection **unioned with** the refs the consolidating caller associated with the consolidated commit itself — artifacts the user activated during the session that ended in the fold, which appear in no child and which a walk over children therefore cannot see. Without that union the artifact files just archived for the consolidated commit would have no node in the tree referring to them at all.

The union deduplicates by the identity of the **archived file** — the plan slug and the note id with their per-commit hash suffix intact, and the external reference's archive key — with the caller's ref winning a collision (its archived copy is the one just written, so it carries the freshest metadata for that file; at equal archived identity both refs address the same file, so nothing is lost by preferring it). A replaced entry keeps the position it held in the inherited list rather than moving to the end.

Retaining the hash suffix in that key is load-bearing. Two children may legitimately hold the same *logical* artifact archived at two different commits — the artifact was worked on again later in the run of commits being folded, so it was archived a second time — and nothing in the fold renames or deletes a child's archived file, so both files survive and both need a node referring to them. Keying on logical identity (suffix stripped) would keep whichever child was walked last and leave the other archived file stranded.

The in-place-rewrite root performs the analogous new-wins union but keys it on **logical** identity, precisely because it unions against a single prior summary rather than many children; there, a re-archived artifact must list once rather than twice (see spec 39). The two key families are not interchangeable in either direction, and using either on the other path leaves an archived file with no node referring to it.

`skills` sit outside that distinction: their fold accumulates per skill instead of keeping one ref per key, so every distinct archived record's increment enters the single surviving row and no contribution can be dropped by the choice of key. (What accumulation does not preserve is a *reference* to every archived record — see the archive-key rule above.) The consolidating commit's freshly-archived skill refs are therefore simply concatenated with the collected child refs before the fold, with no key choice to make.

Each of these fields is emitted on the root only when the merged result is non-empty. The pipelines that supply the caller-side refs, and the archival that produces them, are owned elsewhere (see specs 13 and 39).
...
- **The skill fold carries publish state, and the rule that keeps it safe is a deletion.** Beyond summing counters, each fold decides which of two published-article identifiers survives (inherit only into an empty slot; never overwrite), banks the displaced one so a later push can delete it, and then **removes the survivor from that banked set** — because a bank that still named the survivor would have the cleanup pass delete the article the merged row points at. The first ref's banked set is stripped before the merge rather than inherited, so the recomputation cannot resurrect an identifier this fold just spared. (Surprising; the removal reads as tidying and is the correctness step.)
- **Orphaned document identifiers are no longer a single family.** The node's orphaned-identifier list carries superseded memory-summary identifiers *and* superseded skill-article identifiers, the latter arriving from a per-commit article hoist, from this fold's banked set, and from a legacy-model conversion. Plans, notes and external references never contribute: each is one artifact revised in place. (Notable; the "memory summaries only" reading was true of an earlier model.)
...
- **Two dedupe-key families govern a hoisted root's artifact fields, and swapping them strands archived files.** A consolidated (many-source) root keys its plan / note / external-reference union on the archived file, per-commit suffix intact, because two folded children can hold the same logical artifact archived at different commits and both files survive the fold. An in-place-rewrite root keys the same union on logical identity, because it unions against one prior summary and an artifact re-archived under the new hash must list once. Either family used on the other path leaves an archived file with no node referring to it. Skills are outside the distinction — their fold accumulates rather than keeping one ref per key, so no contribution is lost to the key choice. (Surprising; load-bearing.)
- **A consolidated root's artifact fields are not derivable from its children alone.** They include the refs the consolidating caller associated with the consolidated commit itself, which exist in no child; a reader that recomputes them by walking children will come up short by exactly those. (Notable.)
...
- **Token-breakdown aggregation falls back per segment, not per breakdown.** A present-but-partial breakdown must contribute the segments it does have; falling back only on a wholly-absent breakdown would let a single missing segment turn the tree total into the not-a-number value — which every "is the total above zero?" display gate reads as false, silently hiding the whole aggregate figure instead of surfacing the corruption. (Surprising; intentional.)
...
- **The usage/cost group is the one field group a later user action rewrites.** Detaching a conversation subtracts its recorded share from the owning node and re-derives that node's cost; nothing else on a node is recomputed after write except by a full regenerate. A consequence: `pricesAsOf` on a node can post-date the node's `generatedAt`, and a corrected node's figures are strictly smaller than what its own generation recorded. (Surprising; intentional. See [306 — Conversation Detach Usage Correction].)
...
- The banked-superseded set this fold produces is a pending marker with an owed drain. The drains — into the node's orphaned-identifier list, plus the removal of the key from the persisted row — are performed by **Squash Consolidation Summary** (13), by the legacy-to-unified normalization (06), and by the in-place-rewrite root (39, which unions rather than folds and therefore mints no marker at all). The conversion that reclaims a previously-shipped model's per-record identifiers at push time, and adopts one of them as the commit-level article, is **Legacy Skill-Article Migration** (343).
- The push-side reuse gate that makes an identifier's minting URL load-bearing, the article that a commit-level skill identifier names, and the cleanup pass that drains the orphaned-identifier list are owned by the push specs (94, 231, 236).
