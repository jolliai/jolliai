...
A machine-global store records which repositories have **outbound push to a Jolli Space** turned off. Unlike the repo-wide manual disable flag, which stops *all* capture, this opt-out blocks only outbound sync — memory is still captured and stored locally. That flag has since acquired a second, non-capture role: it is also the per-repository upload **filter** for the machine-wide session-statistics channel, asked of every registered repository once per run rather than of the repository that triggered the run. The store is keyed by each repository's **canonical identity**, not by a working-tree path, so the machine-wide control view (whose rows come from the Memory Bank, which knows repositories by identity) and the per-repo gate (which resolves its own canonical URL) share one key, and a repository checked out in several worktrees shares one decision. A single predicate composes the manual-disable flag with this store, and it is the gate every **per-repository** outbound path on every surface consults — but not every outbound path: the session-statistics upload consults neither the composed predicate nor this store, gating instead on its own configuration switch and the presence of an API key, so a repository with push turned off still uploads its session statistics. New repositories are push-*allowed* by default — a restriction is always an explicit opt-in.
...
- The control surfaces — the command line, the desktop editor's settings, the JVM host's settings, and the local dashboard's settings modal — and the shared engine behind all of them; plus which surfaces do and do not expose the separate session-statistics switch, since one of them sits beside this flag's own list.
...
- The machine-wide session-statistics upload is a third outbound channel, defined by its own topic — it requires no Space binding and carries its own switch. This topic owns only the boundary: that channel consults neither this store nor the composed predicate, and the manual-disable flag reaches it as a per-repository row filter rather than as a gate.
...
- **Local dashboard settings modal** — the same machine-wide list, one row per repository showing its display name, its canonical **identity**, and a switch; **every row written by identity**, and the launch-repository flag deciding only whether the re-enable drain can run. Each toggle applies immediately, with the per-row result reported on a status line under the switch and a failure reloading the persisted list so the switch snaps back. Three things differ from the desktop editor's list, and all three are real behaviour: the **preserved-copy path** a corrupt-store recovery reports is **omitted** (the recovery itself is reported, where the file was preserved is not); the trigger written into the entry and emitted as telemetry is the **command-line** value, because the trigger vocabulary has no dashboard value; and a **failed list load fails WHOLE** — a callout and no rows — rather than degrading the Memory Bank root read to "none" and listing the current repository alone, so this surface has no "the rows below may be wrong" state at all. A failed load additionally closes the guard that would otherwise refetch on every render.

**The session-statistics channel's own switch is not one of these surfaces, and it is not a subset of them.** It exists in two places only: the local dashboard's sync section — where it sits above the per-repository list as its own immediate write, not part of the same Apply — and the command-line configuration setter. Neither editor host exposes it, so a user in the desktop editor or the JVM host can turn a repository's push off from the settings dialog and has no way to reach that switch from the same place.
...
A toggle emits a disable or enable event carrying the surface that triggered it — drawn from a closed vocabulary that has **no dashboard value**, so the dashboard modal's toggles emit the command-line value and are indistinguishable from a command-line toggle afterwards. The same string is what the store records on the entry as its display-only trigger field.
...
- **An unreadable store reports OFF for every repository, so every surface that reports OFF must say which OFF it means.** Attributing a fail-closed read to the user is wrong twice over — they chose nothing, and the condition is machine-wide rather than per-repository — so every reporting surface consumes the state form, not the boolean:
  ...
  - The dashboard's settings modal takes the other option available to it: an unreadable store is the one condition the enumeration propagates, so the list request fails and the section renders a callout with **no rows at all**. It never shows a stale switch, and it never names the store's path either — the failure text is the endpoint's generic "could not list repositories".
...
- **The trigger vocabulary is closed and has no dashboard value, so one surface lies about itself.** The dashboard modal's toggles record and emit the **command-line** trigger, both on the store entry's display-only field and in telemetry — so neither a hand-inspection of the file nor a telemetry query can tell a dashboard toggle from a command-line one. (Surprising; the vocabulary was not extended when the surface was added.)
...
- **This flag governs one outbound channel of three.** It covers per-repository Space push only. The personal Memory-Bank vault sync is unaffected, and so is the machine-wide session-statistics upload — which is not a per-repository channel at all: it requires no Space binding, gates on its own switch plus the presence of an API key, and consults neither this store nor the composed predicate. A repository with push turned off still uploads its session statistics.
- **The manual-disable flag is a row FILTER on that third channel, not a gate on it — and the filter is one-way.** It is asked of every registered repository once per run rather than of the repository that triggered the run, and a withheld repository's rows are withheld permanently: selection pages *over* them, so re-enabling that repository never sends the backlog it held back while switched off. That is the opposite of this flag's own behaviour, where a toggle-on triggers a catch-up drain of exactly that backlog. (Surprising twice over: the disable flag has a second role outside capture entirely, and on that channel it is the one decision with no catch-up.)
...
- The dashboard settings modal that hosts the fourth control surface is defined by **Dashboard Settings Apply** (363) — but only as the surrounding form: what one Apply there validates, persists and reconciles, which of its routes are open reads, and the endpoint the toggle posts to. **The toggle surface itself is owned here**, in full: the row contents, the write-by-identity rule, the per-row status wording, the snap-back on a failed write, the three divergences from the desktop editor's list, and the trigger it is unable to record as its own. That topic states only that the toggle exists there outside Apply and that it identifies itself as the command line.
