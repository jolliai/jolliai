...
The service accepts request protocols 1, 2 and 3. On conflict it refreshes only non-key attributes **present in that incoming row**: an older client omitting a protocol-3 field cannot erase a value a newer replay already supplied, while an explicit `null` remains a real value and clears the column. Rows in one table batch are grouped by their present update columns because Sequelize applies one conflict-update list to an entire bulk insert; without the grouping, a new row's field list could make an old row overwrite that field with null.

Three response shapes are refused rather than read. A **non-2xx** is raised by status, with `404` and `412` each carrying their own class (below). A **2xx whose body does not parse as JSON** is raised as the *same class as a missing endpoint*. A **parsed 2xx carrying neither an `accepted` field nor a `cursor` field while rows were sent** is the JSON form of that same invalid acknowledgement and is refused too. A parsed response carrying either field is returned to the runner even when one table is absent or an accepted count is shorter than the submitted page: only the runner has the batch and can hold that one table while advancing the others, and the at-or-after reader deliberately duplicates one boundary row per non-first page, so the client cannot assume whether `accepted` counts submitted rows or newly stored rows.
...
| `payloadVersion` | Legacy machine-wide replay marker, retained only so older state files round-trip. It is not the current replay authority. |
| `replayByScope` | Per-scope, per-generation historical replay status, completed-table set, and local-only keyset progress. |
...
When an upgraded client first sees an existing scope, it creates generation `skills-mcps-fields-v1` for `sessions`, `session_tool_use` and `skill_invocations`. Those three dedicated replay keysets start at stamp zero without changing the ordinary `byOrigin` cursors. A genuinely unseen scope records the generation complete immediately and keeps the normal 90-day first-run contract; a legacy bare-origin cursor counts as existing progress for the tenant scope.

The replay deliberately does **not** upgrade the wire protocol, clear the server cursor, or depend on old clients stopping. Protocol 3 remains the request shape and protocol 1/2/3 remain accepted. A server cursor already raised by an old client is harmless because the ingest rejects only a client **ahead** of the server; a replay page starting behind it is upserted. While replaying, the client advances from the batch's **local maximum** and ignores the server's higher cursor for pagination. Otherwise the first response would jump over all local pages between zero and that server position. Progress and each drained table are persisted after every acknowledged page, so the ten-batch run ceiling and a crash resume rather than restart; only after all three tables drain are their final local maxima published into `byOrigin` and the scope generation marked complete in the same state-file write.
...
Before the ordinary loop, a pending historical replay reads only its remaining affected tables. Empty tables are marked drained without a request. Submitted tables require the normal presence-based acknowledgement; an omitted table stays queued. Each acknowledged table advances from its local page maximum, never the returned server cursor. Reaching ten batches leaves the replay pending and returns success so the next trigger resumes. Completing a replay after sending rows ends that run; ordinary incremental work resumes on the next trigger.
...
| Neither an `accepted` key nor a cursor key for this table, while the response names another table | **Holds** the current cursor. The server's closed schema stripped a table it does not know; holding only that table keeps its rows queued while acknowledged tables continue. It is withheld for the rest of this run and offered again on the next one. |
| Neither map contains any table key (both maps are present but empty) | The **batch's own last row** on a success, then what the client already held. This wholesale no-detail shape is deliberately distinct from a response that names some tables but omits one; the fallback keeps a server that echoes no per-table detail from leaving the same rows selected for ever and burning the whole ceiling re-sending them. |
...
| Replay generation absent | Run resolves an existing scope | Replay starts at zero for the three affected tables; normal cursors remain unchanged |
| Replay generation absent | Run resolves a new scope | Generation recorded complete; normal first-run window remains in force |
| Replay pending | Page acknowledged | Local replay maxima and drained tables persisted; server cursor ignored for replay pagination |
| Replay pending | All replay tables drained | Final local replay positions published as normal cursors and generation recorded complete |
...
- **Per-table acknowledgement is presence-based, not exact-count based.** An `accepted` key or cursor key says the backend knows the table. A table omitted from both in a response that names another table is held while the acknowledged tables advance; a short accepted count is tolerated because every non-first at-or-after page deliberately contains one duplicate boundary row and backends may count newly stored rows rather than submitted rows.
- **Historical repair is a client-owned replay, not a protocol fence or server-cursor migration.** Old clients keep syncing. An upgraded client walks three local tables from zero with separate persisted keysets and ignores a higher server cursor until the replay drains, so an old client raising that server cursor cannot make the repair skip pages.
