...
**Two different things produce entries of this one shape, and the field does not distinguish them.** Most entries are a relevance verdict — an item the ranker judged unrelated, with the reason the model wrote for it. But every plan the pipeline classified as belonging to a **different repository** also lands here, carrying a **fixed** reason phrase and the lowest tier with no verdict behind it at all: the ranker never received that plan and never tiered it. Nothing downstream inspects the reason text, so a deterministic classification and a model judgement are read identically wherever the field is rendered.

The field is written only when non-empty. It is a **traceability record** ("this item was excluded, and why"), distinct from user manual excludes — which are skipped from association but never stored. Soft-excluded plans/notes are also handled specially by archival (see [42], [43]), and a foreign plan is refused there a second time by archival's own selection rule regardless of what this field says.
...
2. **Track-only split.** The user-kept set is partitioned before the ranker sees it: every reference whose source declares itself **track-only** is lifted out into a separate set and is **not** offered to the relevance ranker at all.
3. **Foreign-plan split.** Every remaining plan whose source file is deterministically classified as belonging to a **different git repository** is removed from the candidate list — no model in the loop. Classification is by repository identity rather than directory containment, and it keeps anything it cannot prove foreign (no source path, no enclosing repository, an unresolvable comparison). Each removed plan is turned into one **excluded-context entry** carrying a fixed reason phrase and the lowest tier, and the entry list this produces becomes the **initial value** of the stage's excluded-context output — before any ranking has run. Whatever survives both splits forms the ranker's candidate list.
4. **AI relevance filter.** The remaining candidates are passed to the relevance ranker (spec 258), which returns the kept candidates **in relevance-ranked order** (most relevant first) plus a soft-excluded set. The change signal handed to the ranker on this path is the commit message, the changed-file list, the declared symbols, **and a capped head of this commit's own diff** (spec 258 owns the caps and the truncation shape). On a change-fingerprint match the panel's persisted ranking is reused; otherwise the ranker recomputes. On success the ranker's soft-excluded set is **unioned onto** the foreign entries from step 3. It is **no longer skipped for any credential provider**: a locally-run agent-CLI provider used to short-circuit the ranking to keep-all, so those users' commits could never carry an excluded-context list or per-item relevance; they now go through the same ranking as everyone else and their summaries can carry both fields.

   **This stage is fail-open, but fail-open no longer means "excludes nothing".** Any failure keeps the full candidate set and applies no verdict — yet because the excluded-context output was already seeded in step 3 and is only unioned with the ranker's result on success, a ranking failure still leaves every foreign-plan entry standing on the stored summary. A commit with a failed ranking and a foreign plan therefore carries a populated excluded-context list and no per-item kept relevance at all.
5. **Track-only splice-back.** The track-only set from step 2 is added back into the kept references **unconditionally** — no verdict, no tier, no reason. This is the whole point of the split: a relevance verdict must never be able to drop a track-only reference from archival. There is **no** matching splice-back for step 3: a foreign plan is gone from the kept set and stays gone.
6. **Prompt-block assembly.** The kept items — in the ranker's order — are rendered into the plans / notes / references blocks handed to this generator. Because the order is relevance-ranked, any per-block budget truncation drops the **least** relevant items, not the oldest.

The track-only split, the foreign-plan split and the splice-back all run identically on the standard commit path and on the amend path.
...
Soft-excluded items are **recorded, not dropped silently**: they are excluded from the prompt blocks but written to the stored summary's excluded-context list (above) for traceability. A foreign plan is recorded in the same list and the same shape, but with a fixed reason and no verdict behind it. Kept items with a non-empty relevance reason are likewise recorded on the stored summary's kept-item relevance list (above). On a fingerprint match the reuse path rebuilds both lists from the persisted per-item ranking without re-running the LLM; on a miss the ranker recomputes. This stage runs on the standard commit path and on the amend path (amend always recomputes fresh rather than reusing the panel's ranking).
...
- **Relevance filtering now runs for every provider, so two summary fields newly appear for local-agent users.** The upstream ranker's provider short-circuit is gone, so a commit made under a locally-run agent-CLI provider can now carry an excluded-context list and per-item kept relevance where previously it never could (those users always got the keep-all shape, which produces neither field). (Notable; behavior change, not a format change.)
- **The excluded-context list is not purely an AI record, and a fail-open ranking no longer leaves it empty.** A plan whose source file belongs to a different repository is split off deterministically, before the ranker runs, and its entry is what the stage's excluded-context output is *initialized* to; the ranker's own verdicts are only unioned on top when the call succeeds. Two consequences: an entry on this field may carry a fixed reason phrase the model never wrote, at the lowest tier, for an item the model was never shown; and "the relevance stage failed open" no longer implies "nothing was excluded", because the foreign entries survive the failure untouched. (Surprising.)
