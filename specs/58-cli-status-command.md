...
6. **Memory database** — a one-line descriptor of whether the local memory database has been built from the stored memories, printed **only when the state could be read at all** (it cannot be, outside a git repository). Five shapes: unavailable with a reason; never built, pointing at the dashboard command; built, with a memory count and a relative age; failed, naming how far it got, the error and the retry command; and in-progress, which itself splits three ways by liveness — the writing process is gone (interrupted, resume), alive but with a stale heartbeat (naming how long there has been no progress and the process id), or alive and fresh (naming the count and the process id). The relative age reads as "just now", minutes, hours below two days, then days.
