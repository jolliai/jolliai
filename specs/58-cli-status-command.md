...
6. **Memory database** — a one-line descriptor of whether this repository's memories have been imported into the machine-level memory database (spec 347), printed whenever that state could be read at all. Six shapes:

   | State | Rendered value |
   | --- | --- |
   | Cannot be read | `Unavailable (<reason>)` |
   | No record for this repository | `Not migrated — run ...` (naming the dashboard command) |
   | Complete | `Migrated (<n> memor{y,ies}[, <age>])` |
   | Failed | `Failed at <done>[/<total>] — <error> (run ... to retry)` |
   | Running, writing process gone | `Interrupted at <done>[/<total>] — run ... to resume` |
   | Running, writing process alive | `Migrating — <done>[/<total>] memories[, no progress for <span>] (pid <n>)` |

   The relative age reads as "just now", minutes, hours below two days, then days. **A record with no lifecycle field at all reads as complete** — such a row was only ever written on success, and reading it as anything else would tell a fully-migrated repository to migrate again.

   The liveness split is three-state, and the middle state is the point: a live process whose heartbeat has gone quiet for ten minutes still reads as *migrating*, with the quiet span named, rather than as interrupted. Only a dead process is evidence of interruption — the import reads thousands of git objects and one fallback path reads them in a single uninstrumented span, so a quiet stretch is normal.

   Two `Unavailable` reasons are specific rather than an error string: a runtime below the database floor (naming the running version), and the alarming file combination where the database is gone but its sidecars remain (naming the recovery command — spec 348).

   This row is **deliberately not part of the machine-readable snapshot**. Adding it there would carry it into the shared status object and from there into the MCP surface, the desktop editor panel and an owed operation for the other IDE host — so it is read locally, by this command only.
7. **Jolli Account** — `Signed in` or `Not signed in`.
8. **Jolli API Key** — `Configured` or `Not configured`.
9. **Jolli Space** — the repo's Space-binding state, printed unconditionally (even "not connected" / "unknown"). See "The Jolli Space: row" below.
10. **Outbound push** — printed **only** when outbound push is off for this repo; absent otherwise. See "The Outbound push: row" below.
11. **AI Provider** — the provider the *next* commit would actually generate with, printed unconditionally. See "The AI Provider: row" below.
12. **Anthropic Key** — `Configured` or `Not configured` (the env variable counts). Printed **only** when the configured provider is explicitly the Anthropic one; omitted for every other provider and when no provider is set. See "The AI Provider: row" below for why the two rows are coupled.
13. **Sessions** — total active session count across all AI agents.
14. **Per-agent breakdown** — one row per AI agent that was *detected* on this machine (see state model below). Undetected agents are omitted entirely to keep the output terse. The list is, in order: Claude, Codex, Gemini, OpenCode, Cursor, Devin, Copilot, Cline, Antigravity, Kimi Code. Undetected agents are omitted.
15. **Merged-row sub-lines** — three rows are each a merged dual-variant row that, when detected, renders an additional indented sub-line beneath itself showing whether its two variants are each present (`✓`/`✗`): Cursor (`↳ IDE: …, CLI: …`), Copilot (`↳ CLI: …, Chat: …`), and Cline (`↳ CLI: …, VS Code: …`). If a variant's scan failed, a further indented line reports that variant's failure kind and message without masking the other variant's working count.
16. **Stored memories** — count of summaries on the orphan branch.
17. **Memory Bank** — where folder-layer memory writes will actually land, or the blocker stopping them, printed unconditionally. The row prints only the *text* of the shared Memory-Bank display record; the record's severity is discarded here, so this row carries no icon and no colour. The state model, the three arms, and every string verbatim are owned by spec 300 — this spec adds only the row's position and unconditionality.
18. **Jolli Site** / **Last signed-in site** — the host portion of the **persisted Jolli site URL** (with `http://` / `https://` scheme stripped), printed only when that URL is stored at all. The label is switched by whether an on-disk credential (an on-disk OAuth token or a Jolli API key) backs it: `Jolli Site:` when one does, `Last signed-in site:` when none does. The URL is **not** derived from the stored Jolli API key.
19. **Orphan branch** — the orphan-branch name used for summary storage.
...
1. **`unavailable — <kind>`** — the agent's session store was found but could not be read (e.g. corrupt database, permission denied, schema mismatch). The `<kind>` is a short machine-style identifier of the failure category. Reachable only for agents that have a failure channel at all: OpenCode, Devin and Antigravity (single-store agents), plus Cursor, Copilot and Cline (the merged dual-variant rows, which reach this state only when **both** of their stores fail — see below). Claude, Codex, Gemini and Kimi Code have **no** failure channel and can never render this state.
...
5. **`detected & enabled`** or **`detected & enabled (<n> session[s])`** — the agent has no hook concept (Codex, OpenCode, Cursor, Devin, Copilot, Cline, Antigravity, Kimi Code — they are discovered passively). The session count is appended when non-zero.

The split is exhaustive in both directions: only the two hook-bearing agents can ever print a hook phrase, and only the passively-discovered ones can ever print `detected & enabled`. The `hook not installed` state prints **no** session count even when sessions were found; every other counted state appends one when it is non-zero.
...
- **Several agents disappear entirely on an older runtime rather than reporting zero.** Detection for the agents whose data lives in an embedded database is gated on the runtime having built-in database support; below the supported floor those detectors answer "not present", so the row is omitted rather than showing a failure or a zero count. The agents whose data is plain files on disk are detected on any runtime. (Surprising — an omitted row can mean "not installed" *or* "this runtime cannot look".)
- **The newest agent has no failure channel at all.** Its sessions are plain files, so there is no scan-error state to report and no per-channel sub-line; it can only ever read as detected-and-enabled or detected-but-disabled. (Notable.)
...
- **The `Memory database:` row is guarded as conditional but is in practice unconditional.** The read it guards catches every failure internally and answers with an `Unavailable` reason — including outside a git repository, where resolving the repository's identity fails — so the guard's omitted branch requires the read to throw, which it has no path to do. Reading it as "this row is absent outside a repository" is wrong: there the row prints, either as `Not migrated` (no database file yet) or as `Unavailable`. (**Unreachable** omission; notable.)
- **A missing database file reads as "never migrated" here, and as "cannot answer" for storage routing.** The same absent-file state is deliberately given opposite meanings by the two readers: the certain, actionable answer is right for this row, and would be a dangerous guess for routing (spec 344). (Notable.)
...
- The machine-level memory database the `Memory database:` row reports on — its location, its runtime floor and its schema — is owned by spec 347; the file-combination alarm one of that row's reasons names is owned by spec 348. This spec owns only the row's position, its conditionality and its wordings.
