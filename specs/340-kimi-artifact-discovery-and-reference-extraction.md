...
7. **On a result event:**
   ...
   - Run the shared context-normalizer (spec 342) with the definition, the tool name, the call's `args`, the decoded payload, and an environment carrying an **always-empty permalink map** plus the caller's extraction options. A `null` return voids the reference and it is dropped.
...
- **Reachable** — the issue-tracker source (declares a generic server prefix alongside its connector alias), the code-forge source, the documentation-lookup source, the self-referential memory source (its generic prefix narrowed by an exact tool allow-list), and the deployment source (again a generic prefix alongside its connector alias).
...
- **The arguments-derived escape hatch is reachable for both definitions that declare the flag**, since both — the documentation-lookup source and the self-referential memory source — are in the reachable set above.
