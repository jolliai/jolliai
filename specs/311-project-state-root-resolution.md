...
- The second implementation's **new observable output**: alongside the directory it now also reports **whether git named it**, as opposed to the answer being the unchanged input. That bit is not cosmetic — it is what decides whether an entry point may key a shared, long-lived, cross-session process on the answer at all.
...
- What the MCP entry point does with the from-git bit once it has it — the shared per-worktree process it keys, the address derived from the root, and the in-process fallback a non-git answer is diverted to (spec 364). This spec owns only the answer and the bit.
...
| CLI bin shim | [`Cli.ts:31`](../cli/src/Cli.ts) | `setLogDir(resolveProjectDir())` — before anything logs or buffers telemetry. Runs for **every** invocation, including the MCP fast path below, which is otherwise skipped past the general startup path. |
...
| CLI bin shim — MCP fast path | bin shim, ahead of command parsing | The bare `jolli mcp` invocation is routed by a pre-parser shortcut and never reaches the command action, so it resolves the root **here**, taking the *from-git* form: it needs both the directory and whether git named it. This is the path that actually runs for a host-spawned MCP process. |
| `jolli mcp` command action | command action | Resolves once, in the from-git form, above the mode branch. The reindex mode feeds the directory to storage construction and the index rebuild; the default mode hands both the directory and the from-git bit to the proxy; the environment-forced mode hands the directory to the in-process server. The claim is deliberately made in **both** places — here and on the fast path — because the fast path is what production takes and this action is reached only when the shortcut declines, so a guard stated once would have a hole exactly where it matters. |
| MCP server startup (per process) | the long-lived server's per-process half | `setLogDir(cwd)` on the already-resolved cwd, placed **after** the two working-directory refusal guards and **before** storage init — a guard must not create a store it then declines to use, and storage must not resolve before the logger knows where it lives. |
| Shared per-worktree MCP daemon | the hidden daemon subcommand | **Does not resolve.** It is handed the root explicitly and takes it verbatim, precisely so it cannot disagree with the process that derived its address; it falls back to resolving only when invoked without that input, which nothing in production does. |
...
- **The same rule has three independent implementations, and nothing pins them in lockstep.** `resolveStateRoot` (keyed memo, explicit `cwd` argument) is one. `resolveProjectDir` is a second: same `git rev-parse --show-toplevel`, same piped-stderr trick, same fall-back-to-the-process-cwd contract — but it takes **no** cwd argument (git inherits the process cwd) and caches into a **single unkeyed slot**, which is exactly why it is the right one for a CLI process that resolves one root for its lifetime and the wrong one for a hook that must resolve a payload-supplied path. That second one has **moved to a leaf module**, and its old home now re-exports it, so the call sites and the single cache are unchanged; the move was purely about what *importing* it costs. Its former home statically pulls in plugin discovery and the whole storage stack, so asking it for one `git rev-parse` dragged all of that into a process whose entire job is to forward bytes and whose resident set is multiplied by every open AI session on the machine. Its cache slot was renamed in the same move, and it gained the companion form described below. A third, async instance is inlined in [`PluginBootstrapHook.ts:62-66`](../cli/src/hooks/PluginBootstrapHook.ts) — `isInsideGitRepo` then `execGit(["rev-parse","--show-toplevel"])`, bailing to `null` rather than falling back. No test, lint rule, or grep gate holds the three together. (Surprising; two of them are byte-comparable in intent and differ only in memo shape.)
- **The second implementation now reports whether git named the directory, and that bit decides whether a shared process is keyed at all.** Collapsed to a string, "git said this is the root" and "git said nothing, so here is the input" are indistinguishable — and at least one caller has to tell them apart, because a directory reached through the fallback is not a worktree root and has no business identifying a long-lived process that many sessions will share. The MCP entry point is that caller: on a **false** bit it declines to key or spawn a shared process at all and serves that session by itself, with one explanatory line on standard error. Measured on a real machine, keying on the fallback answer collapsed every editor session whose registration was spawned at the filesystem root onto **one** shared process rooted there, each answering its memory tools for nothing. Every other caller still wants a plain directory and takes the string form. (What the bit is then used for: spec 364.)
- **The memo is never invalidated in production, and the stale answer is now shared.** A long-lived process that resolved a path *before* it became a git repository keeps the pre-repo answer for its entire lifetime, and vice versa; `git init` in a directory such a process already touched does not re-home its state. That was always true, and it is **sharper now**: the answer a host-spawned MCP process resolved at launch is what keys a shared per-worktree process, so a stale answer does not strand one session — it keys the one process that serves **every** session on that worktree, for as long as it lives (bounded only by its idle reap). `resetStateRootCache` exists but has no production caller.
...
- The shared per-worktree MCP process this resolution keys, the address derived from the root, the from-git bit's use as a spawn gate, and the in-process fallback a non-git answer is diverted to are owned by spec 364.
