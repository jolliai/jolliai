...
- `jolli enable --repo-hooks-only` — run a reduced install covering the shared runtime, the source-neutral git hooks, the acting host's agent hooks, and the project-local `/jolli` menu, while skipping the full skill set, the machine-global instructions block, the deferred state migrations, and MCP registration **for every host except one** (see the Codex exception below). **Silent on success, except for warnings on stderr** — see the dedicated behavior section below. Mutually exclusive with `--integrations-only`.
- `jolli enable --automatic` — a **hidden** flag (absent from `--help`) used by automatic, per-session callers rather than by a person. It has three effects: the install honours the repo-wide manual-disable opt-out instead of overriding it; the install operates on the current worktree only with a short lock budget rather than sweeping every worktree; and it suppresses the installer-warning block in the reduced mode. When the opt-out is honoured the install returns a **zero-write success carrying a manually-disabled discriminator** (spec 44) — this command does not read that discriminator, so the refusal takes the ordinary success branch here and is invisible in the command's own output.

  **The two opt-out flags this command derives are mutually exclusive by construction.** *Respect the opt-out* is set from `--automatic`; *clear it on success* is set from the negation of `--automatic` and of `--integrations-only`. They can therefore never both be on, which is exactly what a caller sending them over a machine interface must also guarantee — nothing downstream rejects the combination.
...
3. **Interactive credential phase** runs only if stdin is a TTY and `--yes` was not passed. It has three ordered parts — the setup wizard, the generation repair ladder, and the sign-in nudge — and the first two are mutually exclusive by design.
   ...
   **3b. Setup wizard.** If a Jolli API key is already stored in the global config, the wizard prints `Jolli API Key:     configured ✓` and skips directly to the Anthropic-key step (step 4) — this is the wizard's **only** path to that step.
   ...
   - **Auto-detect route** — runs only when the gate holds. The wizard performs the cheap multi-tool **presence sweep** (spec 280): filesystem only, no subprocess, every registered tool, milliseconds. It never performs a whole-registry *capability* sweep — the expensive probe is paid for at most one tool per round. Three outcomes:
   ...
   - **Local-agent tool picker.** Reached **two** ways: automatically, when the sweep found two or more tools (above), or explicitly, by choosing `3` on the provider menu. What is offered differs:
     ...
     - **From menu option `3`** — the sweep is re-run and whatever it finds is the list. If it finds **nothing**, all five tools are offered instead, with an extra explanatory line, because a user who explicitly asked for a local agent must not be dead-ended. So option `3` on a machine where one tool is detected offers only that one tool; all five appear only when none were detected.
     ...
     4. OpenCode
     5. Kimi Code
     6. Skip for now (configure later)
     ```
     ...
     **One entry may carry a `  (current)` marker** — two spaces, then the parenthesized word — appended to the label of the tool the configuration already drives, so the user can see what they are on without leaving the menu (the picker gives no other hint). What counts as "current" is narrow and deliberate: it is the stored agent-tool value (defaulting to `claude-code` when that value is absent) **only when the active provider is exactly the local-agent one**, and nothing otherwise. A user who has an agent tool stored but a different provider selected therefore sees **no** marker anywhere in the list — correctly, because that stored tool is driving nothing.

     The marker is computed **once**, before the interactive phase writes anything, and the same value is handed to both entry points into the picker (the auto-detect route and menu option `3`). It is consequently **not** refreshed between re-prompt rounds within a single run: it reflects the configuration as it stood when the run began, not as it stands after a pick. The auto-detect single-tool confirmation path — which skips the picker entirely — prints no marker at all.
     ...
     - **A number naming a listed tool** — that tool is **capability-probed before anything is written** (spec 280).
       - *Usable* → the local-agent provider and the chosen tool are persisted, and the wizard prints three lines followed by the configuration path:
         ...
         followed by `Configuration saved to <absolute config path>`. The third line is the tool's own sign-in instruction from the shared registry — one string per registered tool, the same set the diagnostic command attaches to its failure line (spec 59) — not a pointer at `jolli doctor`.
     ...
     **When the list empties** because every offered tool failed its probe, the closing advice splits on how the list was built — telling someone to install a tool they demonstrably already have would read as a broken diagnosis:
     ...
     when the whole registry was offered blind, versus
...
2. **Reduced install scope.** What runs: the shared machine-global runtime (dispatch scripts and the dist-path registry entry for this source tag), the per-worktree state directory, all five source-neutral git hooks, the acting host's agent hooks and project-local bare `/jolli` menu, the legacy- and retired-skill sweeps, and the union merge of the menu's git-exclude entry. What is skipped: the full agent skill set, the machine-global instructions block, every per-host auto-enable config write, the per-worktree config migration, the deferred state-schema migration, and MCP registration for every host but one.

   **Which host's assets** is decided by the install's *source tag*, not by the compile-time client kind — the `/jolli:init` recipe dispatches through the shared runtime and can land in a `cli` bundle, where the client kind would read "cli" even though a plugin initiated the call. A Claude-plugin tag writes `.claude/**`; a Codex-plugin tag must never write it. An unmapped or missing tag falls back to Claude, because a hand-run `jolli enable --repo-hooks-only` installed the Claude menu and agent hooks before this split existed.

   **The MCP exception: Codex, and only when the acting host is Codex.** That one global `~/.codex/config.toml` entry is written even in this reduced mode, because it is the *only* way MCP reaches a Codex-plugin user: a plugin `.mcp.json` cannot serve them. Codex does not expand its plugin-root variable inside an MCP entry, so such an entry must give a relative command with a plugin-relative working directory — and since every memory tool derives the repository it serves from its working directory, the resulting server answers for the plugin's own cache directory instead of the user's repository: successful-but-empty results, plus a placeholder Memory Bank repo named after the bundle's version directory. A `config.toml` entry carries no working directory and is launched with the session's, which is the only correct one. So the Codex plugin ships no MCP manifest, the server refuses to start under a plugin-cache working directory rather than serve the wrong repository, and this mode registers **Codex alone** — never the other seven global hosts, which a Codex install has no business configuring. Consequence the user sees: the first session after install has skills but no MCP tools, since registrations are read at session start; the skills' CLI fallback covers it. See the MCP client registration spec for the per-host table.
3. **The manual-disable opt-out is NOT gated by this flag.** Honouring the durable repo-wide opt-out is driven by the separate hidden `--automatic` flag, not by the reduced mode. Consequently a **plain `jolli enable --repo-hooks-only`** — the form a person or a shipped setup recipe types — both **ignores** the opt-out (the install proceeds) **and clears** it (a successful install wipes the durable marker), exactly as a full `jolli enable` does. Only a caller that also passes `--automatic` respects the opt-out, and it then returns a **zero-write success carrying a manually-disabled discriminator** without installing anything into the repository. Any claim that "the reduced mode honours the opt-out" is false of the flag itself.
4. **Silent on success — on standard output, always.** The command does not print the success header, the hook-path list, the restart reminder, or any credential prompt. It disarms the short telemetry exit-flush (`markSkipExitFlush()`) and instead awaits an explicit onboarding-funnel snapshot flush capped at `BOUNDED_FLUSH_BUDGET_MS` (2 s) on both the success and failure branch — a one-shot process gets no later tick, so the alternative to the bounded wait is losing the event, not delaying it (see the onboarding funnel snapshot spec). Successful outcomes are recorded only to the diagnostic log. This silence is deliberate and is specifically about *standard output*: an automatic per-session caller may inject it into an agent's context, and one host is stricter still — the Codex plugin's session-start hook validates its whole output against a schema, so a single non-JSON byte on standard output fails the hook and no briefing reaches the model at all.
5. **Installer warnings, on stderr, only when not `--automatic`.** Warnings are surfaced one per line, prefixed, to **standard error** — which keeps standard output byte-clean for the envelope above. The `--automatic` path suppresses them entirely (they remain in the diagnostic log): a per-session bootstrap would otherwise repeat the same warning on every session start. The explicit path does not, because that is the `/jolli:init` case, whose output a person reads and whose warnings report what a person needs to know about machine-global state — today, a config write that failed. Suppressing them on both paths reported a failed save as unqualified success. Note what is deliberately **not** among them any more: the local-agent tool the initiating host would drive is seeded only into a config that holds none, and leaving a configured tool alone is not an event worth a warning line.
6. **Failure surfacing.** If the underlying install reports failure, a one-line error is written to stderr and the exit code is set to `1`. Warnings are printed after it under the same rule as above.
7. **No structured output.** This mode emits nothing machine-readable. The same-session skill-reload request that an earlier `--reload-skills` flag produced here is now emitted by the Claude Code plugin's own session-start bootstrap hook instead.
...
### Onboarding-funnel snapshot at the tail of the report

The very last thing the report does — on **both** the success branch and the error branch — is emit an `onboarding_progressed` snapshot for the target directory (spec 312). Firing on the failure branch too is what makes the "installed but never got into a git repository" drop-off visible, since that failure reports the not-in-a-repo state rather than being silent.

The configuration handed to the snapshot is **re-loaded** at this point rather than reused from earlier in the run. That matters because the interactive credential phase above may have just written the credential the snapshot is reporting on; reusing the configuration read before the wizard would report a user as having no capture route in the same breath as telling them they now have one.

`--repo-hooks-only` never reaches this tail — it returns from the action before the report is built at all. `--integrations-only` does reach it.
...
- **The `(current)` marker answers a narrower question than it looks like it does.** It marks the tool the configuration is *driving*, not the tool the configuration *remembers*. A user with an agent tool stored but a non-local-agent provider selected sees no marker at all — which is right, because that stored tool is driving nothing, but it does mean an empty-marker menu is not proof that no tool was ever chosen. The value is also computed once per run, before any write, so it does not move as the picker re-prompts. (Notable.)
...
- **`--source-tag <tag>` only changes which dist-paths source entry this install stamps.** It lets a non-CLI surface (e.g. IntelliJ) drive a CLI-backed install without overwriting the `cli` entry, so multiple surfaces' runtimes coexist in the registry. It does **not** change the directory recorded in that entry: from this command the recorded runtime is always the executing bundle's own location. An explicitly-named directory is possible for a caller installing on another distribution's behalf, but there is **no flag for it here** — it is reachable only over the machine interface (spec 50).
- **A refusal is invisible in this command's output.** An install refused because the repository is manually disabled reports success, and this command never inspects the discriminator — so the run takes the ordinary success branch. In the reduced mode with `--automatic`, which is the only way to reach the refusal from here, that branch prints nothing at all, so a refused per-session bootstrap is indistinguishable from a successful one at the terminal. (Surprising; observable only in the diagnostic log.)
...
- **`--repo-hooks-only` is deliberately quiet, but no longer flush-free.** It prints nothing on success, and it trades the disarmed exit-flush for an explicit onboarding-funnel snapshot flush bounded at `BOUNDED_FLUSH_BUDGET_MS` (2 s), so an automatic caller's startup stays bounded without losing the event. Only failures reach stderr. This is the one mode where a successful `enable` produces no terminal output.
...
- The onboarding-funnel snapshot emitted at the tail of the report — its state tuple, its dedup ledger, its daily heartbeat, and the consent gate it applies before doing any work — is owned by spec 312. This spec owns only that the tail emits it, on both branches, from re-loaded configuration.
