...
**"No product-held credential is read" is exactly true; "the agent tool's own subscription" is true for only three of the four drivable CLIs.** Three of them authenticate against a subscription the tool itself holds. The fourth authenticates against whichever LLM provider the user configured *inside* it — its own stored provider credentials, or provider API keys left in the environment, which that backend deliberately does **not** scrub. So a `local-agent` pin on that fourth tool still involves no jollimemory-held credential, but it can spend the user's own metered provider credit rather than a flat subscription. The dispatcher makes no distinction: the pin is honored identically for all four.
...
- **A `local-agent` pin reads no stored credential — but it does not always bill a flat subscription.** The dispatcher requires no credential to select it; it is always satisfiable the moment `aiProvider` is set to `"local-agent"`. For three of the four drivable CLIs the spend lands on that tool's own subscription. For the fourth it lands on whichever LLM provider the user configured inside that tool — including provider API keys deliberately left in the child's environment — so the pin can still consume metered credit, just not through a product-held key.
