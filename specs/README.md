...
- [352 — Local Dashboard HTTP Server](352-local-dashboard-http-server.md) — the loopback service behind the dashboard command: how it binds, its route surface, its browser-security layers, and the small set of endpoints that demand a token while the rest do not — the settings view alone also demanding a same-site indication.
...
- [361 — Dashboard Launcher Command](361-dashboard-launcher-command.md) — everything opening the local dashboard does end to end: the refusals that stop it first, the schema it pre-creates so a read-only service has something to open, the lock-guarded reuse-or-replace decision, the browser it opens, the separate stop path, and the history import that runs last and cannot fail the launch.
