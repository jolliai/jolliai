...
- [311 — Project State-Root Resolution](311-project-state-root-resolution.md) — anchor an implicitly-supplied working directory to its enclosing worktree root so a subdirectory can never fork a second per-project state store, and report whether git actually named that root — only a git-named one may key a process every session on the worktree shares.
- [344 — Cutover Routing State Table](344-cutover-routing-state-table.md) — the four-state route resolved from a per-clone freeze marker plus a database witness, the backend each state yields on write and on read, and the shared classifier long-lived hosts rebuild their cached storage on.
...
- [370 — Stale Storage Heal After an Unwitnessed Cutover](370-stale-storage-heal-after-unwitnessed-cutover.md) — how a long-lived process notices, on its own call paths, that another surface froze the ref its cached storage reads, and rebuilds that backend in place rather than restarting — across two hosts whose applies are equivalent only in success.

## Local dashboard and the memory database

- [347 — Machine-Level Memory Database: Store, Schema and Migration Ladder](347-dashboard-sqlite-store-and-schema-migration.md) — the machine-global database file, its two-half schema plus the derived per-day cache declared apart from both halves, owner-only permissions, and an append-only migration ladder identified by name rather than position, which logs every touch inside the database, compares that log against the build reading it and only ever warns — carrying no compatibility gate at all.
...
- [349 — Memory Database Snapshot and Restore](349-dashboard-db-snapshot-and-restore.md) — a daily-gated verified copy with retention floors, asked for on a clock by the resident process and opportunistically by two commit-time sites, and a restore that guarantees the copy's integrity but never its recency.
- [350 — Dashboard Database Repository Backfill](350-dashboard-db-repo-backfill.md) — set reconciliation of commits, summaries, sessions and worktree state into the database, spending no model budget, with branch attribution taken from each memory's own recorded branch rather than from a reachability scan — not to be confused with the historical attribution back-fill.
...
- [352 — Local Dashboard HTTP Server](352-local-dashboard-http-server.md) — the loopback service behind the dashboard command: how it binds, its route surface — five page paths plus the sandboxed document routes the folder-backed pages frame — its browser-security layers, and the small set of endpoints that demand a token while the rest do not, the settings view alone also demanding a same-site indication.
- [353 — Dashboard Read Model Query Layer](353-dashboard-read-model-query-layer.md) — how each view's payload is derived from a read-only handle, the scope, window and dimension axes every view shares, and the breaking-change counter a stale tab reloads on. Calls no model at all.
- [354 — Dashboard Database Write Protocol](354-dashboard-database-write-protocol.md) — the pending-then-project write path and the producer wrappers that cannot fail their caller, so no capture depends on the service being up — including the recall-receipt write stamp and the two paging indexes still written on every receipt for an upload channel that table has since left, and read by nothing.
- [355 — Dashboard Repo Registry and Probe](355-dashboard-repo-registry-and-probe.md) — how a repository becomes known: a machine-global registry file the database's own table projects from, plus the pre-registration folder probe no shipped surface asks any more.
- [356 — Local Dashboard Browser Application](356-local-dashboard-browser-ui.md) — the no-build browser app inlined into every page, its views, its Settings modal that is not a route and whose state outlives a close, its single-view refresh loop, and its zero external network requests.
- [365 — Machine-Global Resident Daemon](365-machine-global-resident-daemon.md) — one background process per machine per user that exists to run maintenance when nobody is asking for it, reached through a never-throwing handshake in which a strictly newer build retires the incumbent and a tie attaches, holding no schedule of its own.
- [366 — Dashboard Knowledge and Graph Reading](366-dashboard-knowledge-and-graph-reading.md) — the two dashboard pages whose source is the Memory Bank folder rather than the database, their separate repository identity space, the filename allowlist that is itself the traversal guard, and a failure posture that prefers a partial page because the layer it reads is regenerated wholesale.
- [361 — Dashboard Command](361-dashboard-launcher-command.md) — everything opening the local dashboard does end to end: the refusals that stop it first, the schema it creates before anything reads, the server it binds in its own process, the browser it opens, the history import that runs behind the live page and cannot fail the launch, and the signal it waits on instead of returning.
- [363 — Dashboard Settings Apply](363-dashboard-settings-apply.md) — what one submitted settings object does to durable state: validated in a fixed order, committed as a single machine-global configuration transaction, then reconciled into every registered repository's agent hooks.
- [374 — Daily Statistics Rollup Cache and the Local-Day Engine](374-dashboard-daily-stats-rollup-cache.md) — a per-local-day pre-aggregate of the spend axes and the token split that is never authoritative, expiring by asking the source tables rather than by being told, over the one calendar engine both the read path and the cache builder derive their day boundaries from — because two derivations would disagree, silently, on exactly the daylight-saving days it exists to get right.
...
- [257 — Multi-Provider Pricing and Cost Estimation](257-multi-provider-pricing-and-cost-estimation.md) — the hand-maintained per-model USD price table keyed by transcript model id, the uniform cost formula, and the write-time estimate stamped on a memory that every token meter prefers over re-estimating.
- [258 — AI Context-Relevance Filtering](258-ai-context-relevance-filtering.md) — one batch LLM call, issued for every provider but only on the commit kinds that derive relevance at all, tiers and soft-excludes low-relevance context items; a least-affinity-first pre-ordering decides who the prompt budget is spent on, and any error keeps every candidate it was handed — a set its caller has already carved down.
...
- [26 — Claude Stop Hook — Session Recording](26-claude-stop-hook-session-recording.md) — record session metadata on every response turn and trigger one incremental discovery pass over plans, references, skills and worktree ownership, with no LLM call and one machine-global write.
...
- [182 — Session Title Resolution Chain](182-session-title-resolution-chain.md) — resolve a session's display title through an ordered source chain, ending at a constant placeholder — plus a second entry point over an already-archived conversation slice that answers absent instead.
...
- [371 — Claude Multi-Owner Session Ownership Ledger](371-claude-multi-owner-session-ownership-ledger.md) — record once per machine every worktree root a session visited or authored into, together with the transcript line each root was first seen at, so a commit made in one checkout can claim a session that sat in another and read that transcript from where this checkout entered the conversation rather than from the top.
- [372 — Transcript Repair and the Empty-Conversation Verdict](372-transcript-repair-and-empty-conversation-verdict.md) — rebuild one memory's missing conversation capture from the transcript history still on this machine, between a per-owner lower bound and the memory's own generation instant, plus the four-state verdict — derived by dry-running that same engine — that tells every memory surface why a memory shows no conversations.
...
- [360 — Working-Tree File Discard](360-working-tree-file-discard.md) — resolve each requested path against one authoritative status read taken at the worktree root, dispatch it to a closed set of index and worktree operations, and report a per-path outcome that never spells a failure as success.
- [373 — Archived Conversation Grouping](373-archived-conversation-grouping.md) — collapse a memory's stored transcript slices into one row per producer-and-session, merging the slices of one session across an amend or squash chain in timestamp order, and hiding only the turn-less records that carry a usage share.
...
- [36 — Summary Attribution by Transcript Cutoff](36-summary-attribution-by-transcript-cutoff.md) — treat the entry's creation timestamp as a read cutoff so post-commit conversation belongs to the next commit, never duplicated within a worktree, with Claude candidates drawn from the session registry together with the ownership ledger that also floors the read.
...
- [42 — Plan Archival on Commit](42-plan-archival-on-commit.md) — snapshot uncommitted plans into storage under a slug-and-hash name and rewrite the registry row into an edit-detecting guard, refusing outright any plan whose file belongs to a different repository.
...
- [62 — `jolli configure`](62-cli-configure-command.md) — set, remove, list, and show per-user config values with type coercion and per-key save-time validation, the key inventory now including a session-statistics switch whose scope is every repository on the machine rather than one repository's memories.
...
- [154 — Built-in external-reference source definitions](154-external-reference-source-adapters.md) — the catalog of built-in sources as data-only definitions, their correctness-sensitive registration order, their field rules, and the ranked synthesized-title family that stops a poorer re-observation overwriting a richer stored one.
...
- [362 — Figma file link recognition and canonicalisation](362-figma-file-link-recognition.md) — recognise a design-file link the user pasted into their own prompt, reject any candidate whose enclosing address does not resolve to the design tool's host, and derive the tool-facing key, a query-stripped link, and the display name the tool's own content can never supply.
...
- [358 — Memory Bank Unused-Folder Archival Sweep](358-memory-bank-unused-folder-archival.md) — on a user-initiated folder-tree Refresh, move every discovered repository folder that provably holds nothing into the vault's archive rather than deleting it, guarding the current repository and skipping silently when the vault write lock is busy.
- [359 — Memory Bank Duplicate-Folder Consolidation](359-memory-bank-duplicate-folder-consolidation.md) — fold the several folders holding one repository into a single survivor, chosen by comparing each folder's set of stored memories against the system of record, merge in what only the losers held, and re-confirm under the lock when the confirmed plan no longer matches what is on disk.

## Jolli Space, sharing and push

- [94 — Summary Push to Jolli Space](94-summary-push-to-jolli-space.md) — a single-attempt authenticated JSON POST carrying a version header and a document-type discriminator, with first-share vs update affordances — plus, on the same client, a second self-retrying session-statistics endpoint that reuses its credential and header composition and none of its document contract.
...
- [310 — Per-Repo Outbound-Push Control](310-per-repo-outbound-push-control.md) — a machine-global, identity-keyed record of which repositories may not push outbound, the one predicate every per-repository outbound path consults — a third, machine-wide channel now sits outside it entirely — and a current-repo toggle on every control surface.
...
- [369 — Session-Statistics Sync Channel](369-session-statistics-sync-channel.md) — a second, machine-wide outbound channel that uploads the memory database's session-activity tables to the user's organization with no Space binding anywhere, one request carrying several repositories' rows, bounded by an explicit whole-schema partition over what may leave and a per-backend-scope keyset cursor over what already has.
...
- [148 — MCP server tool surface](148-mcp-server-tool-surface.md) — the named tools one bound working directory's memory is advertised through — read-only projections plus the Space-reaching ones — each returning a JSON-text response, identical whether the surface is served in-process or by the shared server the spawned process forwards to.
...
- [368 — Cursor Plugin Package](368-cursor-plugin-package.md) — the Codex plugin's structural twin, the five host-contract differences that each fail silently, skills placed across three scopes because this host does not namespace them, and the one host that installs nothing into a repository that has not opted in.
...
- [364 — MCP Per-Worktree Daemon and Proxy](364-mcp-per-worktree-daemon-and-proxy.md) — one shared self-reaping background server per worktree, reached by every session through a byte-forwarding process that serves that session by itself whenever the shared one cannot be reached, leaving the host-facing spawn-a-standard-io-process contract untouched. Its handshake primitives are now shared with the machine-global resident process (365).
...
- [175 — Memory Bank Folder Browser](175-vscode-memory-bank-folder-browser.md) — a lazily-expanded tree interleaving discovered repos with user-dropped siblings, enriched with manifest classification, title, branch, and divergence — and a Refresh that no longer only reads: it archives the folders holding nothing and folds a repository's duplicates into one before re-listing.
...
- [304 — Zero-Write Contract for a Manually-Disabled Repository](304-manually-disabled-zero-write-contract.md) — the in-process suppression flag seeded before the first log line, the full inventory of writes it stops, the durable reads covering the surfaces that flag can never reach, and the carve-outs that still reach disk — one of them unintended.
...
- [367 — Editor Dashboard Launch](367-editor-dashboard-launch.md) — opening the dashboard by sending one command line to a fresh integrated terminal rather than running it in the editor's own process, the three ordered tiers that decide which command line, and the two situations in which the button declines instead.
...
- [122 — Changes Panel](122-intellij-changes-panel.md) — rows from a NUL-separated rename-aware `git status` with checkbox, status badge, muted parent dir, click-to-diff, and a per-row hover discard the panel words, flushes and refreshes around but no longer decides.
...
- [132 — PLANS & NOTES Panel](132-intellij-plans-and-notes-panel.md) — one newest-first list merging plans, notes, references and an aggregate skills row, each badge-tagged and previewable, whose leave-out toggle writes straight through — and where neither read behind it classifies a plan's repository, so a plan the next memory will drop still shows as included.
...
- [313 — IntelliJ Source Presentation Table](313-intellij-source-presentation-table.md) — one table mapping every reference source this host knows — currently the whole shared catalogue, with the lag list empty — to its badge letter, brand hue and label, plus the label-composition policy its consuming panels share.
...
- [317 — IntelliJ Archived Reference Body Read](317-intellij-archived-reference-body-read.md) — derive the command-line surface's archived-reference file stem before the read, since the GitHub, context7 and Sentry sources carry native ids that are not path-safe — and this host's own copy of that set still names only the first two.
