...
Fourteen built-in reference sources — Linear, Confluence, Jira, GitHub, Notion, Slack, Zoom Meeting, Zoom Doc, Asana, monday.com, Context7, Jolli Memory, Vercel, and Figma — are each described as a single **data-only source definition**: a declarative bundle of extraction pipes over an already-parsed tool-result payload plus render metadata, carrying no source-specific code. A shared engine (spec 255) evaluates any definition to project a payload into a uniform cross-source reference record and to render those records into the summarization prompt. This spec is the catalog: the fixed set of built-ins, their **correctness-sensitive** registration order, and each source's field extraction, id/url rules, display fields, description handling, wrapper keys, budgets, and render tags. It also covers the two engine transforms the definitions invoke (HTML-entity decode, ASCII lowercase) and the producer-side payload reshapes the engine-external layer performs before some definitions run (the GitHub issue reshape, the shared Atlassian-Document-Format → plain-text flattener, the Confluence dual-envelope normalization, the Zoom Doc normalization, the monday.com item normalization, and the three arguments normalizations — Context7, Jolli Memory and Figma).

## Scope

**In scope:**

- The fixed ordered set of fourteen built-in source definitions and the fact that registration order is preserved, observable, and — at the Claude tool-name layer — correctness-sensitive (Confluence must precede Jira).
...
- Which built-ins are reachable on each producer (all fourteen on Claude; eleven on Codex — Zoom Doc, Vercel and Figma are not) and why.
- The open source-id model: a source id is a plain string; fourteen built-ins are known, but the id space is not a closed enum.
...
The widened page/doc budgets (Confluence, Notion, Zoom Doc) reflect that documents are typically much larger than ticket descriptions; Zoom Meeting's mid-range budget covers its AI-generated meeting summary. Vercel's are the ticket-shaped defaults. Figma's match Context7's, the other arguments-derived source whose body is a short accumulated trace rather than a document.

**The Context7, Jolli Memory, Vercel and Figma caps are unreachable configuration.** Both call sites that render a prompt block — the commit-summarization assembly and the regeneration rebuild — skip a track-only definition before any rendering happens, and those four are the track-only built-ins. No reachable code path renders any of those blocks, so none of those caps can be observed. They are recorded here for completeness, not as behavior.
...
**The Context7, Jolli Memory, Vercel and Figma rows are unreachable configuration**, for the same reason their budgets are: both prompt-block call sites skip a track-only definition before rendering, so no `context7-libraries`, `jolli-memory-lookups`, `vercel-deployments` or `figma-files` block is ever emitted. The rows record what the definitions declare, not observable output. (Context7's, Jolli Memory's and Figma's field-attribute settings are doubly moot — all three declare no display fields at all. Vercel declares three, so its `yes` would be observable if this block were ever rendered.)
...
- **Codex reachability is 11 of 14.** All fourteen are Claude-reachable. Linear, Confluence, Jira, GitHub, Notion, Zoom Meeting, Asana, monday.com, Slack, Context7, and Jolli Memory are also Codex-reachable (each declares a Codex match rule and has a registered Codex normalizer). **Zoom Doc, Vercel and Figma** are not — none declares a Codex match rule at all (never matched), so all three remain Claude-only (specs 153, 180). Vercel's and Figma's are deliberate holds pending a real Codex capture, not oversights. Slack was Codex-unreachable in an earlier revision (its Codex match had no registered normalizer, so a matched call was dropped after recognition); a Codex Slack normalizer now exists (spec 256), closing that gap.
