...
- The three optional consumer flags a definition may declare (track-only; arguments-derived; accumulate-body), which today three built-ins set between them, and the consequence that a track-only source's declared render tags and budgets are unreachable.
...
- Which built-ins are reachable on each producer (all thirteen on Claude; eleven on Codex — Zoom Doc and Vercel are not) and why.
- The open source-id model: a source id is a plain string; thirteen built-ins are known, but the id space is not a closed enum.
...
A source id is a plain string. Thirteen ids ship as built-in definitions: `linear`, `confluence`, `jira`, `github`, `notion`, `slack`, `zoom-meeting`, `zoom-doc`, `asana`, `monday`, `context7`, `jollimemory`, `vercel`. The id space is **open** — it is not a closed enumeration of a fixed count; persistence and identity layers key off the string directly and tolerate ids they do not currently recognize (spec 179's lenient read path). The thirteen built-ins are the only ones registered today. Each id doubles as the value placed on every record the definition produces and the registry-key prefix (`<id>:<nativeId>`).
...
The widened page/doc budgets (Confluence, Notion, Zoom Doc) reflect that documents are typically much larger than ticket descriptions; Zoom Meeting's mid-range budget covers its AI-generated meeting summary. Vercel's are the ticket-shaped defaults.

**The Context7, Jolli Memory and Vercel caps are unreachable configuration.** Both call sites that render a prompt block — the commit-summarization assembly and the regeneration rebuild — skip a track-only definition before any rendering happens, and those three are the track-only built-ins. No reachable code path renders any of those blocks, so none of those caps can be observed. They are recorded here for completeness, not as behavior.
...
**The Context7, Jolli Memory and Vercel rows are unreachable configuration**, for the same reason their budgets are: both prompt-block call sites skip a track-only definition before rendering, so no `context7-libraries`, `jolli-memory-lookups` or `vercel-deployments` block is ever emitted. The rows record what the definitions declare, not observable output. (Context7's and Jolli Memory's field-attribute settings are doubly moot — both declare no display fields at all. Vercel declares three, so its `yes` would be observable if this block were ever rendered.)
...
- **Codex reachability is 11 of 13.** All thirteen are Claude-reachable. Linear, Confluence, Jira, GitHub, Notion, Zoom Meeting, Asana, monday.com, Slack, Context7, and Jolli Memory are also Codex-reachable (each declares a Codex match rule and has a registered Codex normalizer). **Zoom Doc and Vercel** are not — neither declares a Codex match rule at all (never matched), so both remain Claude-only (specs 153, 180). Vercel's is a deliberate hold pending a real Codex capture, not an oversight. Slack was Codex-unreachable in an earlier revision (its Codex match had no registered normalizer, so a matched call was dropped after recognition); a Codex Slack normalizer now exists (spec 256), closing that gap.
...
- **Context7 was the catalog's first track-only source; Jolli Memory is the second; Vercel is the third.** Each source's references are stored, archived, and displayed like any other, but never enter the summarization prompt and never enter the relevance ranker's input (specs 12, 258).
