...
- **On the JVM IDE surface the resolution no longer drives header emission — it only compares tenants.** That surface's backend calls are issued by the bundled command-line runtime on its behalf, so the `x-tenant-slug` decision for those requests is made by the canonical implementation from the base URL the plugin forwards. The Kotlin port of this resolution is still live, but its remaining job is the same-tenant comparison used by the sign-in and credential-storage rules (origin case-insensitive, first path segment case-**sensitive** — see **Auth Credential Storage**). (Notable; the header-emission half of this spec is canonical-only for that surface.)
